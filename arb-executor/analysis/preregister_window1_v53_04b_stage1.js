...
const implementationCommit = process.argv[3];
const winnerLaw = process.argv[4];
if (!/^[0-9a-f]{40}$/.test(implementationCommit || "")) throw new Error("exact winner implementation commit required");
if (!/^A[0-5]_/.test(winnerLaw || "")) throw new Error("winner arming law required");
const out = path.join(repo, ".claude/window1_v53_04_preregistration_20260820/PRE_REGISTRATION.json");
...
const prior = [
  ["V53_01", "7ba7c3dd23eb9b87d4584f856d4a247d760a03b4", ".claude/window1_v53_preregistration_20260819/PRE_REGISTRATION.json"],
  ["V53_02", "9dca2006fc9865d9989e03d2633e12f7c0f584af", ".claude/window1_v53_02_preregistration_20260820/PRE_REGISTRATION.json"],
  ["V53_03", "98f69961", ".claude/window1_v53_03_preregistration_20260820/PRE_REGISTRATION.json"],
];
const truthBytes = show(truthCommit, truthPath), censusBytes = show(censusCommit, censusPath);
const truth = JSON.parse(truthBytes), census = JSON.parse(censusBytes);
if (truth.rows?.length !== 804 || census.rows?.length !== 1143) throw new Error("V53-04b authority census invalid");
const truthByCode = new Map(truth.rows.map((row) => [row.code, row]));
const excluded = new Set(), authorities = [];
for (const [iteration, commit, receiptPath] of prior) {
  const bytes = show(commit, receiptPath), parsed = JSON.parse(bytes);
  if (parsed.population?.fresh_25?.length !== 25) throw new Error(`${iteration} fresh cohort invalid`);
  parsed.population.fresh_25.forEach((row) => excluded.add(row.code));
  authorities.push({ iteration, commit, path: receiptPath, sha256: sha(bytes), fresh_events_added_to_exclusion_union: 25 });
}
...
if (pins.some((row) => !row.event_id)) throw new Error("standing pin missing from truth table");
const rowsByCode = new Map();
for (const row of census.rows) { if (!rowsByCode.has(row.code)) rowsByCode.set(row.code, []); rowsByCode.get(row.code).push(row); }
const seedMaterial = `V53_04B_FRESH25|${implementationCommit}`, seedSha256 = sha(seedMaterial), rank = (value) => sha(`${seedSha256}|${value}`);
...
for (const [code, rows] of rowsByCode) {
  ...
  strata.get(stratum).push({ code, event_id: truthByCode.get(code).event_id, category, census_stamps: stamps, stratum, role: "FRESH_STRATIFIED_COHORT_NOT_IN_V53_01_THROUGH_V53_03" });
}
...
if (fresh.some((row) => excluded.has(row.code))) throw new Error("prior V53 cohort overlap");
...
const receipt = {
  label: "V53_04_STAGE1_PRE_REGISTRATION",
  variant: "V53_04B_ENGINE_SELECTED_ARMING_LAW",
  created_before_grading: true,
  implementation_commit: implementationCommit,
  winner_arming_law: winnerLaw,
  engine_pins_sweep_pass_required_before_creation: true,
  seed_derivation_law: "SHA256('V53_04B_FRESH25|' + WINNER_IMPLEMENTATION_COMMIT)",
  seed_material: seedMaterial,
  seed_sha256: seedSha256,
  authorities: { truth_table: { commit: truthCommit, path: truthPath, sha256: sha(truthBytes) }, reflex_census: { commit: censusCommit, path: censusPath, sha256: sha(censusBytes) }, V53_pre_registrations: authorities },
...
fs.writeFileSync(out, canonical(receipt));
