...
async function main() {
  ...
  const emitV53Stage1 = async () => {
    ...
    const v53Assertions = isV5304 ? {
      ...
      early_floor_never_posts_before_selected_arming_law: candidateFlow.trace.filter((row) => row.v53_riser_arming?.stance?.value === "EARLY_FLOOR_SIDE" && row.v53_riser_arming.stance.armed !== true && ["PLACE_REST", "REPRICE_REST"].includes(row.final_action)).map((row) => `${row.leg_identity}@${row.receipt}`),
