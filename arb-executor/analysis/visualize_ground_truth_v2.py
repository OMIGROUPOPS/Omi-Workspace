...
(function () {
  ...
  // $ if this exit were deployed on the full corpus at 10 contracts/N (v1 convention)
  const CONTRACTS = 10;
  const dollarsOf = ev => ev == null ? null : ev * M.nTotal / 100 * CONTRACTS;

  let lens = "ev";  // "ev" | "roi"
  const leftVal = d => lens === "ev" ? d.ev : d.roi;
  const leftColor = v => v == null ? "transparent" : (lens === "ev" ? evColor(v) : roiColor(v));
  ...
  function buildPlot(containerId, kind) {
    ...
    function legColAt(t) {
      ...
      return roiColor(-M.roiAbsMax + t * 2 * M.roiAbsMax);
    }
    ...
    function refreshLegend() {
      ...
      else { legL.text((-M.roiAbsMax).toFixed(0) + "%"); legM.text("0"); legR.text("+" + M.roiAbsMax.toFixed(0) + "%"); }
    }
    refreshLegend();

    return { svg, sel, hlRow, hlCol, ovFloor, ovCeil, dens, kind, refreshLegend };
  }
  ...
  function showTip(event, d) {
    ...
    tip.innerHTML =
      ...
      `EV <span class="${evCls}">${fmtEv(d.ev)}c</span> &nbsp; ` +
      `ROI <span class="${roiCls}">${fmtRoi(d.roi)}</span> &nbsp; ` +
      `hit ${fmtHit(d.hit)}%<br>` +
      `<span class="tip-dim">if deployed on all ${M.nTotal.toLocaleString()} N @10ct:</span> <span class="${dolCls}">${fmtDol(dol)}</span>` +
  ...
  function selectRow(c) {
    ...
    const data = cells.filter(d => d.c === c).map(d => ({ R: d.R, T: d.c + d.R, hit: d.hit, ev: d.ev, roi: d.roi, dollars: dollarsOf(d.ev) }));
    sortState = { key: "R", asc: true };
    const effTxt = r && r.effN != null ? r.effN.toFixed(0) : "—";
    renderTable(
      `Cell c=${c}c  ·  ownN=${r ? r.ownN : "—"}  effN=${effTxt}`,
      `Every conceivable R for this entry cent (pooled). Breakeven R≥${r ? r.breakevenFloorR : "—"}, ceiling R≤${r ? r.ceilingMaxR : "—"}. Complement ${r && r.complementC!=null ? r.complementC+"c" : "—"}. $ = pooled EV over all ${M.nTotal.toLocaleString()} N @10ct.`,
      [COL_R, COL_T, COL_HIT, COL_EV, COL_ROI, COL_DOL], data
    );
  }
  function selectCol(R) {
    highlightCol(R);
    const data = cells.filter(d => d.R === R).map(d => ({ c: d.c, T: d.c + d.R, hit: d.hit, ev: d.ev, roi: d.roi, dollars: dollarsOf(d.ev) }));
    sortState = { key: "c", asc: true };
    renderTable(
      `Fixed R = +${R}  ·  cross-cent spectrum`,
      `How the same +${R}c offset performs across every entry cent (pooled). $ = pooled EV over all ${M.nTotal.toLocaleString()} N @10ct.`,
