...
try{
 ...
 assert.deepEqual(errors,[]);const report={base,phase,games:rows,js_errors:errors,inspector:'fixed; no modal',keyboard:'game and gate focus verified',sources:'all five face hashes unchanged',desk:phase==='lab'?'not tested':'disconnected, zero fabricated games'};writeFileSync(resolve(out,`VERIFY_${phase.toUpperCase()}.json`),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
