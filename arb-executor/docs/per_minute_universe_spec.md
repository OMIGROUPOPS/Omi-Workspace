...
| `match_start_ts` | int64 | Inferred from both-sides-simultaneous trade-density signal per LESSONS A35 amended v3 (first minute in a sustained run of K≥3 consecutive minutes where BOTH `own.trade_count_in_minute ≥ M` AND `partner.trade_count_in_minute ≥ M`, with default `M=5`); null if no such run exists |
| `match_start_method` | string | "both_sides_trade_density" / "expected_expiration_fallback" / "unknown" |
...
Per LESSONS C19: Kalshi market-lifecycle timestamps don't track actual match start (pooled stdev 13.5 hours across categories). Per LESSONS A35 amended v3 (2026-05-12 post-T37b Phase 1 v2 visual review): the cleanest match-start anchor available in G9 is the **both-sides-simultaneous trade-density signal** — the first minute in a sustained run of K≥3 consecutive minutes where BOTH own.trade_count_in_minute ≥ M AND partner.trade_count_in_minute ≥ M (default M=5).

Signal evolution: v1 used candle `volume_fp` (LESSONS G19: null throughout for historical-tier markets, returns "unknown" on ~58% of corpus). v2 used per-ticker `trade_count_in_minute > 0` (any non-zero) which fired on sporadic early-premarket trades far ahead of the visible regime change. v3 leverages the paired-leg structure now available via Section 2.9: a real match-start event produces simultaneous trade clusters on BOTH sides of the paired market (RUN's traders and MCD's traders both react to the same line release / odds shift / event news), while sporadic premarket activity tends to be one-sided. Combining the simultaneity requirement with a minimum-trade-count threshold (M=5) filters out single-side noise and isolates the genuine regime change. The producer derives match_start_ts via the v3 signal where possible, falls back to expected_expiration_time only when no such run exists, and emits the method as a column so downstream consumers can filter.

Tuning parameters K and M are spec-level constants exposed in producer code as named defaults; T37c validation gate inspects K=3 / M=5 default behavior on a stratified sample of 50 markets to either confirm the defaults or amend.
