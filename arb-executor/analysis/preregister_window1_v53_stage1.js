...
const out = path.join(repo, ".claude/window1_v53_preregistration_20260819/PRE_REGISTRATION.json");
...
const truthBytes = show(truthCommit, truthPath), censusBytes = show(censusCommit, censusPath), priorBytes = show(priorCommit, priorPath);
const truth = JSON.parse(truthBytes), census = JSON.parse(censusBytes), prior = JSON.parse(priorBytes);
if (truth.rows?.length !== 804 || census.rows?.length !== 1143 || prior.fresh_25?.length !== 25) throw new Error("V53 pre-registration authority census invalid");
...
const seedMaterial = `V53_01_FRESH25|${implementationCommit}`;
...
for (const [code, rows] of rowsByCode) {
  ...
  strata.get(stratum).push({ code, event_id: truthByCode.get(code).event_id, category, census_stamps: stamps, stratum, role: "FRESH_STRATIFIED_COHORT_NOT_IN_V52B_THROUGH_V52R" });
}
...
const receipt = {
  label: "V53_01_STAGE1_PRE_REGISTRATION",
  created_before_grading: true,
  implementation_commit: implementationCommit,
  seed_derivation_law: "SHA256('V53_01_FRESH25|' + V53_IMPLEMENTATION_COMMIT)",
