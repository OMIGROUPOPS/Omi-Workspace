...
const policy = require("./window1_v35_living_rest_evidence_gate.js");
...
const output = path.resolve(arg("--output", path.join(repo, ".claude/window1_live_v4_replay/v35_living_rest_evidence_gate_20260806")));
...
const policyFile = path.join(repo, "arb-executor/analysis/window1_v35_living_rest_evidence_gate.js");
const unitTestFile = path.join(repo, "arb-executor/tests/test_window1_v35_living_rest_evidence_gate.js");
const packageTestFile = path.join(repo, "arb-executor/tests/test_window1_v35_living_rest_evidence_gate_package.js");
...
function simulateW1Mode(baseEvent, tapes, prints, sources, bells, mode, actions, decisions, span, timeline, boundary) {
  ...
  for (const row of timeline) {
    ...
    if (policy.qualifyingAskEvidence(row)) {
      ...
      leg.downward_evidence_rows.push({ ts: row.ts, ordinal: row.ordinal, price: row.ask, kind: "QUALIFYING_ASK_LOW_BAND", receipt: row.receipt });
    }
    ...
    const activeEvidenceFloorCandidates = [receiptLocalEvidenceFloor, runningEvidenceFloor].filter(Number.isFinite);
    const activeEvidenceFloor = activeEvidenceFloorCandidates.length ? Math.min(...activeEvidenceFloorCandidates) : null;
    ...
    if (decision.action === "PLACE_REST" || decision.action === "REPRICE_REST") {
      ...
      leg.first_action ||= detail;
      actions.push({ mode, kind: "FILL", event_id: event.event_id, leg_identity: leg.leg_identity, fill_class: leg.fill_class, entry_cents: leg.entry_cents, receipt: row.receipt, decision_reason: decision.reason, state_label_ignored_for_take_authority: true, active_evidence_floor_cents: activeEvidenceFloor, active_evidence_receipts: activeEvidenceRows.length, displayed_ask: { price_cents: row.ask, size: row.top_ask_size, spread: row.spread, dwell_seconds: row.ask_dwell_seconds, crossed_or_locked: row.bid >= row.ask }, ...w1Clock(row.ts, leg.source, leg.bell, boundary) });
      armW1Sibling(sibling, leg, row, mode, actions, boundary);
...
function differentialV34W1(events, baselineEvents, mode) {
  ...
  for (const event of events) {
    ...
    for (const [id, leg] of Object.entries(event.legs)) {
      ...
      rows.push({ mode, event_id: event.event_id, leg_identity: leg.leg_identity, category: leg.category, price_region: leg.price_region, bell_confidence: event.bell_confidence, disposition, V34_W1: before, V35: after });
    }
  }
  const cells = [...group(rows, (row) => `${row.category}|${row.price_region}|${row.bell_confidence}`).entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([cell, subset]) => ({ cell, legs: subset.length, dispositions: countBy(subset, (row) => row.disposition) }));
...
function bleedDelta(v35Events, v34Events, bleed) {
  const v34By = new Map(v34Events.map((event) => [event.event_id, event]));
  const v35By = new Map(v35Events.map((event) => [event.event_id, event]));
  ...
  for (const before of v34Events) {
    ...
    const after = v35By.get(before.event_id); ensure(after, `bleed event missing ${before.event_id}`);
    const mechanism = mechanismByEvent.get(before.event_id) || "MODE_SPECIFIC_BASELINE_NON_COMPLETION_OUTSIDE_STRICT_BLEED_CENSUS";
    mechanismRows.push({ event_id: before.event_id, category: after.category, starting_price_split: after.starting_price_split, bell_confidence: after.bell_confidence, V34_mechanism: mechanism, V34_completed: before.completed_pair, V35_completed: after.completed_pair, V35_under_par: after.pair_under_par, disposition: after.pair_under_par ? "CONVERTED_TO_UNDER_PAR" : after.completed_pair ? "COMPLETED_NOT_UNDER_PAR" : "NOT_CONVERTED" });
  }
  const newLossRows = v34Events.filter((before) => before.completed_pair && !v35By.get(before.event_id).completed_pair).map((before) => ({ event_id: before.event_id, category: before.category, starting_price_split: before.starting_price_split, bell_confidence: before.bell_confidence, V34_completed: true, V35_completed: false, V35_credited_legs: Object.values(v35By.get(before.event_id).legs).filter((leg) => leg.credited).length }));
  const depthRows = v35Events.flatMap((event) => Object.values(event.legs).filter((leg) => leg.credited).map((leg) => ({ event_id: event.event_id, leg_identity: leg.leg_identity, category: leg.category, price_region: leg.price_region, bell_confidence: event.bell_confidence, V34_mechanism: mechanismByEvent.get(event.event_id) || "V34_COMPLETED_BASELINE", fill_class: leg.fill_class, entry_cents: leg.entry_cents, eventual_print_low_cents: leg.w1_print_backed_achievable_floor_cents, fill_depth_vs_eventual_low_cents: leg.entry_minus_print_backed_floor_cents })));
...
async function causalMain() {
  ...
  for (const commit of [V34_W1_CAUSAL_COMMIT, V34_FULL_LIFE_COMMIT, V34_TRADING_PHASE_COMMIT, V32_COMMIT, R3_COMMIT, RECON_COMMIT, NEARMISS_COMMIT, START_LEDGER_COMMIT, OFFER_COMMIT, BLEED_COMMIT, EXEMPLAR_COMMIT]) requireCommit(commit);
  safeClean(output);
  const baseEvents = loadBaseEvents(); ensure(baseEvents.length === 804, "D must be 804");
  ...
  process.stderr.write("V35_STAGE print_spool_start\n");
  const printLoad = buildPrintSpool(tickers);
  process.stderr.write("V35_STAGE print_spool_complete\n");
  ...
  for (const baseEvent of baseEvents) {
    ...
    if (strictEvents.length % 50 === 0) process.stderr.write(`V35_STAGE replay_events_${strictEvents.length}\n`);
  }
  printLoad.cleanup();
  ...
  const binding = { schema_version: "window1-v35-living-rest-evidence-gate-control-v1", machine: { architecture: "V34_DUAL_SIDE_RESIDENCY_WITH_THREE_AUTHORIZED_V35_REPAIRS", V34_W1_baseline_commit: V34_W1_CAUSAL_COMMIT, V32_lineage_commit: V32_COMMIT, policy_path: path.relative(repo, policyFile).replaceAll("\\", "/"), policy_sha256: fileHash(policyFile), clock_decision_inputs: [], receipt_driven_evidence_aging_only: true }, repairs: { living_rest: "EVERY_BOOK_RECEIPT_REANCHORS_TO_MIN(BEST_BID_MINUS_1,PAIR_CAP)_UP_OR_DOWN_NO_SPREAD_GATE", evidence_take_gate: "DWELL_10_SIZE_5_CAP_AND_ASK_AT_OR_BELOW_RUNNING_PRICE_BEARING_EVIDENCE_FLOOR_STATE_LABEL_IGNORED", no_first_flicker_overpay: `A_NEW_QUALIFYING_ASK_FLOOR_CANNOT_AUTHORIZE_ITS_OWN_TAKE_WHILE_FALLING_QUOTE_OR_PRESSURE_EVIDENCE_REMAINS_UNABSORBED_INSIDE_THE_EXISTING_${policy.LOOKBACK_SECONDS}S_RECEIPT_HORIZON; ESTABLISHED_FLOORS_TAKE_IMMEDIATELY; REASK_ON_EACH_BOOK_RECEIPT; NO_TIMER` }, window_law: { left: "FIRST_TWO_SIDED_BOOK_ON_EITHER_EXPRESSION", right_precedence: ["exact_start_utc", "known_live_by_utc", "schedule_bound_utc"], hard_edge: "NO_REST_WALK_NO_TAKE_NO_FILL_NO_CAP_ARM_NO_STATE_UPDATE_AFTER_EDGE", boundary_is_reporting_and_admission_only_not_AIM_input: true }, close_law: { role: "TELEMETRY_ONLY", grading_reads: 0, settlement_close_reads: 0 }, comparisons: { V34_W1_strict_completed_under_par: 254, V34_W1_census_completed_under_par: 279, R3_original_joint_reference_only: 68, R3_same_V3_window_artifact: "R3_SAME_WINDOW_COMPARISON.json", operator_named_under_par_offer_census_adjusted: 680 } };
  ...
  const core = {
    ...
    "CENSUS_PRICED_65D49B5D_BINDING.json": canonical({ commit: NEARMISS_COMMIT, path: NEARMISS_PATH, source_sha256: sha(nearBytes), source: { waited_and_lost: near.summary.conservation.waited_and_lost_censused, one_cent_near_miss_rests: near.summary.totals.with1, v32_joint: near.summary.conservation.V32_executable_joint, model_free_ceiling: near.summary.conservation.model_free_ceiling_joint }, V35_application: "DYNAMIC_ACTIVE_V35_LIVING_REST_TRAJECTORY_INSIDE_V3_PRE_MATCH_SPAN_ONLY; FIRST_LAWFUL_ONE_CENT_SELLER_PRINT_CONVERTS_AT_CURRENT_REST_IN_CENSUS_COLUMN" }),
