#!/usr/bin/env python3
"""One-shot CASUKA D1-D3 deployment ceremony controller.
...
import casuka_deployment_authorization_verifier_v2 as verifier
...
def confirm_t0_quiet_minute(
    ...
    *,
    ...
    before = collect_operational_state(repo, control)
    if not before.get("audit_halt_clear"):
        raise CeremonyError("T-0 audit halt state is not CLEAR")
    quiet_seconds = control["outcome_proof"]["t0"]["quiet_minute"]["seconds"]
    if (
        isinstance(quiet_seconds, bool)
        or not isinstance(quiet_seconds, int)
        or quiet_seconds < 60
    ):
        raise CeremonyError("invalid quiet-minute duration")
    sleep(quiet_seconds)
    after = collect_operational_state(repo, control)
    if not after.get("audit_halt_clear"):
        raise CeremonyError("T-0 audit halt ceased to be CLEAR")
    if before.get("log_path") != after.get("log_path"):
        raise CeremonyError("live log rolled during quiet-minute confirmation")
    if before.get("conception_event_count") != after.get(
        "conception_event_count"
    ):
        raise CeremonyError("conception burst observed during quiet minute")
    if after.get("sampled_at_epoch", 0) - before.get(
        "sampled_at_epoch", 0
    ) < quiet_seconds:
        raise CeremonyError("quiet minute was not fully observed")
    return {"before": before, "after": after, "quiet_seconds": quiet_seconds}
...
def materialize_remote_control(
    ...
    remote_rollback_script = (
        remote_results + "/casuka_deployment_rollback_v2.sh"
    )
    outcome = control["outcome_proof"]
    rollback = control["rollback"]
    script = f"""set -euo pipefail
...
def invoke_rollback_artifact(
    ...
    args = (
        ...
        deployment["backup_path"],
        deployment["service"],
        deployment["boot_command"],
        deployment["preimage"]["sha256"],
        str(deployment["preimage"]["bytes"]),
        deployment["remote_arb"],
    )
    started = time.time()
    ...
    try:
        ...
        journal.append(
            ...
            "PASS",
            {
                ...
                "quiet_minute_seconds": t0["quiet_seconds"],
                "no_conception_burst": True,
