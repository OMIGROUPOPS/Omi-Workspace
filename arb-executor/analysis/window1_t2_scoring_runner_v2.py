#!/usr/bin/env python3
"""Stdout-safe runner for the corrective T2 scoring package V2."""
...
VERSION = "window1-t2-scoring-runner-v2"
...
def validate_package(
    ...
    *,
    ...
    if (
        ...
        != "window1-t2-scoring-input-manifest-v2"
...
def authorize_execute(
    ...
    if git(repo, "rev-parse", "HEAD") != git(
        repo, "rev-parse",
        "refs/remotes/origin/codex/window1-t2-scoring-package-v2-prerun",
    ):
...
def validation_only(repo: Path, package_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": VERSION + "-validation-only-v2",
