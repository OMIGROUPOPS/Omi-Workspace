...
} = require("./window1_v29r_mirror_armed_second_side_policy.js");
...
const output = path.resolve(arg("--output", path.join(repo, ".claude/window1_live_v4_replay/v29r_mirror_armed_second_side_20260804")));
...
const policyPath = path.join(repo, "arb-executor/analysis/window1_v29r_mirror_armed_second_side_policy.js");
const interimPolicyPath = path.join(repo, "arb-executor/analysis/window1_interim_elimination_v13.js");
const policyTestPath = path.join(repo, "arb-executor/tests/test_window1_v29r_mirror_armed_second_side_policy.js");
const packageTestPath = path.join(repo, "arb-executor/tests/test_window1_v29r_mirror_armed_second_side_package.js");
...
function build() {
  ...
  const completionTargets = new Set(baseTrace.rows.filter((row) => row.first_flag?.layer === "COMPLETION" && row.tape_offered_afterward?.negative_delta_available_past_flag === "YES").map((row) => row.leg_identity));
  ensure(carriedTargets.size === 144 && completionTargets.size === 237, `target mass mismatch ${carriedTargets.size}/${completionTargets.size}`);
  const targetClass = new Map([...carriedTargets].map((id) => [id, "V28_CARRIED"]).concat([...completionTargets].map((id) => [id, "V28_COMPLETION_MIRROR_FN"])));
  const receipts = [], changedLegs = new Set(), changedEvents = new Set();
  for (const event of events) {
    const legs = Object.values(event.legs), filled = legs.filter((l) => l.credited && Number.isFinite(fillTs(l))).sort((a, b) => fillTs(a) - fillTs(b) || a.leg_identity.localeCompare(b.leg_identity));
    if (!filled.length) continue;
    const first = filled[0], armTs = fillTs(first), target = legs.find((l) => l.leg_id !== first.leg_id); ensure(target, `missing sibling ${event.event_id}`);
    if (!targetClass.has(target.leg_identity)) continue;
    const incumbentActive = (target.acted && Number.isFinite(target.action_timestamp_epoch) && target.action_timestamp_epoch <= armTs) || (target.credited && fillTs(target) <= armTs);
    const source = sources.get(target.ticker), rows = incumbentActive ? [] : loadTicks(target.ticker), armBook = incumbentActive ? null : currentAt(rows, armTs), arm = armOtherSide({ incumbentActive, firstFillCents: first.entry_cents, ownLiveAskCents: armBook?.ask ?? null });
    const groupKey = `${target.category}|${regionForShape(target.price_region)}`, shapeGroup = library.groups[groupKey];
    const stopBefore = target.acted && Number.isFinite(target.action_timestamp_epoch) && target.action_timestamp_epoch > armTs ? target.action_timestamp_epoch : source.right + 1;
    const armOrdinal = armBook?.qualified_ask_descent_count ?? null; let evaluated = 0, release = null, terminal = null;
    if (arm.state === "HOLD" && shapeGroup) {
      terminal = evolve(shapeGroup.shapes, rows, (row, survivors) => {
        if (row.ts <= armTs || row.ts >= stopBefore) return null; evaluated += 1;
        ...
        const coherent = usable.length > 0 && verdicts.every((x) => x.verdict === "FLOOR");
        const decision = releaseOtherSide({ armed: arm, bid: row.bid, ask: row.ask, spread: row.spread, askDwellSeconds: row.ask_dwell_seconds, displayedAskSize: row.top_ask_size, strictlyLaterReceipt: row.ts > armTs, ownLaterQualifiedDescent: Number.isInteger(armOrdinal) && row.qualified_ask_descent_count > armOrdinal && row.ask < armBook.ask, coherentOrdinalResolved: coherent });
        terminal = { state: decision.state, reason: decision.reason, row: { ts: row.ts, receipt: row.receipt, bid: row.bid, ask: row.ask, spread: row.spread, ask_dwell_seconds: row.ask_dwell_seconds, top_ask_size: row.top_ask_size, qualified_ask_descent_count: row.qualified_ask_descent_count }, surviving_shape_ids: survivors.map((x) => x.shape_id), usable_shape_ids: usable.map((x) => x.shape_id), ordinal_verdicts: verdicts };
        if (decision.state === "PLACE") { release = { ...terminal, decision }; return terminal; } return null;
      });
    }
    const receipt = { target_class: targetClass.get(target.leg_identity), event_id: event.event_id, category: event.category, starting_price_region: event.starting_price_split, armed_leg_identity: target.leg_identity, armed_ticker: target.ticker, first_filled_leg_identity: first.leg_identity, first_fill_cents: first.entry_cents, first_fill_receipt: first.fill?.evidence_receipt ?? first.fill?.receipt ?? first.placement?.action_receipt ?? null, arm_clock: clocks(armTs, source, bells.get(event.event_id)), live_book_at_arm: armBook ? { receipt: armBook.receipt, timestamp_epoch: armBook.ts, bid: armBook.bid, ask: armBook.ask, spread: armBook.spread, top_ask_size: armBook.top_ask_size, qualified_ask_descent_count: armBook.qualified_ask_descent_count } : null, incumbent_action_timestamp_epoch: target.action_timestamp_epoch, incumbent_active_at_arm: incumbentActive, arm, evaluation: { cadence: "EVERY_CAUSAL_OWN_BOOK_RECEIPT_IN_EVENT_ORDER", polling_interval_seconds: null, causal_receipts_evaluated_after_arm_before_incumbent_or_right_edge: evaluated, elapsed_time_policy_inputs: [], stop_before_incumbent_action_timestamp_epoch: stopBefore <= source.right ? stopBefore : null }, shape_group: groupKey, release: release ? { ...release, row: { ...release.row, clocks: clocks(release.row.ts, source, bells.get(event.event_id)) } } : null, terminal: release ? "RELEASED_AND_CREDITED" : arm.state === "ABSTAIN" ? arm.reason : !shapeGroup ? "SHAPE_GROUP_NOT_BOUND_OVERLAY_ABSTAINS" : terminal?.reason || "HELD_TO_INCUMBENT_OR_WINDOW_END" };
  ...
  const diffRows = resultLegs.map((x) => ({ leg_identity: x.leg_identity, changed: changedLegs.has(x.leg_identity), V28_semantic_sha256: semanticHash(baseByLeg.get(x.leg_identity)), V29R_semantic_sha256: semanticHash(x) }));
  ensure(diffRows.filter((x) => x.changed).every((x) => x.V28_semantic_sha256 !== x.V29R_semantic_sha256), "changed stream unchanged"); ensure(diffRows.filter((x) => !x.changed).every((x) => x.V28_semantic_sha256 === x.V29R_semantic_sha256), "non-target stream drift");
  const eventById = new Map(events.map((e) => [e.event_id, e])), receiptByLeg = new Map(receipts.map((x) => [x.armed_leg_identity, x]));
  const traceRows = baseTrace.rows.map((base) => { const event = eventById.get(base.event_id), leg = Object.values(event.legs).find((x) => x.leg_identity === base.leg_identity), receipt = receiptByLeg.get(base.leg_identity) || null; if (!changedEvents.has(base.event_id)) return { ...clone(base), variant: "V29R_MIRROR_ARMED_SECOND_SIDE", v29r_mirror_arm_effect: receipt ? { arm: receipt.arm, terminal: receipt.terminal } : null }; return { ...clone(base), variant: "V29R_MIRROR_ARMED_SECOND_SIDE", layer_results_in_execution_order: LAYERS.map((layer, i) => ({ order: i + 1, layer, result: "PASS" })), first_flag: null, tape_offered_afterward: null, final_state: event.joint_objective_pass_audited_close ? "JOINT-CAPTURED" : "carried", leg_action_state: { acted: leg.acted, credited: leg.credited, entry_cents: leg.entry_cents, honest_fill_class: leg.honest_fill_class, terminal_reason: leg.terminal_reason }, event_result: { completed_pair: event.completed_pair, combined_entry_cents: event.combined_entry_cents, pair_under_par: event.pair_under_par, both_legs_strictly_below_audited_close: event.both_legs_strictly_below_audited_close, joint_objective_pass: event.joint_objective_pass_audited_close }, v29r_mirror_arm_effect: receipt ? { arm: receipt.arm, release: receipt.release, terminal: receipt.terminal } : { sibling_armed_other_leg: true } }; });
  ensure(traceRows.length === 1608 && new Set(traceRows.map((x) => x.leg_identity)).size === 1608, "trace conservation");
  const trace = { schema: "WINDOW1_DECISION_TRACE_1608_V29R", generated_utc: "2026-08-04T00:00:00Z", variant: "V29R_MIRROR_ARMED_SECOND_SIDE", scope: { events: 804, legs: 1608 }, source_binding: { V28_commit: "3339f30dc9d3136788617bf0e5456708008b845b", V28_trace_sha256: fileHash(tracePath) }, conservation: { rows: traceRows.length, one_row_per_leg: true }, rollup: rollup(traceRows), rows: traceRows };
  ...
  const gradedCeiling = ceilingRows.map((row) => { const old = oldByTicker.get(row.leg_ticker), now = newByTicker.get(row.leg_ticker), changed = Boolean(now && changedLegs.has(now.leg_identity)), converted = row.verdict === "CONVERTIBLE" && changed && (row.class === "b_FN_mirror" ? !old.credited && now.credited : Number.isInteger(now.audited_close_cents) && now.entry_cents < now.audited_close_cents); return { class: row.class, ticker: row.leg_ticker, verdict: row.verdict, changed_by_V29R: changed, converted_by_V29R: converted, V28_entry_cents: old?.credited ? old.entry_cents : null, V29R_entry_cents: now?.credited ? now.entry_cents : null, audited_close_cents_grading_only: now?.audited_close_cents ?? null }; });
  const byCeilingClass = (name, ceiling) => { const rows = gradedCeiling.filter((x) => x.class === name), convertible = rows.filter((x) => x.verdict === "CONVERTIBLE"); return { independent_target_rows: rows.length, independent_convertible_ceiling: ceiling, V29R_converted: convertible.filter((x) => x.converted_by_V29R).length, changed_convertible_rows: convertible.filter((x) => x.changed_by_V29R).length, conversion_share_of_independent_ceiling: ceiling ? convertible.filter((x) => x.converted_by_V29R).length / ceiling : null }; };
  ...
  write(path.join(output, "SCORECARD.json"), canonical({ variant: "V29R_MIRROR_ARMED_SECOND_SIDE", V28_floor: baseScore.V28_score, V29R_score: sc.aggregate, delta, joint_non_regression: sc.aggregate.joint_objective_pairs >= 65, close_bar_policy_input: false, joint_law_role: "EX_POST_GRADE_ONLY", category_x_starting_price_region: sc.category_x_starting_price_region }));
  ...
  write(path.join(output, "INDEPENDENT_CEILING_COMPARISON.json"), canonical({ authority: { commit: ceilingCommit, csv_path: ceilingCsvGitPath, csv_sha256: sha(ceilingCsv), summary_path: ceilingSummaryGitPath, summary_sha256: sha(ceilingSummaryText) }, side_by_side: { completion_mirror_FN_class: byCeilingClass("b_FN_mirror", 29), carried_class: byCeilingClass("a_carried_pair", 119) }, denominator_compatibility: { independent_FN_237: "V11_NON_ACTION_CLASS", V28_completion_trace_237: "ALL_237_ALREADY_CREDITED; ZERO_CURRENTLY_REFUSED_OR_UNPLACED", independent_carried_296: "BOTH_LEGS_OF_148_V11_CARRIED_PAIRS", V28_carried_target_144: "POSITIVE_DELTA_LEG_ONLY_IN_144_V28_STRICT_CARRIED_PAIRS" }, rows: gradedCeiling }));
  write(path.join(output, "TARGET_MASS_AND_CONVERSION.json"), canonical({ requested_target_mass: { V28_carried_positive_legs: carriedTargets.size, V28_completion_layer_false_negative_rows: completionTargets.size }, causal_eligibility: { carried_positive_leg_was_second_fill: receipts.filter((x) => x.target_class === "V28_CARRIED").length, carried_already_active_at_arm: receipts.filter((x) => x.target_class === "V28_CARRIED" && x.incumbent_active_at_arm).length, carried_armed: receipts.filter((x) => x.target_class === "V28_CARRIED" && x.arm.state === "HOLD").length, carried_released: receipts.filter((x) => x.target_class === "V28_CARRIED" && x.release).length, completion_rows_already_credited_in_V28: completionTargetLegs.filter((x) => x?.credited).length, completion_rows_currently_uncredited_in_V28: completionTargetLegs.filter((x) => !x?.credited).length, completion_rows_armed: receipts.filter((x) => x.target_class === "V28_COMPLETION_MIRROR_FN" && x.arm.state === "HOLD").length, completion_rows_released: receipts.filter((x) => x.target_class === "V28_COMPLETION_MIRROR_FN" && x.release).length }, score_effect: delta, ruling: "OVERLAY_ACTS_ONLY_WHEN OTHER SIDE IS REFUSED OR NOT YET PLACED AT FIRST CREDITED FILL; ALREADY-CREDITED TRACE ROWS ARE NOT REWRITTEN" }));
  ...
  write(path.join(output, "RULINGS_RECEIPT.json"), canonical({ dwell_10_seconds: { status: "RATIFIED_PROVISIONALLY_AS_IS", reason: "COMPARABILITY_WITH_ALL_PRIOR_RESULTS", sensitivity_study: "DEFERRED_TO_HOLDOUT_ERA" }, pair_carry_clock: { status: "PROHIBITED", wall_clock_limit: null, elapsed_time_policy_inputs: [] }, V29R: { close_bar: "REMOVED_EX_POST_NOT_BOUND", aim: "min(99-first_fill, own_lawful_live_book_floor)", sequential_and_symmetric: true, joint_law: "EX_POST_GRADE_ONLY" }, orphans_unchanged: [{ name: "DEAD_SPREAD_THRESHOLD", value: 20, status: "AWAITING_RULING" }, { name: "SPREAD_LE_2_POSTING_GATE", value: 2, status: "AWAITING_RULING" }] }));
  ...
  write(path.join(output, "TEST_RESULTS.json"), canonical({ status: "PASS", commands: ["node arb-executor/tests/test_window1_v29r_mirror_armed_second_side_policy.js", "node arb-executor/tests/test_window1_v29r_mirror_armed_second_side_package.js", "node arb-executor/tests/test_window1_v28_anchor_cap_stack_package.js"], omissions: 0, deselections: 0 }));
  ...
  write(path.join(output, "REPORT.md"), `# V29-R — mirror-armed second side\n\nV29-R is an additive overlay on ratified V28 (JOINT 65). The first credited fill on either side arms the other side at min(99-first_fill, its own causal live ask at arm). Release is evaluated on every subsequent own-book receipt and requires its own later decline, coherent V13 ordinal, spread <=1, ten-second dwell, displayed size >=5, and ask <= aim. Audited close is grading-only and never enters the aim.\n`);
  process.stdout.write(canonical({ status: "BUILT", V28_joint: 65, V29R_joint: sc.aggregate.joint_objective_pairs, changed_legs: changedLegs.size, released: receipts.filter((x) => x.release).length, release_delay_median_minutes: delayDistribution.median, FN_conversion: byCeilingClass("b_FN_mirror", 29), carried_conversion: byCeilingClass("a_carried_pair", 119) }));
}
