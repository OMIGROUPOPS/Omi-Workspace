...
1. **T32a — Layer C v1 specification.** Spec doc at docs/layer_c_spec.md per LESSONS B16 layered-realism discipline. Decisions on canonical fee assumption (empirical from kalshi_fills_history.json, maker-zero confirmed by sample fill on KXATPCHALLENGERMATCH-26APR28WALCUI-WAL with fee_cost=$0.00, is_taker=False), policy class taker-exposure handling (limit-bearing maker-only vs time-stop-bearing taker-on-horizon-exit), validation gate (4-check shape mirroring T31c). Pure design doc, no producer code yet.
...
3. **T32c — Layer C v1 coherence read.** check_layer_c_v1_coherence.py + report at layer_c_v1/coherence_report.md. 4-check shape per T31c precedent: (1) realized_capture_after_fees ≤ capture (fees non-negative); (2) limit-bearing policies have realized_capture == capture exactly (maker fees == $0 by canonical assumption); (3) time-stop-bearing policies show fee loss proportional to Kalshi schedule at horizon-minute price; (4) rank correlation between Layer B capture_mean and Layer C realized_capture per cell ≥ 0.99. PASS gates T32d (v2 promotion) and downstream live-trading planning.
