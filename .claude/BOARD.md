...
1. ~~Post-gun forensic~~ ✓ **DONE 07-08 night** (`.claude/postgun_20260708/POST_GUN_FORENSIC.md` + vault entry)
2. **Sequential floor recompute** (queued #15)
3. **Bell recovery** (the gun's blind class: detection 17–62%, median lag 31–46 min — the forensic's numbers are its spec input)
...
13. **ORPHAN-CLASS FIX** — boot re-adopts everything carrying our order fingerprint (client_order_id lineage) so pre-restart own orders never classify manual; kills the class consolidated in LIVING_VAULT "THE ORPHAN CLASS" (07-08). Every restart mints a new batch until this ships.
