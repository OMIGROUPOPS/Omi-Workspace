...
import window1_round3_grid_runner as grid  # noqa: E402
...
def package_fixture() -> tuple[dict, dict]:
    ...
    package = {
        ...
        "controlling_identities": {
            "round3_prerun": grid.AUTHORIZED_PARENT,
            "round2_results": grid.ROUND2_RESULTS,
            "round2_results_audit": {
                "commit": grid.ROUND2_RESULTS_AUDIT,
                "report_path": grid.ROUND2_AUDIT_REPORT_PATH,
                "report_blob_oid": grid.ROUND2_AUDIT_REPORT_BLOB_OID,
            },
            "round3_prerun_audit": {
                "commit": grid.ROUND3_PRERUN_AUDIT,
                "report_path": grid.ROUND3_AUDIT_REPORT_PATH,
                "report_blob_oid": grid.ROUND3_AUDIT_REPORT_BLOB_OID,
            },
        },
        ...
        "candidate_ids": list(grid.FROZEN_CANDIDATES),
        "expected_progress_sequence": grid.expected_progress_sequence(),
        "expected_progress_sequence_sha256": grid.canonical_sha256(
            grid.expected_progress_sequence()
        ),
        "expected_output_inventory": grid.expected_output_inventory(),
        "expected_output_inventory_sha256": grid.canonical_sha256(
            grid.expected_output_inventory()
        ),
        "git_inputs": [
            {
                "path": path,
        ...
        "frozen_source_receipts_sha256": grid.canonical_sha256([]),
...
def test_execution_identity_and_commands_are_fully_fixed():
    ...
    assert grid.EXACT_EXECUTION_COMMAND == (
        ...
        "window1_round3_grid_runner.py --repo . --package "
...
def test_any_dirty_worktree_status_is_refused(monkeypatch, tmp_path):
    monkeypatch.setattr(
        grid,
        "git",
        lambda repo, *args: b"?? preserved_unrelated_evidence/\n",
    )
    assert grid.unexpected_worktree_status(tmp_path) == [
        "?? preserved_unrelated_evidence/"
    ]
