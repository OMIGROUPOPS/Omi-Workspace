...
const output = path.resolve(arg("--output", path.join(repo, ".claude/window1_live_v4_replay/v34_w1_causal_capture_measurement_20260805")));
...
const packageTestFile = path.join(repo, "arb-executor/tests/test_window1_v34_w1_causal_measurement.js");
...
function simulateW1Mode(baseEvent, tapes, prints, sources, bells, mode, actions, decisions, span, timeline, boundary) {
  ...
  for (const row of timeline) {
    ...
    const decision = policy.decide({ state: combined.state, book: row, activeTarget: before, pairCap: leg.pair_cap_cents, runningTradeLow: leg.running_trade_low, runningQualifyingAskLow: leg.running_qualified_ask_low });
    leg.prior_book = row;
    ...
    if (decision.action === "PLACE_REST" || decision.action === "REPRICE_REST_DOWN") {
      ...
      leg.fill_timestamp_epoch = row.ts;
      leg.fill_class = "PROVEN_TAKER_R3_QUALIFIED_SETTLED_ASK";
      leg.terminal_reason = leg.fill_class;
      leg.first_action ||= detail;
      actions.push({ mode, kind: "FILL", event_id: event.event_id, leg_identity: leg.leg_identity, fill_class: leg.fill_class, entry_cents: leg.entry_cents, receipt: row.receipt, displayed_ask: { price_cents: row.ask, size: row.top_ask_size, spread: row.spread, dwell_seconds: row.ask_dwell_seconds, crossed_or_locked: row.bid >= row.ask }, ...w1Clock(row.ts, leg.source, leg.bell, boundary) });
      armW1Sibling(sibling, leg, row, mode, actions, boundary);
    }
...
async function causalMain() {
  ...
  for (const commit of [V34_FULL_LIFE_COMMIT, V34_TRADING_PHASE_COMMIT, V32_COMMIT, R3_COMMIT, RECON_COMMIT, NEARMISS_COMMIT, START_LEDGER_COMMIT, OFFER_COMMIT]) requireCommit(commit);
  safeClean(output);
  const baseEvents = loadBaseEvents(); ensure(baseEvents.length === 804, "D must be 804");
  ...
  process.stderr.write("V34_W1_STAGE print_spool_start\n");
  const printLoad = buildPrintSpool(tickers);
  process.stderr.write("V34_W1_STAGE print_spool_complete\n");
  ...
  for (const baseEvent of baseEvents) {
    ...
    if (strictEvents.length % 50 === 0) process.stderr.write(`V34_W1_STAGE replay_events_${strictEvents.length}\n`);
  }
  printLoad.cleanup();
  ...
  const fullStrictBytes = gitShow(V34_FULL_LIFE_COMMIT, FULL_LIFE_STRICT_PATH), fullCensusBytes = gitShow(V34_FULL_LIFE_COMMIT, FULL_LIFE_CENSUS_PATH);
  const strictDiff = differentialFullLife(strictEvents, readRowsBytes(fullStrictBytes), "STRICT_LAW"), censusDiff = differentialFullLife(censusEvents, readRowsBytes(fullCensusBytes), "CENSUS_PRICED");
  ...
  const scorecard = { schema_version: "window1-v34-w1-causal-close-free-score-v1", comparison_floor: { R3_original_joint_reference_only: 68, R3_same_V3_window: r3Comparison.same_V3_span_metrics, operator_named_under_par_offer_census_adjusted: 680 }, STRICT_LAW: strictScore, CENSUS_PRICED: censusScore, conservation: { D_each_column: [strictScore.aggregate.D, censusScore.aggregate.D], expected_D: 804, pass: strictScore.aggregate.D === 804 && censusScore.aggregate.D === 804 } };
  ...
  const binding = { schema_version: "window1-v34-w1-causal-control-v1", machine: { architecture: "EXACT_V34_DUAL_SIDE_RESIDENCY_MACHINE", V32_lineage_commit: V32_COMMIT, V34_full_life_package_commit: V34_FULL_LIFE_COMMIT, V34_trading_phase_package_commit: V34_TRADING_PHASE_COMMIT, policy_path: path.relative(repo, policyFile).replaceAll("\\", "/"), policy_sha256: fileHash(policyFile), clock_decision_inputs: [] }, window_law: { left: "FIRST_TWO_SIDED_BOOK_ON_EITHER_EXPRESSION", right_precedence: ["exact_start_utc", "known_live_by_utc", "schedule_bound_utc"], hard_edge: "NO_REST_WALK_NO_TAKE_NO_FILL_NO_CAP_ARM_NO_STATE_UPDATE_AFTER_EDGE", boundary_is_reporting_and_admission_only_not_AIM_input: true }, close_law: { role: "TELEMETRY_ONLY", grading_reads: 0, settlement_close_reads: 0 }, comparisons: { R3_original_joint_reference_only: 68, R3_same_V3_window_artifact: "R3_SAME_WINDOW_COMPARISON.json", operator_named_under_par_offer_census_adjusted: 680 } };
  ...
  const core = {
    ...
    "ACTION_AND_FILL_TRACE.jsonl.gz": gzipRows(actions),
    ...
    "DIFFERENTIAL_VS_V34_FULL_LIFE.json": canonical({ STRICT_LAW: { ...strictDiff, rows: undefined }, CENSUS_PRICED: { ...censusDiff, rows: undefined }, conservation: { strict_rows: strictDiff.rows.length, census_rows: censusDiff.rows.length, expected_each: 1608, pass: strictDiff.rows.length === 1608 && censusDiff.rows.length === 1608 } }),
    "DIFFERENTIAL_VS_V34_FULL_LIFE_LEDGER.jsonl.gz": gzipRows([...strictDiff.rows, ...censusDiff.rows]),
    "WAITED_AND_LOST_CENSUS.json": canonical({ STRICT_LAW: { total: strictDiff.waited_and_lost_legs, category_x_price_region_x_bell_confidence: strictDiff.category_x_price_region_x_bell_confidence }, CENSUS_PRICED: { total: censusDiff.waited_and_lost_legs, category_x_price_region_x_bell_confidence: censusDiff.category_x_price_region_x_bell_confidence }, conservation: { strict_legs: strictDiff.rows.length, census_legs: censusDiff.rows.length, expected_each: 1608, pass: true } }),
    "ARNROM_NAMED_RECEIPT.json": canonical({ span: arnSpan, STRICT_LAW: strictArn, CENSUS_PRICED: censusArn }),
    "CLOSE_TELEMETRY_ISOLATION_RECEIPT.json": canonical(closeIsolation),
    "CENSUS_PRICED_65D49B5D_BINDING.json": canonical({ commit: NEARMISS_COMMIT, path: NEARMISS_PATH, source_sha256: sha(nearBytes), source: { waited_and_lost: near.summary.conservation.waited_and_lost_censused, one_cent_near_miss_rests: near.summary.totals.with1, v32_joint: near.summary.conservation.V32_executable_joint, model_free_ceiling: near.summary.conservation.model_free_ceiling_joint }, V34_W1_application: "DYNAMIC_ACTIVE_V34_REST_TRAJECTORY_INSIDE_V3_PRE_MATCH_SPAN_ONLY; FIRST_LAWFUL_ONE_CENT_SELLER_PRINT_CONVERTS_AT_REST_IN_CENSUS_COLUMN" }),
    ...
    "SOURCE_HASH_MANIFEST.json": canonical({ public_committed: Object.fromEntries([builderFile, policyFile, unitTestFile, packageTestFile, floorFile, baseTraceFile, quoteFile, bellFile, spoolHelperFile, spoolHelperSourceFile].map((file) => [path.relative(repo, file).replaceAll("\\", "/"), { sha256: fs.existsSync(file) ? fileHash(file) : null, bytes: fs.existsSync(file) ? fs.statSync(file).size : null }])), git_bound: { [START_LEDGER_PATH]: { commit: START_LEDGER_COMMIT, sha256: sha(startBytes), bytes: startBytes.length }, [R3_EVENT_PATH]: { commit: R3_COMMIT, sha256: sha(r3Bytes), bytes: r3Bytes.length }, [FULL_LIFE_STRICT_PATH]: { commit: V34_FULL_LIFE_COMMIT, sha256: sha(fullStrictBytes), bytes: fullStrictBytes.length }, [FULL_LIFE_CENSUS_PATH]: { commit: V34_FULL_LIFE_COMMIT, sha256: sha(fullCensusBytes), bytes: fullCensusBytes.length }, [RECON_PATH]: { commit: RECON_COMMIT, sha256: sha(reconBytes), bytes: reconBytes.length }, [NEARMISS_PATH]: { commit: NEARMISS_COMMIT, sha256: sha(nearBytes), bytes: nearBytes.length }, [OFFER_PATH]: { commit: OFFER_COMMIT, sha256: sha(offerBytes), bytes: offerBytes.length } }, private_full_life_prints: printLoad.receipt, private_full_life_books: tapeHashes, public_tape_manifest: { sha256: fileHash(publicTapeManifest), bytes: fs.statSync(publicTapeManifest).size } }),
  };
  for (const [name, bytes] of Object.entries(core)) write(name, bytes);
  write("REPORT.md", `# V34-W1 causal capture measurement\n\nThe frozen V34 dual-side residency machine is replayed from each game's first two-sided book through the hard PRE-MATCH edge selected from REAL_START_LEDGER_V3 by exact_start_utc, else known_live_by_utc, else schedule_bound_utc. No action, state update, fill, cap arm, rest walk, or take occurs after that edge. Close values are telemetry only and are proven grade-invariant by deletion.\n\n- STRICT completed / under par: ${strictScore.aggregate.completed_pairs} / ${strictScore.aggregate.pairs_under_par}.\n- CENSUS completed / under par: ${censusScore.aggregate.completed_pairs} / ${censusScore.aggregate.pairs_under_par}.\n- R3 same-window completed / under par: ${r3Comparison.same_V3_span_metrics.completed_pairs} / ${r3Comparison.same_V3_span_metrics.pairs_under_par}; original close-based reference: 68.\n- Operator-named census-adjusted under-par offer: 680.\n- STRICT maker / taker: ${strictFillSplit.aggregate.maker} / ${strictFillSplit.aggregate.taker}.\n- CENSUS maker / taker / one-cent census conversion: ${censusFillSplit.aggregate.maker} / ${censusFillSplit.aggregate.taker} / ${censusFillSplit.aggregate.census_priced}.\n- Post-edge machine rows: ${hardEdgeViolations.length + decisionStats.post_edge}.\n- Full decision rows: ${decisionStats.total}; compact rows: ${strictTrace.length}+${censusTrace.length}.\n- Full-life waited-and-lost at the hard edge: STRICT ${strictDiff.waited_and_lost_legs}; CENSUS ${censusDiff.waited_and_lost_legs}.\n\nThe supplied short identity 84b455c5 does not resolve as a Git object after fetch. The actual 804-row ledger is bound through Git path history at ${START_LEDGER_COMMIT}, SHA-256 ${sha(startBytes)}; no short identity was fabricated.\n`);
  const compareNames = [...Object.keys(core), ...fullTraceParts.parts.map((row) => row.name), "REPORT.md"].sort();
  if (compare) {
