...
- **C-CONCEPTION-HORIZON (T-8h early bound, INTERIM SCAFFOLDING — `6d9ebe31`, deployed 07-08 afternoon)** — no conception/resting entry bid beyond T-8h honest-anchored; router-defer + manage-sweep + chokepoint-refusal; 12-bid pre-deploy census swept (GILOBR exhibit). **The bound is a map edge, NEVER a target; it moves on the early-canvas study + operator ruling.** Watches: `conception_beyond_horizon` audit flag (must stay 0), `conception_horizon_refused` dedup counter, swept pairs re-conceiving at T-8h. See LIVING_VAULT 07-08 afternoon entry.
