...
def test_all_fixture_candidates_and_final_receipt_survive_console_loss(
    ...
    for filename in grid.expected_output_inventory():
        if filename in ("PROGRESS.log", "EXECUTION_MANIFEST.json"):
            continue
        (tmp_path / filename).write_text(
            "synthetic fixture only; no performance\n",
            encoding="utf-8",
            newline="\n",
        )
    grid.write_json(tmp_path / "EXECUTION_MANIFEST.json", {
