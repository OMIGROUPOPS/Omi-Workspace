...
GAP, WIN = 6, 3
FLOOR = 0.50          # v14: COND floor (firewall on the cause). 'majority of visits agree on the center.'
                      # was occ*cond product (v13) -> double-charged occupancy, parked cheap engine-start.
...
def boot_modes(c):
    ...
    for i, g in enumerate(groups):
        ...
        modes.append(dict(center=center, occ=round(occ,3), cond=round(cond,3),
                          oc=round(oc,3), reach=bool(reach),
                          resolved=bool(oc >= FLOOR),              # legacy occ*cond product flag (v13)
                          resolved_cond=bool(cond >= FLOOR)))      # v14 FIREWALL: cond is the CAUSE
...
def fireable_mode(c):
    """Lowest mode whose CENTER is trustworthy (cond>=FLOOR). None -> unresolvable at any basis.
    v14: firewall is cond alone (the CAUSE: 'when I look, is the answer the same?'). occupancy is a
    FREQUENCY, not an epistemic fact -- it flows through the hit-rate (EV/breakeven) and the cushion
    SCALING, never as a hard product pre-screen. The occ*cond product floor (v13) double-charged
    occupancy: it gated resolvability AND was already inside the hit-rate driving EV>breakeven, which
    wrongly parked the cheap engine-start (c5/c6, clearing breakeven by ~5 SE). cond-only firewall
    still kills the c44-class smear (cond 0.33) -- the smear dies on the cause, not the correlate."""
    res = [m for m in CACHE[c]['modes'] if m['resolved_cond']]
    if not res: return None
    return res[0]                     # lowest cond-trustworthy mode; reach modes never lowest (bank precedes reach)
...
if __name__ == '__main__':
    ...
    print("="*80); print(f"v14 FIREWALL — FLOOR(cond)={FLOOR}  [occ -> cushion scaling + hit-rate only]  GAP={GAP} WIN=+-{WIN}"); print("="*80)
