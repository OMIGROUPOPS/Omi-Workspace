#!/usr/bin/env python3
"""Stdout-safe runner for the execution-readiness-repaired T2 package V3."""
...
from window1_t2_scoring_correction_v2 import partition_loss_attribution
from window1_t2_reference_boundary_v3 import (
    adapt_frozen_reference_rows,
    iter_gzip as iter_gzip_v3,
)


VERSION = "window1-t2-scoring-runner-v3"
...
def _validate_artifacts(repo: Path) -> None:
    manifest = read_json(resolve_role(
        repo, f"{PACKAGE_DIRECTORY}/ARTIFACT_HASH_MANIFEST.json"
    ))
    ...
    *,
    ...
    for role in (
        ...
        if role not in package["roles"]:
            raise RunnerError(f"V3 role missing: {role}")
    source = read_json(resolve_role(
        repo, package["roles"]["source_hash_manifest"]
    ))
    _validate_committed(repo, source["committed_source_inputs"])
    _validate_binary(repo, source["frozen_binary_inputs"])
    _validate_artifacts(repo)
    critical = package["input_bundle_payload"][
        "critical_role_identities"
    ]
    for name, receipt in critical.items():
        if name == "guarded_cache_directory":
            continue
        path = resolve_role(repo, str(receipt["path"]))
        if (
            not path.is_file()
            or len(_identity_bytes(path))
            != int(receipt["identity_bytes"])
            or hashlib.sha256(_identity_bytes(path)).hexdigest()
            != receipt["sha256"]
        ):
            raise RunnerError(f"{name} hash mismatch")
    correction = read_json(resolve_role(
        repo, package["roles"]["combined_headroom_arithmetic_receipt"]
    ))
    if (
        correction.get("formula_difference_count") != 0
        or correction.get("target_change_count") != 0
        or correction.get("package_blocked") is not False
    ):
        raise RunnerError("combined-headroom correction is not clear")
    surface = read_json(resolve_role(
        repo, package["roles"]["target_surface_reconciliation"]
    ))
    if (
        surface.get("target_surface_rows") != 4_576_794
        or surface.get("lawful_target_entries") != 2_996_560
        or surface.get("unexplained_residue_count") != 0
        or surface.get("package_blocked") is not False
    ):
        raise RunnerError("target-surface reconciliation is not clear")
...
def authorize_execute(
    ...
    if audit_commit == CONSUMED_AUTHORIZATION:
        raise RunnerError("consumed V2 authorization is rejected by V3")
    ...
    if git(repo, "rev-parse", "HEAD") != git(
        repo, "rev-parse",
        "refs/remotes/origin/codex/window1-t2-scoring-package-v3-prerun",
    ):
...
def validation_only(repo: Path, package_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": VERSION + "-validation-only-v3",
...
def execution_readiness_no_score(
    ...
    """Exercise all real-input joins and stop before the first scorer call."""
    validated = validate_package(
    ...
    if len(events) != D_REQUIRED:
        raise RunnerError("no-score preflight D changed from 804")
    expected_legs = _event_legs(events)
    starts = {
        str(row["event_id"]): row
        for row in read_jsonl(resolve_role(repo, roles["normalized_boundary_ledger"]))
    }
    if len(starts) != 804:
        raise RunnerError("no-score preflight boundary count changed")
    references = adapt_frozen_reference_rows(
        iter_gzip_v3(resolve_role(repo, roles["frozen_reference_ledger"])),
    ...
    if (
        len(expected_legs) != 1608
        or len(references) != 1608
        or len(floors) != 1608
        or set(floors) != set(expected_legs)
        or set(references) != set(expected_legs)
        or len(chains) != 12_864
        or set(chains) != expected_chain_keys
        or len(provenance) != 4_170
        or not set(provenance).issubset(expected_chain_keys)
        or not set(provenance).issubset(set(chains))
        or len(fit_rows) != 804
        or sum(row["slice"] == "fit" for row in fit_rows) != 525
        or sum(row["slice"] == "post_fit" for row in fit_rows) != 279
    ):
        raise RunnerError("real-input no-score joins do not conserve")
    if any(
        row.get("metrics") is not None or row.get("performance") is not None
        for row in list(chains.values()) + list(provenance.values())
    ):
        raise RunnerError("pre-scorer inputs contain populated metrics")
    if resolve_role(repo, RESULTS_DIRECTORY).exists():
        raise RunnerError("V3 results directory must be absent")
    return {
        ...
        "candidate_event_leg_keys_joined": len(expected_chain_keys),
        "boundary_rows_validated": len(starts),
        "reference_rows_validated": len(references),
...
def execute(
    ...
    try:
        ...
        starts = {
            str(row["event_id"]): row
            for row in read_jsonl(resolve_role(
                repo, roles["normalized_boundary_ledger"]
            ))
        }
        fills = adapt_t2_unique_fill_rows(
            iter_gzip(resolve_role(repo, roles["unique_T2_fill_ledger"])),
        ...
        require_claim_fences(claim_manifest)
        references = adapt_frozen_reference_rows(
            iter_gzip_v3(resolve_role(
                repo, roles["frozen_reference_ledger"]
            )),
            expected_legs=expected_legs,
            normalized_boundaries=starts,
        )
        progress.emit(f"frozen_references_validated={len(references)}")
        summaries = []
        for ordinal, candidate in enumerate(CANDIDATE_IDS, 1):
            ...
            for event in events:
                ...
                for leg_row in row["legs"]:
                    ...
                    target_provenance = chain[
                        "target_level_authority_d2_provenance"
                    ]
                    omitted_provenance = target_provenance[
                    ...
                    leg_regret.append({
                        ...
                        **gaps,
                        ...
                        "omitted_d2_inferred_from_successful_fill": False,
            ...
            for slice_name, dates in slice_dates.items():
                legs = [
                    leg
                    for event_row in scored_rows
                    if dates is None
                    or str(event_row["event_date"]) in dates
                    for leg in event_row["regret_by_leg"]
                ]
                authority_attribution[slice_name] = (
                    partition_loss_attribution(legs)
                )
            summary[
                "authority_omitted_d2_loss_attribution"
            ] = authority_attribution
        ...
        result = {
            "schema_version": VERSION + "-eight-candidate-results-v2",
            "execution_id": EXECUTION_ID,
        ...
        if not all(
            set(summary["authority_omitted_d2_loss_attribution"])
            == {"aggregate", "fit", "post_fit"}
            and all(
                value["conservation_pass"] is True
                and value["d2_inferred_from_successful_fill"] is False
            ...
            raise RunnerError(
                "authority x omitted-d2 attribution did not conserve"
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode",
        choices=("validate-only", "preflight-no-score", "execute"),
        required=True,
    )
    ...
    if args.mode == "validate-only":
        ...
        return 0
    if args.mode == "preflight-no-score":
        if args.authorization_commit or args.authorization_report:
            raise RunnerError(
                "authorization forbidden in preflight-no-score"
            )
        ConsoleEchoGuard().stdout(json.dumps(
            execution_readiness_no_score(repo, package),
            sort_keys=True,
        ))
        return 0
