  ...
  return (
    <div className="border border-[#1a1a2e] bg-[#0a0a12] overflow-hidden rounded-none">
      <div className="flex items-center justify-between px-3 py-1.5 bg-black border-b border-[#1a1a2e]">
        <span className="text-[9px] font-mono font-bold text-[#ffffff] uppercase tracking-widest">
          TRADE SPECS
        </span>
        <span className="text-[8px] text-[#ffffff] font-mono">
      ...
      <div className="grid grid-cols-2 divide-x divide-[#1a1a2e]">
        ...
        <div className="px-3 py-2">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] font-mono text-[#ffffff]">{pmTeam}</span>
          </div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[9px] font-mono">
            <div><span className="text-[#ffffff]">Fill Price</span> <span className="text-[#00ff88] ml-1">{pmPrice}</span> <span className="text-[#ffffff] ml-0.5">(${pmCostPerContract.toFixed(2)})</span></div>
            <div><span className="text-[#ffffff]">Qty</span> <span className="text-[#ff8c00] ml-1">{qty}</span></div>
            <div className="col-span-2"><span className="text-[#ffffff]">Total Cost</span> <span className="text-[#00ff88] ml-1 font-bold">${pmTotalCost.toFixed(2)}</span></div>
            <div><span className="text-[#ffffff]">Latency</span> <span className="text-[#ff8c00] ml-1">{t.pm_order_ms || "-"}ms</span></div>
            <div><span className="text-[#ffffff]">Fee</span> <span className="text-[#ff8c00]/80 ml-1">{pmFeeCents.toFixed(2)}c</span></div>
            {t.pm_bid != null && (
              <div className="col-span-2"><span className="text-[#ffffff]">BBO at Detection</span> <span className="text-[#ff8c00] ml-1">{t.pm_bid}c / {t.pm_ask}c</span></div>
            )}
          </div>
          {pmLink && (
          ...
          {t.pm_order_id && (
            <div className="mt-0.5 text-[8px] text-[#ffffff] font-mono truncate">
        ...
        <div className="px-3 py-2">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] font-mono text-[#ffffff]">{kTeam}</span>
          </div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[9px] font-mono">
            <div><span className="text-[#ffffff]">Fill Price</span> <span className="text-[#00bfff] ml-1">{kPrice}</span> <span className="text-[#ffffff] ml-0.5">(${kCostPerContract.toFixed(2)})</span></div>
            <div><span className="text-[#ffffff]">Qty</span> <span className="text-[#ff8c00] ml-1">{qty}</span></div>
            <div className="col-span-2"><span className="text-[#ffffff]">Total Cost</span> <span className="text-[#00bfff] ml-1 font-bold">${kTotalCost.toFixed(2)}</span></div>
            <div><span className="text-[#ffffff]">Latency</span> <span className="text-[#ff8c00] ml-1">{t.k_order_ms || "-"}ms</span></div>
            <div><span className="text-[#ffffff]">Fee</span> <span className="text-[#ff8c00]/80 ml-1">{kFeeCents.toFixed(1)}c</span></div>
            {t.k_bid != null && (
              <div className="col-span-2"><span className="text-[#ffffff]">BBO at Detection</span> <span className="text-[#ff8c00] ml-1">{t.k_bid}c / {t.k_ask}c</span></div>
            )}
          </div>
          {kLink && (
        ...
        return (
          <div className="flex items-center justify-between px-3 py-1.5 bg-black border-t-2 border-[#ff8c00]/30 text-[9px] font-mono">
            <div className="flex items-center gap-3">
              <span className="text-[#ffffff]">{useArbNet ? 'Arb Net' : 'Spread'}</span>
              <span className="text-[#ff8c00]">{useArbNet ? `${arbNetCPC.toFixed(2)}c` : `${t.spread_cents}c`}</span>
              {!useArbNet && (
                <>
                  <span className="text-[#ffffff]">−</span>
                  <span className="text-[#ffffff]">Fees</span>
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[#ffffff]">Net</span>
              <span className={`font-bold text-[10px] ${displayNetCents > 0 ? 'text-[#00ff88]' : displayNetCents < 0 ? 'text-[#ff3333]' : 'text-[#ffffff]'}`}>
              ...
              {qty > 0 && (
                <>
                  ...
                  <span className={`font-bold text-[10px] ${displayTotalDollars > 0 ? 'text-[#00ff88]' : displayTotalDollars < 0 ? 'text-[#ff3333]' : 'text-[#ffffff]'}`}>
  ...
  if (t.direction === "BUY_PM_SELL_K") {
    ...
    return (
      <>
        ...
        <span className="text-[#ffffff] ml-1.5 text-[9px]">[{totalCost.toFixed(0)}c&rarr;{spread.toFixed(0)}c]</span>
      </>
    );
  }
  const kFighter = t.k_yes_team || team;
            ...
            {trades.slice(0, 50).map((t, i) => {
              ...
              return (
                <React.Fragment key={`${t.timestamp}-${t.team}-${i}`}>
                  ...
                  >
                    <td className="px-2 py-1.5 text-[#ffffff] whitespace-nowrap font-mono text-[10px]">
                    ...
                    <td className={`px-2 py-1.5 text-right font-mono ${
                      pnl.totalDollars === null ? "text-[#ffffff]" :
                    ...
                    <tr className="bg-[#0a0a12]">
                      <td colSpan={6} className="px-4 py-3">
                            ...
                            {t.nofill_details && (
                              <div className="mt-1.5 grid grid-cols-4 gap-x-4 gap-y-1 text-[9px] font-mono">
                                {t.nofill_details.pm_data_age_ms != null && (
                                  <div>
                                    <span className="text-[#ffffff]">Data Age</span>
                                ...
                                {t.nofill_details.post_bbo != null && (
                                  <div>
                                    <span className="text-[#ffffff]">Post-BBO</span>
                                    <span className="text-[#ff8c00] ml-1">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-x-6 gap-y-2 text-[10px] font-mono">
                          <div>
                            <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Phase</span>
                            <span className="text-[#ff8c00]">{t.execution_phase || "ioc"}{t.is_maker ? " (maker)" : ""}</span>
                          </div>
                          <div>
                            <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Total Execution</span>
                            <span className="text-[#ff8c00]">{t.execution_time_ms || "-"}ms</span>
                          </div>
                          <div>
                            <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Hedged</span>
                            <span className={t.hedged ? "text-[#00ff88]" : "text-[#ff3333]"}>
                              {t.hedged ? "YES" : "NO"}
                            </span>
                          </div>
                          <div>
                            <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Fills (K / PM)</span>
                            <span className="text-[#ff8c00]">
                              {t.kalshi_fill ?? t.contracts_filled ?? 0}x / {t.pm_fill ?? t.contracts_filled ?? 0}x
                            </span>
                          </div>
                          <div>
                            <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Fees (PM / K)</span>
                            <span className="text-[#ff8c00]">
                          ...
                          {t.sizing_details && (
                            <>
                              {depth.k !== null && (
                                <div>
                                  <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">L1 at Arb Price</span>
                                  <span>
                              ...
                              <div>
                                <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Total Book Depth</span>
                                <span className="text-[#ff8c00]">
                          ...
                          {t.settlement_pnl != null && (
                            <div>
                              <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">Settlement P&L</span>
                          ...
                          {t.gtc_cancel_reason && (
                            <div>
                              <span className="text-[#ffffff] block text-[9px] uppercase tracking-wider">GTC Cancel</span>
                        ...
                        {t.sizing_details?.depth_walk_log && t.sizing_details.depth_walk_log.length > 0 && (
                          <div className="mt-3">
                            <span className="text-[#ffffff] block mb-1 text-[9px] font-mono uppercase tracking-wider">Depth Walk</span>
                            <table className="w-full text-[10px] font-mono">
                              <thead>
                                <tr className="text-[#ffffff] border-b border-[#1a1a2e]">
