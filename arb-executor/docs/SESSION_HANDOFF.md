...
Latest landed analysis: **Path B v4 — per-cell offset re-optimization on net-PnL (T47), DEPLOYABLE**, analytical commit `c90985b` (2026-05-23). Re-sweeps placement×offset per cell on net realized PnL (correcting v1's entry-capture objective): **net ROI 11.73% vs v3 10.70% (+1.024pp; +1.22pp vs canonical v3)** on lower capital, gate 6 material → v4 supersedes v3 as the deployable table. **Net-PnL-optimal offsets are SHALLOW (1-3¢, 27/36 cells), opposite v1's deep favorites** (the atlas fixed-profit exit caps the value of a deep entry discount). The positive complement to the Path C negative arc (Phase 1-3): feature-conditional refinements couldn't beat v3 because the exit cap binds, but re-picking the static offset on the right objective does — the lever was the offset itself. Doc commits `eb272c8` (ROADMAP T47) + `7f80b74` (ANALYSIS_LIBRARY) + this update; **commit 5 (bid_laying_policy Section 12 + per_regime_offsets_v2.csv) follows**. Preceding: Path C Phase 1 (T46, `22f3221`); Path B v3/v2/v1 (T45/T44/T43); worked-example charts.

**Path C arc held unpromoted** (Phase 2 offset-modulation + corrected Phase 3 execution-mode, both negative vs v3) — awaiting operator decision on whether to commit those negative results. Note: Path B v4 (re-optimized static offsets) is the channel that actually beat v3; the Path C feature/execution refinements did not.
