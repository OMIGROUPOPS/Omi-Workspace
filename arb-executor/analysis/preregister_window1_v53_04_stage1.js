...
if (truth.rows?.length !== 804 || census.rows?.length !== 1143) throw new Error("V53-04 authority census invalid");

const truthByCode = new Map(truth.rows.map((row) => [row.code, row]));
const excluded = new Set(), authorities = [];
for (const [iteration, commit, receiptPath] of v53Receipts) {
...
const seedMaterial = `V53_04_FRESH25|${implementationCommit}`, seedSha256 = sha(seedMaterial), rank = (value) => sha(`${seedSha256}|${value}`);
...
const receipt = {
  ...
  seed_derivation_law: "SHA256('V53_04_FRESH25|' + V53_04_IMPLEMENTATION_COMMIT)",
  ...
  authorities: {
    ...
    V53_pre_registrations: authorities,
  },
