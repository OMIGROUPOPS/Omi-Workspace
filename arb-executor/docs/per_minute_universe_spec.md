...
| `volume_in_minute` | float64 | Total contract volume traded in the minute, **trade-tape-sourced** from `sum(g9_trades.count_fp)` over trades in `[minute_ts−60, minute_ts)` where `count_fp > 0`. Zero-filled (never null) — minutes with no positive-size trades get 0. Replaces the v1 candle-sourced path (`g9_candles.volume_fp`) that was 65% null at source and over-reported vs trade tape on 12% of populated minutes. See LESSONS C36. |
| `trade_count_in_minute` | int32 | Number of positive-size trade events in the minute (count of `g9_trades` rows in `[minute_ts−60, minute_ts)` with `count_fp > 0`). Zero-size Kalshi trade events (~0.20% of g9_trades; 67,191 of 33.7M rows) are filtered so this column aligns with `volume_in_minute` by construction: **the invariant `(trade_count_in_minute > 0) ⇔ (volume_in_minute > 0)` holds for every row**. |
| `minute_has_trade` | bool | `trade_count_in_minute > 0` (equivalently: `volume_in_minute > 0`) |
...
| `partner_volume_in_minute` | float64 | Partner's `volume_in_minute` (trade-tape-sourced, zero-filled, never null). Same definition + invariant as own-side. |
| `partner_trade_count_in_minute` | int32 | Partner's positive-size trade events in `[minute_ts−60, minute_ts)`. Same definition as own-side `trade_count_in_minute`. |
...
### 2.10 Depth observability and trade-tape behavioral proxies

#### Depth limitation framing (added 2026-05-12 post-T37b Phase 1 v2 visual review)

Per-minute universe captures **top-of-book BBO at minute resolution** (yes_bid_close, yes_ask_close, plus open/high/low intra-minute ranges per Section 2.2). Full 5-deep orderbook with resting sizes at each level is in **A-tier** (`analysis/premarket_ticks/*.csv`, Apr 18 2026+) per LESSONS A22 / A23 / A24 and can be joined as a v2-of-this-spec for the overlap window. **G9 corpus (pre-Apr-18-2026) does not have explicit depth beyond top-of-book.** Downstream consumers requiring depth must either: (a) filter to the Apr 18+ overlap and join A-tier 5-deep data, or (b) accept top-of-book + trade-tape behavioral proxies as the observable depth surface.

#### Behavioral proxies derived from trade tape (added 2026-05-12 post-T37b Phase 1 v4)

Four columns derived from how g9_trades interact with g9_candles BBO. Each surfaces a different facet of "where is the book thin vs thick" without requiring explicit depth data.

| Column | Type | Notes |
|---|---|---|
| `price_levels_consumed_in_minute` | int32 | Count of distinct `yes_price_dollars` values traded in `[minute_ts−60, minute_ts)`. `1` = book absorbed at a single price level (thick). `3+` = book eaten through multiple levels (thin liquidity). `0` when `trade_count_in_minute == 0`. |
| `bid_consumption_velocity` | float64 | `yes_bid_close[minute_ts] − yes_bid_close[prior_minute]`. Signed: negative = bid eaten down (sellers hit thin bids); positive = bid lifted (buyers absorbed depth on no-side). Null for the first minute (no prior). |
| `ask_consumption_velocity` | float64 | `yes_ask_close[minute_ts] − yes_ask_close[prior_minute]`. Signed: positive = ask lifted (buyers eat through depth); negative = ask lowered (sellers pressure). Null for the first minute. |
| `trade_clustering_in_minute` | float64 | Standard deviation of inter-trade gap times within the minute, in seconds. Computed as `std(diff(trade.created_time_ts))` for trades in `[minute_ts−60, minute_ts)`. Low std = burst (sudden activity, likely event-driven); high std = sparse (passive trading). **Null when `trade_count_in_minute < 3`** (≥3 trades required for ≥2 inter-trade gaps, ≥2 gaps required for non-trivial std). |

**Semantics summary:** `price_levels_consumed_in_minute` measures price-level breadth per minute (depth proxy via how many different prices got hit). `bid_consumption_velocity` and `ask_consumption_velocity` measure intra-tape directional pressure (signed delta of close-quote across minute boundaries). `trade_clustering_in_minute` measures temporal burstiness (whether trades cluster within seconds or spread across the minute). For A-tier-overlap windows (Apr 18+), v2 of this spec joins explicit 5-deep depth columns and these proxies remain available alongside as a control / regression check.
...
- **Phase 2 (corpus subset, stratified by MATCHES not tickers):** up to **80 matches × 2 sides = up to 160 tickers**. Sampling unit is the match (paired event_ticker), not the independent ticker, because the paired-leg signals in Section 2.9 require both sides of every sampled match. Stratification: **4 categories** (ATP_CHALL / ATP_MAIN / WTA_MAIN / WTA_CHALL; no `OTHER` in the tennis corpus per Phase 2 empirical probe — `cell_key_helpers.CATEGORIES` includes OTHER as a safety bucket but yields zero binary-outcome tickers in g9_metadata) × 4 premarket-length quartiles = **up to 16 strata × 5 matches/stratum**. Actual realized count may be lower if (a) a category × quartile stratum has fewer than 5 available matches, or (b) per-category `pd.qcut` binning collapses fewer than 4 quartile bins due to duplicate `premarket_len_min` values (tennis schedules cluster at hour boundaries → identical `expected_expiration_time` offsets → duplicate quartile edges → qcut drops bins). Producer uses `pd.qcut(..., labels=False, duplicates='drop')` wrapped in a `_qcut_safe` helper that catches `ValueError` and falls back to a single bin in the all-identical-values degenerate case. **Actual realized stratification is recorded in `run_summary_phase2.json` field `realized_stratification`** as a dict mapping `(category, quartile) → match_count` per LESSONS C27 (dependency state must be visible in output). Deterministic random seed `PHASE2_SAMPLE_SEED = 42`. Validates corpus-scale runtime and memory behavior. Computes per-ticker compute cost distribution to project Phase 3 runtime accurately. Runtime budget: <30 min (revised from 40 min after Phase 1 v5 anchored per-ticker median at ~3-12 sec; 160 tickers × ~8 sec avg + stratification step ≈ ~22 min + I/O slack). Sampling-by-tickers (the prior framing) would split paired event_tickers ~50% of the time and break paired-leg coverage on those sampled matches — sampling-by-matches preserves pair integrity.
...
**Resolved 2026-05-14 (post-v0.2 publication, corpus checkpoint 3 sha256 `9fde4b5d…`):** The discrepancy is fixed. Producer commit `4b0c3e45` excludes zero-size (`count_fp=0`) trades from `trade_count_in_minute`; backfill aligned both columns on the trade-tape source. The strict invariant `(tc>0) ⇔ (vol>0)` now holds by construction. v0.2's trade-only surge thresholds (Section 7.5.2 z-score, Section 7.5.3 percent-rank) remain calibrated correctly; a future amendment may add a volume-based surge dimension now that volume_in_minute is reliable. **Q4A re-confirmation (2026-05-14) on checkpoint 3:** high-activity cohort shifted from 17,436 → 17,432 tickers (delta −4); v0.2 thresholds stand.
...
- **2026-05-14 ET (volume_in_minute + trade_count_in_minute alignment, post-v0.2):** Producer commit `4b0c3e45` and corpus backfill (checkpoint 3, sha256 `9fde4b5d30e56d99efa0637fe042cb6ca4505274e85e42769b4cedc25e3e5ff4`) align both columns on the trade tape and exclude zero-size (`count_fp=0`) Kalshi events. The strict invariant `(trade_count_in_minute>0) ⇔ (volume_in_minute>0)` now holds for every row; both columns are zero-filled (never null). Section 2 schema rows updated. Section 7.5 (PHASE_3 surge) updated with the resolution note. Q4A re-confirmation: high-activity cohort moved 17,436 → 17,432 tickers (delta −4); v0.2 thresholds stand. See LESSONS C36 (sourcing discipline) + C37 (pre-replace gate discipline + adjudication addendum). Corpus checkpoint chain: c80e5fc2 (raw Phase 3 merge) → f9a71d5c (+ pair_gap_abs) → **9fde4b5d (+ vol/tc trade-tape aligned)**.
