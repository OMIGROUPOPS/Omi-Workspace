...
Latest landed analysis: **Path B v3 — per-regime offsets + atlas exit replay (T45)**, analytical commit `648b8db` (2026-05-23). The deployable measurement: per-regime offsets realize $8,098.50 vs atlas $6,158.20 (+31.5%) on lower capital → blended ROI **12.11% vs 8.70% (+3.41pp, pre-realism)**, beating T44's universal-rule +2.93pp by only +0.48pp (+$269) — diminishing returns; baseline reproduced exactly. Doc-system commits `ff86121` (ROADMAP T45) + `95216b4` (ANALYSIS_LIBRARY) + this SESSION_HANDOFF update (no LESSONS — A40/A41 cover it). Preceding: Path B v2 (T44, `a61830c`), v1 fill mechanics (T43, `1e00818`), worked-example charts v1/v2/v3.

**The bid-laying policy spec is the open downstream T-item.** Empirical inputs now in hand: (a) deployable shape = per-regime offsets (T45), marketable→taker / resting→maker branch (A41), atlas exit from entry (T44/T45); (b) pre-realism ceiling +3.41pp blended over the +8.70% atlas floor → ×B25 realism (0.5-0.7×) for the deployable estimate; (c) per-regime conditioning is a +0.48pp refinement over a universal favorite offset (diminishing returns — don't over-engineer the offset table).
