...
const V43_RESIDUAL_CENSUS_COMMIT = "639e8b19e9e6699f6c99bcd48a9557273234cd93";
...
const output = path.resolve(arg("--output", path.join(repo, isV44 ? ".claude/window1_live_v4_replay/v44_guard_swap_20260809" : isV43 ? ".claude/window1_live_v4_replay/v43_composed_machine_20260809" : isV42 ? ".claude/window1_live_v4_replay/v42_deep_gap_feasibility_guard_20260809" : isV41 ? ".claude/window1_live_v4_replay/v41_maker_machine_20260808" : isV40 ? ".claude/window1_live_v4_replay/v40_incumbent_direction_placement_stack_20260808" : isV39 ? ".claude/window1_live_v4_replay/v39_corrected_placement_stack_20260807" : OUT_REL)));
...
function safeOutput(dir) {
  ...
  ensure(path.basename(resolved).includes(isV44 ? "v44" : isV43 ? "v43" : isV42 ? "v42" : isV41 ? "v41" : isV40 ? "v40" : isV39 ? "v39" : "v38"), `unsafe output ${resolved}`);
...
function armSibling(sibling, filled, row, actions, base) {
  ...
  if (sibling.active_order && sibling.active_order.target_cents > sibling.pair_cap_cents) {
    ...
    if (policy.lawfulCent(sibling.pair_cap_cents)) {
      ...
      if (sibling.lawful_target_levels) sibling.lawful_target_levels.add(sibling.pair_cap_cents);
...
function observeUnionFlow(leg, row) {
  if (!leg.observed_union_flow_levels) return false;
  let price = null, channel = null;
  if (row.kind === "PRINT" && policy.lawfulCent(row.price) && positive(row.size)) {
    price = row.price;
    channel = row.taker_side === "no" ? "STRICT_PRINT_CROSS_OR_TRADED_AT_LEVEL_SELLER" : "TRADED_AT_LEVEL_BUYER_OR_UNKNOWN";
  } else if (row.kind === "BOOK" && policy.qualifyingAskEvidence && policy.qualifyingAskEvidence(row)) {
    price = row.ask;
    channel = "QUOTE_TOUCH_10S_SIZE_FIVE";
  }
  if (!policy.lawfulCent(price)) return false;
  if (!leg.observed_union_flow_levels.has(price)) leg.observed_union_flow_levels.set(price, { price_cents: price, channel, timestamp_epoch: row.ts, receipt: row.receipt });
  return true;
}

function drySiblingStatus(sibling) {
  return policy.drySiblingEvidence({ lawfulLevels: [...sibling.lawful_target_levels], flowLevels: [...sibling.observed_union_flow_levels.keys()] });
}

function maybeLiftDryWithhold(withheldLeg, evidenceLeg, row, actions, base, normalizedClauses) {
  if (!normalizedClauses.dry_sibling_withhold || !withheldLeg.dry_sibling_withhold_active || withheldLeg.credited) return;
  const evidence = drySiblingStatus(evidenceLeg);
  withheldLeg.dry_sibling_last_evidence = evidence;
  if (evidence.withheld) return;
  withheldLeg.dry_sibling_withhold_active = false;
  withheldLeg.dry_sibling_withhold_lifts += 1;
  withheldLeg.dry_sibling_last_lift = { ...clockFields(row.ts, base), receipt: row.receipt, sibling_leg_identity: evidenceLeg.leg_identity, evidence };
  actions.push({ kind: "DRY_SIBLING_WITHHOLD_LIFT", event_id: base.event_id, leg_identity: withheldLeg.leg_identity, ...clockFields(row.ts, base), receipt: row.receipt, sibling_leg_identity: evidenceLeg.leg_identity, evidence });
  if (!withheldLeg.last_placement_inputs) return;
  const before = withheldLeg.active_order?.target_cents ?? null;
  const inputs = { ...withheldLeg.last_placement_inputs, activeTarget: before, pairCap: withheldLeg.pair_cap_cents, siblingBestAsk: normalizedClauses.deep_gap_guard && row.kind === "BOOK" ? row.ask : withheldLeg.last_placement_inputs.siblingBestAsk };
  const decision = policy.decide(inputs);
  if (normalizedClauses.deep_gap_guard) noteV42Guard(withheldLeg, decision, row, base, actions, evidenceLeg.leg_identity);
  const ownBook = withheldLeg.last_placement_inputs.book;
  const detail = { ...clockFields(row.ts, base), receipt: row.receipt, ordinal: row.ordinal, observation: receiptObservation(ownBook), sibling_observation: row.kind === "BOOK" ? receiptObservation(row) : evidenceLeg.prior_book ? receiptObservation(evidenceLeg.prior_book) : null, combined_state: withheldLeg.last_placement_inputs.state, pulse_floor: { floor_cents: withheldLeg.last_placement_inputs.pulseFloor }, pair_cap_cents: withheldLeg.pair_cap_cents, order_before_cents: before, decision, dry_sibling_re_evaluated_on_sibling_receipt: true, dry_sibling_evidence: evidence, order_after_cents: null };
  applyRestDecision(withheldLeg, evidenceLeg, row, decision, withheldLeg.last_placement_inputs.state, detail, actions, base, evidenceLeg.leg_identity);
  detail.order_after_cents = withheldLeg.active_order?.target_cents ?? null;
  withheldLeg.last_decision = detail;
}

function attemptRestFill(leg, sibling, row, fillClass, actions, base, normalizedClauses) {
  if (normalizedClauses.dry_sibling_withhold) ensure(!drySiblingStatus(sibling).withheld, `dry-sibling invariant: live rest reached fill while sibling dry ${leg.leg_identity}`);
  fillLeg(leg, sibling, row, fillClass, actions, base);
  return true;
}

function applyDrySiblingPlacementGuard(leg, sibling, row, decision, actions, base, normalizedClauses) {
  if (!normalizedClauses.dry_sibling_withhold || !policy.lawfulCent(decision.unguarded_decision?.target_cents ?? decision.target_cents)) return decision;
  const evidence = drySiblingStatus(sibling);
  leg.dry_sibling_fill_evaluations += 1;
  leg.dry_sibling_last_evidence = evidence;
  if (!evidence.withheld) return decision;
  leg.dry_sibling_withheld_fill_evaluations += 1;
  if (!leg.dry_sibling_withhold_active) {
    leg.dry_sibling_withhold_active = true;
    leg.dry_sibling_withhold_episodes += 1;
    leg.dry_sibling_first_withhold ||= { ...clockFields(row.ts, base), receipt: row.receipt, potential_target_cents: decision.unguarded_decision?.target_cents ?? decision.target_cents, sibling_leg_identity: sibling.leg_identity, evidence };
    actions.push({ kind: "DRY_SIBLING_WITHHOLD_START", event_id: base.event_id, leg_identity: leg.leg_identity, ...clockFields(row.ts, base), receipt: row.receipt, potential_target_cents: decision.unguarded_decision?.target_cents ?? decision.target_cents, sibling_leg_identity: sibling.leg_identity, evidence });
  }
  leg.dry_sibling_last_withhold = { ...clockFields(row.ts, base), receipt: row.receipt, potential_target_cents: decision.unguarded_decision?.target_cents ?? decision.target_cents, sibling_leg_identity: sibling.leg_identity, evidence };
  const active = leg.active_order?.target_cents ?? null;
  return { action: policy.lawfulCent(active) ? "CANCEL_REST" : "HOLD_REST", target_cents: null, reason: policy.lawfulCent(active) ? "V44_DRY_SIBLING_WITHHOLD_CANCEL_LIVE_REST" : "V44_DRY_SIBLING_WITHHOLD_NEW_REST", placement: decision.placement, guard: decision.guard ?? null, unguarded_decision: decision, dry_sibling: evidence };
}
...
function reevaluateV42WithSiblingBook(withheldLeg, triggeringLeg, row, actions, base, normalizedClauses) {
  ...
  decision = applyDrySiblingPlacementGuard(withheldLeg, triggeringLeg, row, decision, actions, base, normalizedClauses);
  const ownBook = withheldLeg.last_placement_inputs.book;
...
function simulate(base, tapes, prints, mode, clauses = {}) {
  ...
  for (const row of timeline) {
    ...
    observeUnionFlow(leg, row);
    maybeLiftDryWithhold(sibling, leg, row, actions, base, normalizedClauses);
    ...
    if (leg.credited) {
      if (normalizedClauses.deep_gap_guard && row.kind === "BOOK") {
        leg.prior_book = row;
        reevaluateV42WithSiblingBook(sibling, leg, row, actions, base, normalizedClauses);
      }
      continue;
    }
    if (row.kind === "PRINT") {
      if (mode === "STRICT_PRINT_CROSS" && policy.strictPrintCross(leg.active_order, row)) {
        if (attemptRestFill(leg, sibling, row, "STRICT_PRINT_CROSS_SELLER_AGGRESSED_SIZE_FIVE", actions, base, normalizedClauses)) continue;
      }
      if (mode === "MARKET_UNION_REACH" && leg.active_order && policy.strictPrintCross(leg.active_order, row)) {
        if (attemptRestFill(leg, sibling, row, "MARKET_REACH_PRINT_CROSS", actions, base, normalizedClauses)) continue;
      }
      if (mode === "MARKET_UNION_REACH" && policy.tradedAtLevel(leg.active_order, row)) {
        if (attemptRestFill(leg, sibling, row, "MARKET_REACH_TRADED_AT_LEVEL", actions, base, normalizedClauses)) continue;
      }
    ...
    if (mode === "MARKET_UNION_REACH" && policy.quoteTouch(leg.active_order, row)) {
      if (attemptRestFill(leg, sibling, row, "MARKET_REACH_QUOTE_TOUCH_10S_SIZE_FIVE", actions, base, normalizedClauses)) {
        if (normalizedClauses.deep_gap_guard) { leg.prior_book = row; reevaluateV42WithSiblingBook(sibling, leg, row, actions, base, normalizedClauses); }
        continue;
      }
    }
    ...
    let decision = policy.decide(placementInputs);
    const lawfulTarget = decision.unguarded_decision?.target_cents ?? decision.target_cents;
    if (policy.lawfulCent(lawfulTarget)) {
      leg.lawful_target_levels.add(lawfulTarget);
      maybeLiftDryWithhold(sibling, leg, row, actions, base, normalizedClauses);
    }
    if (normalizedClauses.deep_gap_guard) noteV42Guard(leg, decision, row, base, actions, leg.leg_identity);
    decision = applyDrySiblingPlacementGuard(leg, sibling, row, decision, actions, base, normalizedClauses);
    ...
    if (normalizedClauses.deep_gap_guard) reevaluateV42WithSiblingBook(sibling, leg, row, actions, base, normalizedClauses);
  }
  for (const leg of Object.values(event.legs)) {
    ...
    leg.dry_sibling_lawful_levels = [...leg.lawful_target_levels].sort((a, b) => a - b);
    leg.dry_sibling_union_flow = [...leg.observed_union_flow_levels.values()].sort((a, b) => a.timestamp_epoch - b.timestamp_epoch || a.price_cents - b.price_cents);
    delete leg.active_order; delete leg.prior_book; delete leg.directional; delete leg.pulse_visits; delete leg.first_action; delete leg.seller_hits_by_level; delete leg.book_last_trade_hits_by_level; delete leg.downward_evidence_rows; delete leg.last_placement_inputs; delete leg.deep_gap_withhold_active; delete leg.lawful_target_levels; delete leg.observed_union_flow_levels; delete leg.dry_sibling_withhold_active;
  }
...
async function main() {
  ...
  const machineSpecs = isV44 ? [
    { name: "V43_BASELINE", clauses: { deep_gap_guard: true } },
    { name: "GUARD_REMOVED_ONLY", clauses: {} },
    { name: "DRY_SIBLING_ONLY", clauses: { deep_gap_guard: true, dry_sibling_withhold: true } },
    { name: "V44_GUARD_SWAP", clauses: { dry_sibling_withhold: true } },
  ] : isV43 ? [
  ...
  for (const base of [...baseByEvent.values()].sort((a, b) => a.event_id.localeCompare(b.event_id))) {
    index += 1; if (index % 50 === 0) process.stderr.write(`${isV44 ? "V44x4" : isV43 ? "V43x8" : isV42 ? "V42" : isV41 ? "V41" : isV40 ? "V40" : isV39 ? "V39" : "V38"} replay ${index}/804\n`);
  ...
  const primaryRun = machineRuns.get(isV44 ? "V44_GUARD_SWAP" : isV43 ? "V43_ALL_THREE" : "PRIMARY"), marketEvents = primaryRun.marketEvents, strictEvents = primaryRun.strictEvents, allActions = primaryRun.actions;
  ...
  const v44BaselineReproduction = isV44 ? {
    expected: { completed_pairs: 395, completed_locked_cents: 1910, naked_pnl_cents: -162, true_book_net_cents: 1748, frontier: { LE_93: 51, LE_95: 70, LE_97: 141, LT_100: 395 } },
    observed: { ...attributionByName.get("V43_BASELINE").FULL_BOOK, frontier: attributionByName.get("V43_BASELINE").MARKET_UNION_REACH.frontier },
  } : null;
  if (v44BaselineReproduction) v44BaselineReproduction.pass = Object.entries(v44BaselineReproduction.expected).every(([key, value]) => key === "frontier" ? Object.entries(value).every(([tier, expected]) => v44BaselineReproduction.observed.frontier[tier] === expected) : v44BaselineReproduction.observed[key] === value);
  const v44Acceptance = isV44 ? {
    baseline_reproduction: { pass: v44BaselineReproduction.pass },
    completed_pairs: { value: attributionByName.get("V44_GUARD_SWAP").MARKET_UNION_REACH.completed_pairs, minimum: 395, pass: attributionByName.get("V44_GUARD_SWAP").MARKET_UNION_REACH.completed_pairs >= 395 },
    naked_pnl_cents: { value: attributionByName.get("V44_GUARD_SWAP").FULL_BOOK.naked_pnl_cents, minimum: 0, pass: attributionByName.get("V44_GUARD_SWAP").FULL_BOOK.naked_pnl_cents >= 0 },
    true_book_net_cents: { value: attributionByName.get("V44_GUARD_SWAP").FULL_BOOK.true_book_net_cents, strict_minimum: 1748, pass: attributionByName.get("V44_GUARD_SWAP").FULL_BOOK.true_book_net_cents > 1748 },
  } : null;
  ...
  const namedLabels = isV44 ? ["PENTHA", "SHEOLI", "PUTJEA", "KIRSEK", "ARNROM", "KRUFER", "BOSCOP"] : isV43 ? ["KIRSEK", "ARNROM", "KRUFER", "BOSCOP", "PUTJEA", "BORDIM", "ROCBUE", "KREZHE"] : isV42 ? ["PUTJEA", "ROCBUE", "KREZHE", "BORDIM", "ARNROM"] : isV41 ? ["ARNROM", "BOSCOP", "NIKVRB", "WESPAA", "KRUFER"] : ["ARNROM", "BOSCOP", "WESPAA", "NIKVRB", "GANJAN"];
  const named = {};
  ...
  if (isV44) {
    ensure(policy.normalizedClauses({ dry_sibling_withhold: true }).deep_gap_guard === false, "V44 combined retained the removed deep-gap guard");
    ensure(policy.normalizedClauses({ dry_sibling_withhold: true }).dry_sibling_withhold === true, "V44 dry-sibling clause absent");
    ensure(policy.DRY_SIBLING_NEAR_CENTS === 3, "V44 dry-sibling threshold changed");
    ensure(policy.combineState === require("./window1_v36_state_directional_rest_mature_floor.js").combineState, "V44 changed V41/V36 state machine");
  }
  ...
  const v43ResidualPath = ".claude/window1_second_seat/v11_non_action_mechanism_audit_20260803/V43_RESIDUAL_MISS_GEOMETRY.json";
  ...
  const v43RecalibrationBytes = isV44 ? gitShow(V43_RECALIBRATION_COMMIT, v43RecalibrationPath) : null;
  const v43ResidualBytes = isV44 ? gitShow(V43_RESIDUAL_CENSUS_COMMIT, v43ResidualPath) : null;
  const v43RecalibrationReceipt = isV44 ? JSON.parse(v43RecalibrationBytes) : null;
  const v43ResidualReceipt = isV44 ? JSON.parse(v43ResidualBytes) : null;
  if (isMaker41) {
  ...
  if (isV44) {
    ensure(v43RecalibrationReceipt.baseline.completed === 395 && v43RecalibrationReceipt.baseline.true_book === 1748, "b503e4ed V43 baseline binding changed");
    ensure(v43RecalibrationReceipt.A_plus_B.score.completed === 415 && v43RecalibrationReceipt.A_plus_B.score.true_book === 2165, "b503e4ed analytical A+B estimate changed");
    ensure(v43ResidualReceipt.population.completed === 395 && v43ResidualReceipt.class_totals.GUARD_WITHHELD.n === 35, "639e8b19 residual census binding changed");
  }
  const control = isV44
    ? { schema_version: "window1-v44-guard-swap-control-v1", parent: V43_COMMIT, frozen_V43: V43_COMMIT, frozen_union_reach: REACH_COMMIT, controlling_receipts: [V43_RECALIBRATION_COMMIT, V43_RESIDUAL_CENSUS_COMMIT, DEEP_GAP_CENSUS_COMMIT, FULL_BOOK_PNL_COMMIT], architecture: { incumbent: "V43_BYTE_IDENTICAL_EXCEPT_EXACT_GUARD_SWAP", removed: "T10_DEEP_GAP_FEASIBILITY_GUARD_DELETED_FROM_V44_COMBINED", added: "WITHHOLD_POTENTIAL_FILL_WHILE_SIBLING_UNION_FLOW_SO_FAR_HAS_NEVER_COME_WITHIN_3C_OF_ANY_CAUSALLY_OBSERVED_LAWFUL_SIBLING_REST_LEVEL", lift: "FIRST_RECEIPT_ESTABLISHING_SIBLING_FLOW_WITHIN_3C_OF_ANY_CAUSALLY_OBSERVED_LAWFUL_SIBLING_REST_LEVEL", union_channels: ["QUOTE_TOUCH_10S_SIZE_FIVE", "TRADED_AT_LEVEL", "STRICT_PRINT_CROSS"], decision_time_only: true, no_retroactive_fill: true, arm_at_first_evidence: "V43_UNCHANGED", loosen_one_cent: "V43_UNCHANGED", sanity_bound: "V43_UNCHANGED", lazy_leg_1: "V43_UNCHANGED", pair_cap: "V43_UNCHANGED", clocks_as_decision_inputs: [], hard_prebell_edge: "V43_UNCHANGED", take_path: "DELETED_IN_V41" }, acceptance_bar: { true_book_net_cents_strictly_greater_than: 1748, naked_pnl_cents_min: 0, completed_pairs_min: 395, named_at_or_better_and_mechanism_bound: true }, fill_rulers: { market_scoring: "CANON_UNION_CHANNELS", build_verification: "STRICT_PRINT_CROSS", never_swapped: true } }
    : isV43
  ...
  const drySiblingLegs = isV44 ? marketLegs.filter((leg) => leg.dry_sibling_withhold_episodes > 0).map((leg) => ({ event_id: leg.event_id, leg_identity: leg.leg_identity, category: leg.category, bell_confidence: leg.bell_confidence, placement_evaluations: leg.dry_sibling_fill_evaluations, withheld_placement_evaluations: leg.dry_sibling_withheld_fill_evaluations, withhold_episodes: leg.dry_sibling_withhold_episodes, lifts: leg.dry_sibling_withhold_lifts, first_withhold: leg.dry_sibling_first_withhold, last_withhold: leg.dry_sibling_last_withhold, last_lift: leg.dry_sibling_last_lift, credited: leg.credited, entry_cents: leg.entry_cents, terminal_reason: leg.terminal_reason })).sort((a, b) => a.event_id.localeCompare(b.event_id) || a.leg_identity.localeCompare(b.leg_identity)) : [];
  const drySiblingActions = isV44 ? allActions.filter((row) => row.mode === "MARKET_UNION_REACH" && String(row.kind).startsWith("DRY_SIBLING_")) : [];
  const namedV44 = isV44 ? (() => {
    const result = namedAttribution.V44_GUARD_SWAP;
    const threshold = (label, ceiling) => Boolean(result[label].MARKET_UNION_REACH.completed && result[label].MARKET_UNION_REACH.combined_entry_cents <= ceiling);
    const putActions = drySiblingActions.filter((row) => row.event_id.includes("PUTJEA"));
    const putWithheldLegs = [...new Set(putActions.filter((row) => row.kind === "DRY_SIBLING_WITHHOLD_START").map((row) => row.leg_identity))].sort();
    const assertions = {
      PENTHA_at_or_better_55: threshold("PENTHA", 55),
      SHEOLI_at_or_better_92: threshold("SHEOLI", 92),
      PUTJEA_both_withheld_dry_skip_no_naked_PUT: !result.PUTJEA.MARKET_UNION_REACH.completed && Object.values(result.PUTJEA.MARKET_UNION_REACH.legs).every((leg) => !leg.credited) && putWithheldLegs.length >= 1,
      KIRSEK_at_or_better_24: threshold("KIRSEK", 24),
      ARNROM_at_or_better_89: threshold("ARNROM", 89),
      KRUFER_at_or_better_96: threshold("KRUFER", 96),
      BOSCOP_at_or_better_80: threshold("BOSCOP", 80),
    };
    return { law: "NAMED_COMPLETION_PASSES_AT_REQUIRED_COMBINED_PRICE_OR_BETTER; MECHANISM_FINGERPRINT_BINDS_TO_MECHANISM_NOT_A_SPECIFIC_TICK", thresholds: { PENTHA: 55, SHEOLI: 92, KIRSEK: 24, ARNROM: 89, KRUFER: 96, BOSCOP: 80 }, assertions, PUTJEA: { result: result.PUTJEA, dry_sibling_actions: putActions, withheld_leg_identities: putWithheldLegs }, by_machine: namedAttribution };
  })() : null;
  if (v44Acceptance) {
    v44Acceptance.deep_gap_guard_removed = { pass: machineRuns.get("V44_GUARD_SWAP").actions.every((row) => !String(row.kind).startsWith("DEEP_GAP_")) };
    v44Acceptance.named_checks = { pass: Object.values(namedV44.assertions).every(Boolean), assertions: namedV44.assertions };
    v44Acceptance.pass = v44Acceptance.baseline_reproduction.pass && v44Acceptance.completed_pairs.pass && v44Acceptance.naked_pnl_cents.pass && v44Acceptance.true_book_net_cents.pass && v44Acceptance.deep_gap_guard_removed.pass && v44Acceptance.named_checks.pass;
  }
  ...
  const v44AttributionScorecard = isV44 ? {
    order: machineSpecs.map((spec) => spec.name),
    rows: attributionRows.map(({ full_book_rows, ...row }) => row),
    V43_baseline_reproduction: v44BaselineReproduction,
    analytical_estimates_are_not_executable_scores: { recalibration_commit: V43_RECALIBRATION_COMMIT, projected_combined: v43RecalibrationReceipt.A_plus_B.score, status: "ANALYTICAL_ESTIMATE_ONLY_EXECUTABLE_ROWS_CONTROL" },
    deltas_vs_V43: attributionRows.map((row) => ({ machine: row.machine, completed_pairs_delta: row.MARKET_UNION_REACH.completed_pairs - 395, locked_cents_delta: row.FULL_BOOK.completed_locked_cents - 1910, naked_pnl_cents_delta: row.FULL_BOOK.naked_pnl_cents - (-162), true_book_net_cents_delta: row.FULL_BOOK.true_book_net_cents - 1748 })),
    acceptance: v44Acceptance,
  } : null;
  const core = {
    ...
    ...(isV44 ? {
      "GUARD_SWAP_BINDINGS.json": canonical({
        base: { commit: V43_COMMIT, role: "OPERATIVE_V43_EXECUTABLE_BASE", reproduced: v44BaselineReproduction },
        recalibration: { commit: V43_RECALIBRATION_COMMIT, path: v43RecalibrationPath, sha256: shaBytes(v43RecalibrationBytes), role: "ANALYTICAL_ESTIMATE_NOT_EXECUTABLE_SCORE", projected_A_plus_B: v43RecalibrationReceipt.A_plus_B },
        residual_census: { commit: V43_RESIDUAL_CENSUS_COMMIT, path: v43ResidualPath, sha256: shaBytes(v43ResidualBytes), role: "DEFECT_CENSUS", guard_withheld: v43ResidualReceipt.class_totals.GUARD_WITHHELD, no_flow_near: v43ResidualReceipt.class_totals.NO_FLOW_NEAR },
        removed_guard: { original_commit: DEEP_GAP_CENSUS_COMMIT, path: deepGapCensusPath, sha256: shaBytes(deepGapCensusBytes), status: "COMPOSITION_STALE_REMOVED_FROM_V44_COMBINED" },
        full_book_method: { commit: FULL_BOOK_PNL_COMMIT, path: fullBookReceiptPath, sha256: shaBytes(fullBookReceiptBytes) },
      }),
      "COMPOSITION_STALE_RECEIPT.json": canonical({ superseded_calibration_commit: DEEP_GAP_CENSUS_COMMIT, superseding_executable_base: V43_COMMIT, recalibration_commit: V43_RECALIBRATION_COMMIT, ruling: "T10_DEEP_GAP_STANDALONE_CALIBRATION_IS_COMPOSITION_STALE_AND_IS_DELETED_FROM_V44_COMBINED", executable_attribution_controls: true }),
      "DRY_SIBLING_EXECUTABILITY_RECEIPT.json": canonical({ analytical_wording: "A_LEG_FILL_IS_WITHHELD_WHILE_SIBLING_FLOW_IS_DRY", exchange_constraint: "AN_ALREADY_RESTING_ORDER_CANNOT_REFUSE_AN_OPPOSING_FILL", executable_realization: "WITHHOLD_OR_CANCEL_THE_REST_BEFORE_IT_CAN_FILL; PLACE_ON_THE_FIRST_RECEIPT_THAT_MAKES_THE_SIBLING_NON_DRY", impossible_model_rejected: "LEAVE_REST_LIVE_AND_IGNORE_A_LAWFUL_FILL_RECEIPT", no_retroactive_fill: true, parallel_paths_checked: ["ORDINARY_OWN_BOOK_PLACEMENT", "PAIR_CAP_TRANSITION", "DEEP_GAP_CROSS_LEG_REEVALUATION"], hard_invariant: "NO_LIVE_REST_MAY_REACH_A_FILL_WHILE_SIBLING_IS_DRY", consequence: "EXECUTABLE_CAUSAL_RESULT_CONTROLS_OVER_B503E4ED_POST_HOC_ANALYTICAL_ESTIMATE" }),
      "ATTRIBUTION_SCORECARD.json": canonical(v44AttributionScorecard),
      "DRY_SIBLING_WITHHOLD_RECEIPT.json": canonical({ law: control.architecture.added, executable_realization: "REST_WITHHELD_OR_CANCELLED_BEFORE_FILL; PLACED_ON_CAUSAL_LIFT_RECEIPT", threshold_cents: policy.DRY_SIBLING_NEAR_CENTS, lift: control.architecture.lift, union_channels: control.architecture.union_channels, market: { affected_legs: drySiblingLegs.length, withhold_episodes: drySiblingLegs.reduce((sum, row) => sum + row.withhold_episodes, 0), withheld_placement_evaluations: drySiblingLegs.reduce((sum, row) => sum + row.withheld_placement_evaluations, 0), lifts: drySiblingLegs.reduce((sum, row) => sum + row.lifts, 0), by_category: countBy(drySiblingLegs, (row) => row.category) }, two_columns_executable: { V43_baseline: attributionByName.get("V43_BASELINE").FULL_BOOK, dry_sibling_alone: attributionByName.get("DRY_SIBLING_ONLY").FULL_BOOK, V44_combined: attributionByName.get("V44_GUARD_SWAP").FULL_BOOK }, analytical_estimate: { commit: V43_RECALIBRATION_COMMIT, score: v43RecalibrationReceipt.A_plus_B.score, controlling: false } }),
      "DRY_SIBLING_WITHHOLD_LEG_LEDGER.jsonl.gz": gzipRows(drySiblingLegs),
      "DRY_SIBLING_ACTION_LEDGER.jsonl.gz": gzipRows(drySiblingActions),
      "COMPOSITION_ACCEPTANCE_BAR.json": canonical(v44Acceptance),
      "CONSTRUCTION_STATUS.json": canonical({ status: v44Acceptance.pass ? "PASS_OPERATIVE" : "BLOCKED_NOT_OPERATIVE", operative_baseline: v44Acceptance.pass ? "V44_GUARD_SWAP" : "V43_01a58334", reasons: [
        ...(!v44Acceptance.baseline_reproduction.pass ? ["V43_BASELINE_DID_NOT_REPRODUCE"] : []),
        ...(!v44Acceptance.deep_gap_guard_removed.pass ? ["DEEP_GAP_GUARD_SURVIVED_IN_COMBINED"] : []),
        ...(!v44Acceptance.completed_pairs.pass ? ["COMPLETION_BAR_FAILED"] : []),
        ...(!v44Acceptance.naked_pnl_cents.pass ? ["NAKED_BOOK_BAR_FAILED"] : []),
        ...(!v44Acceptance.true_book_net_cents.pass ? ["TRUE_BOOK_BAR_FAILED"] : []),
        ...(!v44Acceptance.named_checks.pass ? ["NAMED_MECHANISM_OR_AT_OR_BETTER_CHECK_FAILED"] : []),
      ], no_forced_values: true, executable_truth_controls: true }),
      "NAMED_V44_RECEIPT.json": canonical(namedV44),
      "FULL_BOOK_PNL.json": canonical({ method: { commit: FULL_BOOK_PNL_COMMIT, receipt_path: fullBookReceiptPath, receipt_sha256: shaBytes(fullBookReceiptBytes), close_audit_path: closeAuditPath, close_audit_sha256: shaBytes(closeAuditBytes), close_column: "replay_close_cents", completed: "100_MINUS_PAIR_ENTRY", naked: "FROZEN_REPLAY_WINDOW1_CLOSE_MINUS_ENTRY_WHERE_AVAILABLE", skip: 0 }, rows: attributionRows.map((row) => ({ machine: row.machine, aggregate: row.FULL_BOOK, by_category: row.by_category.FULL_BOOK })), acceptance: v44Acceptance }),
    } : {}),
    ...(isV43 ? {
    ...
    "NAMED_GAMES.json": canonical({ games: named, action_rows: allActions.filter((row) => namedLabels.some((name) => row.event_id.includes(name)) && (row.kind === "FILL" || String(row.kind).includes("DEEP_GAP") || String(row.kind).includes("DRY_SIBLING") || String(row.reason).includes("PERSISTENT") || String(row.reason).includes("FIRST_OBSERVATION") || String(row.reason).includes("ONE_CENT_LESS_GREEDY") || String(row.reason).includes("WTA_OTHER_EXPRESSION_FALLING"))) }),
    ...
    "SOURCE_HASH_MANIFEST.json": canonical({
      ...
      public: {
        ...
        ...(isV44 ? {
          ...
          "arb-executor/tests/test_window1_v44_guard_swap.js": { sha256: fileHash(path.join(repo, "arb-executor/tests/test_window1_v44_guard_swap.js")), bytes: fs.statSync(path.join(repo, "arb-executor/tests/test_window1_v44_guard_swap.js")).size },
          "arb-executor/tests/test_window1_v44_guard_swap_package.js": { sha256: fileHash(path.join(repo, "arb-executor/tests/test_window1_v44_guard_swap_package.js")), bytes: fs.statSync(path.join(repo, "arb-executor/tests/test_window1_v44_guard_swap_package.js")).size },
        } : {}),
  ...
  write("REPORT.md", isV44
    ? `# V44 guard swap — ${v44Acceptance.pass ? "PASS / OPERATIVE" : "BLOCKED / V43 REMAINS OPERATIVE"}\n\nV44 makes exactly two changes to operative V43: deletes the composition-stale T=10 deep-gap guard and withholds a potential fill while the sibling's decision-time union flow has never come within 3c of any causally observed lawful sibling rest level. The withhold lifts on the first qualifying receipt. Arm-at-first-evidence, +1c loosening, maker-only fills, sanity bound, lazy-leg-1 coupling, pair cap, no-clock law, and hard pre-bell edge remain inherited.\n\n${attributionRows.map((row) => `- ${row.machine}: completed ${row.MARKET_UNION_REACH.completed_pairs}, under par ${row.MARKET_UNION_REACH.under_par_pairs}, locked ${row.FULL_BOOK.completed_locked_cents}c, naked ${row.FULL_BOOK.naked_pnl_cents}c, true book ${row.FULL_BOOK.true_book_net_cents}c, frontier ${row.MARKET_UNION_REACH.frontier.LE_93}/${row.MARKET_UNION_REACH.frontier.LE_95}/${row.MARKET_UNION_REACH.frontier.LE_97}/${row.MARKET_UNION_REACH.frontier.LT_100}; strict ${row.STRICT_PRINT_CROSS.completed_pairs}.`).join("\n")}\n\n- V43 executable baseline reproduction: ${v44BaselineReproduction.pass ? "PASS" : "FAIL"}.\n- b503e4ed's 415/+2165 row is retained as ANALYTICAL_ESTIMATE only; the executable score above controls.\n- Dry-sibling affected ${drySiblingLegs.length} legs across ${drySiblingLegs.reduce((sum, row) => sum + row.withhold_episodes, 0)} episodes with ${drySiblingLegs.reduce((sum, row) => sum + row.lifts, 0)} causal lifts.\n- Bar: completed >=395 ${v44Acceptance.completed_pairs.pass ? "PASS" : "FAIL"}; naked >=0c ${v44Acceptance.naked_pnl_cents.pass ? "PASS" : "FAIL"}; true book >1748c ${v44Acceptance.true_book_net_cents.pass ? "PASS" : "FAIL"}; named ${v44Acceptance.named_checks.pass ? "PASS" : "FAIL"}; overall ${v44Acceptance.pass ? "PASS" : "BLOCKED"}.\n- Named combined entries: PENTHA ${named.PENTHA.MARKET_UNION_REACH.combined_entry_cents ?? "INCOMPLETE"}; SHEOLI ${named.SHEOLI.MARKET_UNION_REACH.combined_entry_cents ?? "INCOMPLETE"}; KIRSEK ${named.KIRSEK.MARKET_UNION_REACH.combined_entry_cents ?? "INCOMPLETE"}; ARNROM ${named.ARNROM.MARKET_UNION_REACH.combined_entry_cents ?? "INCOMPLETE"}; KRUFER ${named.KRUFER.MARKET_UNION_REACH.combined_entry_cents ?? "INCOMPLETE"}; BOSCOP ${named.BOSCOP.MARKET_UNION_REACH.combined_entry_cents ?? "INCOMPLETE"}; PUTJEA no-naked-PUT ${namedV44.assertions.PUTJEA_both_withheld_dry_skip_no_naked_PUT ? "PASS" : "FAIL"}.\n- Market value uses CANON union channels; strict print crossing remains build verification only.\n`
    : isV43
