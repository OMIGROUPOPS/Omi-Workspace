...
const policy = require("./window1_v351_directional_evidence_aging.js");
...
const output = path.resolve(arg("--output", path.join(repo, ".claude/window1_live_v4_replay/v351_directional_evidence_aging_20260806")));
...
const policyFile = path.join(repo, "arb-executor/analysis/window1_v351_directional_evidence_aging.js");
const unitTestFile = path.join(repo, "arb-executor/tests/test_window1_v351_directional_evidence_aging.js");
const packageTestFile = path.join(repo, "arb-executor/tests/test_window1_v351_directional_evidence_aging_package.js");
...
function safeClean(dir) { const r = path.resolve(dir); ensure(path.basename(r).toLowerCase().includes("v35"), `unsafe output ${r}`); ensure(r !== repo && r !== path.parse(r).root, `unsafe output ${r}`); fs.rmSync(r, { recursive: true, force: true }); fs.mkdirSync(r, { recursive: true }); }
...
function simulateW1Mode(baseEvent, tapes, prints, sources, bells, mode, actions, decisions, span, timeline, boundary) {
  ...
  for (const id of ids) {
    ...
    event.legs[id] = {
      ...
      maker_reprices: 0,
      rest_sanity: { tracked_book_receipts: 0, exact_target_receipts: 0, gap_abs_max_cents: 0, reprice_up: 0, reprice_down: 0, target_min_cents: null, target_max_cents: null, best_bid_min_cents: null, best_bid_max_cents: null },
      first_decision: null,
      last_decision: null,
      first_action: null,
  ...
  for (const row of timeline) {
    ...
    const activeEvidenceFloor = policy.directionallyAgedEvidenceFloor({
      state: combined.state,
      runningEvidenceFloor,
      receiptLocalEvidenceFloor,
      reformedQualifyingAskFloor,
      qualifyingAskFloorReformedNow,
    });
    const evidenceAuthority = combined.state === "FALLING" && Number.isInteger(runningEvidenceFloor)
      ...
      : Number.isInteger(receiptLocalEvidenceFloor)
        ...
        : Number.isInteger(reformedQualifyingAskFloor) && activeEvidenceFloor === reformedQualifyingAskFloor && qualifyingAskFloorReformedNow
          ? "RISEN_OR_SETTLED_REFORMED_QUALIFYING_ASK_FLOOR"
    ...
    const decision = policy.decide({ state: combined.state, book: row, activeTarget: before, pairCap: leg.pair_cap_cents, activeEvidenceFloor, floorFirstFlickerLive });
    leg.prior_book = row;
    ...
    if (!leg.credited && leg.active_order) {
      const expected = policy.livingRestTarget({ bid: row.bid, pairCap: leg.pair_cap_cents });
...
function differentialV34W1(events, baselineEvents, mode) {
  ...
  for (const event of events) {
    ...
    for (const [id, leg] of Object.entries(event.legs)) {
      ...
      rows.push({ mode, event_id: event.event_id, leg_identity: leg.leg_identity, category: leg.category, price_region: leg.price_region, bell_confidence: event.bell_confidence, disposition, V34_W1: before, V35_1: after });
    }
  }
  const cells = [...group(rows, (row) => `${row.category}|${row.price_region}|${row.bell_confidence}`).entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([cell, subset]) => ({ cell, legs: subset.length, dispositions: countBy(subset, (row) => row.disposition) }));
...
function differentialV35(events, baselineEvents, mode) {
  ...
  for (const event of events) {
    ...
    for (const [id, leg] of Object.entries(event.legs)) {
      ...
      rows.push({ mode, event_id: event.event_id, leg_identity: leg.leg_identity, category: leg.category, price_region: leg.price_region, bell_confidence: event.bell_confidence, disposition, V35: before, V35_1: after });
    }
  }
  const cells = [...group(rows, (row) => `${row.category}|${row.price_region}|${row.bell_confidence}`).entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([cell, subset]) => ({ cell, legs: subset.length, dispositions: countBy(subset, (row) => row.disposition) }));
...
function bleedDelta(v351Events, v34Events, bleed) {
  const v34By = new Map(v34Events.map((event) => [event.event_id, event]));
  const v351By = new Map(v351Events.map((event) => [event.event_id, event]));
  ...
  for (const before of v34Events) {
    ...
    const after = v351By.get(before.event_id); ensure(after, `bleed event missing ${before.event_id}`);
    const mechanism = mechanismByEvent.get(before.event_id) || "MODE_SPECIFIC_BASELINE_NON_COMPLETION_OUTSIDE_STRICT_BLEED_CENSUS";
    mechanismRows.push({ event_id: before.event_id, category: after.category, starting_price_split: after.starting_price_split, bell_confidence: after.bell_confidence, V34_mechanism: mechanism, V34_completed: before.completed_pair, V351_completed: after.completed_pair, V351_under_par: after.pair_under_par, disposition: after.pair_under_par ? "CONVERTED_TO_UNDER_PAR" : after.completed_pair ? "COMPLETED_NOT_UNDER_PAR" : "NOT_CONVERTED" });
  }
  const newLossRows = v34Events.filter((before) => before.completed_pair && !v351By.get(before.event_id).completed_pair).map((before) => ({ event_id: before.event_id, category: before.category, starting_price_split: before.starting_price_split, bell_confidence: before.bell_confidence, V34_completed: true, V351_completed: false, V351_credited_legs: Object.values(v351By.get(before.event_id).legs).filter((leg) => leg.credited).length }));
  const depthRows = v351Events.flatMap((event) => Object.values(event.legs).filter((leg) => leg.credited).map((leg) => ({ event_id: event.event_id, leg_identity: leg.leg_identity, category: leg.category, price_region: leg.price_region, bell_confidence: event.bell_confidence, V34_mechanism: mechanismByEvent.get(event.event_id) || "V34_COMPLETED_BASELINE", fill_class: leg.fill_class, entry_cents: leg.entry_cents, eventual_print_low_cents: leg.w1_print_backed_achievable_floor_cents, fill_depth_vs_eventual_low_cents: leg.entry_minus_print_backed_floor_cents })));
...
function restSanity(events, actions, mode) {
  ...
  return { law: "ON_EVERY_OWN_BOOK_RECEIPT_ACTIVE_REST_EQUALS_MIN(BEST_BID_MINUS_1,PAIR_CAP); NO_FORMATION_ANCHOR_NO_DOWN_ONLY_CONSTRAINT_NO_SPREAD_GATE", aggregate: { legs: rows.length, tracked_book_receipts: tracked, exact_target_receipts: exact, non_exact_target_receipts: tracked - exact, max_abs_gap_cents: Math.max(...rows.map((row) => row.gap_abs_max_cents)), reprice_up: rows.reduce((sum, row) => sum + row.reprice_up, 0), reprice_down: rows.reduce((sum, row) => sum + row.reprice_down, 0) }, category_x_price_region_x_bell_confidence: cells, named_checks: named, rows, conservation: { rows: rows.length, expected_legs: 1608, cell_legs: cells.reduce((sum, row) => sum + row.legs, 0), pass: rows.length === 1608 && cells.reduce((sum, row) => sum + row.legs, 0) === 1608 } };
}
...
async function causalMain() {
  ...
  for (const commit of [V35_COMMIT, V34_W1_CAUSAL_COMMIT, V34_FULL_LIFE_COMMIT, V34_TRADING_PHASE_COMMIT, V32_COMMIT, R3_COMMIT, RECON_COMMIT, NEARMISS_COMMIT, START_LEDGER_COMMIT, OFFER_COMMIT, BLEED_COMMIT, EXEMPLAR_COMMIT]) requireCommit(commit);
  safeClean(output);
  const baseEvents = loadBaseEvents(); ensure(baseEvents.length === 804, "D must be 804");
  ...
  process.stderr.write("V351_STAGE print_spool_start\n");
  const printLoad = buildPrintSpool(tickers);
  process.stderr.write("V351_STAGE print_spool_complete\n");
  ...
  const decisionStats = { total: 0, strict: 0, census: 0, post_edge: 0, scheduled_clock_non_null: 0, actual_bell_clock_non_null: 0, boundary_clock_non_null: 0 };
  ...
  for (const baseEvent of baseEvents) {
    ...
    strictEvents.push(simulateW1Mode(baseEvent, tapes, prints, sources, bells, "STRICT_LAW", actions, strictDecisionRows, span, timeline, boundary));
    censusEvents.push(simulateW1Mode(baseEvent, tapes, prints, sources, bells, "CENSUS_PRICED", actions, censusDecisionRows, span, timeline, boundary));
    flushDecisions(strictDecisionRows);
    flushDecisions(censusDecisionRows);
    if (strictEvents.length % 50 === 0) process.stderr.write(`V351_STAGE replay_events_${strictEvents.length}\n`);
  }
  printLoad.cleanup();
  ...
  const reconBytes = gitShow(RECON_COMMIT, RECON_PATH), recon = JSON.parse(reconBytes), nearBytes = gitShow(NEARMISS_COMMIT, NEARMISS_PATH), near = JSON.parse(nearBytes), offerBytes = gitShow(OFFER_COMMIT, OFFER_PATH), offer = JSON.parse(offerBytes), publicManifest = JSON.parse(fs.readFileSync(publicTapeManifest));
  ...
  const binding = { schema_version: "window1-v351-directional-evidence-aging-control-v1", machine: { architecture: "V35_WITH_ONE_DIRECTIONAL_EVIDENCE_AGING_REPAIR", V35_baseline_commit: V35_COMMIT, V34_W1_baseline_commit: V34_W1_CAUSAL_COMMIT, V32_lineage_commit: V32_COMMIT, policy_path: path.relative(repo, policyFile).replaceAll("\\", "/"), policy_sha256: fileHash(policyFile), clock_decision_inputs: [], receipt_driven_evidence_aging_only: true }, unchanged_from_V35: { living_rest: "EVERY_BOOK_RECEIPT_REANCHORS_TO_MIN(BEST_BID_MINUS_1,PAIR_CAP)_UP_OR_DOWN_NO_SPREAD_GATE", strict_fill_law: "UNCHANGED", pair_cap: "UNCHANGED", hard_pre_bell_edge: "UNCHANGED", close_free_grading: "UNCHANGED" }, repair: { directional_evidence_aging: `ALL_TIME_DOWNWARD_MINIMUM_REMAINS_AUTHORITATIVE_ONLY_WHILE_COMBINED_OWN_SIDE_STATE_IS_FALLING; OTHERWISE_RECEIPT EVIDENCE_INSIDE_EXISTING_${policy.LOOKBACK_SECONDS}S_HORIZON_REMAINS_AUTHORITATIVE; WHEN_NEITHER_APPLIES_ONLY_A_QUALIFYING_ASK_FLOOR_GENUINELY_REFORMED_ON_THE_CURRENT_NON_FALLING_RECEIPT_CAN_REPLACE_THE_STALE_MINIMUM; AN_OLDER_DESCENT_ORIGIN_SHELF_IS_NOT_RELABELLED; NO_TIMER_NO_WALL_CLOCK_TRIGGER`, no_first_flicker_overpay: `A_NEW_QUALIFYING_ASK_FLOOR_CANNOT_AUTHORIZE_ITS_OWN_TAKE_WHILE_DOWNWARD_EVIDENCE_REMAINS_LIVE; REASK_ON_EACH_BOOK_RECEIPT` }, window_law: { left: "FIRST_TWO_SIDED_BOOK_ON_EITHER_EXPRESSION", right_precedence: ["exact_start_utc", "known_live_by_utc", "schedule_bound_utc"], hard_edge: "NO_REST_WALK_NO_TAKE_NO_FILL_NO_CAP_ARM_NO_STATE_UPDATE_AFTER_EDGE", boundary_is_reporting_and_admission_only_not_AIM_input: true }, close_law: { role: "TELEMETRY_ONLY", grading_reads: 0, settlement_close_reads: 0 }, comparisons: { V35_strict_completed_under_par: 264, V35_census_completed_under_par: 550, V34_W1_strict_completed_under_par: 254, V34_W1_census_completed_under_par: 279, R3_original_joint_reference_only: 68, R3_same_V3_window_artifact: "R3_SAME_WINDOW_COMPARISON.json", operator_named_under_par_offer_census_adjusted: 680 } };
  ...
  ensure(strictArn?.completed_pair && strictArn?.pair_under_par && strictArn?.combined_entry_cents === 94, "ARNROM must complete 38+56=94");
  ensure(strictArn.legs.ARN.entry_cents === 56 && strictArn.legs.ROM.entry_cents === 38, "ARNROM named leg prices failed");
  const acceptanceChecks = {
    strict_completed_at_least_V35_264: strictScore.aggregate.completed_pairs >= 264,
    strict_LE93_at_least_V35_9: strictFrontier.cumulative_frontier.LE_93.completed_pairs >= 9,
    strict_LE95_at_least_V35_25: strictFrontier.cumulative_frontier.LE_95.completed_pairs >= 25,
    strict_LE97_at_least_V35_82: strictFrontier.cumulative_frontier.LE_97.completed_pairs >= 82,
    at_least_one_strict_deep_tier_improves: [
      [strictFrontier.cumulative_frontier.LE_93.completed_pairs, 9],
      [strictFrontier.cumulative_frontier.LE_95.completed_pairs, 25],
      [strictFrontier.cumulative_frontier.LE_97.completed_pairs, 82]
    ].some(([actual, baseline]) => actual > baseline)
  };
  const acceptance = {
    status: Object.values(acceptanceChecks).every(Boolean) ? "PASS" : "REJECTED_DEEP_FRONTIER_REGRESSION_V35_REMAINS_OPERATIVE",
    checks: acceptanceChecks,
    required_floor: { strict_completed_pairs: 264, strict_frontier: { LE_93: 9, LE_95: 25, LE_97: 82 } },
    observed: { strict_completed_pairs: strictScore.aggregate.completed_pairs, strict_frontier: { LE_93: strictFrontier.cumulative_frontier.LE_93.completed_pairs, LE_95: strictFrontier.cumulative_frontier.LE_95.completed_pairs, LE_97: strictFrontier.cumulative_frontier.LE_97.completed_pairs } },
    ruling: "A_NAMED_REGRESSION_PASS_DOES_NOT_OVERRIDE_POPULATION_FRONTIER_REGRESSION; V35.1_IS_FROZEN_AS_A_REJECTED_VARIANT_AND_V35_REMAINS_OPERATIVE"
  };
  ensure(namedEvidenceGate["KXATPCHALLENGERMATCH-26JUL12KRALOR|LOR"].fills.some((row) => row.entry_cents === 5), "KRALOR|LOR must keep 5");
  ensure(bos32Decisions.length > 0, "BOSCOP|BOS must keep 32");
  const arnAgingFill = strictNamedActions.find((row) => row.leg_identity === "KXATPCHALLENGERMATCH-26JUL12ARNROM|ARN" && row.kind === "FILL");
  ensure(arnAgingFill?.entry_cents === 56 && arnAgingFill?.evidence_authority === "RISEN_OR_SETTLED_REFORMED_QUALIFYING_ASK_FLOOR", "ARN directional aging authority failed");
  const namedChecks = { DIRECTIONAL_EVIDENCE_AGING: { ARNROM_ARN: { expected_entry_cents: 56, fill: arnAgingFill, combined_entry_cents: strictArn.combined_entry_cents, pair_under_par: strictArn.pair_under_par }, law: `ALL_TIME_DOWNWARD_EVIDENCE_EXPIRES_AS_AUTHORITY_WHEN_SIDE_IS_NOT_FALLING_AND_NO_DOWNWARD_RECEIPT_REMAINS_INSIDE_${policy.LOOKBACK_SECONDS}S; CURRENT_QUALIFIED_ASK_REFORMS_FLOOR_ON_RECEIPT` }, BOSCOP_COP: { rest_actions_45_to_66: actionSummary(bosCopRequiredBand), seller_sweep_fill_rows_47_to_51: actionSummary(bosCopSweepFills), any_rest_in_required_band: bosCopRequiredBand.length > 0, complete_rows_source: "ACTION_AND_FILL_TRACE_PARTS.json" }, POLKUH_POL: { named_seller_timestamp_epoch: 1783872979, last_rest_action_at_or_before_seller: polActionsBeforeNamedSeller.at(-1) || null }, NO_FIRST_FLICKER_OVERPAY: namedEvidenceGate, ARNROM: { span: arnSpan, STRICT_LAW: strictArn, CENSUS_PRICED: censusArn } };
  const core = {
    ...
    "CENSUS_PRICED_65D49B5D_BINDING.json": canonical({ commit: NEARMISS_COMMIT, path: NEARMISS_PATH, source_sha256: sha(nearBytes), source: { waited_and_lost: near.summary.conservation.waited_and_lost_censused, one_cent_near_miss_rests: near.summary.totals.with1, v32_joint: near.summary.conservation.V32_executable_joint, model_free_ceiling: near.summary.conservation.model_free_ceiling_joint }, V351_application: "DYNAMIC_ACTIVE_V35_LIVING_REST_TRAJECTORY_WITH_DIRECTIONALLY_AGED_TAKE_AUTHORITY_INSIDE_V3_PRE_MATCH_SPAN_ONLY; FIRST_LAWFUL_ONE_CENT_SELLER_PRINT_CONVERTS_AT_CURRENT_REST_IN_CENSUS_COLUMN" }),
    ...
    "SOURCE_HASH_MANIFEST.json": canonical({ public_committed: Object.fromEntries([builderFile, policyFile, unitTestFile, packageTestFile, floorFile, baseTraceFile, quoteFile, bellFile, spoolHelperFile, spoolHelperSourceFile].map((file) => [path.relative(repo, file).replaceAll("\\", "/"), { sha256: fs.existsSync(file) ? fileHash(file) : null, bytes: fs.existsSync(file) ? fs.statSync(file).size : null }])), git_bound: { [START_LEDGER_PATH]: { commit: START_LEDGER_COMMIT, sha256: sha(startBytes), bytes: startBytes.length }, [R3_EVENT_PATH]: { commit: R3_COMMIT, sha256: sha(r3Bytes), bytes: r3Bytes.length }, [V35_STRICT_PATH]: { commit: V35_COMMIT, sha256: sha(v35StrictBytes), bytes: v35StrictBytes.length }, [V35_CENSUS_PATH]: { commit: V35_COMMIT, sha256: sha(v35CensusBytes), bytes: v35CensusBytes.length }, [V34_W1_STRICT_PATH]: { commit: V34_W1_CAUSAL_COMMIT, sha256: sha(v34StrictBytes), bytes: v34StrictBytes.length }, [V34_W1_CENSUS_PATH]: { commit: V34_W1_CAUSAL_COMMIT, sha256: sha(v34CensusBytes), bytes: v34CensusBytes.length }, [BLEED_PATH]: { commit: BLEED_COMMIT, sha256: sha(bleedBytes), bytes: bleedBytes.length }, [RECON_PATH]: { commit: RECON_COMMIT, sha256: sha(reconBytes), bytes: reconBytes.length }, [NEARMISS_PATH]: { commit: NEARMISS_COMMIT, sha256: sha(nearBytes), bytes: nearBytes.length }, [OFFER_PATH]: { commit: OFFER_COMMIT, sha256: sha(offerBytes), bytes: offerBytes.length } }, private_full_life_prints: printLoad.receipt, private_full_life_books: tapeHashes, public_tape_manifest: { sha256: fileHash(publicTapeManifest), bytes: fs.statSync(publicTapeManifest).size } }),
  };
  for (const [name, bytes] of Object.entries(core)) write(name, bytes);
  write("REPORT.md", `# V35.1 directional evidence aging\n\nV35.1 makes one repair to V35 ${V35_COMMIT}: the all-time downward evidence minimum remains authoritative only while the side's combined state is FALLING. Otherwise, downward receipts remain authoritative only inside the existing ${policy.LOOKBACK_SECONDS}-second receipt horizon; after they age and the side is RISING or SETTLED, only a qualifying-ask floor genuinely re-formed by the current non-falling receipt can replace the stale minimum. An older descent-origin shelf cannot be relabeled by a later flicker. The transition is evaluated only on book receipts. There is no timer or wall-clock action trigger. Living-rest mechanics, strict/census fill laws, pair cap, hard V3 PRE-MATCH edge, and close-free grading are unchanged.\n\n- STRICT completed / under par: ${strictScore.aggregate.completed_pairs} / ${strictScore.aggregate.pairs_under_par}; V35: ${v35StrictScore.aggregate.completed_pairs} / ${v35StrictScore.aggregate.pairs_under_par}; V34-W1: ${v34StrictScore.aggregate.completed_pairs} / ${v34StrictScore.aggregate.pairs_under_par}.\n- CENSUS completed / under par: ${censusScore.aggregate.completed_pairs} / ${censusScore.aggregate.pairs_under_par}; V35: ${v35CensusScore.aggregate.completed_pairs} / ${v35CensusScore.aggregate.pairs_under_par}; V34-W1: ${v34CensusScore.aggregate.completed_pairs} / ${v34CensusScore.aggregate.pairs_under_par}.\n- R3 same-window completed / under par: ${r3Comparison.same_V3_span_metrics.completed_pairs} / ${r3Comparison.same_V3_span_metrics.pairs_under_par}; original joint reference: 68.\n- STRICT maker / taker: ${strictFillSplit.aggregate.maker} / ${strictFillSplit.aggregate.taker}.\n- CENSUS maker / taker / one-cent conversion: ${censusFillSplit.aggregate.maker} / ${censusFillSplit.aggregate.taker} / ${censusFillSplit.aggregate.census_priced}.\n- STRICT bleed conversions: ${JSON.stringify(strictBleed.converted_by_V34_mechanism)}; new losses from V34 completed: ${strictBleed.new_losses_from_V34_completed.count}.\n- CENSUS bleed conversions: ${JSON.stringify(censusBleed.converted_by_V34_mechanism)}; new losses from V34 completed: ${censusBleed.new_losses_from_V34_completed.count}.\n- STRICT rest exact tracking: ${strictRestSanity.aggregate.exact_target_receipts}/${strictRestSanity.aggregate.tracked_book_receipts}; max gap ${strictRestSanity.aggregate.max_abs_gap_cents}c; reprices up/down ${strictRestSanity.aggregate.reprice_up}/${strictRestSanity.aggregate.reprice_down}.\n- CENSUS rest exact tracking: ${censusRestSanity.aggregate.exact_target_receipts}/${censusRestSanity.aggregate.tracked_book_receipts}; max gap ${censusRestSanity.aggregate.max_abs_gap_cents}c; reprices up/down ${censusRestSanity.aggregate.reprice_up}/${censusRestSanity.aggregate.reprice_down}.\n- ARNROM STRICT: ${strictArn.legs.ROM.entry_cents} + ${strictArn.legs.ARN.entry_cents} = ${strictArn.combined_entry_cents}, under par ${strictArn.pair_under_par}.\n- Post-edge machine rows: ${hardEdgeViolations.length + decisionStats.post_edge}. Close fields consumed by grading: 0.\n\nThe supplied short identity 84b455c5 remains unresolved. The actual 804-row ledger is bound through Git path history at ${START_LEDGER_COMMIT}, SHA-256 ${sha(startBytes)}.\n`);
  write("REJECTION_REPORT.md", `# V35.1 acceptance ruling\n\nStatus: **${acceptance.status}**.\n\nThe named ARNROM repair succeeds at 38 + 56 = 94, but the strict deep frontier is ${acceptance.observed.strict_frontier.LE_93}/${acceptance.observed.strict_frontier.LE_95}/${acceptance.observed.strict_frontier.LE_97} versus V35 ${acceptance.required_floor.strict_frontier.LE_93}/${acceptance.required_floor.strict_frontier.LE_95}/${acceptance.required_floor.strict_frontier.LE_97}. V35 remains operative. V35.1 is evidence, not a shipping baseline.\n`);
  const compareNames = [...Object.keys(core), ...fullTraceParts.parts.map((row) => row.name), ...actionTraceParts.parts.map((row) => row.name), "REPORT.md", "REJECTION_REPORT.md"].sort();
  if (compare) {
