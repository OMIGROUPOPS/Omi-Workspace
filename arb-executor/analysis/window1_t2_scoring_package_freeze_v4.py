#!/usr/bin/env python3
"""Two-clean-build freezer for the score-free T2 package V4."""
...
from window1_t2_scoring_package_builder_v4 import build


VERSION = "window1-t2-scoring-package-freeze-v4"


class FreezeError(RuntimeError):
    """The two V4 builds were not byte-identical."""
...
def freeze(*, repo: Path, output: Path) -> dict[str, object]:
    if output.exists():
        raise FreezeError("frozen V4 output already exists")
    with tempfile.TemporaryDirectory(
        prefix="w1-t2-scorepkg-v4-a-"
    ) as a_text, tempfile.TemporaryDirectory(
        prefix="w1-t2-scorepkg-v4-b-"
    ) as b_text:
        ...
        if hashes_a != hashes_b:
            ...
            raise FreezeError(
                "clean V4 builds differ: " + ",".join(mismatch[:20])
            )
    ...
    if frozen_regenerable != build_regenerable:
        raise FreezeError("frozen V4 differs from clean builds")
