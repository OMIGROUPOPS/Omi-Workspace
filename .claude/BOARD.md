...
- **EARLY-CANVAS STUDY (Part 2, running detached on the VPS)** — floor-by-hour extended T-8h→T-24h: coverage-first per (cat, hour-bin); lattice vs prints; fillability both conventions; drift information content; anti-selection on early prints; per-game FLOW-STATE transitions fitted from the early-floor population (distributions, not medians). Verdict per cat = does anything before T-8h ever justify a resting bid; findings only — the horizon moves on operator ruling. **Status check 3:35 pm ET 07-08: ALIVE (PID 1459853 since 2:28 pm ET; watcher healthy, 0 restarts), 1800/2460 pairs (~73%), heartbeat fresh; ETA ~9:30 pm ET. Render + commit on landing.**
