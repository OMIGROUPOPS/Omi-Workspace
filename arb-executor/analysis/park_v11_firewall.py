...
GAP, WIN = 6, 3
FLOOR = 0.45          # occ*cond floor (single global). Tuned-once read; justified below by the diff.
...
def fireable_mode(c):
    ...
    low = res[0]                      # already sorted ascending by center
    if low['reach']: return None      # reach-region veto: lowest resolved mode is a jackpot -> dead
    return low
...
def state(c, discount=0):
    ...
    ev, holdEV, sgate = ev_paid_supp_X(c, fm['center'], discount)
    if sgate <= 0: return 'PARK-engine'             # resolved low mode but EV/paid/supp dead now
    if ev <= holdEV: return 'PARK-engine' if ev <= 0 else 'HOLD'
    return 'EXIT'
...
if __name__ == '__main__':
    ...
    print(f"  c5 fireable_mode = {fireable_mode(5)}  -> state={B[5]}  (expect PARK-boundary-ish: engine starts c6)")
