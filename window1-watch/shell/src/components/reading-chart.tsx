...
import { bellTime, cents, plain, readingCard, readingSentence, sentenceFor, type ReadingAction } from '@/lib/reading-view';
...
export function ReadingChart({ game, frame, side, receipt, onReceipt }: {
  ...
  const domain = useMemo(() => {
    const values = [...rows.flatMap(r => [getLast(r), getRest(r)]), ...oracle.map(r => r.perfect), ...actions.map(a => a.marker_cents), floor?.floor_cents].filter((v): v is number => typeof v === 'number' && Number.isFinite(v));
  ...
  const { width: w, height: h } = size, left = 10, right = w - 10, top = 24, bottom = h - 24;
  ...
  const pastActions = actions.filter(a => now.receipt_index != null && a.receipt_index <= now.receipt_index && a.marker_cents != null && a.minutes_to_bell != null);
  const fill = game.face.render.fill_events.find(f => f.leg === side && f.receipt_index <= (now.receipt_index ?? -1));
  const coincident = floor?.floor_cents != null && oracle.some(r => r.perfect != null) && oracle.every(r => r.perfect == null || r.perfect === floor.floor_cents);
  return <section className="reading-side" aria-label={`${side} chart and sentence`}>
    <header className="reading-sentence" title={`Last seen ${cents(sentence?.P)}; forecast ${cents(sentence?.Q)}; expected ${sentence?.X ?? 'no time recorded'} minutes to bell.\n${memberCount == null ? 'No pool count here' : `Pool of ${memberCount} games`}.\nTrace ${game.face.provenance.trace_sha256}\n${receipt?.receipt ?? 'No receipt'}`}>
      <strong>{side}</strong><h2>{readingSentence(sentence)}</h2>
    </header>
    <div className="reading-plot" ref={host} onMouseLeave={() => { setHover(null); setFloorHover(false); }}>
      <svg viewBox={`0 0 ${w} ${h}`} role="img" aria-label={`${side}: recorded tape, our bid, fills, recorded floor and perfect sentence. Hover or tap for exact values.`}>
        ...
        {[.25,.5,.75].map(p => <line key={p} className="reading-grid" x1={left} x2={right} y1={top+(bottom-top)*p} y2={top+(bottom-top)*p}/>) }
        ...
        <g clipPath={`url(#${id}-past)`}>
          <path data-tape-path className="reading-tape" d={paths.tape}/>
      ...
      <div className="reading-line-key"><span>tape</span><span className="reading-bid-key">{fill ? 'bid filled' : 'our bid'}</span><span>{coincident ? 'recorded floor = perfect sentence' : 'recorded floor · perfect sentence'}</span></div>
      ...
      {hover ? <div className="reading-tip" role="tooltip" style={{left:Math.max(8,Math.min(hover.x,w-310))}}>
        <p>{side} · {bellTime(hover.row.minutesToBell)} to bell</p><p>Last trade {cents(getLast(hover.row))} · our bid {getRest(hover.row)==null?'none':cents(getRest(hover.row))}</p>
      ...
      {action ? <div className="reading-tip reading-action-tip" role="tooltip" onKeyDown={e=>{if(e.key==='Escape')setAction(null)}}>
        <FourLineCard lines={readingCard(action)} details={action.details_lines} color="var(--color-rest)"/>
