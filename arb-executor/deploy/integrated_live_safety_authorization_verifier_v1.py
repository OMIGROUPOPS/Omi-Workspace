#!/usr/bin/env python3
"""Fail-closed authorization verifier for the Integrated P0 v1-v4 + CASUKA D1-D3 ceremony.
...
REPORT_TITLE = "# Integrated P0 v1-v4 + CASUKA D1-D3 DEPLOYMENT AUTHORIZATION V2\n\n"
REPORT_EXPLANATION = (
...
def _require_commit(repo: Path, commit: str, label: str) -> None:
    ...
    if check.returncode != 0 or _git(repo, "cat-file", "-t", commit) != "commit":
        raise VerificationError(f"{label} is not an existing commit object")
...
def load_control(repo: Path, control_path: str) -> Mapping[str, Any]:
    ...
    if control["schema_version"] != "integrated-live-safety-control-v2":
...
def expected_authorization_payload(
    ...
    return {
        ...
        "p0_v4_pass": audit["p0_v4_pass"],
        "casuka_d1_d3_pass": audit["casuka_d1_d3_pass"],
        "casuka_repair": audit["casuka_repair"],
...
def _verify_control_values(
    ...
    if package["branch"] != "codex/integrated-p0v4-casuka-prerun":
...
def _verify_git_and_files(
    ...
    # The package is an additions-only control layer. The deployable candidate
    # exists only in the separately frozen file-only integration commit.
    ...
    for label, commit in (
        ...
        ("P0 v4 audit", audit["p0_v4_pass"]),
        ("CASUKA audit", audit["casuka_d1_d3_pass"]),
        ("CASUKA repair", audit["casuka_repair"]),
    ):
        result = _run(
...
import hashlib, json, os, pathlib, re, subprocess, sys
    repo, target, backup, results, service, process, cron_path, cron_backup = sys.argv[1:]
def run(*args):
...
pg = subprocess.run(["pgrep", "-af", "^" + re.escape(process) + "$"], text=True,
    ...
    cron = pathlib.Path(cron_path).read_bytes()
    cron_backup_path = pathlib.Path(cron_backup)
    print(json.dumps({
  ...
  "tmux_session_alive": tmux,
      ...
      "process_rows": rows,
      "installed_crontab_sha256": hashlib.sha256(cron).hexdigest(),
      "installed_crontab_path": cron_path,
      "original_crontab_backup_path": cron_backup,
      "original_crontab_backup_exists": cron_backup_path.is_file(),
      "original_crontab_backup_sha256": (
          hashlib.sha256(cron_backup_path.read_bytes()).hexdigest()
          if cron_backup_path.is_file() else ""
      ),
    }, sort_keys=True))
"""
    result = _run(
...
def _verify_remote_state(
    ...
    expected = {
        ...
        "process_count": 0,
        "installed_crontab_sha256": control["containment"][
            "installed_inhibited_crontab_sha256"
        ],
        "installed_crontab_path": control["containment"][
            "installed_crontab_path"
        ],
        "original_crontab_backup_path": control["containment"][
            "original_crontab_backup"
        ],
        "original_crontab_backup_exists": True,
        "original_crontab_backup_sha256": control["containment"][
            "original_crontab_backup_sha256"
        ],
    }
    mismatches = [
    ...
    if not isinstance(rows, list) or rows:
        raise VerificationError("remote engine is not stopped")
...
def verify(request: VerificationRequest) -> dict[str, Any]:
    ...
    return {
        "schema_version": "integrated-live-safety-authorization-verification-v1",
        "status": "PASS",
        "mode": request.mode,
        "authorization_commit": request.authorization_commit,
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "schema_version": (
                        "integrated-live-safety-authorization-verification-v1"
                    ),
                    "status": "FAIL",
                    "error": str(exc),
                    "no_mutation_performed": True,
