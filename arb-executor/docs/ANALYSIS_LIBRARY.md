...
#### Path B premarket fill mechanics study — per-regime maker bid fill rates (T43, analytical commit `1e00818`)

- **File paths:** arb-executor/data/durable/per_minute_universe/path_b_per_n_fill_results_v1.parquet (589,386 rows; on-disk only, not git-tracked) + path_b_per_regime_fill_summary_v1.parquet (1,512 rows). Finding doc: docs/analysis/premarket_dynamics_v1/path_b_fill_mechanics_findings.md.
- Depth: 1 (corpus-scale fill-mechanics aggregation).
- **Grain:** per (category × anchor_regime × placement_minute × bid_offset) for the summary; per (N × strategy_cell) for the per-N results.
- **Vector:** vector-agnostic.
- **Objective:** entry-side fill mechanics (NOT exit-optimized; complements the exit-optimized atlas T42 — entry edge is additive on top of the locked atlas exit).
- Producer: arb-executor/data/scripts/build_path_b_fill_mechanics_v1.py at commit `884e951`.
- Date created: 2026-05-23.
- Data tier used: FOUNDATION-TIER (premarket_tape_v1, sha256 ff2a63d9) + atlas N membership filter (4 spike_perN parquets).
- Variables used: yes_ask_close, price_close, time_to_match_start_min (from premarket_tape_v1); anchor_price (atlas spike_perN).
- Row counts: 14,033 N × 42 cells = 589,386 per-N rows; 4 × 9 × 6 × 7 = 1,512 summary cells. Gates 1-5 PASS (rowcount; exact per-category N; fill-rate sanity majority; fill_rate monotonic-decreasing in offset 216/216).
- **Strict reading:** Per (category × anchor_regime), the fraction of atlas N's whose maker bid at `anchor_price − offset` would have been touched (price_close ≤ bid OR yes_ask_close ≤ bid) by natural premarket trajectory between placement and T-20m, at minute cadence. expected_improvement_cents = fill_rate × bid_offset. Expected entry-improvement rises monotonically with anchor regime — heavy favorites (r85_94) ~6.3-7.0¢/N via 15¢ offsets, deep underdogs (r05_14) ~1.0-1.2¢/N via 2-3¢ offsets; corpus hindsight ceiling 2.46¢/N (per-regime-optimal), 2.25¢/N best single uniform rule (T-4h, 15¢).
- **Does NOT establish:** realized PnL net of exit (atlas T42 owns the exit side); per-event live deployment decisions (hindsight-optimal placement assumes the regime/offset is known a priori; 2.46¢ ceiling vs 2.25¢ single-rule floor brackets the foresight gap); fill realism downstream (Axis 1 / B25 — a resting bid the trajectory crosses is assumed filled, no queue/sub-minute modeling); sub-minute fills (minute-cadence observation).
- **Strategic implication:** empirical answer to Axis 2 (entry-side maker improvement) per G22 — per-regime fill_rate × offset gives the expected entry-side cents recoverable above the atlas T-20m taker anchor, additive on top of the locked atlas exit. Downstream of the locked atlas; complement not substitute. Favorite books drift up (Scope A T4) so large offsets fill and capture full offset; underdog books drift down to anchor so only small offsets fill. Earlier placement weakly dominates.
- References: LESSONS A39, B16, B25, G22; SCOPE_A T4 drift gradient (empirical foundation); T42 atlas (locked baseline, parent); ROADMAP T43. Reframe note: supersedes an earlier Path B draft that conflated hold-to-settlement PnL with the atlas spike-exit +8.70% baseline (hold-to-settlement taker baseline measures −1.02¢/N).

#### Path B v2 — marketable-vs-resting + atlas exit replay (T44, analytical commit `a61830c`)

- **File paths:** data/durable/per_minute_universe/path_b_v2_per_n_simulation.parquet (14,033 rows; on-disk only) + path_b_v2_per_regime_summary.parquet (36 rows). Finding doc: docs/analysis/premarket_dynamics_v1/path_b_v2_findings.md.
- Depth: 1 (corpus-scale realized-PnL simulation under a single placement rule).
- **Grain:** per-N (one row per atlas N) for the simulation; per (category × anchor_regime) for the summary.
- **Vector:** vector-agnostic.
- **Objective:** entry-side maker placement layered on the locked atlas exit (Axis 2 on top of Axis 1).
- Producer: data/scripts/build_path_b_v2.py.
- Date created: 2026-05-23.
- Data tier used: FOUNDATION-TIER (premarket_tape_v1, entry side) + atlas spike_perN (size_qual_max_250, settlement_value — exit-replay substrate) + descriptive_1c (per-cell best_exit_X / rule).
- Variables used: yes_ask_close, price_close, time_to_match_start_min (entry side); anchor_price, size_qual_max_250, settlement_value, best_exit_X, rule (exit replay).
- **Strict reading:** Single universal rule (place bid at T-240, 15¢ below the T-20m anchor). Per N: classify execution {marketable_taker, maker_resting, miss_fallback}, capture actual entry, replay the atlas cell rule from that entry (trigger iff entry+X ≤ size_qual_max_250, the ≥250ct depth-qualified post-anchor max; else hold winner 99−entry / loser −(entry−1)). Maker placement realizes $7,829.30 vs the atlas baseline $6,158.20 (+27.1%) on LOWER capital ($67,346 vs $70,813) → blended ROI 11.63% vs 8.70% (+2.93pp), PRE-REALISM. Baseline reproduced exactly (ratio 1.0000). All 36 regime cells show positive ROI lift.
- **Does NOT establish:** post-realism deployable dollars (B25 0.5-0.7× NOT applied); the minute, queue position, or sub-minute fill realism of either entry or exit (exit_minute is NULL — the size_qual proxy gives the trigger boolean, conservative lower bound for entry>T-20m); underdog-side capture (the universal 15¢ offset clamps underdog bids to 1¢ → 96-99% miss; per-regime offsets per T43 needed).
- **Strategic implication:** the load-bearing measurement for the bid-laying policy spec — atlas is the floor (+8.70%), maker placement adds the entry-side ceiling (+2.93pp blended, pre-realism), driven by the favorite half of the corpus (the ~15% of N that fill at a discount earn 21-27% ROI; the 85% that miss fall back to the exact atlas outcome). Deployable single-rule policy: marketable→taker at ask, else resting maker bid with T-20m fallback, atlas exit from entry; per-regime + burst override layer on per Plex Round 6.
- References: LESSONS A39, B16, B25, G22, A40, A41; T43 (parent, per-regime fill mechanics); T42 atlas (locked exit floor); ROADMAP T44; Plex Round 6 (staged composition).

#### Path B v3 — per-regime offsets + atlas exit replay (T45, analytical commit `648b8db`)

- **File paths:** data/durable/per_minute_universe/path_b_v3_per_n_simulation.parquet (14,033 rows; on-disk only) + path_b_v3_per_regime_summary.parquet (36 rows). Finding doc: docs/analysis/premarket_dynamics_v1/path_b_v3_findings.md.
- Depth: 1. **Grain:** per-N (simulation) / per (category × anchor_regime) (summary). **Vector:** vector-agnostic. **Objective:** entry-side maker placement layered on the locked atlas exit (deployable per-regime variant of T44).
- Producer: data/scripts/build_path_b_v3.py. Date: 2026-05-23.
- Data tier: FOUNDATION-TIER (premarket_tape_v1) + atlas spike_perN/descriptive_1c + path_b_per_regime_fill_summary_v1 (per-regime offset lookup).
- **Strict reading:** identical dual-improvement methodology to T44 (execution split {marketable_taker, maker_resting, miss_fallback}, actual-entry capture, atlas exit replay via size_qual_max_250) but with per-(category × anchor_regime) optimal offsets from Path B v1 Section 3 instead of the universal 15¢. Realizes $8,098.50 vs atlas $6,158.20 (+31.5%) on lower capital ($66,902) → blended ROI 12.11% vs 8.70% (**+3.41pp**), beating T44's universal-rule +2.93pp by **+0.48pp (+$269)**. Aggregate fill 28.4% (vs T44 15%). Baseline reproduced exactly.
- **Does NOT establish:** post-realism deployable dollars (B25 0.5-0.7× NOT applied); fill/queue/sub-minute realism; that per-regime tuning is transformative (it is a +0.48pp refinement over a universal favorite offset — diminishing returns).
- **Strategic implication:** the deployable measurement for the bid-laying policy spec. Per-regime offsets are the correct deployable shape (each cell's optimum is observable a priori from the anchor regime), and never hurt (every cell ≥ baseline), but the marginal gain over the universal favorite-capture rule is small (+0.48pp) — the incremental value is the underdog/mid harvest (~$385 from r05_14/r15_24/r25_34, clamped to ~zero by the universal rule). Top per-regime ROI lifts are the deep-underdog r05_14 cells (+17 to +21pp, small capital denominator). Gate 4: v3 per-regime fill rates reproduce v1's optimal-cell fills with 0.0pp delta.
- References: LESSONS A39, B16, B25, G22, A40, A41; T44 (parent, universal-rule); T43 (per-regime offset source); T42 atlas (locked exit floor); ROADMAP T45; Plex Round 6 (diminishing-returns thesis confirmed).

#### Path C Phase 1 — 3-feature drift predictor (T46, analytical commit `22f3221`)

- **File paths:** data/durable/per_minute_universe/path_c_phase1_per_n_features.parquet + path_c_phase1_logistic_results.parquet (14,033 rows each; on-disk only). Finding doc: docs/analysis/premarket_dynamics_v1/path_c_phase1_findings.md.
- Depth: 1 (corpus-scale predictive feature analysis). **Grain:** per-N. **Vector:** vector-agnostic. **Objective:** descriptive predictability test (does conditional placement help) — NOT a deployed model.
- Producer: data/scripts/build_path_c_phase1.py. Date: 2026-05-23. Implements Plex Round 7 (`9d11ed9`) Tier 1.
- Data tier: FOUNDATION-TIER (premarket_tape_v1 features 1/3 + drift target; per_minute_features pre-T-4h taker window for feature 2) + atlas anchor/regime + per_regime_offsets CSV.
- Variables used: yes_ask_close, yes_bid_close, spread_close, price_close, paired_arb_gap_maker (tape); taker_flow_in_minute (PMF pre-T-4h); anchor_price (atlas).
- **Strict reading:** logistic regression of drift_reached_bid on {initial_spread, taker_imbalance_30min, paired_arb_gap} + (category×regime) fixed effects, purged CPCV (8 folds, k=2, year-week blocks, 2-week embargo) + Mar–May 2026 holdout; pure-numpy (rank-AUC, IRLS, manual CPCV — no sklearn). **Holdout AUC 0.7298 (>0.62 bar; CPCV 0.6625±0.0418).** drift_reached_bid=3,983 reproduces Path B v3 fill count exactly. paired_arb_gap strongest single feature (within-cell lift to 3.5×, AUC to 0.748); taker_imbalance dominant in the model (permutation 0.022, negative sign — buying pushes price up away from below-anchor bid). All coef CIs exclude 0.
- **Does NOT establish:** a deployed model (this is descriptive predictability); post-realism deployable ROI (B25 not applied); that conditional *placement* helps via skip-gating — it does NOT (Rule A 9.17% / Rule B 9.36% < v3 12.11%, because place-everywhere already falls back safely; the value is offset modulation, Phase 2). pred_p_drift in outputs is in-sample for CPCV rows.
- **Strategic implication:** the 3-feature predictor has real out-of-sample signal → Path C Phase 2 (offset modulation, P(drift)→bid aggressiveness) is worth building; the paired arb gap at T-4h is the most deployable feature (B23-grounded). The binary-skip-gate underperformance is itself the lesson (A42): a fill predictor's value is continuous aggressiveness modulation, not on/off placement when the fallback is safe.
- References: LESSONS A39, B16, B25, G22, A40, A41, A42; T45/T44/T43 (Path B), T42 atlas; ROADMAP T46; Plex Round 7 (`9d11ed9`); B23 (paired_arb_gap mechanism).

#### Path B v4 — per-cell offset re-optimization on net-PnL (T47, analytical commit `c90985b`, DEPLOYABLE)

- **File paths:** data/durable/per_minute_universe/path_b_v4_per_n_simulation.parquet (1,178,772 rows; on-disk only) + path_b_v4_cell_optimum.parquet (36). Finding doc: docs/analysis/premarket_dynamics_v1/path_b_v4_findings.md. Deployable table: docs/policy/per_regime_offsets_v2.csv.
- Depth: 1 (corpus-scale parameter optimization). **Grain:** per (N × config) / per cell. **Vector:** vector-agnostic. **Objective:** net realized PnL (the deployment-correct full-strategy objective; corrects v1's entry-capture objective).
- Producer: data/scripts/build_path_b_v4.py. Date: 2026-05-23.
- Data tier: FOUNDATION-TIER (premarket_tape_v1) + atlas spike_perN/descriptive_1c + per_regime_offsets_v1 (v3 baseline).
- **Strict reading:** re-sweep (placement_minute ∈ {240..20} × offset ∈ {0..20¢}) = 84 configs + v3 exact per cell, argmax net realized PnL (entry + atlas X exit + miss-fallback − 1¢ taker fee). v4 net ROI **11.73% vs v3 10.70% (+1.024pp; +1.22pp vs canonical v3)** on lower capital — gate 6 material, DEPLOYABLE. All 6 gates pass; 25/36 cells material; 34/36 re-optimize. **Net-PnL-optimal offsets are SHALLOW (1-3¢, 27/36 cells), the opposite of v1's deep 15¢-favorite table** — the atlas fixed-profit exit (+X) gives a deeper offset no exit upside, only more misses; shallow bids fill reliably + lower capital.
- **Does NOT establish:** post-realism dollars (B25 0.5-0.7× applies); that net-$ is the right per-cell objective in capital-constrained regimes (3 deep-underdog cells trade ROI for throughput); the ±5min ask fallback lifts the baseline ~1.7% vs canonical v3.
- **Strategic implication:** v4 supersedes v3 as the deployable per-cell table. The positive complement to the Path C negative arc: feature-conditional refinements (Phase 1-3) couldn't beat v3 because the atlas exit cap binds, but **re-picking the static per-cell offset on the correct (net-PnL) objective does** — v1 mis-set the offset by optimizing entry-capture. Deployable: per_regime_offsets_v2.csv + bid_laying_policy_v1 Section 12.
- References: LESSONS A39, B16, B25, G22, A40, A41, A42, E32; T43 (v1 offsets, corrected here), T44/T45 (v2/v3), T42 atlas; ROADMAP T47; Plex R6/R7.

#### Path C arc — feature-conditioned entry refinement, NEGATIVE (T48, analytical commit `701ccc8`)

- **File paths:** data/durable/per_minute_universe/path_c_phase2_per_n_simulation.parquet (14,033×15) + path_c_phase2_per_regime_summary.parquet (36×13); path_c_phase3_corrected_per_n_simulation.parquet (14,033×14) + path_c_phase3_corrected_per_regime_summary.parquet (36×12); all on-disk only. Synthesis doc: docs/analysis/premarket_dynamics_v1/path_c_arc_findings.md. Run summaries: path_c_phase2_run_summary.json + path_c_phase3_corrected_run_summary.json.
- Depth: 1 (corpus-scale strategy refinement test). **Grain:** per-N / per cell. **Vector:** vector-agnostic. **Objective:** descriptive test of whether per-event feature conditioning beats the static Path B table — NEGATIVE result.
- Producers: data/scripts/build_path_c_phase2.py (offset modulation) + data/scripts/build_path_c_phase3_corrected.py (per-cell execution mode). Date: 2026-05-23. Implements Plex Round 7 downstream of T46 Phase 1.
- Data tier: FOUNDATION-TIER (premarket_tape_v1 + per_minute_features taker window) + atlas spike_perN/descriptive_1c + Path B v3 baseline + Phase-1 holdout predictor + Scope A T4 drift gradient.
- **Strict reading (Phase 2 — offset modulation):** map Phase-1 P(drift)→tiered bid offset (aggressive/neutral/conservative) on top of v3; 4 P-threshold parameterizations. **Best variant (broad_060_040) +0.089pp gross (12.194% vs v3 repro 12.105%) = noise; per-cell dominance only 22/36.** NEGATIVE, not promoted.
- **Strict reading (Phase 3 corrected — execution mode):** per-cell, cross-taker immediately on cells where the favorite leg drifts up ≥X¢ (resting bid would be swept past), maker-wait elsewhere; net of 1¢ taker fee; paired-leg inverse logic. Drift read from data reproduces Scope A T4 within 0.19¢. Trigger fires on 8 heavy-favorite cells (r75_84/r85_94). **Best threshold (≥4¢) nets 10.266% vs v3 net 10.514% = −0.248pp; the 8 favorite cells net −$191.1; the cross cohort enters ~1¢ cheaper (75.84¢ vs 76.82¢) but the 1¢ is eaten by the fee and never reaches the realized +X.** NEGATIVE, not promoted. (Supersedes the original binary 55¢-cutoff Phase 3, held unpromoted, which found no entry improvement.)
- **Does NOT establish:** any deployable improvement (both phases negative/noise); post-realism numbers (B25 not applied); that the predictor is invalid (T46 holdout AUC 0.73 is real — it just has no entry-side channel to monetize). Phase 2 gross is pre-fee; net it is no better.
- **Strategic implication (LESSONS A43):** the locked atlas fixed-profit exit (+X regardless of entry) caps the value of per-event entry-side conditioning — the entry discount only changes the hold-to-settlement minority + deployed capital, not the dominant triggered-exit payoff. A working fill predictor cannot beat the static table through offset modulation, execution-mode switching, or skip-gating (A42). The productive lever is the **static per-cell offset re-optimized on net-PnL (T47 Path B v4, +1.024pp)**. Together: locked fixed-profit exit + static per-cell maker placement on the correct objective is the empirical ceiling on this strategy architecture. **Closes the empirical premarket-dynamics strategy arc** — remaining headroom is exit-side, not entry-side (findings Section 7).
- References: LESSONS A39, B16, B25, G22, A40, A41, A42, A43, E32; T46 (Phase 1, parent), T47 (Path B v4, positive contrast), T45/T44/T43 (Path B), T42 atlas; ROADMAP T48; Plex R7.
...
- 2026-05-23 ET: Path B premarket fill mechanics registered in Section 2 (T43). Atlas-downstream Axis 2 (entry-side maker improvement) measurement; producer build_path_b_fill_mechanics_v1.py (commit 884e951), analytical commit 1e00818. Outputs on-disk only (589,386 per-N rows + 1,512 summary cells); finding doc path_b_fill_mechanics_findings.md. Corpus hindsight entry-improvement ceiling 2.46¢/N, monotonic favorite>underdog gradient. Doctrine A39/B16/B25/G22; complements (does not substitute) exit-optimized atlas T42.
- 2026-05-23 ET: Path B v2 registered in Section 2 (T44). Marketable-vs-resting execution split + atlas exit-rule replay from actual entry; single universal rule (T-240, 15¢) over 14,033 atlas N; analytical commit a61830c, producer build_path_b_v2.py. Realized PnL $7,829.30 vs atlas $6,158.20 (+27.1%) on lower capital → blended ROI 11.63% vs 8.70% (+2.93pp), pre-realism; baseline reproduced exactly; favorite-driven. Outputs on-disk only; finding doc path_b_v2_findings.md. Doctrine A39/B16/B25/G22/A40/A41.
- 2026-05-23 ET: Path B v3 registered in Section 2 (T45). Per-(category × anchor_regime) offsets (from v1 Section 3) replace the universal 15¢; analytical commit 648b8db, producer build_path_b_v3.py. $8,098.50 vs atlas $6,158.20 (+31.5%) on lower capital ($66,902) → blended ROI 12.11% vs 8.70% (+3.41pp), beating T44's +2.93pp by +0.48pp (+$269) — diminishing returns; incremental gain is the underdog harvest. Fill 28.4%; baseline exact; gate 4 v3-fill==v1-fill (0.0pp). Outputs on-disk only; finding doc path_b_v3_findings.md.
- 2026-05-23 ET: Path C Phase 1 registered in Section 2 (T46). 3-feature drift predictor (Plex Round 7 Tier 1); analytical commit 22f3221, producer build_path_c_phase1.py. Holdout AUC 0.7298 (>0.62; CPCV 0.6625), drift_reached_bid=3,983 (=Path B v3). paired_arb_gap strongest; taker_imbalance dominant (negative). Key finding: binary skip-gates underperform place-everywhere (Rule A/B 9.17/9.36% vs v3 12.11%) → value is offset modulation not skip-gating (LESSONS A42). Recommend Path C Phase 2. Outputs on-disk only; finding doc path_c_phase1_findings.md.
- 2026-05-23 ET: Path B v4 registered in Section 2 (T47, DEPLOYABLE). Per-cell offset re-optimization on net-PnL (vs v1 entry-capture); analytical commit c90985b. Net ROI 11.73% vs v3 10.70% (+1.024pp), gate 6 material. Net-PnL-optimal offsets shallow (1-3c, 27/36 cells), opposite v1's deep favorites — atlas exit cap binds. Supersedes v3; per_regime_offsets_v2.csv + bid_laying_policy Section 12. Outputs on-disk only; finding doc path_b_v4_findings.md.
- 2026-05-23 ET: Path C arc registered in Section 2 (T48, NEGATIVE). Feature-conditioned entry refinement — Phase 2 offset modulation (best +0.089pp gross = noise) + corrected Phase 3 execution-mode (best −0.248pp net); analytical commit `701ccc8`. Both not promoted. Establishes LESSONS A43: the locked atlas fixed-profit exit caps entry-side feature conditioning (a working predictor, T46 AUC 0.73, has no entry-side channel to monetize); the productive lever is the static per-cell offset on the net-PnL objective (T47 v4). Cross-ref T46 (Phase 1) + T47 (positive contrast). 4 parquets on-disk; both run_summaries + MANIFEST; synthesis doc path_c_arc_findings.md. Closes the empirical premarket-dynamics strategy arc.
