#!/usr/bin/env python3
"""Two-clean-build freezer for the T2 scoring corrective package V2."""
...
from window1_t2_scoring_package_builder_v2 import build


VERSION = "window1-t2-scoring-package-freeze-v2"


class FreezeError(RuntimeError):
    """The score-free V2 builds are not byte-identical."""
...
def freeze(
    *,
    ...
    if output.exists():
        raise FreezeError("frozen V2 output already exists")
    with tempfile.TemporaryDirectory(
        prefix="w1-t2-scorepkg-v2-a-"
    ) as a_text, tempfile.TemporaryDirectory(
        prefix="w1-t2-scorepkg-v2-b-"
    ) as b_text:
        ...
        if hashes_a != hashes_b:
            ...
            raise FreezeError(
                "clean V2 builds differ: " + ",".join(mismatch[:20])
            )
    ...
    if frozen_regenerable != build_regenerable:
        raise FreezeError("frozen V2 package differs from clean builds")
