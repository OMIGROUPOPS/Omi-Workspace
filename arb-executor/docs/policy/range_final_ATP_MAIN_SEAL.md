...
## §7 ADDENDUM (C-STAIRCASE-SHIP-2, Path A) — the D@T-* columns are NOT the validated walk

**Atomic ruling (Plex-ratified Path A; Path B rejected):** the live staircase caller **computes D
live** from the sealed inputs and the formula. It **MUST NOT read the `D@T-*` columns** of
`range_final_ATP_MAIN.csv`, and **MUST NOT write** to the sealed CSV.

Validated depth is defined by:
- `final_target[cell]` (the deepest bid offset at T-240), column `final_target` of this CSV, **and**
- `frac2(t)` from `range_final_walk_schedule.json` (held-step: fraction at the smallest knot ≥ t), **and**
- the formula `D = max(1, int(round(1 + (final_target − 1) · frac2(t))))` (== `abort_validation.py:58`).

The `D@T-*` columns (generated by `analysis/exit_charts/build_range_final.py`) are at a **different
knot set** (240/180/120/90/60/40/30/20/10/5) than the validated walk schedule
(240/210/180/150/120/90/60/30/10) and **disagree with the sim formula on ~55% of overlapping
cell×knot pairs**. They are a build artifact, **not** the validated series. Reading them would deploy
`deployed != validated`.

**Guard:** GATE 4 (`tests/test_dat_guard.py`) fails the suite on any `D@T` reference in `*.py`
outside `build_range_final.py`. **Parity:** GATE 3 (`tests/test_staircase_ship2.py`) asserts the live
caller's D equals the sim's D at non-knot-aligned t (incl. boundaries 150.0/149.999/150.001/30.0/9.999).

**Status:** the clamp `_staircase_target` (Ship 1, 77fd9fa) + the Path-A caller `_staircase_bid` +
the per-cycle walk recast (Ship 2, full-walk, Plex ruled A) wire ATP_MAIN placement AND the knot walk
to the validated formula against a FIXED anchor/cell. D walks monotone non-increasing up the held-step
knot schedule as t→0; restart-survivable. The other 3 categories + engagement + completion are
byte-identical. See the caller contract doc for the 7-risk implementation + walk-trajectory tests.
