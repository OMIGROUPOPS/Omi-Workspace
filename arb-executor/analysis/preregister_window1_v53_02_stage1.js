...
const out = path.join(repo, ".claude/window1_v53_02_preregistration_20260820/PRE_REGISTRATION.json");
...
const v53_01Commit = "7ba7c3dd23eb9b87d4584f856d4a247d760a03b4";
const v53_01Path = ".claude/window1_v53_preregistration_20260819/PRE_REGISTRATION.json";
...
const v53_01Bytes = show(v53_01Commit, v53_01Path);
const truth = JSON.parse(truthBytes), census = JSON.parse(censusBytes), prior = JSON.parse(priorBytes), v53_01 = JSON.parse(v53_01Bytes);
if (truth.rows?.length !== 804 || census.rows?.length !== 1143 || prior.fresh_25?.length !== 25 || v53_01.population?.fresh_25?.length !== 25) throw new Error("V53-02 pre-registration authority census invalid");
...
for (const row of v53_01.population.fresh_25) excluded.add(row.code);
...
const seedMaterial = `V53_02_FRESH25|${implementationCommit}`;
...
for (const [code, rows] of rowsByCode) {
  ...
  strata.get(stratum).push({ code, event_id: truthByCode.get(code).event_id, category, census_stamps: stamps, stratum, role: "FRESH_STRATIFIED_COHORT_NOT_IN_V52B_THROUGH_V53_01" });
}
...
const receipt = {
  label: "V53_02_STAGE1_PRE_REGISTRATION",
  created_before_grading: true,
  implementation_commit: implementationCommit,
  seed_derivation_law: "SHA256('V53_02_FRESH25|' + V53_02_IMPLEMENTATION_COMMIT)",
  ...
  authorities: {
    ...
    V53_01_pre_registration: { commit: v53_01Commit, path: v53_01Path, sha256: sha(v53_01Bytes), fresh_events_added_to_exclusion_union: 25 },
  },
