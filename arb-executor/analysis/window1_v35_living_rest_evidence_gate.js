...
const v34 = require("./window1_v34_dual_side_residency_machine.js");
...
function livingRestTarget({ bid, pairCap = null }) {
  ...
  return v34.lawfulCent(target) ? target : null;
}

function displayedAskHasEvidenceCapacity(book) {
  return Number.isInteger(book?.bid) && Number.isInteger(book?.ask) &&
    Number.isFinite(book.ask_dwell_seconds) && book.ask_dwell_seconds >= v34.QUALIFIED_DWELL_MIN_SECONDS &&
    Number.isFinite(book.top_ask_size) && book.top_ask_size >= v34.QUALIFIED_SIZE_MIN;
}

function qualifyingAskEvidence(book) {
  if (!displayedAskHasEvidenceCapacity(book)) return false;
  if (book.bid >= book.ask) return true;
  return Number.isInteger(book.spread) && book.spread >= 0 && book.spread <= v34.QUALIFIED_SPREAD_MAX_CENTS;
}
...
function evidenceTakePermission({ book, pairCap = null, activeEvidenceFloor = null, floorFirstFlickerLive = false }) {
  ...
  if (!v34.lawfulCent(activeEvidenceFloor)) {
