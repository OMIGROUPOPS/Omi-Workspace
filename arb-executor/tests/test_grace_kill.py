...
# [P0v3 (2) 07-17 SWEEP BEATS FILL] grace RETIRED: live -> always 'cancel',
# whatever the flag/sibling/stamp state. arm/hold no longer exist.
chk("ON, live, held-sibling, no stamp -> 'cancel' (grace retired, sweep first)", bot(True, sib=True)._grace_kill_action(pos(0.0), True, NOW)=="cancel")
chk("ON, live, held-sibling, freshly stamped -> 'cancel' (no hold window)", bot(True, sib=True)._grace_kill_action(pos(NOW-50), True, NOW)=="cancel")
chk("ON, live, held-sibling, old stamp -> 'cancel'", bot(True, sib=True)._grace_kill_action(pos(NOW-400), True, NOW)=="cancel")
chk("bounded at 5min: grace_sec default is 300 (config fossil, named)", LiveV3.__init__ is not None and 300==int(bot(True).match_live_grace_sec))
...
# [07-17] region re-scoped to the HELPER ONLY (def -> next def): the old
# bound (_paired_basis_ok) swallowed _monotonic_cut_eval, whose lawful
# exit_order_id references made this red since the C-MONOTONIC-CUT build
# (pre-existing at HEAD, verified 07-17 — not a grace regression).
gk_region=src[src.index("def _grace_kill_action"):src.index("def _sustained_flow_live")]
