...
const OUTPUT_LABEL = "V54_REPAIR_ITERATION7_FITNESS_WEIGHTED_BASES";
...
function bindCorpusFloorTiming(corpusRows, truthRows) {
  ...
  const boundedWithoutLawfulLow = [];
  for (const candidate of corpusRows) {
    if (candidate.span?.status !== "BOUNDED") continue;
    const eligibleLegRows = (candidate.legs ?? []).filter((leg) => Number.isFinite(leg.low_cents));
    const eligibleLegs = eligibleLegRows.length;
    if (eligibleLegs) add(eligible, candidate.category, eligibleLegs);
    for (const leg of eligibleLegRows) eligibleKeys.add(`${candidate.event_id}|${leg.leg_id}`);
    const timedLegRows = (candidate.legs ?? []).filter((leg) => Number.isFinite(leg.floor_epoch) && Number.isFinite(leg.floor_fraction));
    const timedLegs = timedLegRows.length;
    if (timedLegs) add(after, candidate.category, timedLegs);
    for (const leg of timedLegRows) timedKeys.add(`${candidate.event_id}|${leg.leg_id}`);
    for (const leg of candidate.legs ?? []) if (!Number.isFinite(leg.low_cents)) boundedWithoutLawfulLow.push({ event_id: candidate.event_id, leg_id: leg.leg_id, category: candidate.category });
  }
  const missingEligibleKeys = [...eligibleKeys].filter((key) => !timedKeys.has(key)).sort();
  return {
    ...
    every_eligible_game_bound: missingEligibleKeys.length === 0,
    every_eligible_leg_bound: missingEligibleKeys.length === 0,
    missing_eligible_leg_keys: missingEligibleKeys,
    bounded_legs_without_lawful_low: boundedWithoutLawfulLow.sort((a, b) => `${a.event_id}|${a.leg_id}`.localeCompare(`${b.event_id}|${b.leg_id}`)),
    comparator_repair: "Eligible-path inclusion is checked by event|leg identity. Timed paths outside the low-bearing eligible set are retained as timing coverage but cannot make the eligible comparator false.",
...
async function main() {
  ...
  writeJson(path.join(output, "FLOOR_TIME_BINDING_COVERAGE.json"), {
    label: "V54_REPAIR_ITERATION7_ALL_BELL_BOUNDED_LIBRARY_FLOOR_TIME_BINDING",
    ...corpusFloorTiming,
  });
  ...
  const storiesHeader = `# Four stories — repair iteration 7\n\nLicense: LAW_INDEX @ 99233ab2, sha256 c7c7271501076fefdad0d65044bde5a410ccc718f8f7f5a40d488caf81b3dee6 · all standing laws, welds, pattern law, windows-on-own-clocks (F-VS-065), F-VS-066/068, and the iteration-6 mirror receipts.\n\nThe story is the verdict. Every available evidence basis contributes continuously by receipt-level fitness: time-bearing neighbors, graded neighbors, own-tape presence, and reflex tracking. No category precedence, placement constant, sealed read, live mutation, or full-804 run appears.\n\n`;
  ...
  writeJson(path.join(output, "FITNESS_WEIGHTED_BASES_RECEIPT.json"), {
    ...
    every_sentence_names_weights_and_basis: allDerivations.every((row) => row.sentence.includes("FITNESS_WEIGHTS=") && row.sentence.includes("TARGET_BASIS=FITNESS_WEIGHTED_BLEND")),
    every_applicable_basis_has_positive_weight: allDerivations.every((row) => row.derivation.basis_weights.filter((basis) => basis.raw_fitness > 0).every((basis) => basis.normalized_weight > 0)),
    every_weight_vector_normalized: allDerivations.every((row) => Math.abs(row.derivation.basis_weights.reduce((total, basis) => total + basis.normalized_weight, 0) - 1) < 1e-9),
  });
  ...
  writeJson(path.join(output, "COMPOSITION_PRESENCE_RECEIPT.json"), {
    label: "FITNESS_WEIGHTED_COMPOSITION_AND_PRESENCE_LAW",
    composition: "Time-bearing neighbors, graded neighbors, own evidence, and reflex tracking condition one continuous depth through normalized per-receipt fitness weights; none outranks by category.",
    presence: "Own-tape and reflex applicability are strongest formation-adjacent and are continuously tempered by accrued own evidence and window phase; timing fitness rises only with lawful time-bearing neighbor mass.",
  ...
  if (allDerivations.some((row) => !os.CONDITIONAL_DIP_DECLARATION.authority_order.includes(row.derivation.target_basis))) lawViolations.push("UNNAMED_FITNESS_BLEND_AUTHORITY");
  ...
  if (allDerivations.some((row) => !row.sentence.includes("FITNESS_WEIGHTS=") || !row.sentence.includes("TARGET_BASIS=FITNESS_WEIGHTED_BLEND"))) lawViolations.push("FITNESS_WEIGHTS_OR_BASIS_MISSING_FROM_SENTENCE");
  if (allDerivations.some((row) => Math.abs(row.derivation.basis_weights.reduce((total, basis) => total + basis.normalized_weight, 0) - 1) >= 1e-9)) lawViolations.push("FITNESS_WEIGHTS_NOT_NORMALIZED");
  if (allDerivations.some((row) => row.derivation.basis_weights.some((basis) => basis.raw_fitness > 0 && !(basis.normalized_weight > 0)))) lawViolations.push("APPLICABLE_EVIDENCE_BASIS_SILENCED");
  ...
  writeJson(path.join(output, "REPAIR_GATE_RECEIPT.json"), { label: "FITNESS_WEIGHTED_BASES_GATE", honest_baseline_pins: baselinePins, pins_equal_expected: baselinePins.every((row) => row.completed && row.delta_vs_100_cents === (row.event_id.includes("URSPAL") ? 3 : 6)), required_candidate_floors: SAFETY_FLOORS, candidate_safety_floor_breaks: floorBreaks.map((row) => ({ event_id: row.event_id, completed: row.functionable_v6.completed, delta_vs_100_cents: row.functionable_v6.delta_vs_100_cents, legs: row.functionable_v6.legs })), safety_floor_pass: floorBreaks.length === 0, law_violations: lawViolations, zero_measured_law_violations: lawViolations.length === 0, sentences_carry_weights_basis_allocation_touch_depth: allDerivations.every((row) => row.sentence.includes("FITNESS_WEIGHTS=") && row.sentence.includes("ALLOCATION=") && row.sentence.includes("TARGET_BASIS=FITNESS_WEIGHTED_BLEND") && row.sentence.includes("TOUCH_RELATION=") && row.sentence.includes("CHOSEN_DEPTH_CENTS=") && row.sentence.includes("OWN_WINDOW=") && row.sentence.includes("PAIR_STATE=")), sentences_cite_fills: fillHandoffs.every((row) => row.trade_receipt && row.sentence.includes(row.trade_receipt)), self_stop: floorBreaks.length > 0 || lawViolations.length > 0, stop_reason: floorBreaks.length ? "GIUBAR_OR_URSPAL_OR_LAJSVA_REQUIRED_COMPLETE_NOT_HELD" : lawViolations.length ? "LAW_VIOLATION" : null, full_804_run: false, sealed_read: false, live_mutation: false });
