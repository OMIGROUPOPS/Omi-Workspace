...
const isV52MacroRecognition = isV52m || isV52n || isV52o || isV52Ripeness;
...
function simulate(base, tapes, prints, mode, clauses = {}) {
  ...
  for (const row of timeline) {
    ...
    const ripenessSpan = isV52Ripeness ? groundTruthWindowBinding.byEvent.get(base.event_id) : null;
    const anchorLeg = isV52q ? ripenessSpan?.legs?.[leg.leg_id] : null;
    const macroRecognition = isV52MacroRecognition ? policy.classifyShapeState(leg.v52m_shape_state, {
      ...
      formation_end_epoch: isV52Ripeness ? (clauses.anchor_correction === true ? (anchorLeg?.formation_end_epoch ?? null) : ripenessSpan?.span_start_epoch) : base.left,
      verified_span_end_epoch: isV52Ripeness ? ripenessSpan?.span_end_epoch : base.right,
...
async function main() {
  ...
  const v52oTaxonomyCsvBytes = (isV52o || isV52Ripeness) ? gitShow(SHAPE_TAXONOMY_COMMIT, SHAPE_TAXONOMY_CSV_PATH) : null;
  const v52pRipenessBytes = isV52Ripeness ? gitShow(RIPENESS_COMMIT, RIPENESS_PATH) : null;
  const v52qDiscrepancyBytes = isV52q ? gitShow(ANCHOR_DISCREPANCY_COMMIT, ANCHOR_DISCREPANCY_PATH) : null;
  ...
  const v52qAnchorBinding = isV52q ? {
  ...
  if (isV52q) policy.configureAnchorCorrection(v52qAnchorBinding);
  ...
  const v52ePriorReceipts = isV52e && !isV52eExam ? [
    ...
    ...((isV52n || isV52o || isV52Ripeness) ? [{ iteration: "V52M", commit: V52M_COMMIT, path: ".claude/window1_live_v4_replay/v52m_macro_recognition_20260817/COHORT_SELECTION_RECEIPT.json", bytes: gitShow(V52M_COMMIT, ".claude/window1_live_v4_replay/v52m_macro_recognition_20260817/COHORT_SELECTION_RECEIPT.json") }] : []),
    ...((isV52o || isV52Ripeness) ? [{ iteration: "V52N", commit: V52N_COMMIT, path: ".claude/window1_live_v4_replay/v52n_recognition_confidence_gates_20260817/COHORT_SELECTION_RECEIPT.json", bytes: gitShow(V52N_COMMIT, ".claude/window1_live_v4_replay/v52n_recognition_confidence_gates_20260817/COHORT_SELECTION_RECEIPT.json") }] : []),
    ...(isV52Ripeness ? [{ iteration: "V52O", commit: V52O_COMMIT, path: ".claude/window1_live_v4_replay/v52o_benchmarked_role_instrument_20260817/COHORT_SELECTION_RECEIPT.json", bytes: gitShow(V52O_COMMIT, ".claude/window1_live_v4_replay/v52o_benchmarked_role_instrument_20260817/COHORT_SELECTION_RECEIPT.json") }] : []),
    ...(isV52q ? [{ iteration: "V52P", commit: V52P_COMMIT, path: ".claude/window1_live_v4_replay/v52p_ripeness_gated_role_binding_20260817/COHORT_SELECTION_RECEIPT.json", bytes: gitShow(V52P_COMMIT, ".claude/window1_live_v4_replay/v52p_ripeness_gated_role_binding_20260817/COHORT_SELECTION_RECEIPT.json") }] : []),
  ] : null;
  const v52eCohort = isV52e && !isV52eExam ? (isV52q ? buildV52qCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52p ? buildV52pCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52o ? buildV52oCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52n ? buildV52nCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52m ? buildV52mCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52l ? buildV52lCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52k ? buildV52kCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52j ? buildV52jCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52i ? buildV52iCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52h ? buildV52hCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52g ? buildV52gCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : isV52f ? buildV52fCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts) : buildV52eCohort(baseByEvent, v52eCensusBytes, v52ePriorReceipts)) : null;
  ...
  if (isV52ReadAuthority && !isV52eExam) {
    ...
    if (isV52MacroRecognition) for (let index = 0; index < candidateFlow.trace.length; index += 1) {
      const row = candidateFlow.trace[index];
      if ((isV52q && row.macro_recognition?.candidate_role !== null) || (isV52p && row.macro_recognition?.ripeness?.verified_binding === true) || (isV52o && row.benchmarked_role_instrument?.benchmark_role?.signable === true) || (isV52n && row.recognition_confidence_gate) || (!isV52Ripeness && !isV52o && !isV52n && row.per_shape_floor_depth?.applicable === true)) {
    ...
    const benchmarkRoleSummary = (isV52o || isV52Ripeness) ? (() => {
      ...
      const candidateLegByIdentity = new Map(candidateRun.marketEvents.flatMap((event) => Object.values(event.legs).map((leg) => [leg.leg_identity, leg])));
      const baselineLegByIdentity = new Map(baselineRun.marketEvents.flatMap((event) => Object.values(event.legs).map((leg) => [leg.leg_identity, leg])));
      const v52lRoleRun = isV52q ? machineRuns.get("V52L_FROZEN_BASELINE") : baselineRun;
      ...
      const terminalRows = [...candidateLegByIdentity.entries()].map(([legIdentity, leg]) => {
        ...
        return {
          ...
          role: isV52Ripeness ? (row?.macro_recognition?.bound_role ?? "ABSTAIN") : (row?.macro_recognition?.role ?? "ABSTAIN"), candidate_role: row?.macro_recognition?.candidate_role ?? null, ripeness: row?.macro_recognition?.ripeness ?? null, drift_cents: row?.macro_recognition?.drift_cents ?? null, anchor_correction: row?.macro_recognition?.anchor_correction ?? null,
          ...
          called: isV52Ripeness ? row?.macro_recognition?.bound_role != null : ["ROLE_DOWN", "ROLE_UP"].includes(row?.macro_recognition?.role), correct: expected ? (isV52Ripeness ? row?.macro_recognition?.bound_role : row?.macro_recognition?.role) === expected : null,
          credited: leg.credited, entry_cents: leg.entry_cents, fill_timestamp_epoch: leg.fill_timestamp_epoch,
          depth_consumption: row?.benchmarked_role_instrument ?? null,
        };
      ...
      const v52lReportedRows = isV52q ? groundBoundFourStateRowsFor(v52lRoleRun.marketEvents) : reportedBaselineFourStateRows;
      ...
      const roleReceiptRows = candidateFlow.trace.filter((row) => row.macro_recognition).map((row) => ({ event_id: row.event_id, leg_identity: row.leg_identity, category: row.category, price_region: row.price_region, timestamp_epoch: row.timestamp_epoch, receipt: row.receipt, role: isV52Ripeness ? (row.macro_recognition.bound_role ?? "ABSTAIN") : row.macro_recognition.role, candidate_role: row.macro_recognition.candidate_role ?? null, ripeness: row.macro_recognition.ripeness ?? null, drift_cents: row.macro_recognition.drift_cents, post_formation_open_cents: row.macro_recognition.post_formation_open_cents, last_causal_print_cents: row.macro_recognition.last_causal_print_cents, anchor_correction: row.macro_recognition.anchor_correction ?? null, rule: row.macro_recognition.rule, depth_row_consumed: row.benchmarked_role_instrument?.down_depth_row_consumed ?? null, level_policy: row.benchmarked_role_instrument?.level_policy ?? null, level_policy_consumed: row.benchmarked_role_instrument?.level_policy_consumed ?? false, final_target_cents: row.final_target_cents, final_action: row.final_action }));
      const ungatedParityRows = isV52q ? roleReceiptRows.filter((row) => Number.isInteger(row.post_formation_open_cents) && Number.isInteger(row.last_causal_print_cents) && row.candidate_role).map((row) => {
      ...
      return {
        rule_binding: { rule: policy.benchmarkRuleReceipt(), source_commit: SHAPE_TAXONOMY_COMMIT, source_path: SHAPE_TAXONOMY_PATH, source_csv_path: SHAPE_TAXONOMY_CSV_PATH, source_csv_sha256: shaBytes(v52oTaxonomyCsvBytes), exact_literal_reinterpreted: false, new_constants: 0, ...(isV52Ripeness ? { ripeness: v52pRipenessBinding.provenance, class_gates: policy.CLASS_GATES, category_gates: policy.CATEGORY_GATES, effective_gate_law: "max(candidate_role_class_gate, category_gate)" } : {}), ...(isV52q ? { anchor_correction: v52qAnchorBinding } : {}) },
        down_depth_aggregate_derivation: ["ATP_MAIN", "ATP_CHALL", "WTA_MAIN", "WTA_CHALL"].map((category) => policy.downDepthAggregate(category)),
        terminal_roles: terminalRows,
        role_receipt_count: roleReceiptRows.length,
        coverage: { terminal_grain: "LAST_DECISION_RECEIPT_BEFORE_ENTRY_OR_AVAILABLE_REPLAY_TERMINAL", terminal_legs: terminalRows.length, called_all_legs: calledAll.length, called_all_legs_share: terminalRows.length ? calledAll.length / terminalRows.length : null, benchmark_truth_role_legs: truthEligible.length, called_truth_role_legs: calledTruth.length, called_truth_role_legs_share: truthEligible.length ? calledTruth.length / truthEligible.length : null, benchmark_reference_called_coverage: 0.84, target_band: isV52Ripeness ? [0.60, 1.0] : [0.70, 0.90], lands_in_target_band: truthEligible.length > 0 && calledTruth.length / truthEligible.length >= (isV52Ripeness ? 0.60 : 0.70) && calledTruth.length / truthEligible.length <= (isV52Ripeness ? 1.0 : 0.90) },
        ...
        banked_delta: { V52l: { n: baselineLocked.length, mean_cents: baselineLocked.length ? baselineLocked.reduce((sum, value) => sum + value, 0) / baselineLocked.length : null }, V52o: { n: candidateLocked.length, mean_cents: candidateLocked.length ? candidateLocked.reduce((sum, value) => sum + value, 0) / candidateLocked.length : null }, candidate: { n: candidateLocked.length, mean_cents: candidateLocked.length ? candidateLocked.reduce((sum, value) => sum + value, 0) / candidateLocked.length : null }, exceeds_V52m_1_83c: candidateLocked.length > 0 && candidateLocked.reduce((sum, value) => sum + value, 0) / candidateLocked.length > 1.83, exceeds_claim_bar_cents: isV52q ? 2.14 : isV52p ? 1.94 : 1.83, exceeds_claim_bar: candidateLocked.length > 0 && candidateLocked.reduce((sum, value) => sum + value, 0) / candidateLocked.length > (isV52q ? 2.14 : isV52p ? 1.94 : 1.83) },
    ...
    if (isV52e) Object.assign(sourceFiles, {
      ...
      ...(isV52MacroRecognition ? {
        ...
        ...((isV52o || isV52Ripeness) ? { [`${SHAPE_TAXONOMY_COMMIT}:${SHAPE_TAXONOMY_CSV_PATH}`]: { sha256: shaBytes(v52oTaxonomyCsvBytes), role: "PINNED_VERIFIED_ROLE_TRUTH_AND_BENCHMARK_ROWS" } } : {}),
        ...(isV52Ripeness ? {
    ...
    const v52pFrozenObservationControls = isV52Ripeness ? Object.fromEntries([
      ...
      ...(isV52q ? [["V52P_OBSERVATION_CONTROL", V52P_COMMIT, ".claude/window1_live_v4_replay/v52p_ripeness_gated_role_binding_20260817/OUTCOME_OBSERVATIONS_30.json"]] : []),
    ...
    const v52oObservationControls = isV52Ripeness ? v52pFrozenObservationControls : isV52o ? Object.fromEntries(["V52M_OBSERVATION_CONTROL", "V52N_OBSERVATION_CONTROL"].map((name) => {
...
` : null;
    ...
    if (isV52f || isV52g || isV52h || isV52DepthValidation || isV52CausalOnset) {
      ...
      if (isV52MacroRecognition && !isV52o && !isV52Ripeness) {
      ...
      if (isV52o || isV52Ripeness) {
        await writeGzipRowsFile(path.join(output, "BENCHMARKED_ROLE_CONSUMPTION_LEDGER.jsonl.gz"), candidateCompactTrace.filter((row) => row.macro_recognition).map((row) => ({ event_id: row.event_id, leg_identity: row.leg_identity, category: row.category, price_region: row.price_region, timestamp_epoch: row.timestamp_epoch, receipt: row.receipt, ...row.macro_recognition, benchmarked_role_instrument: row.benchmarked_role_instrument, final_action: row.final_action, final_target_cents: row.final_target_cents })));
