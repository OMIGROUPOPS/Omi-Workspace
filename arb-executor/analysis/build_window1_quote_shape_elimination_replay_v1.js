...
const { mutuallyNarrowPairAndLegs } = require("./window1_pair_interim_elimination_v18.js");
const { narrowBySignableCouples } = require("./window1_pair_couple_elimination_v19.js");
const { evaluateAnchorFreshnessA, advanceLowerSettlementC } = require("./window1_isolated_repair_predicates_v20.js");
...
// One cent is the operator-authorized starting tolerance from the frozen
// 2026-08-03 holdout gate-lag diagnostic; it is not a newly fitted constant.
const FIX_A_ANCHOR_TOLERANCE_CENTS = 1;
...
function replayGame(eventId, sources, library, refs, persistenceLibrary = null) {
  ...
  let tuples = allTuples, pairSurvivorIds = pairHypothesisGroup?.hypotheses?.map((row) => row.pair_hypothesis_id) || [], lastPairNarrowing = null, lastCoupleNarrowing = null, indices = Object.fromEntries(legs.map((l) => [l.leg, 0])); const dynamicPairClosures = [], pairNarrowingTransitions = [], coupleNarrowingTransitions = [], clock = [...new Set(legs.flatMap((l) => l.rows.map((r) => r.ts)))].sort((a, b) => a - b);
  for (const ts of clock) {
    ...
    if (pairInterimV18 && high.last && low.last) {
      const pairBin = Math.max(0, Math.min(100, Math.floor((ts - high.left) / (high.right - high.left) * 100)));
      const priorHigh = [...high.survivor_shapes], priorLow = [...low.survivor_shapes], priorPair = [...pairSurvivorIds];
      lastPairNarrowing = mutuallyNarrowPairAndLegs({ group: pairHypothesisGroup, priorPairIds: pairSurvivorIds, highShapes: high.survivor_shapes, lowShapes: low.survivor_shapes, highRow: { ...high.last, ask_dwell_seconds: high.last.ask_dwell_seconds + Math.max(0, ts - high.last.ts) }, lowRow: { ...low.last, ask_dwell_seconds: low.last.ask_dwell_seconds + Math.max(0, ts - low.last.ts) }, bin: pairBin });
      pairSurvivorIds = lastPairNarrowing.pair_survivor_ids;
      high.survivor_shapes = lastPairNarrowing.high_shapes;
      low.survivor_shapes = lastPairNarrowing.low_shapes;
      tuples = lastPairNarrowing.tuples;
      if (priorHigh.join("|") !== high.survivor_shapes.join("|") || priorLow.join("|") !== low.survivor_shapes.join("|") || priorPair.join("|") !== pairSurvivorIds.join("|")) pairNarrowingTransitions.push({ timestamp_epoch: ts, status: lastPairNarrowing.status, prior_pair_hypothesis_ids: priorPair, surviving_pair_hypothesis_ids: [...pairSurvivorIds], signable_pair_hypothesis_ids: [...lastPairNarrowing.signable_pair_survivor_ids], prior_high_shape_ids: priorHigh, high_shape_ids: [...high.survivor_shapes], prior_low_shape_ids: priorLow, low_shape_ids: [...low.survivor_shapes], high_receipt: high.last.receipt, low_receipt: low.last.receipt, same_clock_observation: true });
    } else if (pairCoupleV19) {
      const priorHigh = [...high.survivor_shapes], priorLow = [...low.survivor_shapes];
      lastCoupleNarrowing = narrowBySignableCouples({ group: pairCoupleGroup, highShapes: high.survivor_shapes, lowShapes: low.survivor_shapes });
      high.survivor_shapes = lastCoupleNarrowing.high_shapes; low.survivor_shapes = lastCoupleNarrowing.low_shapes;
      ...
      if (lastCoupleNarrowing.status === "SIGNABLE_PAIR_COUPLES_NARROWED_BOTH_LEG_SETS" && (priorHigh.join("|") !== high.survivor_shapes.join("|") || priorLow.join("|") !== low.survivor_shapes.join("|"))) coupleNarrowingTransitions.push({ timestamp_epoch: ts, status: lastCoupleNarrowing.status, pair_couple_ids: lastCoupleNarrowing.couples.map((row) => row.pair_couple_id), authorities: lastCoupleNarrowing.couples.map((row) => row.authority), prior_high_shape_ids: priorHigh, high_shape_ids: [...high.survivor_shapes], prior_low_shape_ids: priorLow, low_shape_ids: [...low.survivor_shapes], removed_high_shape_ids: lastCoupleNarrowing.removed_high_shapes, removed_low_shape_ids: lastCoupleNarrowing.removed_low_shapes, high_receipt: high.last?.receipt ?? null, low_receipt: low.last?.receipt ?? null });
    } else tuples = (dynamicRenarrowV6 ? allTuples : tuples).filter((t) => high.survivor_shapes.includes(t.highShape) && low.survivor_shapes.includes(t.lowShape));
    if (!pairInterimV18 && !interimEliminationV13 && dynamicRenarrowV6 && !tuples.length && high.last && low.last && currentPathDirection(high) !== "FLAT" && inverseDirection(currentPathDirection(high)) === currentPathDirection(low)) {
    ...
    for (const leg of legs) {
      ...
      let pairOnlyResolution = null;
      if (pairInterimV18 || pairCoupleV19) {
        const singleMacroShapes = compatibleShapes(leg.group, row, leg.all_shape_ids), singleShapeObjects = singleMacroShapes.map((id) => leg.group.shapes.find((shape) => shape.shape_id === id)).filter(Boolean), singleMicro = microRepairV14Enabled ? microRepairV14(singleShapeObjects, row.qualified_ask_descent_count) : null;
        const singleShapes = microRepairV14Enabled && singleMicro?.usable_shape_ids?.length ? singleMicro.usable_shape_ids : singleMacroShapes;
        const singleVerdicts = microRepairV14Enabled && singleMicro?.mode === "RESOLVED_MACRO_CARRY_AFTER_MICRO_ABSTENTION" ? singleMacroShapes.map((id) => ({ shape_id: id, verdict: "FLOOR" })) : singleShapes.map((id) => ({ shape_id: id, ...shapeVerdict(leg.group, id, row.progress_bin, row, excludeOwnTrainingMember ? eventId : null) }));
        const singleAllFloor = singleShapes.length > 0 && singleVerdicts.every((item) => item.verdict === "FLOOR");
        pairOnlyResolution = { single_leg_shape_ids: singleShapes, single_leg_verdicts: singleVerdicts, single_leg_resolved_floor: singleAllFloor, joint_shape_ids: shapes, joint_resolved_floor: allFloor, resolved_by_pair_hypothesis_not_single_leg_hypothesis: allFloor && !singleAllFloor, pair_hypothesis_ids: lastPairNarrowing?.signable_pair_survivor_ids || [], pair_couple_ids: lastCoupleNarrowing?.couples?.map((row) => row.pair_couple_id) || [], pair_couple_authorities: lastCoupleNarrowing?.couples?.map((row) => row.authority) || [], pair_status: lastPairNarrowing?.status || lastCoupleNarrowing?.status || "PAIR_SOURCE_PENDING" };
      }
      const legRole = leg === high ? "highShape" : "lowShape";
      let microEvidence = null;
      let stableSigningSupport = null;
      if (allFloor || lagDiagnosticV10 || persistenceLibraryEnabled || fixCShapeVerdictSettlement) {
      ...
      let fixAReceipt = null;
      if (fixAAnchorFreshness && allFloor) {
        const anchor = evaluateAnchorFreshnessA({ currentAsk: row.ask, observedLow: leg.first_ask + row.prefix.ask_dip, toleranceCents: FIX_A_ANCHOR_TOLERANCE_CENTS, freshOwnBookReceipt: changedThisTick[leg.leg] });
        fixAReceipt = { ...anchor, current_ask: row.ask, observed_low: leg.first_ask + row.prefix.ask_dip, tolerance_cents: FIX_A_ANCHOR_TOLERANCE_CENTS, own_book_receipt: row.receipt, own_book_timestamp: row.ts };
        if (anchor.placeable && microEvidence) {
          microEvidence = { ...microEvidence, own_micro_position_observed: true, evidence_type: "FIX_A_LIVE_ASK_WITHIN_REARMED_OBSERVED_LOW_ANCHOR_ON_FRESH_OWN_RECEIPT" };
        }
      }
      let fixCReceipt = null;
      if (fixCShapeVerdictSettlement && rawAllLower && microEvidence && stableSigningSupport) {
        const settlement = advanceLowerSettlementC({
          priorRefusal: leg.lower_settlement_anchor_v20,
          currentAsk: row.ask,
          observedLow: leg.first_ask + row.prefix.ask_dip,
          timestamp: row.ts,
          receiptId: row.receipt,
          dwellSeconds: row.ask_dwell_seconds,
          displayedAskSize: row.top_ask_size,
          requiredDwellSeconds: DWELL_SECONDS,
          requiredQuantity: QUANTITY,
          freshOwnBookReceipt: changedThisTick[leg.leg],
          upperLevelsResolved: Boolean(shapes.length && microEvidence.inverse_sibling_resolved && stableSigningSupport.supported),
        });
        leg.lower_settlement_anchor_v20 = settlement.anchor;
        if (settlement.settled) {
          fixCReceipt = settlement;
          verdicts = verdicts.map((item) => ({ ...item, verdict: "FLOOR", prior_verdict: item.verdict, fix_c_shape_verdict_settlement: settlement.settlement }));
          allFloor = true;
        }
      }
      ...
      if (row.raw_row_count < 2) reason = "NO_PRIOR_IN_WINDOW_BOOK";
        ...
        if (fixAAnchorFreshness && !fixAReceipt?.placeable) reason = fixAReceipt?.reason || "NO_FRESH_OWN_BOOK_RECEIPT";
        else if (!ownMicroPositionObserved) reason = "FLOOR_CONSENSUS_BUT_OWN_MICRO_POSITION_UNOBSERVED";
        ...
        else if (!fixAAnchorFreshness && !liveBookFloorV15 && row.prefix.ask_net !== row.prefix.ask_dip) reason = "FLOOR_CONSENSUS_BUT_CURRENT_ASK_IS_ABOVE_OBSERVED_LOW";
      ...
      if (state === "PLACE") { const sibling = legs.find((candidate) => candidate !== leg), baseOrder = { price_cents: row.ask, quantity: QUANTITY, action_ts: ts, action_receipt: row.receipt, same_receipt_fill_forbidden: true, surviving_shapes: verdicts, macro_surviving_shapes: macroShapes, micro_repair_v14: microRepair, pair_shape_tuples: tuples.map((t) => ({ ...t })), pair_interim_v18: pairOnlyResolution, inverse_sibling_proof_type: row.inverse_sibling_proof_type ?? null, stable_signing_support: row.stable_signing_support ?? null, live_book_floor_authority_v15: row.live_book_floor_authority_v15 ?? null, persistence_floor_v15: persistenceFloor?.authority === "LEAVE_ONE_LEG_OUT_FITTED_SAME_PRICE_PERSISTENCE" ? persistenceFloor : null, fix_a_anchor_freshness_v20: fixAReceipt, fix_c_shape_verdict_settlement_v20: fixCReceipt, pre_action_evidence: { own: preActionEvidence(leg), sibling: preActionEvidence(sibling) } }; leg.order = stableSamePriceConfirmation ? { ...baseOrder, action_receipt: leg.last.receipt, own_book_receipt_at_action: leg.last.receipt, own_book_ts_at_action: leg.last.ts, sibling_book_receipt_at_action: sibling.last?.receipt ?? null, sibling_book_ts_at_action: sibling.last?.ts ?? null, micro_position_evidence_type: row.micro_position_evidence_type ?? "ASK_PRICE_TRANSITION" } : baseOrder; }
...
function main() {
  ...
  const receipt = {
    schema_version: pairCoupleV19 ? "WINDOW1_PAIR_COUPLE_ABSTENTION_REPLAY_V19" : pairInterimV18 ? "WINDOW1_SYNCHRONIZED_PAIR_INTERIM_ELIMINATION_REPLAY_V18" : interimEliminationV13 ? "WINDOW1_INTERIM_ELIMINATION_REPLAY_V13" : dynamicRenarrowV6 ? "WINDOW1_QUOTE_SHAPE_DYNAMIC_RENARROW_REPLAY_V6" : "WINDOW1_QUOTE_SHAPE_REPLAY",
    cold: true,
    ...
    pair_interim_v18: pairInterimV18,
    pair_couple_v19: pairCoupleV19,
