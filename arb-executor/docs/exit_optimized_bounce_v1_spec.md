...
- **G7 band support**: frame-aware (probe /tmp/g7_probe.log, full-cohort measured). Frame I (dense, ~2,128/band) keeps the hard `n_positions >= 200` reliability bar. Frame P is structurally sparse BY CONSTRUCTION (one T-20m+/-2 conservative fill per ticker -- this IS the conservative-fill assumption, not a data deficiency; measured per-band min 27, median 172, mean 177.9, zero bands <10, 39/40 >=30). Frame P uses a calibrated floor `n_positions >= 30` (below the populated-band cluster p10=80, above the lone extreme-dislocation band idx39 n=27, which carries a `low_support` flag rather than hard-failing the gate). The v0.1 flat-200 was a frame-blind defect that would reject the deliverable for correctly implementing the chosen dual-conservative-frame model -- corrected per the G23/4f55339 honest-provenance discipline (probe-grounded, not silently patched).
