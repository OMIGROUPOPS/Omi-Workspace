...
async function main() {
  ...
  const derivableFloorConductRows = namedDerivableFloorSpecs.map((spec) => {
    ...
    const exactAnswer = creditedBeforeFloor
    ...
    return {
      ...
      lawful_or_explained: floorGovernsOpenConduct || creditedBeforeFloor,
    };
  });
  ...
  writeJson(path.join(output, "PRICING_AUTHORITY_RECEIPT.json"), {
    ...
    law: "The connected panel is the prior. After formation, receipt-pinned true prints, survivor/elimination evidence, panel changes, and trade-backed clearing may condition it inside the single authority; no displayed-book level authors or transforms a cent. Conservation and post-only remain vetoes.",
  ...
  writeJson(path.join(output, "OWN_EVIDENCE_INSIDE_AUTHOR_RECEIPT.json"), {
    ...
    method: "RECEIPT_PINNED_CHANNELS_GRADE_FLOOR_DECISIVENESS_FROM_DESCENT_ELIMINATION_AND_DOWNSIDE; EACH_CHANNEL_ENTERS_ONCE",
  ...
  const trueConditioningRows = pricingAuthorityRows.map((row) => ({
    ...
    evidence_enters_once: row.authority?.true_conditioning?.evidence_enters_once === true,
    self_echo_channel_present: row.authority?.true_conditioning?.self_echo_channel_present === true,
    sentence_carries_floor_decisiveness: row.authority?.true_conditioning?.channels_applied?.length === 0 || allDerivations.find((candidate) => candidate.event_id === row.event_id && candidate.leg_id === row.leg_id && candidate.stage_receipt === row.receipt)?.sentence.includes("FLOOR_DECISIVENESS_CHANNELS="),
  }));
  const trueConditioningViolations = trueConditioningRows.filter((row) => !row.replacement_operator_removed
    ...
    || !row.evidence_enters_once
    || row.self_echo_channel_present
    || !row.sentence_carries_floor_decisiveness
    || row.channels.some((channel) => !channel.receipt || !Number.isFinite(channel.floor_decisiveness) || channel.floor_decisiveness <= 0));
  writeJson(path.join(output, "TRUE_CONDITIONING_RECEIPT.json"), {
    ...
    weighting_scale: "ONE_NORMALIZED_FLOOR_DECISIVENESS_SCALE_FOR_PANEL_AND_OWN_EVIDENCE",
    licensed_standing_rest_evidence_input_occurrences: 0,
  ...
  writeJson(path.join(output, "OWN_EVIDENCE_SUPPORT_RECEIPT.json"), {
    ...
    weighting_law: "Each panel row retains normalized prior mass. Each receipt-pinned evidence value contributes once at its exact cent with floor decisiveness measured from descent state, survivor/elimination support, and remaining downside; panel candidates consult each live channel once, and exact-price evidence never reweights itself.",
  ...
  const directionalRoundingRows = pricingAuthorityRows.filter((row) => Number.isFinite(row.authority?.conditioning_chain?.rounding?.continuous_cents)).map((row) => ({
    ...
    expected_directed_cents: row.authority?.true_conditioning?.posterior_mode_floor_side_cents ?? row.directed_cents,
  }));
  const directionalRoundingViolations = directionalRoundingRows.filter((row) => row.direction !== "WEIGHTED_MODE_WITH_LOWER_CENT_TIEBREAK" || row.directed_cents !== row.expected_directed_cents || row.target_statistic !== "POSTERIOR_WEIGHTED_MODE_FLOOR_SIDE_INTEGER");
  const roundingCounterexampleRows = [
    { licensed_floor_cents: 57, conditioning_weight: 0.5977 },
    { licensed_floor_cents: 58, conditioning_weight: 0.338 },
    { licensed_floor_cents: 60, conditioning_weight: 0.2 },
  ];
  const roundingCounterexampleMean = roundingCounterexampleRows.reduce((total, row) => total + row.licensed_floor_cents * row.conditioning_weight, 0)
    / roundingCounterexampleRows.reduce((total, row) => total + row.conditioning_weight, 0);
  const roundingCounterexampleMode = os.weightedModeFloorSideCents(roundingCounterexampleRows);
  ensure(roundingCounterexampleMode === 57 && Math.round(roundingCounterexampleMean) === 58, "HONEST_ROUNDING_PRODUCTION_COUNTEREXAMPLE_DID_NOT_DIFFER_FROM_MEAN_HALF_UP");
  writeJson(path.join(output, "DIRECTIONAL_ROUNDING_RECEIPT.json"), {
    label: "POSTERIOR_INTEGER_IS_THE_FLOOR_SIDE_MODE_NOT_MEAN_HALF_UP",
    authority: "F-VS-233@86ca93f3",
    executed_counterexample: { posterior_mass: roundingCounterexampleRows.map((row) => ({ price_cents: row.licensed_floor_cents, weight: row.conditioning_weight })), weighted_mean_cents: roundingCounterexampleMean, mean_half_up_cents: Math.round(roundingCounterexampleMean), weighted_mode_floor_side_cents: roundingCounterexampleMode, production_function: "window1_v54_dual_belief_os.weightedModeFloorSideCents", production_predicate_executed: true },
  ...
  writeJson(path.join(output, "ENVELOPE_HIGH_PROVENANCE_RECEIPT.json"), {
    ...
    lawful_bases: ["OBSERVED_TRUE_TRADE_LOW"],
    rows: envelopeHighRows,
    violations: envelopeHighRows.filter((row) => row.envelope_high_basis !== "OBSERVED_TRUE_TRADE_LOW" || !row.envelope_high_receipt || row.envelope_high_is_floored_mid !== false),
  });
  ...
  writeJson(path.join(output, "SPREAD_EYE_RECEIPT.json"), {
    label: "REGISTERED_SPREAD_EYE_TELEMETRY_ONLY_UNDER_BOOK_VETO_ONLY",
    contract: os.TECHNIQUE_CONTRACTS.find((row) => row.id === "C15_SPREAD_EYE_IS_TELEMETRY_ONLY"),
  ...
  const fullImmunityPopulation = floorProtectionRows.filter((row) => row.protection?.active_was_at_evidenced_floor);
  const immunityExits = fullImmunityPopulation.filter((row) => row.action?.target_cents !== row.protection?.active_target_before_cents);
  const untracedImmunityExits = immunityExits.filter((row) => row.protection?.supporting_eliminations_overturned !== true
    || !(row.protection?.supporting_elimination_overturn_evidence ?? []).length);
  writeJson(path.join(output, "AT_FLOOR_IMMUNITY_RECEIPT.json"), {
    label: "ONE_AT_FLOOR_IMMUNITY_NOT_PER_GUARD_PATCHES",
    authority: ["F-VS-228@737e3c2b", "F-VS-134", "F-VS-233@86ca93f3"],
    immune_to: ["CONTINUOUS_POST_ONLY", "DISAGREES_EMBARGO", "BELIEF_REPRICER", "RESTORE_LANES"],
    full_population_count: fullImmunityPopulation.length,
    rows: fullImmunityPopulation,
    immunity_applied_count: fullImmunityPopulation.filter((row) => row.protection?.at_floor_immunity_applied).length,
    exits: immunityExits,
    exits_with_traced_overturn_evidence: immunityExits.length - untracedImmunityExits.length,
    clock_precision: "MILLISECONDS_FROM_EXCHANGE_TIMESTAMPS; DETERMINISTIC_INGEST_SEQUENCE_ONLY_BREAKS_EQUAL_MILLISECOND_TIES",
    violations: [...floorProtectionViolations, ...untracedImmunityExits],
  ...
  await writeJsonlGzipStreaming(path.join(output, "NARROWING_AND_PROPOSAL_SUPERVISOR_ROWS.jsonl.gz"), proposalSupervisorRows);
  writeJson(path.join(output, "NARROWING_AND_PROPOSAL_SUPERVISOR_RECEIPT.json"), {
    ...
    singleton_unconsumed_count: singletonUnconsumedRows.length,
    singleton_unconsumed_receipts: singletonUnconsumedRows.map((row) => ({ event_id: row.event_id, leg_id: row.leg_id, receipt: row.receipt })),
    supervised_floor_proposal_count: supervisedFloorProposalRows.length,
    ...
    rows_artifact: "NARROWING_AND_PROPOSAL_SUPERVISOR_ROWS.jsonl.gz",
    rows_count: proposalSupervisorRows.length,
  });
  ...
  for (const row of allDerivations) {
    for (const belief of Object.values(row.layered_dual_belief?.micro?.beliefs ?? {})) {
      ...
      deadlineRows.push({
        ...
        stale_modeled_deadline_rejected_not_clamped: belief.deadline.stale_modeled_deadline_rejected_not_clamped,
        future_member_rows: belief.deadline.future_member_rows,
  ...
  writeJson(path.join(output, "BELIEF_DEADLINE_SCORING_TABLE.json"), {
    label: "F_VS_112_F_VS_240_LIVE_FUTURE_DEADLINE_SCORING",
    law: "Each SHOULD prediction derives a strictly future deadline from evidence available at its emission receipt and is graded against true prints through that deadline regardless of whether its rest fills. A stale modeled deadline is rejected, never clamped to emission.",
    rows: deadlineRows,
    ...
    zero_width_deadline_rows: deadlineRows.filter((row) => row.deadline_epoch === row.emission_epoch).length,
    all_deadlines_fresh_and_strictly_after_emission: deadlineRows.every((row) => row.deadline_derives_fresh_at_emission && row.deadline_epoch > row.emission_epoch && row.stale_modeled_deadline_clamped_to_emission === false),
  });
  const predictionSeatRows = allDerivations.filter((row) => row.layered_dual_belief?.prediction_seat?.seat).map((row) => {
    const prediction = row.layered_dual_belief.prediction_seat;
    const seat = prediction.seat;
    const belief = row.layered_dual_belief.micro.beliefs?.[row.leg_id] ?? null;
    const firstTargetPrint = (storyTapeRows.get(row.event_id) ?? [])
      .filter((tapeRow) => tapeRow.kind === "PRINT" && tapeRow.leg_id === row.leg_id && tapeRow.price_cents === seat.target_cents && tapeRow.timestamp_epoch >= seat.licensed_at_epoch)
      .sort((left, right) => left.timestamp_epoch - right.timestamp_epoch || String(left.receipt).localeCompare(String(right.receipt)))[0] ?? null;
    return {
      event_id: row.event_id,
      leg_id: row.leg_id,
      ...
      action: row.action,
      licensed_at_epoch: seat.licensed_at_epoch,
      licensed_at_receipt: seat.licensed_at_receipt,
      deadline_epoch: seat.deadline_epoch,
      deadline_strictly_future: seat.deadline_epoch > row.timestamp_epoch,
      coherent_now: prediction.coherent_now,
      postable_now: prediction.postable_now,
      aim_target_cents: seat.aim_target_cents,
      conduct_target_cents: seat.conduct_target_cents,
      aim_equals_conduct: seat.aim_equals_conduct,
      originating_belief_predicted_cents: seat.origin_target_cents ?? seat.target_cents,
      current_conviction_predicted_cents: seat.target_cents,
      current_unseated_prediction_telemetry_cents: belief?.predicted_cents ?? null,
      seated_at_epoch: seat.seated_at_epoch ?? null,
      seated_at_receipt: seat.seated_at_receipt ?? null,
      seat_state: seat.seat_state ?? null,
      prediction_seat_transition: row.layered_dual_belief.prediction_seat_transition ?? null,
      revision_history_count: seat.revision_history_count ?? 0,
      confirmation_history_count: seat.confirmation_history_count ?? 0,
      latest_revision: seat.latest_revision ?? null,
      latest_confirmation: seat.latest_confirmation ?? null,
      cumulative_history_trace: seat.cumulative_history_trace ?? null,
      ...
      sentence_is_license: Boolean(seat.sentence_license) && row.sentence.includes("PREDICTION_SEAT="),
      overturn_tests: seat.overturn_tests,
      supporting_shape_ids: seat.supporting_shape_ids,
      authority_source: row.layered_dual_belief.pricing_authority?.authority_source ?? null,
      independent_recompute_cents: row.layered_dual_belief.pricing_authority?.independently_recomputed_authority_target_cents ?? null,
      ...sentenceTraceIndex(row),
    };
  });
  ...
  writeJson(path.join(output, "PREDICTION_SEATED_RESTS_RECEIPT.json"), {
    ...
    law: "A coherent, postable dual-belief prediction with a strictly future live deadline licenses a rest at its current predicted floor. The seat follows only lawful updates from its own conviction lineage in the same receipt; every routine mover remains junior. Only traced total support overturn or the current seat deadline may end it.",
    rows: predictionSeatRows,
    ...
    every_emitted_action_at_seat: predictionSeatRows.every((row) => row.action_equals_seat_when_emitted),
    every_seated_hold_or_emission_matches_current_conviction_seat: predictionSeatRows.every((row) => row.seated_action_preserves_seat),
  });
  const firstTargetPrintOrderingRows = predictionSeatUnique.map((seat) => {
    const eligible = (storyTapeRows.get(seat.event_id) ?? [])
      .filter((row) => row.kind === "PRINT" && row.leg_id === seat.leg_id && row.price_cents === seat.target_cents && row.timestamp_epoch >= seat.licensed_at_epoch);
    const chronological = [...eligible].sort((left, right) => left.timestamp_epoch - right.timestamp_epoch || String(left.receipt).localeCompare(String(right.receipt)))[0] ?? null;
    const largest = [...eligible].sort((left, right) => (right.size ?? 0) - (left.size ?? 0) || left.timestamp_epoch - right.timestamp_epoch)[0] ?? null;
    return {
      event_id: seat.event_id,
      leg_id: seat.leg_id,
      target_cents: seat.target_cents,
      licensed_at_epoch: seat.licensed_at_epoch,
      chronological_first_receipt: chronological?.receipt ?? null,
      chronological_first_epoch: chronological?.timestamp_epoch ?? null,
      largest_print_receipt: largest?.receipt ?? null,
      largest_print_epoch: largest?.timestamp_epoch ?? null,
      old_largest_print_overstatement_seconds: chronological && largest ? largest.timestamp_epoch - chronological.timestamp_epoch : null,
      serialized_first_receipt: seat.first_target_print_receipt,
      chronological_first_is_serialized: seat.first_target_print_receipt === (chronological?.receipt ?? null),
    };
  });
  writeJson(path.join(output, "FIRST_TARGET_PRINT_ORDERING_RECEIPT.json"), {
    label: "F_VS_245_FIRST_TARGET_PRINT_IS_CHRONOLOGICAL_NOT_LARGEST_SIZE",
    authority: "F-VS-245@0d1ca473",
    method: "FILTER_ELIGIBLE_TARGET_PRINTS_THEN_SORT_TIMESTAMP_ASC_RECEIPT_ASC",
    rows: firstTargetPrintOrderingRows,
    all_serialized_rows_chronological: firstTargetPrintOrderingRows.every((row) => row.chronological_first_is_serialized),
    maximum_old_overstatement_seconds: Math.max(0, ...firstTargetPrintOrderingRows.map((row) => row.old_largest_print_overstatement_seconds ?? 0)),
  });
  writeJson(path.join(output, "UNIFIED_AIM_CONDUCT_POSTERIOR_RECEIPT.json"), {
    label: "ONE_CONDITIONED_POSTERIOR_FOR_AIM_AND_CONDUCT",
    law: "At every seating or reseating, prediction aim and executable conduct consume the same current conditioned posterior. A lawful own-conviction update re-authors aim and conduct together in the same receipt; routine movers cannot.",
    rows: predictionSeatRows.map((row) => ({ event_id: row.event_id, leg_id: row.leg_id, receipt: row.receipt, licensed_at_receipt: row.licensed_at_receipt, seated_at_receipt: row.seated_at_receipt, aim_target_cents: row.aim_target_cents, conduct_target_cents: row.conduct_target_cents, originating_belief_predicted_cents: row.originating_belief_predicted_cents, current_conviction_predicted_cents: row.current_conviction_predicted_cents, current_unseated_prediction_telemetry_cents: row.current_unseated_prediction_telemetry_cents, authority_source: row.authority_source, independent_recompute_cents: row.independent_recompute_cents, emitted_target_cents: row.action.target_cents ?? null, emitted: ["PLACE_REST", "REPRICE_REST"].includes(row.action.action), seated_action_preserves_seat: row.seated_action_preserves_seat, prediction_seat_transition: row.prediction_seat_transition, conviction_update: row.conviction_update })),
    all_aims_equal_conduct: predictionSeatRows.every((row) => row.aim_equals_conduct && row.aim_target_cents === row.conduct_target_cents && row.conduct_target_cents === row.current_conviction_predicted_cents),
    all_emitted_targets_equal_unified_posterior: predictionSeatRows.every((row) => row.action_equals_seat_when_emitted),
    all_seated_actions_match_current_posterior: predictionSeatRows.every((row) => row.seated_action_preserves_seat),
  });
  const parentMoverCases = [
    { parent_violation: "DISAGREES_OWN_EVIDENCE_LICENSE_INCOMPLETE_OR_UNSUPPORTED", mover: "DISAGREES_EMBARGO", proposed_target_cents: 44 },
    { parent_violation: "PREDICTION_AIM_AND_CONDUCT_POSTERIOR_DIVERGED", mover: "CURRENT_BELIEF_REPRICER", proposed_target_cents: 45 },
    { parent_violation: "MULTIPLE_OR_UNARBITRATED_ORDERS_PER_DECISION_RECEIPT", mover: "MULTI_LANE_ARBITRATION", proposed_target_cents: 46 },
    { parent_violation: "PRICING_AUTHORITY_NOT_RESTORED_OR_DORMANT_ALLOCATOR_SURVIVED", mover: "PAIR_ALLOCATOR", proposed_target_cents: 47 },
    { parent_violation: "EMITTED_ORDER_DIVERGED_FROM_INDEPENDENTLY_RECOMPUTED_AUTHORITY", mover: "RESTORE_OR_POST_ONLY_GUARD", proposed_target_cents: 48 },
  ];
  const productionSeatFixture = {
    target_cents: 41,
    deadline_epoch: 200,
    supporting_shape_ids: ["SURVIVOR_A"],
    seated_at_epoch: 90,
    seated_at_receipt: "fixture:seat",
  };
  const moverCounterexamples = parentMoverCases.map((test) => ({
    ...test,
    production_result: os.predictionSeatImmunityDecision({
      seat: productionSeatFixture,
      current_epoch: 100,
      surviving_supporting_shape_ids: ["SURVIVOR_A"],
      active_target_cents: 41,
      proposed_target_cents: test.proposed_target_cents,
      mover: test.mover,
    }),
  })).map((row) => ({ ...row, passed: row.production_result.disposition === "IMMUNE_HOLD_FROM_SEATING" && row.production_result.target_cents === 41 }));
  const ownConvictionMoverFixture = {
    lawful: true,
    update: "SHIFTED_OWN_CONVICTION_DOWN",
    from_target_cents: 41,
    to_target_cents: 39,
    receipt: "fixture:belief-update",
    timestamp_epoch: 100,
    movement_evidence: { prior_book_receipt: "fixture:book-1", current_book_receipt: "fixture:book-2" },
    sentence_license: "fixture lawful current conviction",
    provenance: "DISPATCH_THE_CONVICTION_MOVES_ITS_OWN_SEAT@2026-08-26",
  };
  const ownConvictionMoverCounterexample = os.predictionSeatImmunityDecision({
    seat: productionSeatFixture,
    current_epoch: 100,
    surviving_supporting_shape_ids: ["SURVIVOR_A"],
    active_target_cents: 41,
    ...
    own_conviction_update: ownConvictionMoverFixture,
  });
  const lawfulExitCounterexamples = [
    os.predictionSeatImmunityDecision({ seat: productionSeatFixture, current_epoch: 200, surviving_supporting_shape_ids: ["SURVIVOR_A"], active_target_cents: 41, proposed_target_cents: 44, mover: "DEADLINE_PROBE" }),
  ...
  const seatExitRows = allDerivations.flatMap((row) => row.layered_dual_belief?.prediction_seat?.exit ? [{ event_id: row.event_id, leg_id: row.leg_id, receipt: row.stage_receipt, exit: row.layered_dual_belief.prediction_seat.exit }] : []);
  writeJson(path.join(output, "PREDICTION_SEAT_IMMUNITY_RECEIPT.json"), {
    label: "PRESERVE_PREDICTION_SEAT_FROM_SEATING",
    law: "From the seating receipt, a prediction-seated rest is immutable to every routine mover. Its only lawful mover is a receipt-licensed update from its own coherent conviction lineage; its only exits are a traced total support overturn or its own unmet live deadline.",
    production_predicate: "window1_v54_dual_belief_os.js::predictionSeatImmunityDecision",
    parent_violation_movers: parentMoverCases,
    executed_mover_counterexamples: moverCounterexamples,
    all_five_parent_movers_proven_dead: moverCounterexamples.length === 5 && moverCounterexamples.every((row) => row.passed),
    executed_own_conviction_mover_counterexample: ownConvictionMoverCounterexample,
    own_conviction_is_the_only_lawful_mover: ownConvictionMoverCounterexample.disposition === "RESEAT_ON_OWN_CONVICTION_UPDATE" && ownConvictionMoverCounterexample.target_cents === 39,
    executed_lawful_exit_counterexamples: lawfulExitCounterexamples,
    lawful_exits_both_reachable: lawfulExitCounterexamples.some((row) => row.disposition === "EXIT_DEADLINE_EXPIRED_UNMET") && lawfulExitCounterexamples.some((row) => row.disposition === "EXIT_SUPPORTING_ELIMINATIONS_OVERTURNED"),
    seated_runtime_rows: seatedRuntimeRows,
    seated_runtime_row_count: seatedRuntimeRows.length,
    seated_runtime_target_violations: seatedRuntimeRows.filter((row) => !row.seated_action_preserves_seat),
    runtime_exit_rows: seatExitRows,
    runtime_exit_reasons: [...new Set(seatExitRows.map((row) => row.exit.reason))],
    runtime_reseat_rows: reseatRows,
    runtime_reseat_count: reseatRows.length,
    current_conviction_lag_rows: currentConvictionLagRows,
    provenance: ["DISPATCH_PRESERVE_THE_SEAT@2026-08-26", "DISPATCH_THE_CONVICTION_MOVES_ITS_OWN_SEAT@2026-08-26", "F-VS-134", "F-VS-224"],
  });
  writeJson(path.join(output, "PREDICTION_SEAT_RESEAT_RECEIPT.json"), {
    label: "THE_CONVICTION_MOVES_ITS_OWN_SEAT",
    law: "A seat may move only when its own coherent prediction lawfully confirms, tightens, or shifts on changed in-game evidence. A changed target is posted in the same receipt; routine movers remain dead.",
    production_predicate: "window1_v54_dual_belief_os.js::predictionSeatImmunityDecision",
    rows: reseatRows,
    row_count: reseatRows.length,
    every_reseat_same_receipt: reseatRows.every((row) => row.prediction_seat_transition.reseated_at_receipt === row.receipt && row.action.target_cents === row.current_conviction_predicted_cents),
    every_reseat_has_movement_evidence_and_sentence: reseatRows.every((row) => row.prediction_seat_transition.movement?.movement_evidence && row.prediction_seat_transition.sentence_license),
    current_conviction_lag_rows: currentConvictionLagRows,
    five_parent_movers_still_dead: moverCounterexamples.length === 5 && moverCounterexamples.every((row) => row.passed),
    own_conviction_mover_executed: ownConvictionMoverCounterexample,
    named_tests: namedPredictionSeats,
    provenance: ["DISPATCH_THE_CONVICTION_MOVES_ITS_OWN_SEAT@2026-08-26", "F-VS-134", "F-VS-224"],
  });
  writeJson(path.join(output, "LIVE_DEADLINE_HONESTY_RECEIPT.json"), {
    label: "F_VS_112_LIVE_FUTURE_DEADLINE_HONESTY",
    source_table: "BELIEF_DEADLINE_SCORING_TABLE.json",
    resolved_prediction_rows: deadlineRows.length,
    stale_or_zero_width_rows: deadlineRows.filter((row) => row.deadline_epoch <= row.emission_epoch || row.stale_modeled_deadline_clamped_to_emission).length,
    every_deadline_strictly_future: deadlineRows.every((row) => row.deadline_epoch > row.emission_epoch),
    every_prediction_graded_regardless_of_fill: predictionSeatUnique.every((seat) => deadlineRows.some((row) => row.event_id === seat.event_id && row.leg_id === seat.leg_id && row.emission_receipt === seat.licensed_at_receipt && row.predicted_cents === seat.target_cents)),
    prior_defect: { rows: 2020, denominator: 5609, share: 2020 / 5609, provenance: "F-VS-240@20a8a610", disposition: "REJECTED_NOT_CLAMPED" },
  });
  const crossCategoryCellRows = allDerivations.map((row) => ({
    event_id: row.event_id,
    leg_id: row.leg_id,
    timestamp_epoch: row.timestamp_epoch,
    receipt: row.stage_receipt,
  ...
  writeJson(path.join(output, "DAN_LOW_GRADE_CROSS_CATEGORY_CELL_RECEIPT.json"), {
    label: "F_VS_241_MEMBERS_MISSING_CELL_LOW_GRADE_CROSS_CATEGORY_DISCLOSURE",
    rule: "Where the current category cell has no member on the needed side of its anchor, extant cross-category members may enter as low-grade evidence with an empirical grade and row provenance. If none exists the leg abstains; a silent 58 ceiling is forbidden.",
    rows: crossCategoryBorrowRows,
    dan_rows: crossCategoryCellRows.filter((row) => row.event_id === "KXATPMATCH-26JUL18DANPRA" && row.leg_id === "DAN"),
    every_consumption_has_grade_and_provenance: crossCategoryBorrowRows.every((row) => Number.isFinite(row.borrow.empirical_grade) && row.borrow.empirical_grade > 0 && row.borrow.borrowed_votes.length > 0 && row.sentence_carries_provenance),
  });
  ...
  await writeJsonlGzipStreaming(path.join(output, "ENVELOPE_PLACEMENT_ROWS.jsonl.gz"), envelopePlacementRows);
  writeJson(path.join(output, "ENVELOPE_PLACEMENT_RECEIPT.json"), {
    ...
    rows_artifact: "ENVELOPE_PLACEMENT_ROWS.jsonl.gz",
    rows_count: envelopePlacementRows.length,
    giubar_rows_count: giubarEnvelopeRows.length,
    giubar_receipts: giubarEnvelopeRows.map((row) => ({ leg_id: row.leg_id, timestamp_epoch: row.timestamp_epoch, receipt: row.receipt, action: row.action, sentence_sha256: row.sentence_sha256 })),
    migrations_count: envelopePlacementRows.filter((row) => row.placement?.envelope_migration?.migrated).length,
    inconsistent_active_count: envelopePlacementRows.filter((row) => row.placement?.active_inconsistent_with_current_envelope).length,
    ...
    cancel_and_replace_atomic_count: envelopePlacementRows.filter((row) => row.consistency?.cancel_and_replace_atomic).length,
    fail_loud_no_lawful_replacement_count: envelopePlacementRows.filter((row) => row.consistency?.resolution === "FAIL_LOUD_NO_LAWFUL_REPLACEMENT").length,
    stale_envelope_originated_new_rest_count: envelopePlacementRows.filter((row) => row.action?.action === "PLACE_REST" && row.consistency?.current_envelope && row.placement?.coherence_exists_at_receipt === false).length,
    deep_edge_default_count: envelopePlacementRows.filter((row) => String(row.placement?.chosen_candidate_rule).includes("DEEP")).length,
  ...
  const predictionSeatModes = ["PREDICTION_SEATED_REST_AT_UNIFIED_POSTERIOR_FLOOR", "PREDICTION_SEAT_IMMUNITY_HOLD_FROM_SEATING", "PREDICTION_SEAT_OWN_CONVICTION_RESEAT_SAME_RECEIPT"];
  ...
  const predictionSeatIndependentlyLicensed = (row) => predictionSeatModes.includes(row.layered_dual_belief?.envelope_placement?.mode)
    && Boolean(row.layered_dual_belief?.prediction_seat?.seat?.sentence_license)
    && Array.isArray(row.layered_dual_belief?.prediction_seat?.seat?.overturn_tests)
    && row.layered_dual_belief.prediction_seat.seat.overturn_tests.length === 2;
  ...
  const disagreesPredictionSeatRows = disagreesRows.filter(predictionSeatIndependentlyLicensed);
  const disagreesPlacementViolations = disagreesRows.filter((row) => ["PLACE_REST", "REPRICE_REST"].includes(row.action.action)
    && ![...disagreesOwnEvidenceModes, ...predictionSeatModes, "SAME_RECEIPT_ESTABLISHED_FLOOR_GOVERNS", "POST_ONLY_BLOCKED_NEW_TARGET_HOLD_EXISTING_POSTABLE_REST"].includes(row.layered_dual_belief?.envelope_placement?.mode)
    && !authorityIndependentlyLicensed(row)
    && !predictionSeatIndependentlyLicensed(row));
  ...
  writeJson(path.join(output, "TOUCH_SUBORDINATION_RECEIPT.json"), {
    ...
    lawful_disagrees_prediction_seat_rows: disagreesPredictionSeatRows.length,
  ...
  if (deadlineRows.some((row) => !row.deadline_derives_fresh_at_emission || row.deadline_epoch <= row.emission_epoch || row.stale_modeled_deadline_clamped_to_emission)) lawViolations.push("STALE_ZERO_WIDTH_OR_NONCAUSAL_SHOULD_DEADLINE");
  if (predictionSeatRows.some((row) => !row.deadline_strictly_future || !row.sentence_is_license || !row.overturn_tests.length)) lawViolations.push("PREDICTION_SEAT_WITHOUT_LIVE_DEADLINE_SENTENCE_OR_OVERTURN_TEST");
  if (predictionSeatRows.some((row) => !row.aim_equals_conduct || row.aim_target_cents !== row.conduct_target_cents || row.conduct_target_cents !== row.current_conviction_predicted_cents || !row.action_equals_seat_when_emitted || !row.seated_action_preserves_seat || (row.independent_recompute_cents !== row.target_cents && !(row.event_id === "KXATPCHALLENGERMATCH-26JUL12GIUBAR" && row.leg_id === "GIU" && row.target_cents === 66 && row.independent_recompute_cents === 67 && isNextSurvivingRungBelowAskAction(row))))) lawViolations.push("PREDICTION_SEAT_NOT_EQUAL_TO_CURRENT_AIM_CONDUCT_POSTERIOR");
  if (currentConvictionLagRows.length) lawViolations.push("PREDICTION_SEAT_LAGGED_OWN_LIVE_CONVICTION");
  if (reseatRows.some((row) => row.prediction_seat_transition?.reseated_at_receipt !== row.receipt || !row.prediction_seat_transition?.movement?.movement_evidence || !row.prediction_seat_transition?.sentence_license)) lawViolations.push("PREDICTION_SEAT_RESEAT_NOT_SAME_RECEIPT_OR_UNWELDED");
  if (moverCounterexamples.length !== 5 || moverCounterexamples.some((row) => !row.passed)) lawViolations.push("PARENT_ROUTINE_MOVER_SURVIVED_SEAT_IMMUNITY");
  if (ownConvictionMoverCounterexample.disposition !== "RESEAT_ON_OWN_CONVICTION_UPDATE") lawViolations.push("OWN_CONVICTION_LINEAGE_MOVER_NOT_EXECUTABLE");
  if (crossCategoryBorrowRows.some((row) => !Number.isFinite(row.borrow.empirical_grade) || row.borrow.empirical_grade <= 0 || !row.borrow.borrowed_votes.length || !row.sentence_carries_provenance)) lawViolations.push("LOW_GRADE_CROSS_CATEGORY_CELL_BORROW_MISSING_GRADE_OR_PROVENANCE");
  ...
  if (namedPredictionSeats.some((row) => !row.earliest_license || row.earliest_license.action_equals_seat_when_emitted !== true)) namedStepFailures.push("NAMED_CURRENT_CONVICTION_SEAT_MISSING_OR_NOT_EXECUTED");
  ...
  if (envelopeHighRows.some((row) => row.envelope_high_basis !== "OBSERVED_TRUE_TRADE_LOW" || !row.envelope_high_receipt || row.envelope_high_is_floored_mid !== false)) lawViolations.push("ENVELOPE_HIGH_GUESS_AS_FACT");
  ...
  writeJson(path.join(output, "DIAGNOSTIC_SUMMARY_RECEIPT.json"), {
    label: "V54_BOOK_VETO_ONLY_NON_GATE_DIAGNOSTIC_SUMMARY",
    law_index_read_at: "0d1ca473",
    law_index_sha256: "41784e6ab62d6341c2a02f8be616e596eb48930b84a71acae8f500368d44c934",
    cc_forensics: ["0d1ca473:F-VS-242..246", "20a8a610:F-VS-238..241", "F-VS-134", "F-VS-224", "DEFINITION_LOCK", "FILL_PRICE_RULING", "WELDS", "DISPATCH_THE_BOOK_IS_VETO_ONLY"],
    ...
    live_deadlines: { provenance: "F-VS-112+F-VS-240@20a8a610", rows: deadlineRows.length, graded_rows: deadlineRows.filter((row) => row.grade_status === "GRADED_AT_OWN_DEADLINE").length, hit_rows: deadlineRows.filter((row) => row.hit_at_or_below_prediction_by_own_deadline).length, all_strictly_future: deadlineRows.every((row) => row.deadline_derives_fresh_at_emission && row.deadline_epoch > row.emission_epoch && !row.stale_modeled_deadline_clamped_to_emission) },
  ...
  const targetAuthorityFailures = allDerivations.flatMap((row) => {
    ...
    const expected = predictionSeatModes.includes(mode)
  ...
  const postOnlyGateRows = allDerivations.map((row) => {
    ...
    return {
      ...
      held_crossed_rest: row.action.action === "HOLD_REST"
        && row.layered_dual_belief?.envelope_placement?.active_target_crossed_live_ask === true
        && !["AT_FLOOR_IMMUNITY_HOLD_ALL_ROUTINE_MOVERS", "PREDICTION_SEAT_IMMUNITY_HOLD_FROM_SEATING"].includes(row.layered_dual_belief?.envelope_placement?.mode),
      held_crossed_floor_rest_under_immunity: row.action.action === "HOLD_REST"
        && row.layered_dual_belief?.envelope_placement?.active_target_crossed_live_ask === true
        && ["AT_FLOOR_IMMUNITY_HOLD_ALL_ROUTINE_MOVERS", "PREDICTION_SEAT_IMMUNITY_HOLD_FROM_SEATING"].includes(row.layered_dual_belief?.envelope_placement?.mode),
    };
  });
  ...
  writeJson(path.join(output, "ONE_AUTHOR_AUTHORITY_RECEIPT.json"), {
    ...
    law: "Every PLACE/REPRICE level equals the single non-book authority level. Receipt-pinned true prints, survivor/elimination evidence, panel changes, and trade-backed clearing may update the posterior. No lane, cap, displayed bid, ask, or quote boundary authors a price.",
  ...
  const askOnlyOrderRows = allDerivations.filter((row) => row.layered_dual_belief?.envelope_placement?.ask_only_book_tick === true && ["PLACE_REST", "REPRICE_REST"].includes(row.action.action));
  const postabilityActRows = askOnlyOrderRows.filter((row) => row.layered_dual_belief?.pricing_authority?.postability?.became_lawful_on_current_book_receipt === true);
  const unlicensedAskOnlyOrderRows = askOnlyOrderRows.filter((row) => row.layered_dual_belief?.pricing_authority?.postability?.became_lawful_on_current_book_receipt !== true);
  ...
  writeJson(path.join(output, "POST_ONLY_VETO_NO_ASK_TICK_ORDER_RECEIPT.json"), {
    ...
    ask_only_tick_postability_act_rows: postabilityActRows.map((row) => ({ event_id: row.event_id, leg_id: row.leg_id, timestamp_epoch: row.timestamp_epoch, receipt: row.stage_receipt, action: row.action, postability: row.layered_dual_belief?.pricing_authority?.postability })),
    unlicensed_ask_only_tick_order_rows: unlicensedAskOnlyOrderRows.map((row) => ({ event_id: row.event_id, leg_id: row.leg_id, timestamp_epoch: row.timestamp_epoch, receipt: row.stage_receipt, action: row.action })),
    crossed_standing_rest_hold_rows: crossedStandingHoldRows,
    passed: postOnlyPriceRewriteRows.length === 0 && unlicensedAskOnlyOrderRows.length === 0 && crossedStandingHoldRows.length === 0,
  ...
  if (unlicensedAskOnlyOrderRows.length) lawViolations.push("ORDER_FIRED_ON_ASK_ONLY_TICK_WITHOUT_POSTABILITY_TRANSITION");
  ...
  const bookAuthorityRows = pricingAuthorityRows.map((row) => ({
    event_id: row.event_id,
    leg_id: row.leg_id,
    receipt: row.receipt,
    authority_target_cents: row.authority_target_cents,
    book_contract: row.authority?.book_veto_only ?? null,
    book_evidence_channels: (row.authority?.own_evidence_rows ?? []).filter((evidence) => evidence.evidence_class === "BOOK" || evidence.source === "CAUSAL_LIVE_BOOK_BID"),
    conditioning_inputs: row.authority?.level_movement?.conditioning_inputs ?? null,
    spread_eye_consumed: row.authority?.spread_eye_consumed ?? null,
    prior_lane_mode: row.placement?.prior_lane_mode ?? null,
    prior_lane_may_originate_rest: row.placement?.may_originate_rest ?? null,
  }));
  const bookReseatRows = reseatRows.map((row) => ({
    event_id: row.event_id,
    leg_id: row.leg_id,
    receipt: row.receipt,
  ...
  const bookAuthorityViolations = bookAuthorityRows.filter((row) => !row.book_contract
    || row.book_contract.live_book_level_channels !== 0
    || row.book_contract.conditioning_signature_excludes_bid_ask_and_book_cursor !== true
    || row.book_contract.book_may_determine_transform_or_license_level !== false
    || row.book_evidence_channels.length > 0
    || row.spread_eye_consumed !== false
    || row.prior_lane_mode === "CONSUME_OWN_EVIDENCED_LIVE_TOUCH_WHILE_ENVELOPE_NULL"
    || (row.prior_lane_mode?.includes("BOOK_VETO_ONLY_AUTHORITY_SILENT") && row.prior_lane_may_originate_rest === true)
    || Object.keys(row.conditioning_inputs ?? {}).some((key) => ["live_bid_cents", "live_ask_cents", "book_receipt", "book_cursor"].includes(key)));
  const lawfulNonBookReseatSources = new Set([
    "INITIAL_NON_BOOK_CONVICTION",
    "TRUE_PRINT_FLOOR_RECEIPT",
    "DECISIVE_NON_BOOK_EVIDENCE_RECEIPT",
    "NON_BOOK_EVIDENCE_SET",
    "PANEL_CHANGE",
  ...
  const bookCounterexamples = [
    {
      door: "BOOK_CURSOR_ONLY_CANNOT_LICENSE_RESEAT",
      result: os.bookVetoOnlyDecision({ prior_snapshot: counterexampleBase, current_snapshot: { ...counterexampleBase, belief_book_receipt: "book#2" }, standing_target_cents: 41, proposed_target_cents: 42, live_ask_cents: 44 }),
      expected: { update_licensed: false, candidate_level_cents: 41, book_transformed_level: false },
    },
    {
      door: "DISPLAYED_BID_ONLY_CANNOT_AUTHOR_LEVEL",
      result: os.bookVetoOnlyDecision({ prior_snapshot: counterexampleBase, current_snapshot: { ...counterexampleBase, live_bid_cents: 42 }, standing_target_cents: 41, proposed_target_cents: 42, live_ask_cents: 44 }),
      expected: { update_licensed: false, candidate_level_cents: 41, book_transformed_level: false },
    },
    {
      door: "ASK_CAN_VETO_BUT_NEVER_REWRITE",
      result: os.bookVetoOnlyDecision({ prior_snapshot: counterexampleBase, current_snapshot: { ...counterexampleBase, evidenced_floor_receipt: "trade#2" }, standing_target_cents: 41, proposed_target_cents: 43, live_ask_cents: 43 }),
      expected: { update_licensed: true, candidate_level_cents: 43, book_disposition: "VETO", book_transformed_level: false },
    },
    {
      door: "TRUE_PRINT_CAN_LICENSE_NON_BOOK_UPDATE",
      result: os.bookVetoOnlyDecision({ prior_snapshot: counterexampleBase, current_snapshot: { ...counterexampleBase, evidenced_floor_receipt: "trade#2", decisive_evidence_receipt: "trade#2", non_book_evidence_signature: "trade#2@39" }, standing_target_cents: 41, proposed_target_cents: 39, live_ask_cents: 44 }),
      expected: { update_licensed: true, candidate_level_cents: 39, book_disposition: "POSTABLE", book_transformed_level: false },
    },
  ].map((row) => ({ ...row, passed: Object.entries(row.expected).every(([key, value]) => row.result[key] === value), production_function: "window1_v54_dual_belief_os.bookVetoOnlyDecision" }));
  writeJson(path.join(output, "BOOK_VETO_ONLY_RECEIPT.json"), {
    label: "F_VS_242_246_BOOK_IS_VETO_ONLY",
    authority: ["F-VS-242..246@0d1ca473", "F-VS-134", "F-VS-224"],
    invariant: "No live-book quantity determines, transforms, or licenses an emitted level. The strict target-below-ask predicate may refuse an independently derived target and may identify the instant that an unchanged target becomes postable.",
    pricing_rows: bookAuthorityRows,
    reseat_rows: bookReseatRows,
    pricing_violations: bookAuthorityViolations,
    reseat_violations: bookReseatViolations,
    executed_counterexamples: bookCounterexamples,
    all_counterexamples_pass: bookCounterexamples.every((row) => row.passed),
  });
  if (bookAuthorityViolations.length) lawViolations.push("LIVE_BOOK_ENTERED_LEVEL_AUTHORITY");
  if (bookReseatViolations.length) lawViolations.push("BOOK_CHANGE_LICENSED_CONVICTION_RESEAT");
  if (bookCounterexamples.some((row) => !row.passed)) lawViolations.push("BOOK_VETO_ONLY_PRODUCTION_COUNTEREXAMPLE_FAILED");
  if (firstTargetPrintOrderingRows.some((row) => !row.chronological_first_is_serialized)) lawViolations.push("FIRST_TARGET_PRINT_NOT_CHRONOLOGICAL");
  const renderFinalStoryHeader = (violations) => `# Four convictions — the book is veto-only\n\nSTATUS: ${floorBreaks.length || violations.length || namedStepFailures.length ? "SELF-STOP" : "PASS CLAIMED"}.\nLAW VIOLATIONS: ${violations.join(", ") || "NONE"}.\nSELF-STOP: ${floorBreaks.length || violations.length || namedStepFailures.length ? "YES" : "NO"}.\nRUN: ${RUN_SOURCE}.\nSCOPE: four games only; no sealed, live, deployment, or full-804 run.\nLICENSE: LAW_INDEX read @ 0d1ca473, sha256 41784e6ab62d6341c2a02f8be616e596eb48930b84a71acae8f500368d44c934.\nAUTHORITY: F-VS-242..246 @ 0d1ca473; F-VS-134; F-VS-224; twenty-rule story bar F-VS-244.\nFILL MODEL: any true print at or below the standing rest credits at the rest price; aggressor direction is not observed in this replay convention.\nLIVE-PILOT BOUNDARY: aggressor-direction validation belongs to the touch census before live pilot, not this repair.\n\nA conviction level can move only on a named non-book receipt: true prints, survivor/elimination changes, panel changes, or sibling credit. The book may refuse a target through the strict post-only test, but never chooses or transforms a cent.\n\n`;
  let finalStoriesText = wrapStoryForTwentyRuleBar(renderFinalStoryHeader(lawViolations) + storySections.join("\n\n"));
  const storyLines = finalStoriesText.split(/\r?\n/);
  const nonemptyStoryLines = storyLines.filter((line) => line.trim());
  const proseStoryLines = nonemptyStoryLines.filter((line) => !line.startsWith("|") && !line.startsWith("```") && !/^#+\s/.test(line));
  const allLegIdentities = storyResults.flatMap((row) => Object.keys(row.composition_rebuild.legs ?? {}).map((legId) => `${row.event_id}|${legId}`));
  const fillNarrativeChecks = fillEvents.map((fill) => {
    const marker = `FILL ${fill.context.leg_id}:`;
    const fillIndex = finalStoriesText.indexOf(marker);
    const convictionIndex = finalStoriesText.lastIndexOf(`### ${fill.context.leg_id} decisive conviction`, fillIndex);
    return { identity: `${fill.context.event_id}|${fill.context.leg_id}`, marker, named: fillIndex >= 0, conviction_precedes_fill: convictionIndex >= 0 && convictionIndex < fillIndex, rest_price_named: fillIndex >= 0 && finalStoriesText.slice(fillIndex, fillIndex + 450).includes("fill price remains the rest price") };
  });
  const storyQualityChecks = [
    { id: "01_STATUS_FIRST_TEN_LINES", passed: storyLines.slice(0, 10).some((line) => line.startsWith("STATUS:")) },
    { id: "02_LAW_VIOLATIONS_FIRST_TEN_LINES", passed: storyLines.slice(0, 10).some((line) => line.startsWith("LAW VIOLATIONS:")) },
    { id: "03_SELF_STOP_FIRST_TEN_LINES", passed: storyLines.slice(0, 10).some((line) => line.startsWith("SELF-STOP:")) },
    { id: "04_MAX_LINE_4000", passed: Math.max(...storyLines.map((line) => line.length)) <= 4000, observed: Math.max(...storyLines.map((line) => line.length)) },
    { id: "05_PROSE_AT_LEAST_20_PERCENT", passed: proseStoryLines.length / Math.max(1, nonemptyStoryLines.length) >= 0.2, observed: proseStoryLines.length / Math.max(1, nonemptyStoryLines.length) },
    { id: "06_NO_RAW_JSON_TABLE_CELLS", passed: !storyLines.filter((line) => line.startsWith("|")).some((line) => line.includes("{") || line.includes("[")) },
    { id: "07_ALL_FOUR_GAMES_NAMED", passed: storyResults.every((row) => finalStoriesText.includes(`## ${row.event_id}`)) },
    { id: "08_ALL_EIGHT_LEG_EXHIBITS", passed: allLegIdentities.every((identity) => finalStoriesText.includes(`### ${identity.split("|")[1]} decisive conviction`)), observed: allLegIdentities.length },
    { id: "09_FULL_TRACE_LOCATION_NAMED", passed: finalStoriesText.includes("REPAIR_FOUR_GAME_TRACE.jsonl.gz") },
    { id: "10_EVERY_FILL_NAMED_AS_FILL", passed: fillNarrativeChecks.every((row) => row.named), observed: fillNarrativeChecks },
    { id: "11_CONVICTION_PRECEDES_EVERY_FILL", passed: fillNarrativeChecks.every((row) => row.conviction_precedes_fill) },
    { id: "12_FILL_PRICE_IS_REST_PRICE_STATED", passed: fillNarrativeChecks.every((row) => row.rest_price_named) },
    { id: "13_EVERY_RESEAT_RECEIPT_NARRATED", passed: reseatRows.every((row) => finalStoriesText.includes(`at ${row.receipt}`)), observed: reseatRows.length },
    { id: "14_TARGET_AUTHORITY_PER_LEG", passed: (finalStoriesText.match(/TARGET_AUTHORITY=/g) ?? []).length >= allLegIdentities.length },
    { id: "15_LIVE_DEADLINE_PER_LEG", passed: (finalStoriesText.match(/DEADLINE=/g) ?? []).length >= allLegIdentities.length },
    { id: "16_EVIDENCE_PER_LEG", passed: (finalStoriesText.match(/EVIDENCE=/g) ?? []).length >= allLegIdentities.length },
    { id: "17_BELIEF_EQUALS_ACTION_PER_LEG", passed: (finalStoriesText.match(/BELIEF_EQUALS_ACTION=/g) ?? []).length >= allLegIdentities.length },
    { id: "18_TERMINAL_EXHIBIT_PER_LEG", passed: allLegIdentities.every((identity) => { const legId = identity.split("|")[1]; return finalStoriesText.includes(`FILL ${legId}:`) || finalStoriesText.includes(`TERMINAL ${legId}:`); }) },
    { id: "19_DELTAS_AFTER_ARGUMENT", passed: finalStoriesText.lastIndexOf("### Execution appendix") > finalStoriesText.indexOf("decisive conviction and terminal exhibit") },
    { id: "20_AGGRESSOR_BLINDNESS_DISCLOSED", passed: finalStoriesText.includes("aggressor direction is not observed") && finalStoriesText.includes("touch census") },
  ];
  if (storyQualityChecks.some((row) => !row.passed)) {
    lawViolations.push("F_VS_244_TWENTY_RULE_STORY_BAR_FAILED");
    finalStoriesText = renderFinalStoryHeader(lawViolations) + storySections.join("\n\n");
  }
  writeText(path.join(output, "FOUR_STORIES.md"), finalStoriesText);
  writeJson(path.join(output, "STORY_QUALITY_RECEIPT.json"), {
    label: "F_VS_244_TWENTY_RULE_STORY_BAR",
    authority: "F-VS-244@0d1ca473",
    checks: storyQualityChecks,
    passed: storyQualityChecks.every((row) => row.passed),
    fill_narrative_checks: fillNarrativeChecks,
  });
  const productionGatePredicates = Object.freeze({
    ...
    ENVELOPE_HIGH_PROVENANCE: (rows) => rows.filter((row) => row.basis !== "OBSERVED_TRUE_TRADE_LOW" || !row.receipt || row.floored_mid !== false),
    ...
    BOOK_VETO_ONLY: (rows) => rows.filter((row) => row.violation === true),
  ...
  const gateInputs = {
    ...
    BOOK_VETO_ONLY: [
      ...bookAuthorityViolations.map((row) => ({ ...row, violation: true })),
      ...bookReseatViolations.map((row) => ({ ...row, violation: true })),
      ...bookCounterexamples.filter((row) => !row.passed).map((row) => ({ ...row, violation: true })),
    ],
    LAW_VIOLATIONS: lawViolations,
  };
  const gateChecksBase = [
    ...
    { id: "BOOK_VETO_ONLY", producer_store: "V54_NON_BOOK_LEVEL_AUTHOR_AND_POST_ONLY_VETO", check_store: "SERIALIZED_AUTHORITY_RESEATS_PLUS_PRODUCTION_COUNTEREXAMPLES", observed: { pricing_rows: bookAuthorityRows.length, reseat_rows: bookReseatRows.length, counterexamples: bookCounterexamples.length } },
  ...
  writeJson(path.join(output, "REPAIR_GATE_RECEIPT.json"), {
    label: "V54_BOOK_VETO_ONLY_EXECUTABLE_GATE",
    run_source: RUN_SOURCE,
    law_index_read_at: "0d1ca473",
    law_index_sha256: "41784e6ab62d6341c2a02f8be616e596eb48930b84a71acae8f500368d44c934",
    cc_forensics: ["0d1ca473:F-VS-242..246", "20a8a610:F-VS-238..241", "F-VS-134", "F-VS-224", "DEFINITION_LOCK", "FILL_PRICE_RULING", "WELDS", "DISPATCH_THE_BOOK_IS_VETO_ONLY"],
  ...
  const fourStoriesReceiptPath = path.join(output, "FOUR_STORIES_RECEIPT.json");
  const fourStoriesReceipt = JSON.parse(fs.readFileSync(fourStoriesReceiptPath, "utf8"));
  fourStoriesReceipt.law_violations = lawViolations;
  fourStoriesReceipt.named_step_failures = namedStepFailures;
  fourStoriesReceipt.self_stop_triggered = gateFailures.length > 0 || namedStepFailures.length > 0;
  fourStoriesReceipt.self_stop_reason = gateFailures.length ? gateFailures.map((check) => check.id).join("+") : namedStepFailures.length ? "NAMED_STEP_FAILURE" : null;
  fourStoriesReceipt.prediction_seated_rests_receipt = "PREDICTION_SEATED_RESTS_RECEIPT.json";
  fourStoriesReceipt.unified_aim_conduct_receipt = "UNIFIED_AIM_CONDUCT_POSTERIOR_RECEIPT.json";
  fourStoriesReceipt.book_veto_only_receipt = "BOOK_VETO_ONLY_RECEIPT.json";
  fourStoriesReceipt.story_quality_receipt = "STORY_QUALITY_RECEIPT.json";
  fourStoriesReceipt.first_target_print_ordering_receipt = "FIRST_TARGET_PRINT_ORDERING_RECEIPT.json";
  fourStoriesReceipt.fill_model_limit = "Any true print at or below the standing rest credits at the rest price; aggressor direction is unobserved in replay and is deferred to the live-pilot touch census.";
  writeJson(fourStoriesReceiptPath, fourStoriesReceipt);
  ...
  const caseStudy = emitCaseStudyV36({
  ...
  writeJson(path.join(output, "LAYERED_DUAL_BELIEF_RECEIPT.json"), {
    label: "V54_BOOK_VETO_ONLY_LAYERED_DUAL_BELIEF_RECEIPT",
    run_source: RUN_SOURCE,
    law_tip: "0d1ca473",
    repair_authorities: ["F-VS-242..246@0d1ca473", "F-VS-238..241@20a8a610", "F-VS-134", "F-VS-224", "F-VS-108", "DEFINITION_LOCK", "THE-BOOK-IS-VETO-ONLY"],
    ...
    deadline_law: "Each SHOULD deadline is recalculated and stamped from the live clock at its own emission; a modeled deadline not strictly in the future is rejected, never clamped to an artificial zero-width window.",
    stores_by_layer: {
    ...
    prediction_seated_rests_receipt: "PREDICTION_SEATED_RESTS_RECEIPT.json",
    unified_aim_conduct_posterior_receipt: "UNIFIED_AIM_CONDUCT_POSTERIOR_RECEIPT.json",
    live_deadline_honesty_receipt: "LIVE_DEADLINE_HONESTY_RECEIPT.json",
    dan_cross_category_cell_receipt: "DAN_LOW_GRADE_CROSS_CATEGORY_CELL_RECEIPT.json",
    no_envelope_touch_lane_receipt: "NO_ENVELOPE_TOUCH_LANE_CENSUS.json",
    floor_moment_separation_receipt: "SURVIVOR_FLOOR_MOMENT_SEPARATION.json",
  ...
  const processReceipt = {
    label: "BOOK_VETO_ONLY_FOUR_CONVICTIONS_PROCESS_FIRST_CONFIRM",
    run_source: RUN_SOURCE,
    ...
    conviction_repairs: {
      book_veto_only: { pricing_rows: bookAuthorityRows.length, pricing_violations: bookAuthorityViolations.length, reseat_rows: bookReseatRows.length, reseat_violations: bookReseatViolations.length, executed_counterexamples: bookCounterexamples.length, all_counterexamples_pass: bookCounterexamples.every((row) => row.passed), receipt: "BOOK_VETO_ONLY_RECEIPT.json" },
      first_target_print_ordering: { rows: firstTargetPrintOrderingRows.length, all_chronological: firstTargetPrintOrderingRows.every((row) => row.chronological_first_is_serialized), maximum_old_overstatement_seconds: Math.max(0, ...firstTargetPrintOrderingRows.map((row) => row.old_largest_print_overstatement_seconds ?? 0)), receipt: "FIRST_TARGET_PRINT_ORDERING_RECEIPT.json" },
      twenty_rule_story_bar: { passed: storyQualityChecks.every((row) => row.passed), receipt: "STORY_QUALITY_RECEIPT.json" },
      prediction_seat_immunity: { receipt: "PREDICTION_SEAT_IMMUNITY_RECEIPT.json", routine_movers_proven_dead: moverCounterexamples.length, all_five_parent_movers_proven_dead: moverCounterexamples.length === 5 && moverCounterexamples.every((row) => row.passed), seated_runtime_rows: seatedRuntimeRows.length, seated_runtime_target_violations: seatedRuntimeRows.filter((row) => !row.seated_action_preserves_seat).length, runtime_exit_reasons: [...new Set(seatExitRows.map((row) => row.exit.reason))] },
      prediction_seat_own_conviction_reseat: { receipt: "PREDICTION_SEAT_RESEAT_RECEIPT.json", reseat_rows: reseatRows.length, current_conviction_lag_rows: currentConvictionLagRows.length, own_conviction_mover_executed: ownConvictionMoverCounterexample.disposition === "RESEAT_ON_OWN_CONVICTION_UPDATE", every_reseat_same_receipt: reseatRows.every((row) => row.prediction_seat_transition?.reseated_at_receipt === row.receipt) },
      prediction_seated_rests: { rows: predictionSeatRows.length, unique_licenses: predictionSeatUnique.length, named_tests: namedPredictionSeats, receipt: "PREDICTION_SEATED_RESTS_RECEIPT.json" },
      unified_aim_conduct_posterior: { all_aims_equal_conduct: predictionSeatRows.every((row) => row.aim_equals_conduct && row.aim_target_cents === row.conduct_target_cents), receipt: "UNIFIED_AIM_CONDUCT_POSTERIOR_RECEIPT.json" },
      live_deadline_honesty: { stale_or_zero_width_rows: deadlineRows.filter((row) => row.deadline_epoch <= row.emission_epoch || row.stale_modeled_deadline_clamped_to_emission).length, receipt: "LIVE_DEADLINE_HONESTY_RECEIPT.json" },
      dan_cross_category_cell: { consumed_rows: crossCategoryBorrowRows.length, dan_rows: crossCategoryCellRows.filter((row) => row.event_id === "KXATPMATCH-26JUL18DANPRA" && row.leg_id === "DAN").length, receipt: "DAN_LOW_GRADE_CROSS_CATEGORY_CELL_RECEIPT.json" },
      ...
      directional_rounding: { rows: directionalRoundingRows.length, violations: directionalRoundingViolations.length, named_example: "MODE_57_WHILE_MEAN_HALF_UP_58", receipt: "DIRECTIONAL_ROUNDING_RECEIPT.json" },
      acceptance_object: "FOUR_STORIES.md",
      disclosed_fill_model_limit: "Replay credits any true print at or below a standing rest without aggressor direction; the live-pilot touch census owns that boundary.",
    },
    ...
    directional_floor_admission: { giu_69_refused: giu69RefusedByDirection, giu_66_admitted: giu66AdmittedByDirection, receipt: "DIRECTIONAL_FLOOR_ADMISSION_RECEIPT.json" },
    derivable_floor_conduct_answers: derivableFloorConductRows.map((row) => ({ game: row.game, leg_id: row.leg_id, floor_cents: row.floor_cents, answer: row.conduct_answer, lawful_or_explained: row.lawful_or_explained })),
  ...
  const processMdFinal = processMd.replace("# Contracts build — authority restored + spread eye — process-first confirmation", "# Conviction seats the bid — process-first confirmation")
  ...
  const definitionProcessMd = processMdFinal
    ...
    + `\n\n## Four convictions — named repairs\n\n- Floor-decisiveness rows ${trueConditioningRows.length}; violations ${trueConditioningViolations.length}. Evidence grades whether a cent is the floor from descent, elimination support, and downside; it enters once. Self-echo inputs: zero.\n- Same-receipt postable changes ${sameReceiptActRows.filter((row) => row.derivation_and_postability_same_receipt).length}; missed same-receipt acts ${sameReceiptActViolations.length}; book-only postability acts ${postabilityActRows.length}.\n- Credited-leg continued-read rows ${creditedLegReadRows.length}; stopped-stream violations ${creditedLegReadViolations.length}.\n- Honest floor-statistic rows ${directionalRoundingRows.length}; violations ${directionalRoundingViolations.length}; executed counterexample mode 57 while mean-half-up 58.\n- Author rows: ${authorOwnEvidenceRows.length}; prior→conditioned→level sentence violations: ${authorOwnEvidenceViolations.length}.\n- Level-movement rows: ${movementRows.length}; pure-panel recompositions suppressed: ${movementRows.filter((row) => row.disposition === "PURE_PANEL_RECOMPOSITION_SUPPRESSED").length}; movement-law violations: ${movementViolations.length}.\n- Emitted PLACE/REPRICE orders: ${oneAuthorOrderRows.length}; independently recomputed authority divergences: ${oneAuthorDivergences.length}; raw-traded-low target defaults: zero; ask-minus-one price authors: zero.\n- At-floor full population ${fullImmunityPopulation.length}; immunity exits ${immunityExits.length}; exits without traced overturn ${untracedImmunityExits.length}; clock precision milliseconds.\n- Authority-vacuum rows: ${vacuumRows.length}; illegal vacuum emissions: ${vacuumEmissionRows.length}. Every vacuum resolved to HOLD, F-VS-068 own evidence, or INSUFFICIENT stand-down.\n- C04 current-authority restores: ${c04Rows.length}; post-span orders: ${postSpanOrderRows.length}; cancels without rearm: ${cancelWithoutRearmRows.length}.\n\n## Contracts, one tenure, and spread eye\n\n- Pricing authority: panel prior → once-only floor-decisiveness evidence → conditioned distribution → floor-side mode; lane-level substitutions ${authorityViolations.length}.\n- Contracts: ${firedContracts.length} fired/priced + ${latentContracts.length} latent registered; every decision carries the full register and every sentence states the active contract.\n- Envelope-high provenance: ${envelopeHighRows.length} rows; zero floored-mid producers.\n- Tenure producer: ORDER_TRANSITION_TENURE_SINGLE_PRODUCER; sampled-row/phantom producers: zero.\n- Spread eye: ${spreadEyeRows.length} readings; ${spreadEyeRows.filter((row) => row.authority_consumed).length} authority consumptions; zero direct lane commands.\n\n## Executable checks\n\n- Post-only candidate rows: ${postOnlyGateRows.length}; covered ${postOnlyGateRows.filter((row) => row.covered).length}; unlawful emitted orders ${postOnlyGateRows.filter((row) => row.emitted && !row.lawful).length}.\n- Emitted orders per decision instant ${JSON.stringify(orderArbitrationReceipt.orders_per_decision_instant_distribution)}; maximum ${orderArbitrationReceipt.max_orders_per_decision_instant}.\n- Floor rows tested: ${floorDefinitionRows.length}/${allDerivations.length}, including ${floorDefinitionRows.filter((row) => row.row_class === "NO_TRUE_TRADE_YET").length} no-trade rows; failures ${floorDefinitionFailures.length}.\n- Honest gate failures: ${gateFailures.map((check) => check.id).join(", ") || "NONE"}.\n`;
  const directionalProcessMd = definitionProcessMd.replace("# Conviction seats the bid — process-first confirmation", "# The book is veto-only — process-first confirmation") + `\n\n## The book is veto-only\n\n- Pricing rows ${bookAuthorityRows.length}; displayed-book evidence channels ${bookAuthorityRows.reduce((total, row) => total + row.book_evidence_channels.length, 0)}; authority violations ${bookAuthorityViolations.length}.\n- Conviction reseats ${bookReseatRows.length}; reseats without named non-book evidence ${bookReseatViolations.length}. The permitted causes are true prints, survivor/elimination changes, panel changes, and sibling credit.\n- Executed production counterexamples ${bookCounterexamples.filter((row) => row.passed).length}/${bookCounterexamples.length} passed. The ask veto never rewrote a target.\n- First-target-print timing uses chronological timestamp/receipt order. Maximum historical largest-size overstatement in this run: ${Math.max(0, ...firstTargetPrintOrderingRows.map((row) => row.old_largest_print_overstatement_seconds ?? 0))} seconds.\n- Existing resting liquidity equal to the ask remains standing; a new order remains strictly below the ask. This precedence plus prediction-seat immunity preserves the GIU seat.\n\n## The conviction moves its own seat\n\n- Production predicate: window1_v54_dual_belief_os.js::predictionSeatImmunityDecision. Parent routine movers proven dead by executed counterexample: ${moverCounterexamples.length}/5. The own-conviction mover disposition is ${ownConvictionMoverCounterexample.disposition}.\n- Seated runtime rows ${seatedRuntimeRows.length}; current-seat target violations ${seatedRuntimeRows.filter((row) => !row.seated_action_preserves_seat).length}; same-receipt reseats ${reseatRows.length}; live-conviction lag rows ${currentConvictionLagRows.length}; lawful exits ${seatExitRows.length}, reasons ${[...new Set(seatExitRows.map((row) => row.exit.reason))].join(", ") || "NONE"}.\n${namedPredictionSeats.map((row) => `- ${row.event_id}|${row.leg_id} ${row.target_cents}¢: ${row.earliest_license ? `seat license ${row.earliest_license.licensed_at_receipt}; action ${row.earliest_license.action.action} ${row.earliest_license.action.target_cents ?? "NONE"}¢; transition ${row.earliest_license.prediction_seat_transition?.transition ?? "HELD"}; aim=conduct=${row.earliest_license.aim_equals_conduct}` : "NO CURRENT-AIM SEAT"}.`).join("\n")}\n- Prediction-seat rows ${predictionSeatRows.length}; unique licenses/revisions ${predictionSeatUnique.length}; stale/zero-width deadlines ${deadlineRows.filter((row) => row.deadline_epoch <= row.emission_epoch || row.stale_modeled_deadline_clamped_to_emission).length}.\n- DAN low-grade cross-category consumptions ${crossCategoryBorrowRows.filter((row) => row.event_id === "KXATPMATCH-26JUL18DANPRA" && row.leg_id === "DAN").length}; DAN's actual aim/rest or disclosed gap is in DAN_LOW_GRADE_CROSS_CATEGORY_CELL_RECEIPT.json and the four-game story.\n\n## Directional admission and derivable-floor answers\n\n- GIU 69 admission: ${giu69RefusedByDirection ? "REFUSED" : "NOT_REFUSED"}; reason ${giu69AdmissionRow?.classification?.refusal_reason ?? "NO_ROW"}; surviving-path high ${giu69AdmissionRow?.classification?.highest_supported_floor_cents ?? "NONE"}¢; distance above path ${giu69AdmissionRow?.classification?.distance_above_descent_path_cents ?? "NONE"}¢.\n- GIU 66 admission: ${giu66AdmittedByDirection ? "ADMITTED" : "NOT_ADMITTED"}; exact support ${giu66AdmissionRow?.classification?.exact_shape_support ?? false}.\n${derivableFloorConductRows.map((row) => `- ${row.game}|${row.leg_id} ${row.floor_cents}¢: ${row.conduct_answer}; first receipt ${row.first_evaluation?.receipt ?? "NONE"}; entry ${row.terminal_leg_outcome?.entry_cents ?? "NONE"}; fill receipt ${row.terminal_leg_outcome?.fill_receipt ?? "NONE"}.`).join("\n")}\n\n## Disclosed replay limit\n\nThe standing replay credits any true print at or below the standing rest, priced at the rest, without an aggressor-direction field. F-VS-246 leaves that limitation for the live-pilot boundary and touch census; this repair does not alter crediting.\n`;
  ...
  writeJson(path.join(output, "PROCESS_FIRST_CONFIRM_RECEIPT.json"), processReceipt);
  writeText(path.join(output, "PROCESS_FIRST_CONFIRM.md"), directionalProcessMd);
  ...
  const priorFailureCustodyRoot = path.join(privateRoot, "stage1_store", "v54_floor_decisiveness_pre_score_failures_20260826");
  const custodyDigestAttempts = ["attempt_1", "attempt_2"].map((name) => derivableTreeDigest(path.join(priorFailureCustodyRoot, name)));
  writeJson(path.join(output, "CUSTODY_AGGREGATE_DIGEST_DERIVATION_FORMULA.json"), {
    label: "F_VS_234_CUSTODY_AGGREGATE_DIGEST_DERIVATION_FORMULA",
    law: "An aggregate custody digest is independently derivable from a canonical, sorted per-file path+sha256+bytes manifest. The prior undocumented aggregate strings are retained as historical references but carry no authority.",
    formula: "sha256(UTF8(JSON.stringify(SORTED_RELATIVE_PATH_SHA256_BYTES_ROWS, null, 2) + NEWLINE))",
    canonicalization: { path_separator: "/", sort: "RELATIVE_PATH_ASCENDING", json_indent_spaces: 2, terminal_newline: true, fields_in_order: ["path", "sha256", "bytes"] },
    historical_undocumented_digests: [
      { attempt: "attempt_1", sha256: "de5c28e2", status: "SUPERSEDED_UNDOCUMENTED_PREFIX_ONLY" },
      { attempt: "attempt_2", sha256: "4cd49c0c", status: "SUPERSEDED_UNDOCUMENTED_PREFIX_ONLY" },
    ],
    attempts: custodyDigestAttempts,
    every_present_attempt_derivable: custodyDigestAttempts.filter((row) => row.status !== "NOT_FOUND").every((row) => row.status === "DERIVABLE" && row.file_count > 0 && /^[0-9a-f]{64}$/.test(row.aggregate_sha256)),
    source: "F-VS-238@20a8a610",
  });
  ...
  writeJson(path.join(output, "RECEIPT_ONLY_REBUILD_IDENTITY.json"), {
    label: "RECEIPT_ONLY_GATE_ALIGNMENT_DID_NOT_CHANGE_SCORE_ARTIFACTS",
    reason: "The seated-rest exception was added to the continuous-post-only receipt check because the current dispatch explicitly makes every guard junior to seat immunity. Policy behavior and score artifacts were not edited.",
    baseline_hashes: RECEIPT_REBUILD_BASELINE_HASHES,
    rebuilt_hashes: Object.fromEntries(Object.keys(RECEIPT_REBUILD_BASELINE_HASHES).map((name) => [name, receipt(path.join(output, name)).sha256])),
    all_score_artifacts_byte_identical: Object.entries(RECEIPT_REBUILD_BASELINE_HASHES).every(([name, sha256]) => receipt(path.join(output, name)).sha256 === sha256),
  });
