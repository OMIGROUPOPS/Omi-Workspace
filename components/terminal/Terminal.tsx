...
// OMNI Terminal — Main grid layout
// Bloomberg-style terminal with live scanner panels and mock data.
...
const MOCK_WATCHLIST: WatchlistItem[] = [
  ...
  { ticker: "KXNBA-LAL-SP-Y", info: mkInfo("KXNBA-LAL-SP-Y", "LAL Lakers -3.5", "nba-lal-gsw", "spread"), best_bid: 53, best_ask: 56, mid: 55, spread: 3, bid_size: 120, ask_size: 95, move_30s: 2, kyle_lambda: 0.0065 },
  { ticker: "KXNBA-GSW-SP-Y", info: mkInfo("KXNBA-GSW-SP-Y", "GSW Warriors +3.5", "nba-lal-gsw", "spread"), best_bid: 44, best_ask: 47, mid: 45, spread: 3, bid_size: 95, ask_size: 120, move_30s: -2, kyle_lambda: 0.0065 },
  ...
  { ticker: "KXMLB-NYM-TOT-O", info: mkInfo("KXMLB-NYM-TOT-O", "NYM o8.5 Runs", "mlb-nym-phi", "total"), best_bid: 49, best_ask: 53, mid: 51, spread: 4, bid_size: 60, ask_size: 45, move_30s: 1, kyle_lambda: 0.0120 },
  { ticker: "KXNHL-CHI-ML-Y", info: mkInfo("KXNHL-CHI-ML-Y", "CHI Blackhawks", "nhl-chi-det", "moneyline"), best_bid: 65, best_ask: 69, mid: 67, spread: 4, bid_size: 80, ask_size: 70, move_30s: 2, kyle_lambda: 0.0095 },
];

const MOCK_SIGNALS: ScanSignal[] = [
  { scan_type: "momentum_lag", severity: "HIGH", ticker: "KXMLB-NYY-ML-Y", game_id: "mlb-nyy-bos", entry_side: "buy_yes", entry_price: 63, target: 68, stop: 58, description: "ML moved +5c (\u0394logit=+0.21) but spread only +2c (\u0394logit=+0.08)", depth: 180, timestamp: Date.now() - 12000 },
  ...
  { scan_type: "whale_momentum", severity: "MEDIUM", ticker: "KXCRYPTO-BTC40-Y", game_id: "crypto-btc-40k", entry_side: "buy_yes", entry_price: 73, target: 78, stop: 68, description: "Whale buy 500ct BTC>40K YES@72 taker=yes", depth: 650, timestamp: Date.now() - 25000 },
  ...
  { scan_type: "momentum_lag", severity: "MEDIUM", ticker: "KXNHL-CHI-ML-Y", game_id: "nhl-chi-det", entry_side: "buy_yes", entry_price: 69, target: 74, stop: 64, description: "Spread moved +3c (\u0394logit=+0.12) ML lagging +1c (\u0394logit=+0.04)", depth: 70, timestamp: Date.now() - 60000 },
...
export default function Terminal() {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div className="flex items-center justify-between px-4 h-8 bg-[#111] border-b border-[#222] shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span
            style={{ color: "#555", fontSize: "10px", letterSpacing: "0.15em" }}
          >
            EXCHANGE TERMINAL
          </span>
        ...
        <div className="flex items-center gap-4" style={{ fontSize: "10px" }}>
          ...
          <span style={{ color: "#555" }} suppressHydrationWarning>
            {clock}
          </span>
      ...
      {/* Main grid */}
      <div className="flex-1 grid grid-cols-12 grid-rows-6 gap-px bg-[#1a1a1a] overflow-hidden min-h-0">
        {/* Watchlist — left column */}
        <div className="col-span-2 row-span-6 bg-[#0d0d0d] p-2 min-h-0 overflow-hidden">
          <Watchlist
        ...
        {/* Chart — center top */}
        <div className="col-span-7 row-span-3 bg-[#0d0d0d] p-2 min-h-0 overflow-hidden">
          <Chart ticker={selectedTicker ?? undefined} />
        </div>

        {/* Orderbook — right top */}
        <div className="col-span-3 row-span-3 bg-[#0d0d0d] p-2 min-h-0 overflow-hidden">
          <Orderbook ticker={selectedTicker ?? undefined} />
        </div>

        {/* Scanner — bottom left */}
        <div className="col-span-4 row-span-3 bg-[#0d0d0d] p-2 min-h-0 overflow-hidden">
          <Scanner
