...
## SHADOW-ARM ADDENDUM (Plex source ruling, 2026-07-06)
**This document is also the OUTCOME_PROOF for armed state `c2a59a62`** — the Plex-ratified SHADOW arm
(`per_match_clock_shadow` + `scale_gun_shadow`, both together, nothing else bundled). Two-lane statement:
observe-only by construction — trading behavior is byte-identical (the staged build's proven OFF/ON identity
covers every decision path; the flags add log lines only), so BOTH lanes are unchanged by definition. The
shadow's purpose per the ruling: prove WIRING, not facts. Graduation + rollback: `SHADOW_GRADUATION.md`.
...
|---|---|---|---|
| **riser_post revision** | **WIN** — 89/107 riser fills retained (83%) at 2–3¢ better Δaim across the FULL slate, matching the pre-registered N=69 curves; pair-completion structure held (18 lost fills flagged per-row) | **+$6.50** (n=107 legs) | **WINS BOTH → DEPLOYS (`4d7b065c`)** |
