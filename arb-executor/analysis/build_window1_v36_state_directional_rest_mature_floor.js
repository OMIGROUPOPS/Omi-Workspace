...
const policy = require("./window1_v36_state_directional_rest_mature_floor.js");
...
const output = path.resolve(arg("--output", path.join(repo, ".claude/window1_live_v4_replay/v36_state_directional_rest_mature_floor_20260806")));
...
const policyFile = path.join(repo, "arb-executor/analysis/window1_v36_state_directional_rest_mature_floor.js");
const unitTestFile = path.join(repo, "arb-executor/tests/test_window1_v36_state_directional_rest_mature_floor.js");
const packageTestFile = path.join(repo, "arb-executor/tests/test_window1_v36_state_directional_rest_mature_floor_package.js");
...
function safeClean(dir) { const r = path.resolve(dir); ensure(path.basename(r).toLowerCase().includes("v36"), `unsafe output ${r}`); ensure(r !== repo && r !== path.parse(r).root, `unsafe output ${r}`); fs.rmSync(r, { recursive: true, force: true }); fs.mkdirSync(r, { recursive: true }); }
...
function simulateW1Mode(baseEvent, tapes, prints, sources, bells, mode, actions, decisions, span, timeline, boundary) {
  ...
  for (const row of timeline) {
    ...
    const decision = policy.decide({ state: combined.state, book: row, activeTarget: before, pairCap: leg.pair_cap_cents, activeEvidenceFloor, floorFirstFlickerLive, floorMature });
    leg.prior_book = row;
...
async function causalMain() {
  ...
  for (const commit of [V35_COMMIT, V34_W1_CAUSAL_COMMIT, V34_FULL_LIFE_COMMIT, V34_TRADING_PHASE_COMMIT, V32_COMMIT, R3_COMMIT, RECON_COMMIT, NEARMISS_COMMIT, START_LEDGER_COMMIT, OFFER_COMMIT, BLEED_COMMIT, EXEMPLAR_COMMIT, AUTOPSY_COMMIT]) requireCommit(commit);
  safeClean(output);
  const baseEvents = loadBaseEvents(); ensure(baseEvents.length === 804, "D must be 804");
  ...
  process.stderr.write("V36_STAGE print_spool_start\n");
  const printLoad = buildPrintSpool(tickers);
  process.stderr.write("V36_STAGE print_spool_complete\n");
  ...
  for (const baseEvent of baseEvents) {
    ...
    if (strictEvents.length % 50 === 0) process.stderr.write(`V36_STAGE replay_events_${strictEvents.length}\n`);
  }
  printLoad.cleanup();
  ...
  const binding = { schema_version: "window1-v36-state-directional-rest-mature-floor-control-v1", machine: { architecture: "V35_WITH_STATE_DIRECTIONAL_REST_AND_MATURE_FLOOR_TAKE", V35_baseline_commit: V35_COMMIT, V34_W1_baseline_commit: V34_W1_CAUSAL_COMMIT, V32_lineage_commit: V32_COMMIT, policy_path: path.relative(repo, policyFile).replaceAll("\\", "/"), policy_sha256: fileHash(policyFile), clock_decision_inputs: [], state_transitions_and_maturity_evaluated_only_on_receipts: true }, unchanged_from_V35: { strict_fill_law: "UNCHANGED", evidence_gate: "UNCHANGED_EXCEPT_TAKE_AUTHORITY_REQUIRES_CAUSAL_FLOOR_MATURITY", pair_cap: "UNCHANGED", hard_pre_bell_edge: "UNCHANGED", close_free_grading: "UNCHANGED" }, repair: { FALLING_rest: "V32_NO_CHASE_MIN(PRIOR_REST,BEST_BID_MINUS_1,PAIR_CAP); NEVER_TRACK_UP_IN_LIVE_DECLINE", RISING_or_SETTLED_rest: "V35_LIVING_REST_MIN(BEST_BID_MINUS_1,PAIR_CAP)_TRACK_UP_AND_DOWN_BYTE_SEMANTIC_UNCHANGED", mature_floor_take: `NO_NEW_LOW_RECEIPT_INSIDE_EXISTING_${policy.LOOKBACK_SECONDS}S_TRAILING_HORIZON; MATURITY_REASKED_ONLY_ON_BOOK_RECEIPTS; NEW_LOW_RESETS; NO_TIMER_NO_WALL_CLOCK_TRIGGER`, reformed_floor_authority: "A_NONFALLING_QUALIFYING_ASK_FLOOR_MAY_REPLACE_AN_OLDER_MINIMUM_ONLY_AFTER_MATURITY; A DESCENT_ORIGIN_SHELF_CANNOT RELABEL ITSELF" }, window_law: { left: "FIRST_TWO_SIDED_BOOK_ON_EITHER_EXPRESSION", right_precedence: ["exact_start_utc", "known_live_by_utc", "schedule_bound_utc"], hard_edge: "NO_REST_WALK_NO_TAKE_NO_FILL_NO_CAP_ARM_NO_STATE_UPDATE_AFTER_EDGE", boundary_is_reporting_and_admission_only_not_AIM_input: true }, close_law: { role: "TELEMETRY_ONLY", grading_reads: 0, settlement_close_reads: 0 }, comparisons: { V35_strict_completed_under_par: 264, V35_census_completed_under_par: 550, V34_W1_strict_completed_under_par: 254, V34_W1_census_completed_under_par: 279, R3_original_joint_reference_only: 68, R3_same_V3_window_artifact: "R3_SAME_WINDOW_COMPARISON.json", operator_named_under_par_offer_census_adjusted: 680 } };
  ...
  const acceptanceChecks = {
    ...
    strict_LE93_recovers_to_V34_23: strictFrontier.cumulative_frontier.LE_93.completed_pairs >= 23,
    strict_LE95_recovers_to_V34_34: strictFrontier.cumulative_frontier.LE_95.completed_pairs >= 34,
    strict_LE97_preserves_V34_68: strictFrontier.cumulative_frontier.LE_97.completed_pairs >= 68,
    named_regressions_all_pass: Object.values(namedRegressionChecks).every(Boolean),
  };
  const acceptance = {
    status: Object.values(acceptanceChecks).every(Boolean) ? "PASS_V36_BAR" : "REJECTED_V36_BAR_V35_REMAINS_OPERATIVE",
    checks: acceptanceChecks,
    named_regression_checks: namedRegressionChecks,
    required_floor: { strict_completed_pairs: 264, strict_frontier: { LE_93: 23, LE_95: 34, LE_97: 68 }, rule: "COMPLETION_BAR_AND_DEEP_FRONTIER_BAR_MUST_BOTH_PASS" },
  ...
  const core = {
    ...
    "CENSUS_PRICED_65D49B5D_BINDING.json": canonical({ commit: NEARMISS_COMMIT, path: NEARMISS_PATH, source_sha256: sha(nearBytes), source: { waited_and_lost: near.summary.conservation.waited_and_lost_censused, one_cent_near_miss_rests: near.summary.totals.with1, v32_joint: near.summary.conservation.V32_executable_joint, model_free_ceiling: near.summary.conservation.model_free_ceiling_joint }, V36_application: "DYNAMIC_STATE_DIRECTIONAL_REST_TRAJECTORY_WITH_MATURE_FLOOR_TAKE_AUTHORITY_INSIDE_V3_PRE_MATCH_SPAN_ONLY; FIRST_LAWFUL_ONE_CENT_SELLER_PRINT_CONVERTS_AT_CURRENT_REST_IN_CENSUS_COLUMN" }),
