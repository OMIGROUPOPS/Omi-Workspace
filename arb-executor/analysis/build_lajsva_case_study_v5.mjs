...
const REPAIR_ROOT = ".claude/window1_live_v4_replay/v54_repair_iteration4_composition_presence_20260822";
const OUT_ROOT = ".claude/window1_live_v4_replay/lajsva_case_study_v5_20260822";
const V4_ROOT = ".claude/window1_live_v4_replay/lajsva_case_study_v4_20260822";
...
function main() {
  ...
  source = source
    ...
    .replaceAll("V1_V2_V3_SIDE_BY_SIDE.md", "V1_V2_V3_V4_V5_SIDE_BY_SIDE.md")
    .replaceAll("repair iteration 2", "repair iteration 4")
    .replaceAll("LAJSVA v3", "LAJSVA v5")
    .replaceAll("v3 repaired machine", "v5 repaired machine")
    .replaceAll("FOUNDATION_CONDITIONAL_DIP_EARLY_RISER", "COMPOSITION_PRESENCE")
    .replaceAll("LAJSVA_CASE_STUDY_V3", "LAJSVA_CASE_STUDY_V5")
    .replaceAll("V3 receipt", "V5 receipt");
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "lajsva-v5-"));
  ...
  const v4ReceiptFile = path.join(repo, V4_ROOT, "CASE_STUDY_RECEIPT.json");
  const v4 = JSON.parse(fs.readFileSync(v4ReceiptFile, "utf8"));
  const result = caseReceipt.v1_v2_v3?.v3 ?? null;
  const prior = v4.v1_v2_v3_v4?.v4 ?? null;
  const outcome = (value) => value?.completed ? `${value.pair_cents}, Δ${value.delta_cents}` : "partial";
  const spine = `# LAJSVA case-study spine — v1 / v2 / v3 / v4 / v5
...
| v4 | strict pre-bell minutes; subtractive graded retrieval; inert split | ${outcome(prior)} | self-stopped |
| v5 | absolute-floor composition; fresh per-receipt split; touch-presence sentences | ${outcome(result)} | ${gate.self_stop ? `self-stop: ${gate.stop_reason}` : "gate pass"} |

V4 receipt: ${V4_ROOT}/CASE_STUDY_RECEIPT.json@sha256:${hash(fs.readFileSync(v4ReceiptFile))}

V5 gate: ${REPAIR_ROOT}/REPAIR_GATE_RECEIPT.json@sha256:${hash(fs.readFileSync(gateFile))}
`;
  fs.writeFileSync(path.join(out, "V1_V2_V3_V4_V5_SIDE_BY_SIDE.md"), spine, "utf8");
  caseReceipt.label = "LAJSVA_CASE_STUDY_V5_COMPOSITION_PRESENCE";
  caseReceipt.v1_v2_v3_v4_v5 = { v1: "COMPLETE_94_DELTA_6_BROKEN_REASONING", v2: "PARTIAL", v3: "PARTIAL", v4: prior, v5: result };
  ...
  const manifest = { label: "LAJSVA_CASE_STUDY_V5_MANIFEST", files: rows, all_under_50_mb: rows.every((row) => row.bytes <= 50 * 1024 * 1024) };
  ...
  if (expected && expected !== manifestHash) throw new Error(`CASE_STUDY_V5_DETERMINISM_FAILED ${expected} != ${manifestHash}`);
  if (expected) {
    fs.writeFileSync(determinismFile, canonical({ label: "LAJSVA_CASE_STUDY_V5_DETERMINISM_X2", two_clean_builds: true, first_manifest_sha256: expected, second_manifest_sha256: manifestHash, byte_identical: true }), "utf8");
    const finalRows = [...rows, receipt(repo, determinismFile)];
    fs.writeFileSync(path.join(out, "ARTIFACT_HASH_MANIFEST.json"), canonical({ label: "LAJSVA_CASE_STUDY_V5_MANIFEST", files: finalRows, all_under_50_mb: finalRows.every((row) => row.bytes <= 50 * 1024 * 1024) }), "utf8");
  }
