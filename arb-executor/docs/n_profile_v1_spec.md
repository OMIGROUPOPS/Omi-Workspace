...
**44 columns total** (43 originally enumerated + n_minutes_observed added at this commit). The earlier v0.1 header said "44" while the table enumerated 43 (a known off-by-one). This is now reconciled correctly: n_minutes_observed is the 44th column, earning its slot by serving gate 3 (§4.1 RHS) and the §4.2 observation-coverage ratio — not a phantom filler. Single row per N. Estimated emitted row count post-dropouts: ~17,000-19,000 (final number determined by Phase 1 probe; dropouts include missing trade tape, missing match_start_ts inference, partner-ticker resolution failures).
