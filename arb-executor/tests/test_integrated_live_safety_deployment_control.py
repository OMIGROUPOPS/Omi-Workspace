#!/usr/bin/env python3
"""Focused and adversarial tests for integrated live-safety deployment-control V1."""
...
import integrated_live_safety_authorization_verifier_v1 as verifier  # noqa: E402
import integrated_live_safety_ceremony_v1 as ceremony  # noqa: E402
import build_integrated_live_safety_prerun as builder  # noqa: E402
    ...
    def __init__(self) -> None:
        ...
        self.control = {
            ...
            "containment": {
                ...
                "installed_inhibited_crontab_sha256": "2" * 64,
                "installed_crontab_path": "/var/spool/cron/crontabs/root",
                "original_crontab_backup": "/root/original.cron",
                "original_crontab_backup_sha256": "3" * 64,
                "tennis_entry_buys": 0,
                "cron_restoration": "after all postchecks pass",
                "failure_state": "stopped and inhibited",
            },
            "outcome_proof": {
        ...
        run(
            ...
            "git",
            "update-ref",
            "refs/remotes/origin/codex/integrated-p0v4-casuka-prerun",
            self.package,
        )
        run(
            self.repo,
        ...
        self.control["audit"]["p0_v4_pass"] = self.v1_pass
        self.control["audit"]["casuka_d1_d3_pass"] = self.v1_pass
        self.control["audit"]["casuka_repair"] = self.v1_pass
        ...
        audit_receipt = {
            ...
            "p0_v4_pass": self.v1_pass,
            "casuka_d1_d3_pass": self.v1_pass,
            "integration_commit": self.integration,
            "rollback_commit": self.rollback,
        ...
        current["audit"]["p0_v4_pass"] = self.v1_pass
        current["audit"]["casuka_d1_d3_pass"] = self.v1_pass
        current["audit"]["casuka_repair"] = self.v1_pass
        ...
        remote_state = {
            "schema_version": "integrated-live-safety-remote-preflight-state-v1",
            "host": "root@example.test",
            "remote_repo": "/remote/repo",
            "repo_head": self.base,
            "repo_branch": "main",
            ...
            "process_count": 0,
            "process_rows": [],
            "installed_crontab_sha256": "2" * 64,
            "installed_crontab_path": "/var/spool/cron/crontabs/root",
            "original_crontab_backup_path": "/root/original.cron",
            "original_crontab_backup_exists": True,
            "original_crontab_backup_sha256": "3" * 64,
        }
...
class RunnerAndBuilderTests(unittest.TestCase):
    def test_stopped_containment_and_parked_rollback_are_frozen(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / "package"
            builder.build(ROOT, output)
            control = json.loads(
                (output / "DEPLOYMENT_CONTROL_V1.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertEqual(0, control["containment"]["engine_process_count"])
            self.assertEqual(
                builder.INSTALLED_CRON_SHA256,
                control["containment"]["installed_inhibited_crontab_sha256"],
            )
            contract = json.loads(
                (output / "PRE_DEPLOYMENT_OUTCOME_PROOF_CONTRACT.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertEqual(0, contract["rollback"]["authorized_rollback_restarts"])
            self.assertEqual(
                "FORBIDDEN", contract["rollback"]["cron_restoration_on_failure"]
            )

    def test_rollback_script_never_boots_unsafe_preimage(self) -> None:
        source = (
            ROOT
            / "arb-executor/deploy/integrated_live_safety_rollback_v1.sh"
        ).read_text(encoding="utf-8")
        self.assertNotIn("tmux new-session", source)
        self.assertNotIn("BOOT_CMD", source)
        self.assertIn("engine parked; cron inhibited", source)

    def test_cron_restoration_is_after_postcheck_collection(self) -> None:
        source = (
            ROOT
            / "arb-executor/deploy/integrated_live_safety_ceremony_v1.py"
        ).read_text(encoding="utf-8")
        evidence = source.index("post = collect_post_deployment_evidence(")
        restoration = source.index(
            "cron_restoration = restore_original_crontab_after_pass("
        )
        self.assertLess(evidence, restoration)
