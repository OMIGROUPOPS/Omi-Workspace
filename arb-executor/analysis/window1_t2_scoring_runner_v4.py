#!/usr/bin/env python3
"""Stdout-safe runner for the transitive-boundary-repaired T2 package V4."""
...
from window1_t2_scoring_runtime_v4 import (
    ...
    mark_candidate_completed,
    no_score_seam_probe,
)


VERSION = "window1-t2-scoring-runner-v4"
    ...
    *,
    ...
    for role in (
        ...
        if role not in package["roles"]:
            raise RunnerError(f"V4 role missing: {role}")
...
def authorize_execute(
    ...
    if audit_commit in {
        ...
        raise RunnerError("consumed V2/V3 authorization is rejected by V4")
    ...
    if git(repo, "rev-parse", "HEAD") != git(
        repo, "rev-parse",
        "refs/remotes/origin/codex/window1-t2-scoring-package-v4-prerun",
    ):
...
def validation_only(repo: Path, package_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": VERSION + "-validation-only-v4",
...
def execution_readiness_no_score(
    ...
    """Exercise the exact shared execute arguments without scoring."""
    validated = validate_package(
        repo, package_path, verify_private_inputs=True
    )
    seam = no_score_seam_probe(
        repo=repo,
        roles=validated["package"]["roles"],
...
def execute(
    ...
    def persist_accounting(current: ScoreAccounting) -> None:
        snapshot = current.to_dict()
        start_receipt.update(snapshot)
        start_receipt["scorer_invocations"] = (
            current.scorer_call_attempts
        )
        write_json(
    ...
    try:
        ...
        prepared_calls = list(iter_prepared_scorer_calls(
            repo=repo,
            roles=roles,
            candidate_ids=CANDIDATE_IDS,
            candidate_to_parent=CANDIDATE_TO_PARENT,
        ))
        if len(prepared_calls) != 6_432:
            raise RunnerError("prepared scorer calls do not conserve")
        prepared_by_candidate = {
            candidate: [
                call for call in prepared_calls
                if call.candidate_id == candidate
            ]
            for candidate in CANDIDATE_IDS
        }
        progress.emit("prepared_raw_V5_scorer_calls=6432")
        summaries = []
        for ordinal, candidate in enumerate(CANDIDATE_IDS, 1):
            ...
            for prepared in prepared_by_candidate[candidate]:
                event = prepared.event
                event_id = prepared.event_id
                fill_map = prepared.fills_by_leg
                row = invoke_prepared_score(
                    prepared=prepared,
                    scorer=score_t2_event,
                    accounting=accounting,
                    persist=persist_accounting,
                )
                leg_regret = []
                for leg_row in row["legs"]:
                    leg_id = str(leg_row["leg_id"])
                    chain = prepared.regret_chains_by_leg[leg_id]
                    ...
                    reference = prepared.references_by_leg[leg_id]
        ...
        write_json(result_dir / "EXECUTION_MANIFEST.json", {
            **start_receipt,
            ...
            "exit_code": 0,
            "scorer_invocations": accounting.scorer_call_attempts,
            "scorer_call_attempts": accounting.scorer_call_attempts,
            "completed_event_rows": accounting.completed_event_rows,
            "completed_candidates": accounting.completed_candidates,
        ...
        write_json(result_dir / "EXECUTION_FAILURE.json", {
            ...
            "completed_event_rows": accounting.completed_event_rows,
            "completed_candidates": accounting.completed_candidates,
            "active_candidate_id": accounting.active_candidate_id,
            "active_event_id": accounting.active_event_id,
        })
        write_json(
            result_dir / "OUTPUT_HASH_MANIFEST.json",
            _output_hashes(result_dir),
