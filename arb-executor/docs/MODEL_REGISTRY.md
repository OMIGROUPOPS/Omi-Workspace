...
**DRIFT ATLAS (07-15, C-TREND-PATH v1):** ATLAS_V1 (.claude/trendpath/), 30 pages: W1 path quantiles per slice + bottom excursion per cat x side x price-cell; tour g9<Jul10 walk-forward, ITF live-era BRANDED. Week grade at path prices x reach law: +29.65 over actual, 9.0% yield incl. pessimistic; N=285 underpowered; shadow live (trendpath_shadow), cutover on forward n>=300 CI-off-zero + operator word.

**CONTENTION SCORE (07-15, C-CONTENTION-LAW v1):** per atlas page (ATLAS_V1 contention blocks): achievable band swing at the path entry vs ride-loss weighted by ENTERED-legs win partition; band-touch exit convention; reach-law entry-fill annotation; recomputed 12:05a as pages harden; NO-OPINION on thin tiers. Selector verdicts live (shadow) in trendpath_shadow; nightly SELECTOR section. Week grade: 22.6%/23.0% yield vs 8% bar, N=176 underpowered; cutover = forward n>=300 CI-off-zero + operator word.

**ORIENT_V1 (07-15, C-PAIR-LAW AMENDMENT):** directional tells per event pair (f_drift/f_range/f_flow on the discovery hour; spread asymmetry SILENT in corpus, named), walk-forward (tour<Jul10; ITF live-era), refits nightly 12:05a; operating point n>=10 rate 0.65/0.35 (v1, named). Week grade: 8% coverage, 62% of calls correct, n=8/300 -- SHADOW, own clock before money. 15 callable buckets/6 cats.

**ENTRY DOSSIER (07-14, C-VAULT-WIRED-ENTRY v1):** every placement/refusal logs the ten-surface consultation (atlas page, contention, pair state, reach-law p_fill at aim, M15 range cell [ENTRY-band mapping = NAMED GAP/intake], dip timing, flow state, refuse margins, fill regime, honest clock) + SHADOW aim variants (range-shape slice, timing delay). Census nightly; GAPs auto-board. A silently-ignored registry surface is a named violation.
