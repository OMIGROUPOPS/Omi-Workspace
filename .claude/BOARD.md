...
- **C-CONCEPTION-HORIZON (T-8h early bound, INTERIM SCAFFOLDING — `6d9ebe31`, deployed 07-08 afternoon, `f41330f` PID 1456032)** — no conception/resting entry bid beyond T-8h honest-anchored; router-defer (18 live fires hour 1) + manage-sweep + chokepoint-refusal; census swept **11/11 by direct API** (GILOBR exhibit — survivors were manual-class restart orphans on LYING kalshi_primary clocks, invisible to the in-code sweep; see vault amendment). **The bound is a map edge, NEVER a target; it moves on the early-canvas study + operator ruling.** Watches: `conception_beyond_horizon` audit flag (must stay 0 — caveat: blind where the clock lies short), `conception_horizon_refused`, swept pairs re-conceiving at T-8h. **Aftermath addendum `b0661bc2`: failed walk-recovery now frees the leg (3 starved legs 14:23 healed at the deploy restart).** **Residuals routed: lying-clock/pm-honest-matcher miss → queued #11 + kalshi-occ lane (GILOBR the exhibit); restart-orphans-read-as-manual → BOT_DOWN/audit-twin lane.**
- **EARLY-CANVAS STUDY (Part 2, running detached on the VPS)** — floor-by-hour extended T-8h→T-24h: coverage-first per (cat, hour-bin); lattice vs prints; fillability both conventions; drift information content; anti-selection on early prints; per-game FLOW-STATE transitions fitted from the early-floor population (distributions, not medians). Verdict per cat = does anything before T-8h ever justify a resting bid; findings only — the horizon moves on operator ruling.
