...
of the 39 V13 interim leg hypotheses, hard `n >= 30`, explicit structural
parent borrowing, no silent interpolation, and abstention to frozen V11. An
unusable pair couple never vetoes the single-leg path. FRONTIER and REGRET
GAUGE print together. This lane remains development-only and in-sample.
...
https://raw.githubusercontent.com/OMIGROUPOPS/Omi-Workspace/refs/heads/codex/window1-live-consolidated/.claude/window1_live_v4_replay/pair_couple_abstention_v19_20260803/PRE_RUN_REPORT.md

Every operator-facing reply in this lane ends with a `VAULTED:` line naming
the committed addendum or explicitly stating no new vault mutation was made.
...
**SESSION-ZERO READ: `truth/INDEX.json` FIRST** — laws (309) · surfaces (7) · sites (6) · studies (61). SECOND READ: truth/OPEN_LEDGER.md (every open thread; gate-checked every C50). Everything below is a pointer INTO the index's members:
...
## STATE (2026-07-20 CLOSE — LOOP 9 EXIT (b) — C50 keeps this current):
LAP 1 DONE, EXIT (b) TAKEN per the stopping law (66b781d2): census
= sealed p90 completes 0% whole-market (came-shallower 503 legs,
med gap 4c; never-came 222); drill best 11%x29% fit / 7%x11%
holdout; CEILING dense 19% (5.0 g/d; mirror 0/8), pool 34%; perfect
capture misses par by 1c median; bound curve 97->19 / 98->30 /
saturates ~31. NAMED: the 97->98 cent = +11pp (pair-97 law's cent,
OPERATOR'S DESK); ~40pp = both-leg divots never arrive (no divot
policy reaches it); WIDENING WORK = 13.7 g/d -> book-led join +
park frames next drill. Mirror law lap-1: link 2/18, receipt n=0
said. Gate #4 live (reschedule law, boot fde5664b PID 413245, 11s
stop, audit PASS). Overnight: nightly 01:10 (joint+tripwire) ·
retest 02:00 · wall 02:50. All four capture parts CLOSED.

(prior:) ## STATE (2026-07-20 LATE — CAPTURE STANDARD ARC — C50 keeps this current):
...
## V35.1 DIRECTIONAL EVIDENCE AGING - REJECTED - 2026-08-06

V35.1 fixed ARNROM locally at 38+56=94, while retaining KRALOR|LOR 5,
BOSCOP|BOS 32, and ARNROM|ROM 38. It is rejected at population scale:
STRICT completions rose 264->283, but the deep frontier regressed from
...
## FRESH HOLDOUT EXAM STOPPED AT SEAL - 2026-08-06

The read-only post-July-26 census found 172 paired big-4 events. Git history
excludes 171 as previously touched. Only `KXATPMATCH-26JUL28MICMCD` is sealed,
so `N=1 < 60`; V36, V35, and R3 were not run. There are zero runner
