...
def transaction_fee_cents(price_cents: int, quantity: int) -> int:
    """Kalshi fee: ceil(7 * quantity * p * (1-p)) cents, p=price/100."""
...
def control_reconciliation(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "population": len(rows),
        "fee_law": {
            "formula": "ceil(7 * quantity * p * (1-p)) cents per transaction",
...
def render_markdown(result: dict[str, Any], ledger: Path) -> str:
    ...
    lines = [
        ...
        + " |",
        "| full-tape five-contract opportunity, actual fee curve | "
