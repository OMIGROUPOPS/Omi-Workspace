...
<script>
  ...
  function posRestingRow(l, sched_ep){
    const bid = l.exit_resting ? l.exit_resting.price_c : (l.aim_c || null);
    const tx = fmtTx(txMin(sched_ep, l.placed_ep));
    const gap = (l.l1 && bid !== null) ? (l.l1.bid - bid) : null;
    return '<div class="legrow orders-shape">' +
      '<span class="player">' + esc(l.last_name) + '</span>' +
      '<span class="num">' + esc(l.qty) + '</span>' +
      '<span class="num">' + cents(bid) + '</span>' +
      '<span class="grey">' + (l.placed_et ? esc(l.placed_et).replace(' ET','') : mdash('POSTED')) + '</span>' +
      '<span class="grey">' + (tx || mdash('TX')) + '</span>' +
      '<span class="num">' + (l.l1 ? l.l1.bid + '/' + l.l1.ask : mdash('BEST')) + '</span>' +
      '<span>' + (gap === null ? mdash('GAP') : signedCents(gap)) + '</span>' +
      '</div>';
  }
  ...
  async function loadPositions(){
    ...
    body.innerHTML = games.map(g => {
      ...
      if(bothFilled){
        ...
        rows = (g.legs || []).map(l => posRestingRow(l, sched_ep)).join('');
      }
  ...
  function ordRow(l, sched_ep){
    // [reconciliation 07-17] the unshipped slots are SHIPPED: POSTED =
    // orders_ledger created_time; T−X from its raw epoch; BEST/GAP =
    // fresh L1 (30s TTL).
    const tx = fmtTx(txMin(sched_ep, l.placed_ep));
    const gap = (l.l1 && l.aim_c !== null && l.aim_c !== undefined)
      ? (l.l1.bid - l.aim_c) : null;
    return '<div class="legrow orders-shape">' +
      '<span class="player">' + esc(l.last_name) + '</span>' +
      '<span class="num">' + esc(l.qty) + '</span>' +
  ...
  async function loadOrders(){
    ...
    body.innerHTML = games.map(g => {
      const legs = (g.legs || []).map(l => ordRow(l, (g.anchors||{}).sched_ep)).join('');
