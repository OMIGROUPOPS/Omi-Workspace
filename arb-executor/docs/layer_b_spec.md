...
A second threshold applies at the per-cell-trajectory level: each cell must have at least 50 simulated trajectories from sampled tickers with valid forward windows. **Definition: a "trajectory" is a (ticker, entry_moment) tuple, not a ticker.** A single sampled ticker contributes one trajectory per minute in its candle history that matches the cell's entry_band, regime, spread_band, volume_intensity, and category — typically tens to hundreds of trajectories per ticker for active markets. The 50-trajectory threshold therefore operates at moment-granularity, not ticker-granularity. The sample_manifest.json ticker cap (SAMPLE_PER_CELL=30 from Layer A v1) does not bound the trajectory count: 30 sampled tickers × ~100 cell-matching minutes each = ~3000 trajectories per cell on average. **Threshold-check is deferred to T31b-beta scope** (single-cell test mode) because computing the actual trajectory count requires reading g9_candles per ticker and applying cell-key classification at moment-granularity — which the dry-run mode (T31b-alpha) deliberately avoids per LESSONS C28 streaming discipline. Cells where the threshold is not met after T31b-beta moment-counting will be excluded from the T31b-gamma full sweep and flagged in the producer log. This handles the case where a "substantial" cell has 25 markets but only sparse candle coverage with few minute-level moments matching the cell key.
...
1. **Capture distributions are bounded by physical limits, with minute-cadence overshoot expected on the fire-side.** For a +Xc limit policy, capture_p10 across **fired** trajectories should be ≥ +Xc (once a policy fires, its realized capture is at least the threshold). Right-tail overshoot above +Xc is expected and not a failure: when bid moves from below entry+Xc at minute t-1 to above entry+Xc at minute t, the realized capture is the minute-t closing bid (which can substantially exceed +Xc on volatile minutes). Across all trajectories (fired + horizon_expired + settled_unfired), capture_p10 should be ≥ -100c (basic sanity floor — no contract pays less than -$1.00 per share). The original "capture_p90 should not exceed +Xc" formulation in T31a v1 was incorrect; it conflated threshold-trigger semantics with realized-fill semantics. Patch 5 correction surfaced by --test-cell in_match__1__medium__high__ATP_CHALL Session 7 run: +5c limit policy showed cap_p90 = +0.090, fire_rate 69%, capture_p10 over fired ≈ +0.05 (as expected). Spec validation gate code in T31c must operate on the corrected formulation.
...
3. **Layer B inherits Layer A's bounce structure faithfully (peak-capture policies).** In cells with positive-mean bounce (per Layer A bounce_60min_mean > 0), Layer B *limit policies* should show capture_p90 monotone increasing with the limit threshold limit_c — because limit policies fire at the maximum favorable excursion (MFE) within the walk window, mirroring Layer A's bounce_Xmin_mean (also an MFE-style metric). capture_p90 (not capture_mean) is the correct metric: it isolates the upper tail of the all-outcome distribution, which is dominated by fired trajectories whose captures are >= limit_c by construction. capture_mean conflates fired (MFE-style) with unfired (endpoint/settlement, often negative in mean-reverting markets), so the MFE signal is washed out at high limit_c thresholds where fire_rate drops. PASS if >= 60% of positive-bounce cells exhibit positive Spearman between limit_c and capture_p90. (Time-stop policies test a different property — endpoint capture at horizon, not MFE — and may show negative trend with horizon in mean-reverting markets where the bid peaks early then reverts. Per LESSONS B21, this is an MFE-vs-endpoint distinction, not a producer-correctness or strategy-coherence violation. Time-stop horizon trend in positive-bounce cells is reported as informative-INCONCLUSIVE; not a gating check.) Patch 7 correction surfaced by T31c first run: original Check 3 wording assumed time-stop endpoint would mirror Layer A MFE, but Layer A bounce_60min_mean (median +0.0557 in 531 positive cells) and Layer B time_stop capture_mean (median Spearman vs horizon -0.321 across 354 cells, only 38.7% positive-rho) measure structurally different things.
