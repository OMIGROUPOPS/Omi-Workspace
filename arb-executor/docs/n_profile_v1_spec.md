...
| 15a | `n_minutes_observed` | int | Count of per_minute_features rows observed for this ticker (= n_minutes_premarket + n_minutes_in_match by construction; the phase partition is exhaustive over observed minutes). This is the gate-3 RHS (§4.1) and the §4.2 observation-coverage-ratio denominator. NOT wall-clock — see lifetime_minutes (col 11) for market wall-clock. Persisted so gate 3 is checkable from the reloaded-from-disk parquet (C37). |
...
| 40 | `both_sides_active_minutes` | int | CORPUS-SCOPED count of minutes where BOTH this N AND its paired partner N had at least one trade (count_fp>0 in the same minute). Partner activity is read directly from per_minute_features regardless of any processing batch — this is the true bilateral-feasibility primitive (B23/E18), batch-independent by definition. NOTE (commit 51b1cd60 / operator adjudication 2026-05-16): pre-51b1cd60 the producer computed this batch-scoped (only when the partner ticker was co-present in the processed sample), which spuriously zeroed ~93% of rows at sub-corpus scale. Prior Phase-1/2 artifacts' near-zero values were that BATCH-SCOPING ARTIFACT — NOT a sampling property that "resolves at full corpus" as described in earlier analysis. Corpus-scoped is the corrected, intended semantic; see §7 resolution-log row 13. |
...
| 45 | `match_start_method` | string | The per_minute_features match-start signal tier that fired for this N: `both_sides_price_discovery` (tier 1) / `both_sides_trade_density` (tier 2) / `expected_expiration_fallback` (tier 3) / `unknown` (no signal). Sourced directly from per_minute_features.match_start_method (T37-derived; same one-time narrow-projection read as category/partner_ticker). LESSONS F35: tier-3 (`expected_expiration_fallback`) N's have boundary-unreliable premarket/in-match splits (match_start anchored to near-settlement expected_expiration, ~14.8h post-open); `unknown` N's are C32-canonical early-settle (no in_match phase). Downstream consumers filter on this column to exclude the 6.4% tier-3 cohort for any premarket/in-match-dependent analysis until T37-RECAL recalibration lands. Column ordinal 45 (col 44 = n_minutes_observed; the "15a" / "45" labels are intentional non-renumbering per the established single-concern pattern — the column NAME is what the producer keys on, not the table ordinal). |

**45 columns total** (43 originally enumerated + n_minutes_observed [col 44, gate-3 RHS + §4.2 denominator] + match_start_method [col 45, F35 interim mitigation — tier-3 filterability]). The v0.1 header's "44" off-by-one was reconciled when n_minutes_observed landed; match_start_method is the deliberate 45th, added per LESSONS F35 to make the boundary-unreliable tier-3 cohort downstream-filterable. Both added columns earn their slots by purpose, not as phantom fillers. Single row per N. Estimated emitted row count post-dropouts: ~17,000-19,000 (final number determined by Phase 1 probe; dropouts include missing trade tape, missing match_start_ts inference, partner-ticker resolution failures).
...
5. Pass 3: compute both_sides_active_minutes CORPUS-SCOPED — read the active-minute set (trade_count_in_minute>0) for ALL tickers via a single pushed-down per_minute_features read (filters=[("trade_count_in_minute",">",0)], 3 cols), then for each N count minutes where BOTH N and its paired partner are active. Partner lookup is independent of which tickers are in the current processing batch (the pre-51b1cd60 batch-scoping defect — see §2.1 col 40 note + §7 row 13). At full-corpus scale the active-minute map is bounded (active minutes are a small fraction of total per G19).
...
3. **Phase partition exhaustiveness.** Checked from the reloaded-from-disk parquet (C37) against the persisted `n_minutes_observed` column (§2.1 col 44). Three sub-invariants, zero violations each: (a) EXHAUSTIVE PARTITION — n_minutes_premarket + n_minutes_in_match == n_minutes_observed exactly (zero tolerance; the phase split at match_start_ts must exhaustively and non-overlappingly cover every observed minute, premarket xor in_match). (b) ACTIVE SUBSET — n_active_minutes_premarket <= n_minutes_premarket AND n_active_minutes_in_match <= n_minutes_in_match (active minutes are a subset of observed minutes per phase by definition; violation = accounting bug). (c) NON-NEGATIVE — all four phase counts >= 0. NOTE: this is an internal-consistency invariant over the OBSERVATION WINDOW, NOT a wall-clock check — lifetime_minutes (§2.1 col 11, market wall-clock) is definitionally larger than n_minutes_observed because per_minute_features (T37) is a bounded window that starts after market open. Comparing phase_sum to lifetime_minutes is a category error (original v0.1 gate-3 defect, surfaced Phase 1 retry #4). For no-match-start N's (match_start_ts NaT) all observed minutes classify premarket (n_minutes_in_match = 0); invariant (a) still holds (premarket count == observed count).
...
- **match_start_method distribution + zero-in-match breakdown (LESSONS F35):** per-method N counts (tier1/tier2/tier3/unknown) and, within each method, the count and fraction with n_minutes_in_match = 0. Corpus expectation per F35 investigation: ~87.5% tier-1/2 (healthy, ~0% zero-in-match), ~6.4% tier-3 (`expected_expiration_fallback`, boundary-unreliable), ~6.1% `unknown` (C32-canonical early-settle, ~100% zero-in-match). Logged in validation_report so the tier-3 (boundary-unreliable) and unknown (no-in-match-phase) cohorts are explicitly traceable in every n_profile build; consumers use this to scope analyses to the reliable tier-1/2 majority until T37-RECAL lands.
...
| 11 | Gate 3 disk-checkability — n_minutes_observed column added | Redefined gate 3 (row 10) compares phase_sum to observed-row-count, but gate 3 runs on the reloaded-from-disk 43-col parquet (C37) and observed-row-count was not persisted (guaranteed by construction in compute_per_ticker_row but not re-verifiable post-hoc from disk). RESOLVED: n_minutes_observed added as §2.1 col 44 (= len of ticker's per_minute_features rows). Serves gate 3 RHS AND §4.2 coverage-ratio denominator. Gate 3 expanded to three disk-checkable sub-invariants: exhaustive partition (sum==observed), active-subset (active<=total per phase), non-negative. Also reconciles the v0.1 spec-header "44 columns total" off-by-one — header is now literally correct, the column earning the 44th slot by purpose not coincidence. | Operator adjudication 2026-05-16 ET (Option 3: most rigorous without over-engineering) |
| 12 | match_start_method exposed as col 45 (F35 interim mitigation) | LESSONS F35 (commit 19fdd5a) established 6.4% tier-3 `expected_expiration_fallback` N's have boundary-unreliable premarket/in-match splits, recoverable only via deferred T37-RECAL. F35's prescribed interim mitigation: surface match_start_method so consumers can exclude the tier-3 cohort. ADDED as §2.1 col 45 in the sample-quality family (also closes a pre-existing gap — §1.1 already listed "match-start signal method" but §2.1 omitted it). §4.2 gains a method-distribution + zero-in-match informative measurement so the tier-3/unknown cohorts are traceable in every build. n_profile is NOT the defect locus (faithfully reflects PMF); this column is the consumer-side filter until the upstream T37-RECAL fix lands. | LESSONS F35; operator direction 2026-05-16 ET |
| 13 | both_sides_active_minutes: batch-scoped → corpus-scoped (defect correction) | Phase-2 re-validation post PMF-pushdown remediation 51b1cd60: equivalence check vs validated baseline 498d8e10 showed 43/44 data cols + all timestamps byte-identical, but both_sides_active_minutes differed on 923/1000 rows, unidirectionally (BASE exactly 0 → NEW substantive; 0 rows new<base; the 77 unchanged = the 72 partner-co-present + 5 coincidental). Disk-evidence root cause: pre-51b1cd60 Pass-3 was BATCH-SCOPED (computed only when partner ticker co-present in the processed batch) → spuriously zeroed ~93% at sub-corpus scale; the 51b1cd60 I/O remediation's Pass-3 rewrite reads partner activity from per_minute_features pushdown independent of batch → corpus-scoped, the correct value. 51b1cd60's commit message "no data/logic change — same per-ticker results" is FACTUALLY FALSE for this column (history pushed, not rewritable — this row + LESSONS are the honest provenance record per the 4f553397 provenance-integrity discipline). Operator adjudication: corpus-scoped is the correct intended semantic, accept not revert; prior "resolves at full corpus" framing described a latent bug, not a sampling property. Equivalence gate for Phase-2 re-re-validation redefined: 43 non-bsam cols byte-identical AND bsam differs ONLY in base-zero→new-positive direction. | Operator adjudication 2026-05-16 ET; equivalence check disk evidence; cross-ref LESSONS (this session) |
