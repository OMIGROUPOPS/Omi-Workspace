...
Writes /tmp/on_ledger.tsv, /tmp/on_participation.tsv, /tmp/on_agg.txt, /tmp/on_dump.json"""
...
LOG='logs/live_v3_20260702.jsonl'
# restart boundary: final live boot 22:09:10 ET Jul 2 == epoch 1783044550. But include full evening slate.
# We tag each leg with which_boot but process the whole overnight file.
BOOT_LIVE = datetime(2026,7,2,22,9,10,tzinfo=ET).timestamp()
...
json.dump({'legs':rows_out,'events':evrows,'discovery':discovery,
    'flags_note':'tape_gated_abandon=T per_side_placement=T inside_buffer_off=F completion_all_cells=T completion_combined_ceiling=T kalshi_occ_observe=T',
    'abandon_defer_total':sum(abandon_defer.values()),'occ_obs_total':sum(occ_obs.values())},
    open('/tmp/on_dump.json','w'),default=str)

tf=open('/tmp/on_ledger.tsv','w')
...
pf=open('/tmp/on_participation.tsv','w')
