...
const replayPath = path.join(repo, "arb-executor/analysis/build_window1_quote_shape_elimination_replay_v11_frozen.js");
const secondLegXLibraryPath = path.join(repo, ".claude/window1_live_v4_replay/second_leg_x_pricer_fit_v17_20260803/SECOND_LEG_X_CONDITIONAL_LIBRARY.json");
...
function prepareInputs(work) {
  ...
  const v11Events = zlib.gunzipSync(fs.readFileSync(v11EventPath)).toString("utf8").trim().split(/\r?\n/).map(JSON.parse);
  const v11LegByKey = Object.fromEntries(v11Events.flatMap((event) => event.legs.map((leg) => [leg.leg_identity, leg])));
  ...
  for (const event of ceiling.events) {
    ...
    for (const row of rows) {
      ...
      legs[row.leg] = {
        entry_cents: null,
        own_window1_close_cents: v11LegByKey[`${event.event_id}|${row.leg}`]?.own_window1_close_cents ?? null,
  ...
  return { ceiling, capacityByKey, quoteByEvent, bellByKey, v11LegByKey, refsPath, windowsPath };
}
...
function enrich(events, inputs) {
  ...
  return events.map((event) => {
    ...
    for (const legId of legIds) {
      ...
      const close = inputs.v11LegByKey[`${event.event_id}|${legId}`]?.own_window1_close_cents ?? null;
      ...
      legs[legId] = {
        ...
        delta_to_own_window1_close_cents: entry === null || !Number.isInteger(close) ? null : entry - close,
        ...
        ask_reachable_floor_minus_own_window1_close_cents: Number.isInteger(floor) && Number.isInteger(close) ? floor - close : null,
        ...
        second_leg_x_pricer_decisions: replayLeg.second_leg_x_pricer_decisions || [],
...
async function main() {
  ...
  const conservation = {
    schema_version: "WINDOW1_SECOND_LEG_X_PRICER_POPULATION_CONSERVATION_V17",
    D: ledger.length,
    legs: allLegs.length,
    ...
    carried_pairs_strict_one_above_one_below_close: ledger.filter((event) => { const deltas = Object.values(event.legs).map((leg) => leg.delta_to_own_window1_close_cents); return event.candidate_completed_pair && deltas.some((delta) => delta > 0) && deltas.some((delta) => delta < 0); }).length,
