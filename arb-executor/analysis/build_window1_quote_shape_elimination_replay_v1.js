...
const libraryPath = path.join(outDir, "QUOTE_SHAPE_LIBRARY.json");
...
const TARGETS = new Set(["KXATPCHALLENGERMATCH-26JUL19NIKVRB", "KXATPCHALLENGERMATCH-26JUL19HURBIG"]);
...
function prefixRows(rows, left, right) {
  ...
  for (const row of rows) {
    ...
    output.push({ ...row, prefix, ask_dwell_seconds: row.ts - lastAskChange, ask_change_after_first_timestamp: askChangeAfterFirstTimestamp, raw_row_count: count, progress_bin: Math.max(0, Math.min(100, Math.floor((row.ts - left) / (right - left) * 100))) });
  }
...
function replayGame(eventId, sources, library, refs) {
  const legs = sources.map((source) => { const loaded = loadRows(source); return { ...source, source_receipt: loaded.source, rows: prefixRows(loaded.rows, source.left, source.right), first_book_ts: loaded.rows[0].ts, first_ask: loaded.rows[0].ask, first_bid: loaded.rows[0].bid, price_region: region(loaded.rows[0].bid), order: null, fill: null, last: null, survivor_shapes: null, decisions: [] }; });
  ...
  for (const ts of clock) {
    ...
    for (const leg of legs) {
      ...
      if (leg.order && row.ts > leg.order.action_ts && row.ask <= leg.order.price_cents && row.ask_dwell_seconds >= DWELL_SECONDS && capacityAtOrBelow(row, leg.order.price_cents) >= QUANTITY) { leg.fill = { price_cents: leg.order.price_cents, quantity: QUANTITY, evidence_ts: row.ts, evidence_receipt: row.receipt, evidence_type: "STRICTLY_LATER_ASK_DWELL_AND_DISPLAYED_CAPACITY", ask_cents: row.ask, ask_dwell_seconds: row.ask_dwell_seconds, capacity: capacityAtOrBelow(row, leg.order.price_cents) }; leg.decisions.push({ ts, state: "FILLED", receipt: row.receipt, order: leg.order, fill: leg.fill }); continue; }
      ...
      if (row.raw_row_count < 2) reason = "NO_PRIOR_IN_WINDOW_BOOK";
        ...
        else if (!leg.resolved_direction || !sibling.independent_direction || inverseDirection(leg.resolved_direction) !== sibling.independent_direction || !directionObserved(sibling, sibling.independent_direction)) reason = "FLOOR_CONSENSUS_BUT_SIBLING_DIRECTION_NOT_INDEPENDENTLY_OBSERVED";
...
function main() {
  ...
  const events = [...TARGETS].sort().map((id) => replayGame(id, sourcesByEvent[id], library, refs)), receipt = { schema_version: "WINDOW1_QUOTE_SHAPE_ELIMINATION_TWO_GAME_REPLAY_V1", cold: true, outcome_knowledge_consumed: false, score_free: true, library_sha256: sha256(fs.readFileSync(libraryPath)), decision_law: { macro: "category + first one-tick-spread lawful-book price band initializes every empirical quote topology", pair: "the first independently resolved ask-path direction removes non-inverse sibling directions; historically observed pair tuples remain the provenance set", micro: "at each own-book tick, exact fitted distance minima first retain the closest ask-net/dip shapes, then dwell/spread/cadence/displayed-size/top-five-depth resolves only the remaining tie; eliminated shapes never return", micro_micro: "the joint clock re-evaluates carried lawful BBO state on either leg's tick; PLACE additionally requires an own ask transition after the first timestamp, an independently observed inverse sibling direction, current ask equal to observed ask low, inherited 10-second dwell, and displayed ask quantity >=5", states: ["PLACE", "HOLD", "INSUFFICIENT_EVIDENCE"], place: "all surviving pair-constrained shape medoids say FLOOR and micro-micro evidence is executable", hold: "all surviving shape medoids say LOWER", insufficient: "no prior in-window book, disagreement, no own micro position, unresolved sibling direction, missing library evidence, or execution evidence unavailable", fill: "strictly later ask receipt, ask<=X, ask dwell>=10 seconds, displayed capacity>=5" }, events };
  fs.mkdirSync(outDir, { recursive: true }); fs.writeFileSync(path.join(outDir, "TWO_GAME_REPLAY.json"), canonical(receipt)); for (const event of events) fs.writeFileSync(path.join(outDir, `${event.event_id.includes("NIKVRB") ? "NIKVRB" : "HURBIG"}_QUOTE_SHAPE_REPLAY.svg`), svgChart(event, sourcesByEvent[event.event_id]));
