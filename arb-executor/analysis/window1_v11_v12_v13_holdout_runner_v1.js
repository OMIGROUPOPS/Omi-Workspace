...
/* One-shot, frozen V11/V12/V13 Window-1 holdout evaluation.
 *
 * The runner consumes only the declared 2026-07-24..26 BBO files and frozen
 * event catalog, fetches receipt-identified public trades, and applies the
 * three already-fitted policies without refitting.  It also builds the
 * requested V11->V13 development loss crosswalk from committed ledgers.
 */
...
async function requestJson(url, attempts = 5) {
  let last;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const response = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "omi-window1-holdout-v1" }, signal: AbortSignal.timeout(30000) });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return await response.json();
    } catch (error) { last = error; if (attempt + 1 < attempts) await new Promise((resolve) => setTimeout(resolve, Math.min(10000, 500 * 2 ** attempt))); }
  }
  throw last;
}

async function fetchTape(tickers, rawDir, progressFile) {
  ensure(!fs.existsSync(rawDir), "public-trade raw directory already exists; one-shot evaluation refuses reuse"); fs.mkdirSync(rawDir, { recursive: true });
  const queue = [...tickers], results = [], failures = []; let complete = 0;
  async function worker() {
    while (queue.length) {
      ...
      try {
        ...
        results.push({ ticker, trades, page_count: page, raw_sha256: sha256(bytes), raw_bytes: bytes.length });
...
function prepareInputs({ ticksRoot, metadataRoot, workspace, tape }) {
  ...
  for (const [eventId, legs] of [...byEvent].sort(([a], [b]) => a.localeCompare(b))) {
    const meta = catalogMap.get(eventId); ensure(meta, `${eventId}: missing catalog row`); const scheduled = finite(meta.sched_honest), right = finite(meta.right_edge), left = scheduled === null ? null : scheduled - 8 * 3600;
    const positiveWindow = left !== null && right !== null && right > left; const actualBell = finite(meta.official_ts ?? meta.onset_est);
    ...
    eventEvidence.push({ event_id: eventId, category: legs[0].category, event_date: legs[0].event_date, starting_price_split: split, scheduled_start_ts: scheduled, guarded_right_ts: right, guarded_right_source: meta.right_edge_src, actual_bell_ts: actualBell, both_closes_properly_late: evidenceLegs.every((x) => x.close_properly_late), legs: evidenceLegs, ceilings: { absolute_traded_low: pairNegative(absolute, closes), traded_low_print_size_at_least_five: pairNegative(size5, closes), capacity_proven_ask_floor: pairNegative(askFloors, closes), lowest_seller_aggressed_trade_floor: pairNegative(seller, closes), maker_reachable: pairNegative(askFloors, closes) && evidenceLegs.every((x) => Boolean(x.maker_reachable_at_ask_floor)) } });
...
function runVariant(repo, ticksRoot, prepared, workspace, version) {
  ...
  const library = version === "V11" ? path.join(repo, ".claude/window1_live_v4_replay/five_exact_dynamic_renarrow_v6_20260801/QUOTE_SHAPE_LIBRARY_DYNAMIC_RENARROW_V6.json") : version === "V12" ? path.join(repo, ".claude/window1_live_v4_replay/coherent_shape_v12_fit_20260802/COHERENT_QUOTE_SHAPE_LIBRARY_V12.json") : path.join(repo, ".claude/window1_live_v4_replay/interim_shape_v13_fit_20260803/INTERIM_SHAPE_LIBRARY_V13.json");
  ...
  if (version === "V12" || version === "V13") argv.push("--coherent-shape-v12"); if (version === "V13") argv.push("--interim-elimination-v13");
...
function normalizeVariant(run, evidenceMap) {
  ...
  for (const rawEvent of run.replay.events) {
    ...
    for (const evLeg of evidence.legs) {
      const raw = rawEvent.legs?.[evLeg.leg_id] || {}; const entry = integer(raw.honest_credited_entry_cents), acted = integer(raw.proposed_entry_cents) !== null;
...
async function main() {
  ...
  ensure(args.includes("--execute-once"), "runner requires --execute-once"); const repo = path.resolve(value("--repo", ".")), inputRoot = path.resolve(value("--input-root", "C:/tmp/window1_holdout_v11_v13_20260803")), out = path.resolve(value("--output", path.join(repo, ".claude/window1_live_v4_replay/v11_v12_v13_holdout_20260803")));
  const ticksRoot = path.join(inputRoot, "ticks"), metadataRoot = path.join(inputRoot, "metadata"), marker = path.join(inputRoot, "HOLDOUT_EVALUATION_CONSUMED.json"), workspace = path.join(inputRoot, "execution_workspace"), rawDir = path.join(inputRoot, "public_trade_raw"), progress = path.join(inputRoot, "HOLDOUT_PROGRESS.log");
  ...
  writeJson(marker, { execution_started_at_utc: new Date().toISOString(), runner: rel(repo, __filename), git_head: spawnSync("git", ["rev-parse", "HEAD"], { cwd: repo, encoding: "utf8" }).stdout.trim(), attempts: 1, retries: 0, holdout_dates: [...HOLDOUT_DATES].sort() });
  fs.mkdirSync(workspace, { recursive: true }); fs.writeFileSync(progress, `${new Date().toISOString()} START\n`);
  const identities = fs.readdirSync(ticksRoot).map(tickerIdentity).filter(Boolean); ensure(identities.length === 456, "sealed ticker count changed"); const tape = await fetchTape(identities.map((x) => x.ticker).sort(), rawDir, progress);
  const prepared = prepareInputs({ ticksRoot, metadataRoot, workspace, tape }); const evidenceMap = new Map(prepared.eventEvidence.map((x) => [x.event_id, x])); const runs = VERSIONS.map((version) => runVariant(repo, ticksRoot, prepared, workspace, version)); const normalized = runs.map((run) => normalizeVariant(run, evidenceMap)); const dev = developmentComparison(repo);
  ...
  const v13 = normalized.find((x) => x.version === "V13"); writeJson(path.join(out, "V13_EXECUTION_FLOOR_COLLAPSE_DIAGNOSTIC.json"), { schema_version: "WINDOW1_V13_EXECUTION_FLOOR_DIAGNOSTIC_V1", development_source: "committed V13 population leg ledger", holdout_source: "single sealed evaluation", development_category_and_price_region: [...group(readGzipRows(path.join(repo, ".claude/window1_live_v4_replay/interim_elimination_v13_20260803/POPULATION_LEG_LEDGER.jsonl.gz")).filter((x) => x.proposed_entry_cents !== null), (x) => `${x.category}|${x.price_region}`).entries()].sort(([a], [b]) => a.localeCompare(b)).map(([key, xs]) => ({ category: key.split("|")[0], price_region: key.split("|")[1], legs: xs.length, qualifying_ask_floor_gap: distribution(xs.map((x) => integer(x.honest_credited_entry_cents) !== null && integer(x.qualifying_ask_floor_cents) !== null ? x.honest_credited_entry_cents - x.qualifying_ask_floor_cents : null), xs.length), objective_traded_low_gap: distribution(xs.map((x) => integer(x.honest_credited_entry_cents) !== null && integer(x.objective_traded_low_cents) !== null ? x.honest_credited_entry_cents - x.objective_traded_low_cents : null), xs.length) })), sealed_holdout_category_and_price_region: v13.summary.category_and_leg_price_region });
...
module.exports = { tickerIdentity, capacityFloor, pairNegative, distribution, metrics };
