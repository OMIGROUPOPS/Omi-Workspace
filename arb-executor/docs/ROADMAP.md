...
T28. **Foundation commit + ANALYSIS_LIBRARY G9 entry.** **CLOSED 2026-05-04 (commit ea84e74).** G9 parquets sha256-pinned in MANIFEST.md (g9_candles 9.5M rows, g9_trades 33.7M rows, g9_metadata 20,110 rows). T27 verification 9/9 PASS. ANALYSIS_LIBRARY canonical foundation entry registered. Foundation pointer for all downstream Layer A/B/C work per LESSONS C27. Per LESSONS C27 step 2-3:
...
T29. **Layer A v1 producer (combined visuals + tabular metrics).** **CLOSED 2026-05-04 (producer commit 1398c39, MANIFEST commit 37a5216, ANALYSIS_LIBRARY commit cf31903).** Combined T29 visuals + T30 tabular into single producer per simplification. Output: cell_stats.parquet (671 cells, 371 substantial, 8.98M moments aggregated from 19,603 markets), sample_manifest.json, build_layer_a_v1.log, 15 visual PNGs (5 categories x 3 regimes). Producer runtime 60.7 min, clean exit. Foundation pointer T28 ea84e74. Validity gated through T21. Per LESSONS C27 step 4-5 + B16 (Layer A separation) + E31 (regime-aware) + B17 (one-sided book filtering) + G18 (candle close fields = BBO snapshots):
...
T30. **Layer A v1 tabular metrics.** **CLOSED 2026-05-04 (subsumed into T29 combined producer).** Tabular metrics output (per-cell forward-bounce distribution stats, drawdown stats, breakeven-threshold fractions) folded into cell_stats.parquet alongside visual PNGs in single T29 producer run. No separate metrics producer needed. Per LESSONS C27 step 6 + B16. After T29 visuals reveal which cells show clean signal, run the tabular metrics producer on those cells specifically (or all cells with sufficient sample size):
...
T31a. **Layer B v1 specification.** Per LESSONS B16 + ROADMAP G10 (promoted to T31 in commit faf51d9, split into T31a/T31b in this commit). Pure design doc at arb-executor/docs/layer_b_spec.md. Resolves five operational decisions: (1) simulation methodology — per-trajectory walk against g9_candles re-read by sampled ticker, Methodology A chosen over distribution-based or closed-form alternatives because Layer B is the strategy gate and honesty matters more than speed; (2) cell-minimum-sample-size — inherits n_markets >= 20 from T21 substantial-cell scope, plus per-cell threshold of 50 simulated trajectories with valid forward windows; (3) source columns — yes_ask_close for entry, yes_bid_close for exit (conservative spread-cross fill assumption); (4) policy parameter space — 55 policies per cell across limit-exit, time-stop, trailing-stop, and combined types, ~20K (cell, policy) tuples per channel; (5) non-fire handling — explicit settled_unfired (resolves to settlement value from g9_metadata) and horizon_expired buckets in capture distribution. Settlement-zone Layer B deferred to v2 per T21 Check 3 finding (structural directional asymmetry requires different exit logic). Two-channel scope per E31: premarket (5-dim cell key) and in-match (4-dim cell key, volume_intensity dropped per F30). Substantial cells only (371 from T21). Foundation pointers: T28 ea84e74, T29 1398c39 / 37a5216 / cf31903, T21 faf51d9. Output schema and producer architecture detailed in spec doc. CLOSED 2026-05-05 ET. Spec stable at docs/layer_b_spec.md. Promoted to T31a/T31b/T31c at commit 0915d92; 8 patches landed across 7ea70b1 (1: scalar exclusion), 6001d62 (2: in_match 5-dim retention), 8df82d5 (3: trajectory definition), 105f3c3 (4: cent/dollar units), d2c3065 (5: Check 1 wording relax), 488bddc (6: drop 30s horizon from 55-policy grid), c9cdd1b (7: Check 3 MFE vs endpoint distinction per LESSONS B21), f0d1b04 (8: Check 3a metric capture_mean → capture_p90). Spec referenced in ANALYSIS_LIBRARY Layer B v1 entry.

T31b. **Layer B v1 implementation.** Code per T31a spec. Produces arb-executor/data/durable/layer_b_v1/exit_policy_per_cell.parquet plus per-(channel, category) summary visual PNGs. Producer at arb-executor/data/scripts/build_layer_b_v1.py. Memory discipline per LESSONS C28 (streaming row-group column projection on g9_candles, per-cell trajectory cache cleared after processing). Producer log to build_layer_b_v1.log. Estimated runtime: 30-60 min comparable to T29. Output gets full C27 treatment: MANIFEST sha256 entry + ANALYSIS_LIBRARY subsection with foundation-pointer to T28, producer pointer to commit hash of producer code, validity status PENDING T31c. Gates Layer C (G11). CLOSED-DATA-PATH 2026-05-05 ET. Producer at 28e8ab7 (data path); shared cell_key_helpers refactor at 8174ec0; T31b-α skeleton at 739bfdd; T31b-β functions at b67f6d8 + 2582a98 (key fix) + 8cda92f (3 fixes); T31b-γ-1 full-run at 28e8ab7; followup MANIFEST + ANALYSIS_LIBRARY registration at 846a10b. Output exit_policy_per_cell.parquet validated by T31c PASS at 2654a54. T31b-γ-2 visuals (per-(channel, category) PNGs) deferred to v2 / Priority 3 per Session 7→8 handoff.

T31c. **Layer B v1 coherence read.** CLOSED 2026-05-05 ET via commit 5cf45e0 (script: data/scripts/check_layer_b_v1_coherence.py) + report at data/durable/layer_b_v1/coherence_report.md (sha256 72f1747b). Verdict: 4/4 gating-checks PASS (Check 1 capture-bounded spot-check, Check 2 fire-rate monotonic, Check 3a limit-policy capture_p90 trend in positive-bounce cells, Check 4 premarket vs in_match). 1 informative-only Check 3b INCONCLUSIVE (time-stop horizon endpoint trend per LESSONS B21 — structurally distinct from MFE, not gating). Layer B v1 outputs are validated and cleared for downstream Layer C (G11) consumption. Spec went through 8 patches before PASS (patches 5, 7, 8 substantively changed Check wording; patches 1-4 + 6 were earlier producer/scope corrections).

T32. **Layer C realized economics — v1 scope.** Per LESSONS B16 layered-realism discipline (Layer A → Layer B → Layer C, each layer adds one modeling concept and gets its own coherence read). Promoted from G11 at this commit; gate (T31c PASS) satisfied at 2654a54. **v1 scope (single concept added: empirical fees from bot fill history):** consume Layer B exit_policy_per_cell.parquet (19,170 rows) + bot's kalshi_fills_history.json (7,489 fills, fee_cost + is_taker per fill); add per-(cell, policy) realized_capture_after_fees, entry_fee_assumed, exit_fee_assumed, policy_class_taker_exposure_flag. For limit-bearing policies (maker entry + maker exit): both legs pay an empirical maker fee derived per (yes_price_bucket) from kalshi_fills_history.json. Maker fees are NOT zero (37.7% of bot maker fills are non-zero, parabolic per Kalshi schedule); realized_capture < capture by an amount that depends on entry_price and exit_price. For time-stop / limit_time_stop policies whose time-stop leg fires: exit is taker, fee = parabolic Kalshi schedule at horizon-minute close price. **v1 idealized fills assumed (per Layer B); v1 strictly per-cell output per E20 (no portfolio aggregate); v1 single-leg per E18 (bilateral capture is G12-blocked).** v2 spec adds empirical entry fill probability from g9_trades; v3 adds exit fill probability; v4 adds capital constraints. Layer D = bot-execution / taker-fallback / queue position (separate from Layer C). T32 chain (planned, parallel to T31a/b/c shape): T32a (spec doc layer_c_spec.md), T32b (producer build_layer_c_v1.py + output layer_c_v1/realized_economics_per_cell.parquet), T32c (coherence read check_layer_c_v1_coherence.py + report). T32c PASS gates v2.



T33. **Per-minute Open Interest producer (TOTAL OI from g9_candles + per-side flow from g9_trades) — AMENDED 2026-05-06.** Per LESSONS F31 (amended) + B24 + C30. Foundation: g9_candles.parquet (T28 sha256-pinned at ea84e74) for total per-ticker per-minute OI; g9_trades.parquet for per-side directional flow. **AMENDMENT 2026-05-06 ET:** Original T33 framing planned a ~2-3 hour reconstruction of OI from g9_trades cumulative signed count_fp, on the premise that OI was not tracked at the data layer. Session 9 morning probe revealed g9_candles.open_interest_fp is populated for 96.4% of 19,687 markets (verified vs metadata_final_oi); historical-tier markets show OI=0 throughout (backfill gap); 64.6% of rows have NULL OI in pre-formation/inactive minutes. **Revised producer scope:** (1) Total per-ticker per-minute OI = direct read from g9_candles.open_interest_fp + null/zero handling for inactive periods + historical-tier fallback to trade-tape cumsum where candle OI=0. (2) Per-side directional flow (yes-side trades vs no-side trades) = trade-tape signed cumsum by taker_side, providing the long-YES vs long-NO breakdown that total OI does not expose (every YES contract has a NO counterparty, so total OI is symmetric per ticker). Output: data/durable/oi_v1/oi_per_minute.parquet, columns [ticker, ts_minute, total_oi (from candle), yes_side_flow_cum (from trades), no_side_flow_cum (from trades), source ('candle'|'trade_reconstruction')]. Per-market aggregation; ~20,018 markets. C28 streaming discipline still mandatory for trade-tape per-side computation (~33.7M trade rows); candle read is single-pass single-column. **Revised estimated runtime: ~20-30 minutes** (was ~2-3 hours). T33 chain (planned, mirrors T29/T31b shape): T33a (spec doc oi_v1_spec.md with column definitions + computation method + per-market correctness check sample), T33b (producer build_oi_v1.py + output parquet + log), T33c (coherence read check_oi_v1_coherence.py + report — 4-check shape: monotone non-negative OI, per-market OI sum equals total open positions at last observed minute, per-side delta sums equal trade counts on that side, sample-cell anchor evidence reproduces Dzumhur=2,070 + Mannarino=8,594). C27 treatment: MANIFEST sha256 entry + ANALYSIS_LIBRARY subsection with foundation-pointer to T28. Validity gate: T33c PASS gates downstream Layer C v2/v3 fill-probability modeling (T32d v2). Cross-reference: F31 (the lesson), B24 (4× OI asymmetry anchor evidence), B22 (cell anchoring framework — OI growth tells us which cells are reachable when), E16 (in-match liquidity), C28 (streaming discipline mandate).

T34. **Combined-trade-price distortion event detection producer.** Per LESSONS F32 + B23. Foundation: g9_trades.parquet (T28 sha256-pinned at ea84e74). Combined trade prices `yes_close + no_close > $1.00` are direct alpha-bearing distortion events not currently flagged anywhere. Computation: per-market, walk g9_trades minute-bins, compute (yes_close + no_close) per minute, flag minutes where `combined > $1.00 + ε` (ε=$0.01 for tick-discreteness). Output: data/durable/distortion_v1/distortion_events.parquet, columns [ticker, ts_minute, yes_close, no_close, combined_price, distortion_magnitude, regime, category]. Per-event row (only minutes with distortions present); plus data/durable/distortion_v1/distortion_per_cell.parquet aggregating per-cell distortion frequency for direct join into Layer A/B downstream. **Treated as producer not one-off:** alpha-bearing data deserves canonical persistence per E27/E28 methodology drift discipline (operator confirmed Session 8: "you know" affirming producer-not-diagnostic shape). Estimated runtime ~30-60 min from g9_trades minute-bin walk. T34 chain: T34a (spec doc distortion_v1_spec.md), T34b (producer build_distortion_v1.py), T34c (coherence read 4-check: distortion-magnitudes non-negative, per-tier frequency reasonable, in-match dominates premarket per E16 expectation, sample-cell anchor evidence). C27 treatment. Validity gate: T34c PASS opens distortion-frequency feature for Layer A v2 / B v2 cell-key dim or filter. Cross-reference: F32 (the lesson), B23 (bilateral mechanism — distortions are negative-cost-basis extreme), E16 (in-match volatility expected to dominate distortion frequency), F31 (OI context for why distortions occur — thin OI = thin orderbook = more distortion).


**CLOSED 2026-05-06 ET (Session 9, superseded by Cat 9 negative finding — commit 631e653, sha256 `653e9d74888376f1`):** Producer was never built. Cat 9 ran T34's intended computation at corpus scale and found **zero events** across 33,727,162 trade rows × 20,018 markets × 3,236,159 candle-minutes at ε=$0.01 per-minute close. Per LESSONS F32 amendment, sub-claim (1) — combined > $1.00 over-pay distortions — is empirically absent at the first operationalization. T34 is closed without a producer artifact; the empirical absence is canonicalized as anchor evidence in ANALYSIS_LIBRARY Section 4. Finer-ε / sub-minute / tick-level re-probes are deferred to optional new T-items if the operator chooses to pursue any of them — they are not part of T34's scope retroactively. Cross-reference: LESSONS F32 (amended), LESSONS C31 (new), ANALYSIS_LIBRARY Section 4, SIMONS_MODE.md Section 8.

T35. **Formation gate per-market field producer.** Per LESSONS F34 + E12 + B22. Foundation: g9_metadata.parquet (T28 sha256-pinned at ea84e74) + g9_trades.parquet first-row-per-ticker. Formation gate = time gap between line release / market_open and first observed trade per market. Currently no per-market field captures this; E12 names the concept narratively, the bot uses uniform 4-hour gate. Computation: per-market, take g9_metadata.market_open_ts (probe required to confirm exact field name — likely `created_time` or `open_time`) minus g9_trades min(created_time) per ticker. Output: data/durable/formation_gate_v1/formation_gate_per_market.parquet, columns [ticker, market_open_ts, first_trade_ts, formation_gate_minutes, category]. Per-market scalar field; ~20,018 markets. Estimated runtime ~30 min. T35 chain: T35a (spec doc formation_gate_v1_spec.md), T35b (producer build_formation_gate_v1.py), T35c (coherence read 4-check: formation_gate_minutes non-negative, per-tier distribution reasonable, ATP main faster than ITF expectation per F34, sample-market anchor evidence). C27 treatment. Validity gate: T35c PASS enables Decision 6 / Check 5 of layer_c_spec.md (formation contamination measurement) — soft dependency for T32c, NOT blocking T32a/b. Cross-reference: F34 (the lesson), E12 (formation phase concept), B22 (cell anchoring — formation gate is when cells become trade-anchorable), Decision 6 in layer_c_spec.md.
...
G11. **Layer C realized economics.** PROMOTED 2026-05-05 ET to T32 (see T-section). Gate (formerly "G10 delivery") was renamed to "T31c PASS" in faf51d9 split; T31c PASSED at 2654a54 (Layer B v1 outputs registered + cleared for downstream consumption), so promotion gate is satisfied. See T32 entry for v1 scope.
...
G13. **Depth-chain orderbook-snapshot data collection (blocked-track).** Per LESSONS F33 + B24. Foundation: NEW data collection — NOT recoverable from existing g9 data. g9_candles records BBO close per minute (yes_bid_close + yes_ask_close); does NOT record orderbook depth at +1¢, +2¢, +5¢ from BBO. Anchor evidence (Session 8 / Mannarino YES depth chain): orderbook snapshot showed bids at 26¢ × 67 contracts, 25¢ × 4,210, 24¢ × 5,549, asks at 27¢ × 1,697, 28¢ × 14,710 (massive wall), 29¢ × 9,335, 30¢ × 8,050. The 28¢ wall is a structural barrier capping upside trajectory width. Without depth-chain data, fill-probability modeling at non-BBO maker-post depths is impossible, trajectory-width upper-bound modeling from wall locations is impossible, and "wall removed" regime-change detection is impossible. Recovery scope: implement Kalshi `/orderbook` polling at premarket-relevant intervals (suggested: 1-minute snapshots during full premarket window, 5-second snapshots in last 30 minutes pre-match), persist as data/durable/orderbook_snapshots_v1/. Per-market estimated cost: depth chain × snapshot interval × premarket+in_match duration. Order-of-magnitude: 20,018 markets × ~6 hours premarket avg × 60 snapshots/hr × ~10 levels per side × 2 sides = ~144M rows over the corpus IF full historical replay is desired (likely impractical — Kalshi historical orderbook may not be retrievable retroactively). More likely path: prospective collection going forward, incrementing during live operation. **Blocked-track gap, not immediate work.** Cross-reference: F33 (the lesson), B24 (orderbook asymmetry — depth chain is the underlying cause), F31 (OI is the recoverable proxy; this is the irrecoverable signal), B23 (bilateral fill feasibility depends on wall locations).
...
### Immediate sequence (post-T31c)

1. **T32a — Layer C v1 specification.** Spec doc at docs/layer_c_spec.md per LESSONS B16 layered-realism discipline. Decisions on canonical fee assumption (empirical from kalshi_fills_history.json per (is_taker, yes_price_bucket); fee schedule is parabolic for BOTH makers and takers per Kalshi rules — maker median $0.00 with range $0.00-$1.65 across 5,495 fills, taker median $0.17 with range $0.00-$23.91 across 1,994 fills; 37.7% of maker fills carry non-zero fee at non-extreme prices), policy class taker-exposure handling (limit-bearing maker-only vs time-stop-bearing taker-on-horizon-exit), validation gate (4-check shape mirroring T31c). Pure design doc, no producer code yet.

2. **T32b — Layer C v1 producer.** Code per T32a spec. build_layer_c_v1.py consuming Layer B exit_policy_per_cell.parquet (sha256-pinned per T31b-γ-1) + kalshi_fills_history.json. Output: layer_c_v1/realized_economics_per_cell.parquet, per-(cell, policy) realized fee-adjusted distributions. Producer log to build_layer_c_v1.log. Same C27 treatment: MANIFEST sha256 entry + ANALYSIS_LIBRARY subsection with foundation-pointer to T28 (G9) + T29 (Layer A) + T31b (Layer B), validity status PENDING T32c.

3. **T32c — Layer C v1 coherence read.** check_layer_c_v1_coherence.py + report at layer_c_v1/coherence_report.md. 4-check shape per T31c precedent: (1) realized_capture_after_fees ≤ capture (fees non-negative); (2) empirical fee schedule matches Kalshi parabolic shape: both maker and taker fee distributions exhibit parabolic shape vs |P-0.50|, with taker median exceeding maker median at 50¢ peak by factor ≥ 3 (consistent with published 4:1 taker:maker ratio); (3) time-stop-bearing policies show fee loss proportional to Kalshi schedule at horizon-minute price; (4) rank correlation between Layer B capture_mean and Layer C realized_capture per cell ≥ 0.99. PASS gates T32d (v2 promotion) and downstream live-trading planning.

4. **T32d — Layer C v2 promotion.** Pending T32c PASS. v2 adds empirical entry fill probability from g9_trades (33.7M rows, C28 streaming discipline mandatory). Spec scope opens at T32d-spec start.

### Parallel / deferred

- **G12 — Per-event paired moments dataset.** Blocked on U8 / E18. Required for bilateral capture analysis (paired YES/NO leg per match per moment) at Layer C v2+. Separate track from T32 chain.
- **T31b-γ-2 — Layer B v1 visuals.** Per-(channel, category) PNGs analogous to T29 15-PNG output. ~10-20 min producer run. Display-only, not on T32 critical path. Resume when narrative deliverable is needed.
- **T22 — TAXONOMY refactor.** Cross-session canonical-doc-evolution work; resume once T32 chain is stable.
- **T33 — OI reconstruction producer.** Per LESSONS F31 + B24. ~2-3 hr from g9_trades. Independent of T32 critical path. Output gates Layer C v2/v3 fill-probability modeling.
- **T34 — Combined-price distortion event detection producer.** Per LESSONS F32 + B23. ~30-60 min from g9_trades. Independent of T32 critical path. Alpha-bearing direct signal.
- **T35 — Formation gate per-market field producer.** Per LESSONS F34 + E12 + B22. ~30 min from g9_metadata + g9_trades. Soft dependency for T32c Check 5 (formation contamination); does NOT block T32a/b.
- **G13 — Depth-chain orderbook-snapshot data collection (blocked-track).** Per LESSONS F33 + B24. Not recoverable from existing data; requires new prospective Kalshi /orderbook polling. Order-of-magnitude infeasible for historical replay.

### Foundation-phase commit chain

Phase 1 — G9 foundation: bd83412 (T17 producer); ea84e74 (T28 sha256-pin in MANIFEST).

Phase 2 — Layer A v1: 8889012 (T29 producer build_layer_a_v1.py); 1398c39 (T29 refactor — canonical pointer); 37a5216 (MANIFEST sha256 entry); cf31903 (ANALYSIS_LIBRARY subsection).

Phase 3 — T21 coherence read (Layer A v1 PASS): faf51d9 (PASS verdict + G10 → T31 rename); 00f2604 + 03be86b + 53d3598 (cleanup commits).

Phase 4 — Session 6 → 7 transition: 31182f2 (ROADMAP cleanup); 5bf93b4 (Session 6 → 7 handoff doc); d207d51 (Session 7 opening / D17 catch-up); c19cb7c (Section 8 bullet-4 orphan fix).

Phase 5 — Layer B specification (T31a chain, 9 commits): 0915d92 (T31 split into a/b/c); 7ea70b1 (patch 1: scalar exclusion); 6001d62 (patch 2: in_match 5-dim retention); 8df82d5 (patch 3: trajectory definition); 105f3c3 (patch 4: cent/dollar units); d2c3065 (patch 5: Check 1 wording relax); 488bddc (patch 6: drop 30s horizon); c9cdd1b (patch 7: MFE vs endpoint per B21); f0d1b04 (patch 8: capture_mean → capture_p90).

Phase 6 — Layer B implementation (T31b chain, 7 commits): 8174ec0 (cell_key_helpers refactor); 739bfdd (T31b-α skeleton); b67f6d8 (T31b-β functions evaluate_policy + walk_trajectory + --test-cell); 2582a98 (β key fix policy_type → type); 8cda92f (β 3 fixes pre-γ); 28e8ab7 (T31b-γ-1 full-run producer code); 846a10b (γ-1 followup: MANIFEST + ANALYSIS_LIBRARY entries + LESSONS G21).

Phase 7 — Layer B coherence read (T31c chain, 4 commits): 58cb8aa (T31c first run, FAIL on Check 3); 5cf45e0 (T31c code update for patches 7+8); 2654a54 (T31c PASS-verdict followup, validity flip + coherence_report registered).

Phase 8 — Session 7 closure: 97b88ef (handoff docs/handoffs/session_07_to_08.md + LESSONS C29 set -e + grep verification gate brittleness).
...
- 2026-05-04 (Session 6 Phase 5-iv / T31a): T31 split into T31a (spec) + T31b (impl) + T31c (coherence read), mirroring T19/T20/T21 pattern. Spec doc landed at arb-executor/docs/layer_b_spec.md with five operational decisions resolved (simulation methodology Method-A per-trajectory walk, n_markets>=20 + 50-trajectory threshold, conservative spread-cross fills, 55-policy parameter grid, explicit settled_unfired/horizon_expired buckets). Settlement-zone deferred to v2 Layer B variant per T21 Check 3 structural-asymmetry finding. T31b/T31c remain OPEN.
- 2026-05-04 (Session 6 Phase 5-iv-fix / T31a patch): layer_b_spec.md amended after T31b pre-flight probe surfaced spec gap — g9_metadata.result has 3 values (yes/no/scalar) not 2. Decision 5 amended to map yes→1.00, no→0.00 explicitly and to exclude scalar markets (496 of 20,110, ~2.5% of universe) from Layer B v1 scope. Out-of-scope section adds scalar-markets bullet. Producer architecture amended to verify result is in {yes,no} before processing trajectories. Producer-architecture step 2 also notes filter pushdown efficiency (row groups are ticker-sorted per probe). Layer B v2 may add scalar variant if strategy expands.
- 2026-05-04 (Session 6 Phase 5-iv-fix2 / T31a patch 2): layer_b_spec.md amended after T31b-alpha pre-flight probe surfaced two spec gaps. (1) sample_manifest.json keys are 5-dimensional uniformly (built by T29 before F30 finding); Layer B v1 retains 5-dim in_match cell keys for invariant preservation, with volume_intensity recorded but flagged as uninformative per F30 — downstream consumers collapse at filter time. Rebuilding sample_manifest with 4-dim in_match keys would require T29 re-run + fan-out aggregation drift. (2) Producer architecture step 1 amended with cell-key parsing detail: keys split on `__`, entry_band_idx maps via ENTRY_BANDS[idx] from cell_key_helpers (refactor commit 8174ec0).
- 2026-05-04 (Session 6 Phase 5-iv-fix3 / T31a patch 3): layer_b_spec.md Decision 2 amended after T31b-alpha dry-run surfaced spec ambiguity. Defined `trajectory` as (ticker, entry_moment) tuple — not ticker. The sample_manifest cap of 30 tickers per cell does not bound trajectory count: each ticker contributes one trajectory per cell-matching minute in its candle history, typically yielding ~3000 trajectories per cell. Threshold-check (50 trajectories minimum per cell) deferred to T31b-beta scope where moment-granularity g9_candles reads happen. T31b-alpha dry-run log line clarified to flag ticker-level proxy is not the real threshold check.
- 2026-05-04 (Session 6 Phase 5-iv-fix4 / T31a patch 4): layer_b_spec.md Decision 4 amended after T31b-beta pre-flight probe surfaced unit-convention gap. Policy parameters (limit thresholds, trailing offsets) are integer CENTS (limit_c=5 means $0.05). g9_candles yes_bid_close / yes_ask_close are float DOLLARS (0.0 to 1.0). Producer's evaluate_policy converts thresholds to dollars at function entry (limit_dollars = limit_c / 100); price-comparison loop operates in dollar units. Convention single-sourced; downstream parquet receives cent-denominated policy_params and dollar-denominated capture_* columns.
- 2026-05-05 (Session 6 → Session 7 handoff): SESSION_HANDOFF doc landed at arb-executor/docs/handoffs/session_06_to_07.md per LESSONS D14. Foundation phase complete, T21 PASS verdict landed, Layer B v1 spec (T31a) reconciled across 4 patches, Layer B v1 producer skeleton (T31b-α) landed with dry-run verifying input loading + policy grid construction. Session 7 opens on T31b-β single-cell test mode. 14 commits in Session 6 chain, all single-concern and web-fetch verified.
- 2026-05-05 (Session 7 opening, ROADMAP+MANIFEST+LESSONS catch-up commit): T28/T29/T30 status flipped from OPEN to CLOSED with commit pointers (T28 ea84e74, T29 1398c39+37a5216+cf31903, T30 subsumed into T29). Section 8 NEXT MOVES rewritten to reflect T31a/b/c split (was single T31 entry). MANIFEST.md Layer A v1 validity status updated PENDING T21 -> PASSED T21 2026-05-04 (commit faf51d9). LESSONS D17 added (session-handoff documents do not substitute for keeping canonical indices current). Pattern caught by Session 7 opening probe per LESSONS D14/D15/C27 family.
- 2026-05-05 (Session 7 / T31a patch 5): layer_b_spec.md Validation Gate Check 1 wording corrected after T31b-beta --test-cell verification surfaced minute-cadence overshoot. Original wording "capture_p90 ≤ +Xc (modulo ±1c)" conflated threshold-trigger with realized-fill semantics; minute-cadence price observation means realized capture on fire is the minute-close bid which can substantially exceed the threshold on volatile minutes. Corrected formulation: "capture_p10 across fired trajectories ≥ +Xc; right-tail overshoot expected and valid; capture_p10 across all trajectories ≥ -100c floor." T31c coherence-read code must implement the corrected formulation. Surfaced by +5c limit policy showing cap_p90 = +0.090 with 69% fire rate on cell in_match__1__medium__high__ATP_CHALL.
- 2026-05-05 (Session 7 / T31a patch 6): layer_b_spec.md Decision 4 amended to drop sub-minute time-stop horizons from v1 scope. Original time-stop set {30s, 1min, ..., settle} = 9 policies; corrected set {1min, 5min, ..., settle} = 8 policies. Rationale: g9_candles minimum cadence is 1-minute, so a 30-second time-stop horizon is not expressible in v1 (first observable forward minute is at entry+60s, past any sub-minute horizon). Sub-minute time-stops require trade-tape resolution per existing v2 open-items section. Total policy grid: 55 -> 54. Producer TIME_STOPS constant updated correspondingly. Surfaced by --test-cell verification in_match__1__medium__high__ATP_CHALL Session 7 (post-Fix-C run): 30s policy showed 0% fire / 100% horizon_expired even after int-truncation fix because no candle exists at entry+30s in minute-cadence data. Cleaner than retaining a known-noop policy in the grid.
- 2026-05-05 ET (Session 7 / T31a patch 7): layer_b_spec.md Validation Gate Check 3 wording corrected after T31c first run (commit 58cb8aa) emitted FAIL on Check 3 (median Spearman -0.321, only 38.7% positive-rho across 354 cells). Diagnosis: original wording "Time-stop policies at long horizons capture more than at short horizons in positive-bounce cells" conflated two different metrics. Layer A bounce_Xmin_mean is MFE (max favorable excursion, monotone non-decreasing with window by definition); Layer B time_stop capture_mean is endpoint capture at horizon (bid AT minute t+X, not max within). In mean-reverting markets, endpoint reverts to fair value at long horizons. Both metrics are correct; they answer different questions. Patch 7 rewrites Check 3 to test the *right* property: Layer B *limit* policies (which fire at MFE within window, mirroring Layer A bounce structure) should show capture_mean monotone increasing with limit_c in positive-bounce cells. Time-stop horizon trend reclassified as informative-INCONCLUSIVE per LESSONS B21. T31c re-run will use the corrected Check 3 logic.
- 2026-05-05 ET (Session 7 / T31a patch 8): layer_b_spec.md Check 3a metric refined from capture_mean to capture_p90. Patch 7 corrected the *direction* of Check 3 (limit policies, not time-stop, mirror Layer A MFE) but specified the wrong *metric* (capture_mean conflates fired MFE-style outcomes with unfired endpoint outcomes; in mean-reverting markets the unfired tail loss washes out the MFE signal at high limit_c thresholds). capture_p90 isolates the upper tail of the all-outcome distribution, dominated by fired trajectories whose captures are >= limit_c by construction. Pre-validation on disk: capture_p90 monotone increasing across +1c -> +30c limit thresholds in test-cell run (+0.08 -> +0.32). Patch 8 is a follow-on refinement to patch 7 within the same conceptual fix; both surfaced by T31c first-run FAIL and dry-run probes. Cross-reference: B21 (MFE vs endpoint distinction extends to mean-vs-tail of all-outcome distribution).
- 2026-05-05 ET (Session 7 / T31c PASS-verdict followup): T31c CLOSED. coherence_report.md (sha256 72f1747b) registered in MANIFEST data/durable/layer_b_v1 section + ANALYSIS_LIBRARY Layer B v1 entry. Layer B v1 validity status flipped PENDING T31c -> PASSED T31c (4/4 gating PASS, 1 informative-only INCONCLUSIVE). Layer B v1 outputs cleared for downstream Layer C (G11) consumption. T31a + T31b status flips and G11 -> T32 promotion deferred to separate ROADMAP-hygiene commit (Commit B, single-concern) per LESSONS C1 single-concern discipline; T31a (8 patches landed, spec stable), T31b (gamma-1 data path closed; gamma-2 visuals still pending), Section 8 NEXT MOVES rewrite, and the duplicate "1." numbering bug all bundle there.
- 2026-05-05 ET (Session 8 opening / ROADMAP hygiene commit): Post-T31c ROADMAP brought current. T31a CLOSED (8 patches landed, spec stable). T31b CLOSED-DATA-PATH (γ-1 done; γ-2 visuals deferred). G11 PROMOTED to T32 (gate T31c PASS satisfied at 2654a54); T32 entry added to T-section with v1/v2/v3/v4 layered scope per LESSONS B16. Section 8 NEXT MOVES rewritten: T32a/b/c as immediate sequence; G12 + T31b-γ-2 + T22 as parallel/deferred. Section 8 Post-T31 orphan subsection (with duplicate "1." numbering bug) deleted. Foundation-phase commit chain restructured from flat 7-entry list to 8 phased buckets (G9 → Layer A → T21 → Session 6→7 transition → T31a → T31b → T31c → Session 7 closure), 25 additional verified hashes added. Single concern: ROADMAP brought current to post-T31c reality.
- 2026-05-05 ET (Session 8 / T32 fee-canonical correction): ROADMAP T32 entry + Section 8 Immediate sequence bullets 1 & 3 brought into alignment with layer_c_spec.md Decision 1. Three sites corrected: T32 entry "both legs $0 fee" wording, T32a bullet "maker-zero confirmed by sample fill" wording, T32c Check 2 "maker fees == $0" wording. The maker-zero claim was empirically wrong — 37.7% of bot maker fills carry non-zero fee, and the parabolic Kalshi schedule applies to both makers and takers (with maker coefficient ~25% of taker per published rules + bot empirical: maker median $0.00 due to skewed distribution at extreme prices, taker median $0.17 at typical prices). Spec docs/layer_c_spec.md (commit 4bed07f) is canonical truth; ROADMAP is summary; this commit reconciles them. No new analysis claims, no T-item status changes.
- 2026-05-05 ET (Session 8 / commit Y — ROADMAP additions T33+T34+T35+G13 from LESSONS X1+X2): T33 added (OI reconstruction producer per F31; ~2-3hr from g9_trades; gates Layer C v2/v3 fill-probability). T34 added (combined-price distortion event detection producer per F32; ~30-60min from g9_trades; alpha-bearing direct signal, treated as producer-not-diagnostic per E27/E28 methodology drift discipline). T35 added (formation gate per-market field producer per F34; ~30min from g9_metadata + g9_trades; soft dep for T32c Check 5). G13 added (depth-chain orderbook-snapshot data collection per F33; blocked-track, not recoverable from existing data, requires new prospective Kalshi /orderbook polling). Section 8 Parallel/deferred update DEFERRED in this commit (regex mismatch on disk heading text "### Parallel / deferred" with spaces around slash + blank-line-aware match needed; landed in immediate follow-up). Cross-references close the loop with X1+X2 forward-pointers from B22/B23/B24 + F31/F32/F33/F34. No T-item status changes to existing items; T32 critical path (T32a CLOSED, T32b/c PENDING) unaffected. Single concern: ROADMAP additions mapping X1+X2 lessons to deliverable scope.
- 2026-05-05 ET (Session 8 / commit Y follow-up — Section 8 Parallel/deferred + CHANGELOG correction): Section 8 Parallel/deferred subsection updated with T33/T34/T35/G13 bullets (gap from c8eb017 closed). Plus CHANGELOG entry from c8eb017 corrected — original wording "updated with all four" was empirically wrong (regex skipped silently); replaced with accurate description of what landed and what was deferred. Two coordinated edits + new CHANGELOG entry. C29 family lesson reinforced: regex assumptions about disk text need probe-first verification, and CHANGELOG entries must describe what landed not what was intended.
- 2026-05-06 ET (Session 9 morning / T33 producer plan amendment + F31 amendment + C30 lesson): T33 producer scope simplified per F31 amendment. Original plan was ~2-3 hour reconstruction of total OI from g9_trades. Probe of g9_candles.open_interest_fp confirmed 96.4% match vs metadata across 19,687 markets — total per-minute OI is directly readable from existing candle column. Revised producer reads candle column for total OI (with historical-tier fallback to trade-tape) + uses trade-tape signed cumsum only for per-side directional flow (long-YES vs long-NO breakdown not exposed by total OI). Estimated runtime collapses to ~20-30 min. T33a/T33b/T33c chain shape preserved. T33c coherence read updated to verify candle-vs-metadata final-OI match + per-tier OI population reliability + sample-cell anchor reproduction (Dzumhur/Mannarino paired-event cross-ticker comparison, NOT within-ticker per-side). No T-item status changes; T34/T35/G13 unaffected. Single concern: T33 plan empirical correction.
