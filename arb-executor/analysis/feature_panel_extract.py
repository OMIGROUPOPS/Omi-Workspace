...
def span_months(pairs):
    """Archive partitions intersecting exact half-open event spans."""
    result=set()
    for pair in pairs.values():
        first=datetime.fromtimestamp(pair["formation"],ET)
        last=datetime.fromtimestamp(pair["bell"],ET)
        end_index=last.year*12+last.month-1
        if pair["bell"]>datetime(last.year,last.month,1,tzinfo=ET).timestamp():
            end_index+=1
        for index in range(first.year*12+first.month-1,end_index):
            year,month=divmod(index,12)
            result.add(f"{year:04d}-{month+1:02d}")
    return result
...
def run_supplemental(args):
    ...
    source_manifest=[];archive_partitions=[]
    with closing(sqlite3.connect(index_path)) as index:
        ...
        if args.odds_dir:
            if not args.odds_receipt and not args.record_odds_hashes_only:
                raise ValueError("ODDS_RECEIPT_REQUIRED")
            if args.record_odds_hashes_only:
                # No desktop audit metadata is transferred. These are new
                # read-only source observations, not a historical pin claim.
                months=span_months(pairs)
                bindings={}
                for path in sorted(args.odds_dir.glob("*.parquet")):
                    footer=pq.ParquetFile(path);column=footer.schema.names.index("polled_at")
                    stats=[footer.metadata.row_group(i).column(column).statistics for i in range(footer.metadata.num_row_groups)]
                    if not stats or any(not(item and item.has_min_max) for item in stats):
                        raise ValueError("ODDS_PARTITION_TIMESTAMP_BOUNDS_MISSING:"+path.name)
                    lower_stamp=min(item.min for item in stats);upper_stamp=max(item.max for item in stats)
                    if lower_stamp[:7]!=path.stem or upper_stamp[:7]!=path.stem:
                        raise ValueError("ODDS_PARTITION_CLOCK_MISMATCH:"+path.name)
                    included=path.stem in months
                    archive_partitions.append(dict(path=str(path),min_polled_at=lower_stamp,max_polled_at=upper_stamp,
                        selected=included,rule="Verified footer month intersects exact selected half-open library spans"))
                    if included:
                        bindings[path.name]=dict(path=str(path),sha256=sha256_file(path))
                if not bindings:
                    raise ValueError("NO_REMOTE_ODDS_ARCHIVES")
            else:
            ...
            for name,binding in sorted(bindings.items()):
                ...
                for batch in pf.iter_batches(batch_size=32768,columns=["event_ticker","polled_at"]):
                    ...
                    if any(row["polled_at"][:7]!=path.stem for row in selected_rows):
                        raise ValueError("ODDS_RETAINED_ROW_PARTITION_MISMATCH:"+name)
                    unique={(row["event_ticker"],datetime.fromisoformat(row["polled_at"]).replace(tzinfo=ET).timestamp()) for row in selected_rows}
                ...
                if sha256_file(path)!=binding["sha256"]:
                    raise ValueError("ODDS_ARCHIVE_CHANGED_SHA:"+name)
                source_manifest.append(dict(kind="ODDS_POLL_AVAILABILITY",path=str(path),sha256=binding["sha256"],
                    ...
                    sha256_start=binding["sha256"],sha256_end=binding["sha256"],
                    historical_binding=("REMOTE_HASH_RECORDED_REQUIRES_LOCAL_PIN_VALIDATION" if args.record_odds_hashes_only else "FILED_ARCHIVE_HASH_VERIFIED"),
        ...
        with partial.open("xb") as raw,gzip.GzipFile(filename="",mode="wb",fileobj=raw,mtime=0) as zipped:
            for ticker,leg in specs.items():
                ...
                row=dict(leg,schema="FEATURE_SUPPLEMENTAL_AVAILABILITY_V1",
                    ...
                    odds_poll_epochs=odds,odds_status=("REMOTE_HASH_RECORDED_REQUIRES_LOCAL_PIN_VALIDATION" if args.record_odds_hashes_only else "EXACT_EVENT_IN_SPAN_ARCHIVE_POLLS") if odds else ("NO_EXACT_EVENT_POLLS_IN_READ_ARCHIVES" if args.odds_dir else "NOT_EXTRACTED"),
        ...
        result=dict(status="REMOTE_HASH_RECORDED_REQUIRES_LOCAL_PIN_VALIDATION" if args.record_odds_hashes_only else "VERIFIED",schema="FEATURE_SUPPLEMENTAL_AVAILABILITY_V1",counts=dict(counts),
            library_sha256=sha256_file(args.library),extractor_sha256=sha256_file(Path(__file__)),
            sources=source_manifest,archive_partition_selection=archive_partitions,limit_legs=args.limit_legs,
            output=dict(path=str(destination),sha256=sha256_file(partial),bytes=partial.stat().st_size),
...
def parser():
    ...
    p.add_argument("--record-odds-hashes-only",action="store_true",help="Read remote originals and record hashes; desktop must validate against its local audit receipt before using odds fields")
    return p
