#!/usr/bin/env python3
"""
NCAAMB Sell-The-Bounce (STB) — Live Trading Module
====================================================
Buy dislocation dips on NCAAMB match markets, exit +7c via resting sell.
20-minute time stop: if resting sell hasn't filled after 20m, market sell at bid.
Standalone module — does NOT touch tennis_stb.py or swing_ladder.py.
"""
...
class Position:
    ...
    filled: bool = False
    settled: bool = False
    time_stopped: bool = False   # True if exited via time stop
...
class NcaambSTB:
    ...
    def check_ncaamb_game_state(self, ticker: str, game_data: dict) -> Optional[str]:
        """Check NCAAMB game state filters. Returns rejection reason or None if OK."""
        live = game_data.get("live_data", {})
        details = live.get("details", {})
        status = details.get("status", "").lower()

        # Reject ended/completed games
        if status in ("ended", "completed", "closed", "finished"):
        ...
        # Allow halftime explicitly
        if status == "halftime":
            return None

        # Get score and period info
        try:
            away_pts = int(details.get("away_points", 0) or 0)
            home_pts = int(details.get("home_points", 0) or 0)
        except (ValueError, TypeError):
            away_pts = 0
            home_pts = 0

        period = details.get("period", "")
        remaining = details.get("period_remaining_time", "")

        # Reject if trailing by 15+ in 2nd half (period 2)
        try:
            period_num = int(period) if period else 0
        except (ValueError, TypeError):
            period_num = 0
        ...
        if period_num >= 2:
            ...
            if diff >= 15:
                return f"down {diff}pts in 2H ({away_pts}-{home_pts})"

            # Reject if under 3 minutes game clock in period 2
            if remaining:
                try:
                    ...
                    if total_secs < 180:  # 3 minutes
                        return f"under 3min game clock ({remaining})"
                except (ValueError, IndexError):
                    pass

        # Reject period 1 entries UNLESS score differential >= 5
    ...
    async def discover_markets(self):
        """Find all open NCAAMB match markets. Filters out pre-game markets."""
        all_tickers = []
        now = time.time()
        skipped_pregame = 0
        for series in SERIES:
            ...
            while pages < 10:
                ...
                for m in markets:
                    ...
                    # Store/update expiry for pre-game filter
                    ...
                    # Pre-game filter: skip if expiry > MAX_HOURS_TO_EXPIRY from now
                    if expiry_epoch:
                        hours_until = (expiry_epoch - now) / 3600.0
                        if hours_until > MAX_HOURS_TO_EXPIRY:
                            side = ticker.split("-")[-1]
                            if event_ticker not in self.known_events:
                                log(f"[SKIP_PREGAME] {side} expires={expiry_str[:16]} "
                                    f"hours_until={hours_until:.1f} — game not started")
                            skipped_pregame += 1
                            continue
                ...
                if not cursor:
                    break
        if skipped_pregame:
            log(f"[DISCOVERY] Skipped {skipped_pregame} pre-game tickers "
    ...
    async def execute_entry_92plus(self, ticker: str):
        ...
        # Depth filter: require depth_ratio_5c >= 0.15
        ...
        result = await api_post(self.session, self.api_key, self.private_key,
                                order_path, payload, self.rl)
        if not result or result.get("error"):
            err = result.get("error", {}) if result else {}
            log(f"[92+_BID_FAIL] {side} price={maker_price}c err={err}")
            return
        ...
        sell_target = 99  # settlement play -- resting sell at 99c
        ...
        pos = Position(
            ...
            contracts=CONTRACTS_92PLUS if buy_confirmed else 0,
        ...
        dr5_str = f"{dr5:.3f}" if isinstance(dr5, (int, float)) else "?"
        log(f"[92+_BID_POSTED] {now} {et} {side} maker_bid={maker_price}c "
            f"ask={ask}c bid={bid}c spread={spread}c "
            f"combined_mid={combined_mid:.1f}c depth_ratio_5c={dr5_str} "
            f"target_sell=99c oid={order_id[:12]} status={status}")

        if buy_confirmed:
            # Maker bid filled immediately (unlikely but handle it)
            pos.contracts = CONTRACTS_92PLUS
            self._csv_write_entry(pos)
            await self.place_exit_sell(ticker)
        else:
            log(f"  [92+_RESTING] Maker bid resting at {maker_price}c, "
                f"sell posts on fill")
    ...
    async def execute_entry(self, ticker: str, is_reentry: bool = False, original_price: int = 0):
        ...
        if game_data:
            ...
            away_pts = details.get("away_points", "?")
            home_pts = details.get("home_points", "?")
            period = details.get("period", "?")
            remaining = details.get("period_remaining_time", "?")
            possession = details.get("possession", "?")
            game_state_log = (
                f" score={away_pts}-{home_pts} period={period}"
                f" clock={remaining} possession={possession} status={status}"
            )
        ...
        maker_payload = {
            ...
            "yes_price": maker_bid_price, "post_only": True,
            "client_order_id": str(uuid.uuid4()),
        }
            ...
            if maker_status == "executed":
                ...
                if chk_status == "executed":
                    ...
                    await api_delete(self.session, self.api_key, self.private_key,
                                    del_path, self.rl)
                    if chk_fills > 0:
                        log(f"[MAKER_PARTIAL] {side} bid={maker_bid_price}c {chk_fills}/{CONTRACTS}ct filled")
                        result = maker_result
                        result["order"] = chk_order
                        ask = maker_bid_price
                    else:
        ...
        entry_fee = max(1, CONTRACTS // 5) if entry_type == "maker" else CONTRACTS
        ...
        pos = Position(
            ...
            entry_ask=ask, entry_bid=bid, entry_spread=spread,
            combined_ask=int(combined_mid), entry_ts=time.time(),
            contracts=CONTRACTS, sell_price=ask + EXIT_BOUNCE,
            buy_order_id=order_id, buy_confirmed=buy_confirmed,
            buy_fill_ts=buy_fill_ts,
        )
        ...
        log(f"[ENTRY] {now} {et} {side} ask={ask}c bid={bid}c "
            ...
            f"buy_status={status} entry_type={entry_type}")
    ...
    async def check_time_stops(self):
        """Conditional time stop for all NCAAMB positions.
        ...
        Race condition safety: we set pos.time_stopped = True FIRST (before
        any async calls) so check_fills() won't act on this position concurrently.
        """
        now = time.time()
    ...
    async def _cancel_stale_buy(self, ticker: str, pos, elapsed: float):
        ...
        if chk_result:
            ...
            if status == "executed":
                ...
                expected_ct = CONTRACTS_92PLUS if pos.entry_mode == "stb_92plus" else CONTRACTS
        ...
        if fill_count > 0:
            ...
            expected_ct_r = CONTRACTS_92PLUS if pos.entry_mode == "stb_92plus" else CONTRACTS
            ...
            if book and remaining > 0:
                ...
                if price_ok:
                    ...
                    log(f"  [PARTIAL_FILL] {pos.side} — {fill_count}ct filled of {CONTRACTS} "
                ...
                if result:
                    ...
                    if status == "executed":
                        ...
                        if fill_count and fill_count > 0:
                            ...
                            pos.filled = True
                        continue
                # Don't check sell or settlement until buy is confirmed
                continue


            # --- 92c+ stop loss DISABLED (maker entries hold to settle) ---
            if False and pos.buy_confirmed and pos.entry_mode and "92plus" in pos.entry_mode:
                book = self.books.get(ticker)
                current_bid = book.best_bid if book and book.best_bid is not None else None
                if current_bid is not None and current_bid <= pos.entry_ask - 10:
                    log(f"[92+_STOP] {pos.side} entry={pos.entry_ask}c bid={current_bid}c "
                        f"loss={pos.entry_ask - current_bid}c — stop triggered at -10c")
                    # Check if resting sell already filled before cancelling
                    if pos.sell_order_id:
                        chk_path = f"/trade-api/v2/portfolio/orders/{pos.sell_order_id}"
                        chk_result = await api_get(self.session, self.api_key, self.private_key,
                                                   chk_path, self.rl)
                        if chk_result:
                            ...
                            if status == "executed":
                                ...
                                hold_sec = pos.exit_ts - pos.entry_ts
                                now_str = time.strftime("%H:%M:%S")
                                log(f"[FILL_BEAT_92STOP] {now_str} {pos.event_ticker} {pos.side} "
                                    f"entry={pos.entry_ask}c sold={pos.sell_price}c "
                                    f"profit={profit - pos.contracts}c — sell filled before stop")
                                self.log_trajectory(pos, "WIN")
                                await self._csv_write_exit(pos, "fill_7c", pos.sell_price, profit - pos.contracts)
                                self.resting_sells.pop(ticker, None)
                                await self.print_balance()
                        ...
                        await api_delete(self.session, self.api_key, self.private_key,
                                         del_path, self.rl)
                        self.resting_sells.pop(ticker, None)
                    # Aggressive taker sell at bid-2c
                    aggressive_price = max(1, current_bid - 2)
                    sell_path = "/trade-api/v2/portfolio/orders"
                    sell_payload = {
                        "ticker": ticker,
                    ...
                    if result and not result.get("error"):
                        ...
                        hold_sec = pos.exit_ts - pos.entry_ts
                        now_str = time.strftime("%H:%M:%S")
                        log(f"[92+_STOP] {now_str} {pos.event_ticker} {pos.side} "
                            f"entry={pos.entry_ask}c exit={current_bid}c (limit={aggressive_price}c) "
                            f"pnl={net_pnl}c net hold={hold_sec:.0f}s ({hold_sec/60:.1f}m) "
                            f"sell_status={sell_status}")
                        self.log_trajectory(pos, "STOP_92PLUS")
                        await self._csv_write_exit(pos, "stop_10c", current_bid, net_pnl)
                        await self.print_balance()
                    else:
                        err = result.get("error", {}) if result else {}
                        log(f"  [92+_STOP_SELL_FAIL] {ticker} bid={current_bid}c "
                            f"limit={aggressive_price}c err={err}")
                    continue
            ...
            if pos.sell_order_id:
                ...
                if result:
                    ...
                    if status == "executed":
                        ...
                        profit = (pos.sell_price - pos.entry_ask) * pos.contracts
                        # Maker exit = 0 fee
                        self.total_pnl_cents += profit - (pos.contracts)  # subtract entry fee
                        self.total_wins += 1
                        hold_sec = pos.exit_ts - pos.entry_ts
                        now = time.strftime("%H:%M:%S")
    ...
    def _csv_write_entry(self, pos):
        ...
        row = {
            ...
            "ticker": pos.ticker,
            "series": series,
            "sport": self._detect_sport(pos.ticker),
    ...
    async def _csv_write_exit(self, pos, exit_type, exit_price, pnl_cents):
        ...
        if updated:
            ...
            row_str = (
                ...
                f"{self._detect_series(pos.ticker)},{self._detect_sport(pos.ticker)},{pos.entry_ask},{pos.side},"
    ...
    async def print_balance(self):
        ...
        if bal is not None:
            ...
            log(f"  [BAL] ${bal:.2f}  P&L: ${pnl_dollars:+.2f}  "
                ...
                f"time_stops={self.total_time_stops} entries={self.total_entries}")
            ...
            for t, pos in self.positions.items():
                if not pos.filled and not pos.settled and not pos.time_stopped:
                    ...
                    log(f"    {pos.side} entry={pos.entry_ask}c bid_now={bid_now}c "
                        f"target={pos.sell_price}c hold={hold:.0f}m{time_left}")
        log("")
            ...
            if partner:
                ...
                if (book and pbook and book.best_ask is not None
                    ...
                    if combined_int < prev_min:
                        ...
                        if combined_int <= 98:
                            side = ticker.split("-")[-1]
                            log(f"[LOW_COMBINED] {et[-25:]} combined_mid={combined_mid:.1f}c "
            ...
            if book92 and book92.best_ask is not None and book92.best_ask >= 95:
                ...
                if not (et92 and et92 in self.entered_events):
                    ...
                    if not skip_expiry:
                        ...
                        elif self.sustained_90_ticks.get(ticker, 0) > 0:
                            side92 = ticker.split("-")[-1]
                            log(f"[92+_WAIT] {side92} ask={book92.best_ask}c "
                                f"sustained={self.sustained_90_ticks[ticker]}/5")
            elif book92 and book92.best_ask is not None and book92.best_ask < 90:
                ...
                if ticker in self.positions:
                    pos90 = self.positions[ticker]
                    if pos90.entry_mode == "stb_92plus" and not pos90.buy_confirmed:
                        side90 = ticker.split("-")[-1]
                        log(f"[92+_BID_CANCEL] {side90} ask dropped to "
                            f"{book92.best_ask}c < 90c -- cancelling maker bid")
                        try:
                            del_path = f"/trade-api/v2/portfolio/orders/{pos90.buy_order_id}"
                            await api_delete(self.session, self.api_key, self.private_key,
                                             del_path, self.rl)
                        except Exception as e:
                            log(f"[92+_BID_CANCEL_ERR] {side90}: {e}")
                        # Clean up position and dedup sets
                        self.entered_sides.discard(ticker)
                        et90 = self.ticker_to_event.get(ticker, "")
                        if et90:
                            self.entered_events.discard(et90)
                        self.mode_92_entered.discard(ticker)
                        del self.positions[ticker]
    ...
    async def discovery_loop(self):
        """Periodically discover new NCAAMB markets and subscribe."""
        while True:
            try:
    ...
    async def run(self):
        ...
        log(f"  Game state: reject 15+pt deficit in 2H, <3min clock, ended/scheduled")
        log("=" * 60)
        ...
        try:
            ...
            # Cancel ALL resting NCAAMB orders before anything else
            ...
            # Launch all loops (includes time_stop_loop)
    ...
    async def cancel_all_resting_orders(self):
        """Cancel ALL resting orders on NCAAMB tickers. Called once at startup."""
        cancelled = 0
        cursor = ""
        pages = 0
        while pages < 10:
            ...
            for o in orders:
                oid = o.get("order_id", "")
                ticker = o.get("ticker", "")
                is_ncaamb = any(ticker.startswith(s) for s in SERIES)
                if not is_ncaamb:
                    continue
        ...
        if cancelled:
            ...
            log("[INIT] No resting NCAAMB orders to cancel")
    ...
    async def rebuild_entered_sides(self):
        ...
        while pages < 10:
            ...
            for f in fills:
                ...
                if action == "buy" and ticker not in self.entered_sides:
                    self.entered_sides.add(ticker)
                    # Fix 3: Also rebuild entered_events
                    evt = self.ticker_to_event.get(ticker, "")
                    if evt:
                        self.entered_events.add(evt)
                    recovered += 1
            cursor = result.get("cursor", "")
            pages += 1
            if not cursor:
        ...
        if recovered:
            log(f"[INIT] Recovered {recovered} entered sides from fill history: "
                f"{', '.join(t.split('-')[-1] for t in sorted(self.entered_sides))}")
        else:
            log("[INIT] No previous NCAAMB entries found in fill history")
    ...
    async def reconcile_existing_positions(self):
        """Check for existing Kalshi positions in NCAAMB markets at startup.
        ...
        for p in result.get("market_positions", []):
            ...
            # Check if this is a NCAAMB ticker
            is_ncaamb = any(ticker.startswith(s) for s in SERIES)
            if not is_ncaamb:
                continue
