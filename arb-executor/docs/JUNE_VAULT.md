...
**GRADE REFRESH (2026-07-02, +overnight slate):** re-ran on **122 events / 233 legs** — completion **27%→48%**, but combined **med 99→100** with **31/58 both-fills now ≥100** (zero/negative locked edge), realized **−$11→−$55** (64 legs settled). The walk still carries **71%** of both-fills but now to **par**, not sub-par: more volume, worse quality. (Gap metric med +9¢ flagged corrupted by low-print tape artifacts; trust completion↑ / combined↑ / P&L↓.)

### 4H addendum #9 — FIX #1 BUILD: C-KALSHI-OCC observe-only mode (2026-07-02) — STAGED, not armed
Fix #1 of the §4H settled-conclusion queue. **Code + tests live on the deploy branch `blend/kalshi-occ-fallback` @ `c8d7b336`** (which carries the C-KALSHI-OCC base; disjoint from this canonical `agent-derivation` history — its `JUNE_VAULT.md` is now a stub pointing back here). New gated flag **`kalshi_occ_observe` (default OFF = byte-identical; live_v4.py +30/−0, purely additive)**. When ON it is **pure-observe** — sets no start, opens no envelope, places no order: (1) on the primary-miss path it logs `kalshi_occ_observe {event, category, occurrence_datetime, would_coarse_start, would_resolve, would_trade}`; (2) for **every resolved** event it logs `kalshi_occ_delta {resolver_source, resolver_start, kalshi_occurrence, delta_sec}` — measuring whether **Kalshi-as-PRIMARY diverges from the ESPN chain** (if deltas are negligible → the follow-up promotes Kalshi start to primary and removes ESPN from the entry path = operator's stated end-state). Once per event. **`tests/test_kalshi_occ_observe.py` — 19 checks, all pass**: flag-off no-op, AST-proven zero-state-mutation + zero-order-path, correct log fields, once-per-event. **⚠ DEPLOY NEEDS A RESTART (live positions on the book) — operator's call; NOT armed. HOLD.** Unpulled-before-restart note: VPS was at `01dcc8b`, origin deploy-branch tip was `d9f001ae` = docs-only (`OMQS_ADVERSE_SELECTION_STRANDED.md`, no code) — my build sits on top at `c8d7b336`.

### 4H addendum #6 — P6 CAPPED-WALK REPLAY + CEILING SWEEP (2026-07-02) — the walk-augmented test addendum #4 demanded
The walk-augmented replay (`analysis/stranded/OMQS_P6_CAPPED_WALK.md`, `p6.py`). Drives the bot's **actual** logged walk (`v4_place` + every `v4_move_repost.new_target`), imposes a **combined cap by construction** (leader clamped `≤ CAP − followerTgt(t)`), replays fills on the L1 tape, hold-to-settle. Swept `CAP ∈ {95…99, uncap}` over **111 paired events / 1.95 d**. **VALIDATION PASS:** uncapped reproduces the box (combined med 99, 70% in 98-100, 44.6 comp/day) → walk model calibrated, axes judgeable.
- **VERDICT @ cap-97 = FAIL → CLOSURE FIRES (pre-committed).** axis① ≥25/day **PASS (31.3)**; axis② median≤95 & no 96-97 pile **FAIL (median 97, 54/61 piled at 96-97 — the exact named failure mode)**; axis③ signature reproducible. **The static combined-cap-at-97 walk branch is CLOSED.**
- **INVARIANT 6 (the cap is a clamp, not a price-improver):** median combined **= the cap** (cap-95→med95, cap-97→med97, cap-99→med99); fills pile at cap−0/−1. The cap does NOT deepen fills or find cheaper prints — it guarantees `combined ≤ cap` by clamping the leader, at a **monotonic completion cost (44.6→29.8/day as 99→95)**. "Sub-par median" is a **restatement of the cap level**, not discovered edge. (The lone combined-**80** double-fill appears at every cap = genuinely cheap on tape, cap-independent.)
- **INVARIANT 7 (locked edge is exit-policy-gated):** E1 hold-to-settle edge is real and **monotonic in the cap (+$4.20 uncap → +$15.35 at cap-95, ≥25/day throughout)** — but under the **live band-sell exit (E2) the *entire* frontier is P&L-negative (−$6→−$18)** (the FUCKUP-3 winner-cap/loser-hold defect); positive only under hold (E1) or asymmetric winner-ride/loser-stop (E3). **Capping without changing the exit converts locked edge into a loss.** (E2/E3 underpowered, n=4-12 settled; E1 analytic/robust; sign matches the −$55 grade + FUCKUP-3 ledger.)
- **CONTINGENT CLOSURE (Plex's rule):** cap-97 refuted on its own signature → **closed.** One orthogonal frontier exception (cap-95 clears axis② at 29.8/day, E1 +$15.35) exists, but per INV-6 it's a tautology of the cap level and per INV-7 its realization **requires abandoning the live band-sell exit** — an **exit-policy** result, **not** a cap vindication, and **the operator's call, not silent continuation.** ~~Convergent with P5: the next lever is the EXIT~~ **SUPERSEDED by §0A standing order (2026-07-02): exits are OUT OF SCOPE; P6's E2-negative reading is an artifact of broken entries, struck as exit evidence. The next lever is the ENTRY — fill both sides ≤97.**

### 4H addendum #8 — C-KALSHI-OCC OBSERVE (read-only, 2026-07-02) — the never-touched ≤97 slate
`analysis/stranded/OMQS_KALSHI_OCC_OBSERVE.md` (+ `.json`, `kalshi_occ_observe.py`). The gated coarse-start fallback in `live_v4.py` (`kalshi_occurrence_fallback`, default-OFF) was **NOT armed** — its only arm is a live-entry arm and the bot holds live positions; per the operator's "do NOT place orders," this is a **read-only measurement** of what it *would* resolve. **(a) Resolvability:** 82 schedule_gap events → **57 book-bearing (occ-resolvable): ITF_W 33 · ITF_M 18 · ATP_CHALL 6**; 25 thin/excused; 0 historically occ-resolved (flag confirmed OFF all box). **(b) Loop-lag:** current loop_lag mean 1.91s / p95 4.26s / **max 19.16s**; **the 57 resolvable events = 112,591 skip-logs = 92% of all schedule_gap-spam** → resolving them strips 92% of the loop's gap-churn (exact after-lag deferred to an armed observe run). **(c) Byte-identical:** 1,027 events resolved via primary; the occ-fallback is Fallback-2 inside the primary-miss `else`, flag consulted only there → resolved events take the branch above it, on/off byte-identical (11 book_gap events also resolved late via primary → occ helps only pre-resolution). **CENSUS (the headline):** of the 57, 51 have a computable combined and **48 (94%) offered achievable combined ≤97 — median 72** (ATP_CHALL 6/6 med60 · ITF_M 14/15 med76 · ITF_W 28/30 med76). **The single largest pool of un-captured ≤97 entries found — the exact §0A entry target, forfeited entirely by schedule_gap.** Top: KHODEL 20 / KOBMAX 21 / WOLHAR 22 … (sub-30 tail leans on the low-print artifact; robust signal = 48/51 ≤97 aggregate + mid-range 48-65). **Argues directly for arming C-KALSHI-OCC (observe form / next slate — operator's call).** Then **HOLD**.

### 4H addendum #7 — ERROR LEDGER, current box, ENTRY-SIDE (2026-07-02)
`analysis/stranded/OMQS_ERROR_LEDGER_CURRENT.md` (+ `.json`, `ledger.py`). 6-category, per-game, dual-anchor timing (T-minus vs scheduled start AND true tape-onset), letter-graded, mechanically-named errors, dollar-sorted. **122 graded POSITIONS** of 548 box ticker-events. **Grades A:2 · B:25 · C:31 · D:38 · F:26** — only **2 clean A** (both ≤97 near-fillable) in the whole box; **64/122 never completed** (D+F); **31 C = combined ≥100 locked loss.** **Entry-side damage $88.15, and 76% of it is OVERPAY**: `posted-over-fillable` **52 games / $37.40** + `posted-late/over-fillable` **20 / $30.00** = **$67.40**; then the stale-clock cancels `pulled-by-t20m` **12 / $12.15** + `pulled-by-match-live` **4 / $1.05**; `blocked-by-volume-floor` **4 / $5.70**. **The two entry problems, quantified:** we post ABOVE the size-backed fillable level (→ combined ≥100 / ≥97) and the t20m/match-live clock pulls worked bids before the tape dips to them. **Census (→ ITEM 3):** 82 schedule_gap-skipped events, **57 book-bearing** (ITF_W 33 / ITF_M 18 / ATP_CHALL 6) never touched. WTA_CHALL had 0 events in box. ⚠ newest-date (26JUL02+) rows are provisional (matches not yet started).
