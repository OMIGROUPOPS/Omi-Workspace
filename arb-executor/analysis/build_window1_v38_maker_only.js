...
const AIM_ERROR_CENSUS_COMMIT = "17664b3fb6f6cafbd9260487054941155056f23c";
...
const isV49b = variant === "v49b";
const isV51 = variant === "v51";
const isTradeTruthVariant = isV48 || isV49 || isV49b || isV51;
...
function safeOutput(dir) {
  ...
  ensure(path.basename(resolved).includes(isV51 ? "v51" : isV49b ? "v49b" : isV49 ? "v49" : isV48 ? "v48" : isV47 ? "v47" : isV46 ? "v46" : isV45 ? "v45" : isV43 ? "v43" : isV42 ? "v42" : isV41 ? "v41" : isV40 ? "v40" : isV39 ? "v39" : "v38"), `unsafe output ${resolved}`);
...
function simulate(base, tapes, prints, mode, clauses = {}) {
  ...
  for (const row of timeline) {
    ...
    if (row.kind === "PRINT") {
      leg.cumulative_volume_lots += Number.isFinite(row.size) ? row.size : 0;
    ...
    if ((isV49b || isV51) && Number.isInteger(doctrineP)) {
    ...
    const doctrineStanding = (isV49b || isV51) && leg.v49b_doctrine ? { ...leg.v49b_doctrine, authorized: doctrineEvidence.length > 0, causal_evidence: doctrineEvidence, evaluated_timestamp_epoch: row.ts, evaluated_receipt: row.receipt } : null;
    const continuityDoctrine = isV51 && leg.v51_continuity ? { ...leg.v51_continuity, authorized: true, causal_evidence: [{ source: "AIM_ERROR_CENSUS_EVIDENCE_NAMED_P", binding_commit: AIM_ERROR_CENSUS_COMMIT, P_source: leg.v51_continuity.P_source }], evaluated_timestamp_epoch: row.ts, evaluated_receipt: row.receipt } : null;
    const placementInputs = { state: combined.state, book: row, priorAsk: prior?.ask ?? null, askGapCents, activeTarget: before, pairCap: leg.pair_cap_cents, pulseFloor: pulse.floor_cents, persistentJoinLevel: isPlacementStack ? leg.persistent_join_level : null, wtaInverseFalling, causalOwnReachLow, activeEvidenceFloor, floorFirstFlickerLive: activeEvidenceFloor === leg.running_qualified_ask_low && leg.running_qualified_ask_low_unabsorbed, floorMature, recentTradeLow, priorTrueTradeLow: leg.prior_true_trade_low_cents, priorTrueTradeLowReceipt: leg.prior_true_trade_low_receipt, priorExactBidEvidence, evidencedStandingLevel: leg.evidenced_standing_level_cents, evidencedStandingAuthority: leg.evidenced_standing_authority, doctrineStanding, continuityDoctrine, cumulativeVolumeLots: leg.cumulative_volume_lots, siblingBestAsk: normalizedClauses.deep_gap_guard ? (sibling.prior_book?.ask ?? null) : undefined, siblingEntryCents: sibling.entry_cents, siblingCredited: sibling.credited, clauses: normalizedClauses };
    ...
    if (isV51) {
      leg.continuity_evaluations += 1;
      if (decision.continuity_of_standing?.applied) {
        leg.continuity_applied_receipts += 1;
        if (decision.continuity_of_standing.incumbent_reason?.includes("WITHHOLD")) leg.continuity_s12_overrides += 1;
        else leg.continuity_walk_away_overrides += 1;
        const continuityReceipt = { ...clockFields(row.ts, base), receipt: row.receipt, order_before_cents: before, order_after_cents: decision.target_cents, flow: decision.continuity_of_standing.flow, incumbent_action: decision.continuity_of_standing.incumbent_action, incumbent_reason: decision.continuity_of_standing.incumbent_reason };
        leg.continuity_first ||= continuityReceipt;
        leg.continuity_last = continuityReceipt;
      }
    }
...
async function main() {
  ...
  const identity81Bytes = (isV49b || isV51) ? gitShow(IDENTITY_81_COMMIT, identity81Path) : null;
  const identity81 = (isV49b || isV51) ? JSON.parse(identity81Bytes) : null;
  const v51AimPath = ".claude/window1_second_seat/v11_non_action_mechanism_audit_20260803/AIM_ERROR_CENSUS.json";
  const v51AimBytes = isV51 ? gitShow(AIM_ERROR_CENSUS_COMMIT, v51AimPath) : null;
  const v51Aim = isV51 ? JSON.parse(v51AimBytes) : null;
  const v51AimByKey = isV51 ? new Map(v51Aim.rows.map((row) => [`${row.event}|${row.leg}`, row])) : new Map();
  if (isV51) {
    ensure(v51Aim.conservation.rrwp_legs === 362 && v51Aim.conservation.scored === 355 && v51AimByKey.size === 355, "AIM_ERROR_CENSUS target conservation failed");
    ensure(v51Aim.predictability.d_volume_tier[">10k"].n === 61, "AIM_ERROR_CENSUS elevated tier changed");
  }
  ...
  for (const span of spans) {
    ...
    for (const leg of span.per_leg) {
      ...
      const aimRow = v51AimByKey.get(`${span.event_id}|${legId}`);
      base.legs[legId] = { leg_id: legId, leg_identity: leg.leg_identity, ticker: leg.ticker, category: span.category, price_region: prior.price_region, leg_direction: reach.leg_direction, reach, ...((isV49b || isV51) ? { v49b_doctrine: v49bDoctrineByEventLeg.get(`${span.event_id}|${legId}`) ?? null } : {}), ...(aimRow ? { v51_continuity: { identity_key: `${span.event_id}|${legId}`, source_event: span.event_id, leg_id: legId, level_cents: aimRow.P, P_source: aimRow.Psrc, frozen_volume_tier: aimRow.vol } } : {}) };
  ...
  if (isV49b || isV51) ensure([...baseByEvent.values()].flatMap((base) => Object.values(base.legs)).filter((leg) => leg.v49b_doctrine).length === 93, "V49b doctrine identities did not bind to replay legs");
  if (isV51) ensure([...baseByEvent.values()].flatMap((base) => Object.values(base.legs)).filter((leg) => leg.v51_continuity).length === 355, "V51 continuity identities did not bind to replay legs");
  const machineSpecs = isV51 ? [
    { name: "V49B_FROZEN_BASELINE", market_mode: "MARKET_TRADES_AS_TRUTH", clauses: { arm_at_first_evidence: true, deep_gap_guard: true, loosen_one_cent: true, release_guard_on_sibling_credit: true, same_tick_arm: true, trades_as_truth: true, faithful_stand_at_p: true, continuity_of_standing: false } },
    { name: "V51_CONTINUITY_OF_STANDING", market_mode: "MARKET_TRADES_AS_TRUTH", clauses: { arm_at_first_evidence: true, deep_gap_guard: true, loosen_one_cent: true, release_guard_on_sibling_credit: true, same_tick_arm: true, trades_as_truth: true, faithful_stand_at_p: true, continuity_of_standing: true } },
  ] : isV49b ? [
  ...
  let index = 0;
  for (const base of [...baseByEvent.values()].sort((a, b) => a.event_id.localeCompare(b.event_id))) {
    index += 1; if (index % 50 === 0) process.stderr.write(`${isV51 ? "V51x2" : isV49b ? "V49bx2" : isV49 ? "V49x2" : isV48 ? "V48x5" : isV47 ? "V47x2" : isV46 ? "V46x2" : isV45 ? "V45x2" : isV43 ? "V43x8" : isV42 ? "V42" : isV41 ? "V41" : isV40 ? "V40" : isV39 ? "V39" : "V38"} replay ${index}/804\n`);
  ...
  const namedLabels = (isV49 || isV49b || isV51) ? ["HERKAZ", "ARNROM", "KIRSEK", "KRUFER", "BOSCOP", "PANFAL"] : isV48 ? ["LUZTSE", "SALIBR", "ARNROM", "KIRSEK", "KRUFER", "BOSCOP", "PANFAL"] : isV47 ? ["SURECH", "ARNROM", "KIRSEK", "KRUFER", "BOSCOP", "PANFAL"] : isV46 ? ["PANFAL", "ARNROM", "KIRSEK", "KRUFER", "BOSCOP"] : isV45 ? ["LUZTSE", "COLCER", "SMIYUN", "VANLEE", "SAINUG", "PENTHA", "SHEOLI", "ARNROM", "KRUFER", "KIRSEK"] : isV43 ? ["KIRSEK", "ARNROM", "KRUFER", "BOSCOP", "PUTJEA", "BORDIM", "ROCBUE", "KREZHE"] : isV42 ? ["PUTJEA", "ROCBUE", "KREZHE", "BORDIM", "ARNROM"] : isV41 ? ["ARNROM", "BOSCOP", "NIKVRB", "WESPAA", "KRUFER"] : ["ARNROM", "BOSCOP", "WESPAA", "NIKVRB", "GANJAN"];
  const named = {};
  ...
  // V51's requested evidence is the 355-row continuity ledger plus the two
  // scorecard rows.  Re-emitting four full retained-state attribution ledgers
  // duplicates frozen V49b/V51 event ledgers and exceeds the bounded builder
  // memory envelope without adding a V51 decision receipt.
  if (isAttribution && !isV51) {
  ...
  const standabilityBytes = (isV49 || isV49b || isV51) ? gitShow(STANDABILITY_V2_COMMIT, standabilityPath) : null;
  const standabilityMdBytes = (isV49 || isV49b || isV51) ? gitShow(STANDABILITY_V2_COMMIT, standabilityMdPath) : null;
  ...
  const standabilityReceipt = (isV49 || isV49b || isV51) ? JSON.parse(standabilityBytes) : null;
  ...
  const control = isV51
    ? { schema_version: "window1-v51-continuity-of-standing-control-v1", base: V49B_COMMIT, frozen_V49b: V49B_COMMIT, controlling_receipts: [AIM_ERROR_CENSUS_COMMIT], architecture: { change: "EXACT_P_REST_ALREADY_ARMED_REMAINS_STANDING_DURING_ELEVATED_FLOW", time_discipline_only: true, price_transform: "NONE", elevated_flow: { causal_measure: "CUMULATIVE_TRUE_PRINT_VOLUME_LOTS_AT_OR_BEFORE_CURRENT_RECEIPT", strict_threshold_lots: policy.ELEVATED_FLOW_THRESHOLD_LOTS, provenance: policy.ELEVATED_FLOW_PROVENANCE }, overridden_vacate_paths: ["V49B_AT_P_CURRENTLY_UNLAWFUL_NO_OFFSET_FALLBACK", "V43_C2_WITHHOLD_CANCEL_INFEASIBLE_INCUMBENT_REST"], never_creates_a_rest: true, pair_cap_never_waived: true, staying_means_accepting_fills: true, incumbent_elsewhere: "V49B_BYTE_IDENTICAL", V50_price_bound_family: "NOT_CONSUMED" }, target_population: { distinct_RRWP_legs: 362, scoreable_with_lawful_floor: 355, no_lawful_floor: 7 }, fill_rulers: { market_scoring: "ANY_TRUE_TRADE_AT_OR_BELOW_PRIOR_LAWFUL_REST", build_verification: "STRICT_SELLER_AGGRESSED_PRINT_SIZE5_AT_OR_BELOW_PRIOR_REST", never_swapped: true }, acceptance: "MECHANISM_BOUND_MEASUREMENT_NO_AUTO_RATIFICATION_BAR" }
    : isV49b
  ...
  const v51Package = isV51 ? (() => {
    const baselineRun = machineRuns.get("V49B_FROZEN_BASELINE"), candidateRun = machineRuns.get("V51_CONTINUITY_OF_STANDING");
    const baseline = attributionByName.get("V49B_FROZEN_BASELINE"), candidate = attributionByName.get("V51_CONTINUITY_OF_STANDING");
    const frozenScore = JSON.parse(gitShow(V49B_COMMIT, ".claude/window1_live_v4_replay/v49b_faithful_stand_at_p_20260811/MARKET_GRADE_SCORECARD.json")).score;
    const frozenStrict = JSON.parse(gitShow(V49B_COMMIT, ".claude/window1_live_v4_replay/v49b_faithful_stand_at_p_20260811/STRICT_BUILD_VERIFICATION_SCORECARD.json")).score;
    const baselineReproduction = {
      market_score_byte_identical: canonical(baseline.MARKET) === canonical(frozenScore),
      strict_score_byte_identical: canonical(baseline.STRICT_PRINT_CROSS) === canonical(frozenStrict),
    };
    baselineReproduction.pass = baselineReproduction.market_score_byte_identical && baselineReproduction.strict_score_byte_identical;
    ensure(baselineReproduction.pass, "V49b frozen score reproduction failed");

    const aimPath = ".claude/window1_second_seat/v11_non_action_mechanism_audit_20260803/AIM_ERROR_CENSUS.json";
    const aimBytes = gitShow(AIM_ERROR_CENSUS_COMMIT, aimPath), aim = JSON.parse(aimBytes);
    ensure(aim.conservation.rrwp_legs === 362 && aim.conservation.scored === 355, "AIM_ERROR_CENSUS population changed");
    ensure(aim.predictability.d_volume_tier["1k-10k"].n === 150 && aim.predictability.d_volume_tier[">10k"].n === 61, "AIM_ERROR_CENSUS elevated tiers changed");
    const aimByKey = new Map(aim.rows.map((row) => [`${row.event}|${row.leg}`, row]));
    const targetKeys = new Set(aimByKey.keys());
    ensure(targetKeys.size === 355, `AIM scoreable identity count ${targetKeys.size}`);
    const baselineEvents = new Map(baselineRun.marketEvents.map((event) => [event.event_id, event]));
    const tradedFloorByLeg = new Map(candidate.traded_floor_rows.map((row) => [row.leg_identity, row]));
    const rows = [];
    for (const event of candidateRun.marketEvents) for (const leg of Object.values(event.legs)) {
      const baseLeg = baselineEvents.get(event.event_id).legs[leg.leg_id];
      const key = `${event.event_id}|${leg.leg_id}`;
      if (!targetKeys.has(key)) continue;
      const baselineCredited = baseLeg.credited, candidateCredited = leg.credited;
      const disposition = !baselineCredited && candidateCredited ? "CONVERTED_BY_CONTINUITY" : baselineCredited && candidateCredited ? "ALREADY_CREDITED" : "REMAINED_UNCREDITED";
      const floor = tradedFloorByLeg.get(leg.leg_identity)?.lowest_traded_price_cents ?? null;
      const fallingKnife = !baselineCredited && candidateCredited && Number.isInteger(leg.entry_cents) && Number.isInteger(floor) ? leg.entry_cents - floor : null;
      rows.push({ event_id: event.event_id, leg_identity: leg.leg_identity, category: leg.category, price_region: leg.price_region, volume_tier: aimByKey.get(key).vol, evidence_named_P_cents: aimByKey.get(key).P, state: disposition, baseline_credited: baselineCredited, candidate_credited: candidateCredited, baseline_entry_cents: baseLeg.entry_cents, candidate_entry_cents: leg.entry_cents, traded_floor_cents: floor, falling_knife_cents: fallingKnife, continuity_applied_receipts: leg.continuity_applied_receipts, continuity_walk_away_overrides: leg.continuity_walk_away_overrides, continuity_s12_overrides: leg.continuity_s12_overrides, first_continuity_receipt: leg.continuity_first, final_continuity_receipt: leg.continuity_last });
    }
    ensure(rows.length === 355, `V51 target row count ${rows.length}`);
    const states = ["CONVERTED_BY_CONTINUITY", "ALREADY_CREDITED", "REMAINED_UNCREDITED"];
    const stateCensus = Object.fromEntries(states.map((state) => [state, rows.filter((row) => row.state === state).length]));
    ensure(Object.values(stateCensus).reduce((a, b) => a + b, 0) === 355, "V51 three-state conservation failed");
    const changed = rows.filter((row) => row.state === "CONVERTED_BY_CONTINUITY");
    const byVolume = Object.fromEntries(["1-99", "100-999", "1k-10k", ">10k"].map((tier) => {
      const tierRows = rows.filter((row) => row.volume_tier === tier), converted = tierRows.filter((row) => row.state === "CONVERTED_BY_CONTINUITY");
      return [tier, { target_legs: tierRows.length, states: countBy(tierRows, (row) => row.state), conversions: converted.length, falling_knife_cents: distribution(converted.map((row) => row.falling_knife_cents)) }];
    }));
    const byCategory = Object.fromEntries([...new Set(rows.map((row) => row.category))].sort().map((category) => {
      const categoryRows = rows.filter((row) => row.category === category), converted = categoryRows.filter((row) => row.state === "CONVERTED_BY_CONTINUITY");
      return [category, { target_legs: categoryRows.length, states: countBy(categoryRows, (row) => row.state), conversions: converted.length, falling_knife_cents: distribution(converted.map((row) => row.falling_knife_cents)) }];
    }));
    const scoreDelta = {
      market_completed_pairs: candidate.MARKET.completed_pairs - baseline.MARKET.completed_pairs,
      market_under_par_pairs: candidate.MARKET.under_par_pairs - baseline.MARKET.under_par_pairs,
      true_book_cents: candidate.FULL_BOOK.true_book_net_cents - baseline.FULL_BOOK.true_book_net_cents,
      strict_completed_pairs: candidate.STRICT_PRINT_CROSS.completed_pairs - baseline.STRICT_PRINT_CROSS.completed_pairs,
      frontier: Object.fromEntries(Object.keys(candidate.MARKET.frontier).map((key) => [key, candidate.MARKET.frontier[key] - baseline.MARKET.frontier[key]])),
    };
    const differential = ladderDifferential(baselineRun.marketEvents, candidateRun.marketEvents, closeByTicker, "V51_CONTINUITY_OF_STANDING");
    const outsideTargetChanges = differential.rows.filter((row) => !targetKeys.has(row.leg_identity));
    const mechanismApplications = rows.reduce((sum, row) => sum + row.continuity_applied_receipts, 0);
    const namedChecks = [...changed].sort((a, b) => (b.falling_knife_cents ?? -Infinity) - (a.falling_knife_cents ?? -Infinity) || a.leg_identity.localeCompare(b.leg_identity)).slice(0, 6);
    const acceptance = { mechanism_bound: true, baseline_reproduction: baselineReproduction, target_population: { distinct_rrwp_legs: 362, scoreable_legs: 355, unavailable_floor_legs: 7 }, elevated_flow_threshold: { operator: ">", lots: policy.ELEVATED_FLOW_THRESHOLD_LOTS, provenance: policy.ELEVATED_FLOW_PROVENANCE }, mechanism_applications: mechanismApplications, conversions: changed.length, falling_knife_cost_cents: distribution(changed.map((row) => row.falling_knife_cents)), changed_legs_outside_355_target: outsideTargetChanges.length, build_pass: baselineReproduction.pass && outsideTargetChanges.length === 0, ratified: null, operator_ruling_required: true };
    return { aimPath, aimBytes, aim, rows, stateCensus, byVolume, byCategory, scoreDelta, differential, namedChecks, acceptance };
  })() : null;
  ...
  const core = {
    ...
    ...(isV51 ? {
      "V51_RECEIPT_BINDINGS.json": canonical({ frozen_V49b: { commit: V49B_COMMIT, baseline_reproduction: v51Package.acceptance.baseline_reproduction }, AIM_ERROR_CENSUS: { commit: AIM_ERROR_CENSUS_COMMIT, path: v51Package.aimPath, sha256: shaBytes(v51Package.aimBytes), bytes: v51Package.aimBytes.length, population: v51Package.acceptance.target_population }, elevated_flow_authority: { condition: `CAUSAL_CUMULATIVE_PRINT_VOLUME_LOTS_GT_${policy.ELEVATED_FLOW_THRESHOLD_LOTS}`, provenance: policy.ELEVATED_FLOW_PROVENANCE, fitted_parameter: false, census_tier_boundary: true, eventual_event_volume_not_consumed: true } }),
      "ATTRIBUTION_SCORECARD.json": canonical({ order: machineSpecs.map((spec) => spec.name), rows: attributionRows.map(({ full_book_rows, traded_floor_rows, traded_floor_games, ...row }) => row), delta: v51Package.scoreDelta, differential: { aggregate: v51Package.differential.aggregate, by_category: v51Package.differential.by_category, score_delta: v51Package.differential.score_delta }, acceptance: v51Package.acceptance }),
      "FRONTIER.json": canonical({ V49B: attributionByName.get("V49B_FROZEN_BASELINE").MARKET.frontier, V51: marketScore.frontier, delta: v51Package.scoreDelta.frontier, denominator: 804 }),
      "THREE_STATE_CENSUS.json": canonical({ population: v51Package.acceptance.target_population, states: v51Package.stateCensus, by_category: v51Package.byCategory, by_volume_tier: v51Package.byVolume, conservation: { states_sum: Object.values(v51Package.stateCensus).reduce((a, b) => a + b, 0), expected: 355, pass: Object.values(v51Package.stateCensus).reduce((a, b) => a + b, 0) === 355 } }),
      "CONTINUITY_LEG_LEDGER.jsonl.gz": gzipRows(v51Package.rows),
      "VOLUME_TIER_DELTAS.json": canonical(v51Package.byVolume),
      "FALLING_KNIFE_COST_RECEIPT.json": canonical({ law: "ENTRY_MINUS_LOWEST_LAWFUL_TRUE_TRADE_FLOOR_ON_CONVERTED_LEGS", converted_legs: v51Package.acceptance.conversions, distribution_cents: v51Package.acceptance.falling_knife_cost_cents, per_category: v51Package.byCategory, per_volume_tier: v51Package.byVolume }),
      "NAMED_CONTINUITY_CHECKS.json": canonical({ selection_law: "TOP_SIX_CONVERTED_TARGET_LEGS_BY_FALLING_KNIFE_CENTS_THEN_LEG_IDENTITY", rows: v51Package.namedChecks, all_from_frozen_355_target: v51Package.namedChecks.every((row) => row.state === "CONVERTED_BY_CONTINUITY"), pass: v51Package.namedChecks.length === Math.min(6, v51Package.acceptance.conversions) }),
      "V49B_V51_DIFFERENTIAL_RECEIPT.json": canonical({ aggregate: v51Package.differential.aggregate, by_category: v51Package.differential.by_category, score_delta: v51Package.differential.score_delta }),
      "V49B_V51_DIFFERENTIAL_LEDGER.jsonl.gz": gzipRows(v51Package.differential.rows),
      "COMPOSITION_ACCEPTANCE_BAR.json": canonical(v51Package.acceptance),
      "CONSTRUCTION_STATUS.json": canonical({ status: v51Package.acceptance.build_pass ? "PASS_MEASURED_NOT_RATIFIED" : "BLOCKED_BUILD_DEFECT", operative_candidate: "V49B_47B51FD2", V51_ratification: null, mechanism_bound_only: true }),
      "FULL_BOOK_PNL.json": canonical({ method: { commit: FULL_BOOK_PNL_COMMIT, close_column: "replay_close_cents", completed: "100_MINUS_PAIR_ENTRY", naked: "FROZEN_REPLAY_WINDOW1_CLOSE_MINUS_ENTRY_WHERE_AVAILABLE", skip: 0 }, rows: attributionRows.map((row) => ({ machine: row.machine, aggregate: row.FULL_BOOK, by_category: row.by_category.FULL_BOOK })) }),
      "TRADES_AS_TRUTH_CREDIT_LAW.json": canonical({ order_must_preexist_print: true, price_relation: "TRUE_TRADE_PRICE_LE_REST_LEVEL", asks_role: "PLACEMENT_ONLY", strict_build_verification_separate: true }),
    } : {}),
    ...(isV48 ? {
    ...
    "SOURCE_HASH_MANIFEST.json": canonical({
      ...
      public: {
        ...
        ...(isV51 ? {
          "arb-executor/analysis/window1_v49b_faithful_stand_at_p.js": { sha256: fileHash(path.join(repo, "arb-executor/analysis/window1_v49b_faithful_stand_at_p.js")), bytes: fs.statSync(path.join(repo, "arb-executor/analysis/window1_v49b_faithful_stand_at_p.js")).size },
          "arb-executor/tests/test_window1_v51_continuity_of_standing.js": { sha256: fileHash(path.join(repo, "arb-executor/tests/test_window1_v51_continuity_of_standing.js")), bytes: fs.statSync(path.join(repo, "arb-executor/tests/test_window1_v51_continuity_of_standing.js")).size },
          "arb-executor/tests/test_window1_v51_continuity_of_standing_package.js": { sha256: fileHash(path.join(repo, "arb-executor/tests/test_window1_v51_continuity_of_standing_package.js")), bytes: fs.statSync(path.join(repo, "arb-executor/tests/test_window1_v51_continuity_of_standing_package.js")).size },
        } : {}),
  ...
  write("REPORT.md", isV51
    ? `# V51 continuity-of-standing - ${v51Package.acceptance.build_pass ? "PASS / MEASURED" : "BLOCKED"}\n\nV51 changes one time discipline over frozen V49b: on a frozen AIM_ERROR_CENSUS target leg, after an exact-P rest is already standing and causal cumulative true-print volume crosses the census's named elevated-flow tier (> ${policy.ELEVATED_FLOW_THRESHOLD_LOTS} lots), the rest remains at P through incumbent walk-away and pre-credit S12/deep-gap-withhold decisions. V51 never creates the rest, never changes P, never waives the pair cap, consumes no V50 price bound, and leaves every non-authority receipt on V49b.\n\n- Frozen V49b reproduction: ${v51Package.acceptance.baseline_reproduction.pass ? "PASS" : "FAIL"}.\n- Three-state target census: converted ${v51Package.stateCensus.CONVERTED_BY_CONTINUITY}; already credited ${v51Package.stateCensus.ALREADY_CREDITED}; remained uncredited ${v51Package.stateCensus.REMAINED_UNCREDITED}; total 355.\n- Continuity applications: ${v51Package.acceptance.mechanism_applications}; changes outside the frozen 355-leg target: ${v51Package.acceptance.changed_legs_outside_355_target}.\n- CANON trades-truth V49b -> V51: completed ${attributionByName.get("V49B_FROZEN_BASELINE").MARKET.completed_pairs} -> ${marketScore.completed_pairs}; under par ${attributionByName.get("V49B_FROZEN_BASELINE").MARKET.under_par_pairs} -> ${marketScore.under_par_pairs}; frontier <=93/<=95/<=97/<100/any ${marketScore.frontier.LE_93}/${marketScore.frontier.LE_95}/${marketScore.frontier.LE_97}/${marketScore.frontier.LT_100}/${marketScore.frontier.ANY_PRICE}.\n- True book V49b -> V51: ${attributionByName.get("V49B_FROZEN_BASELINE").FULL_BOOK.true_book_net_cents}c -> ${attributionByName.get("V51_CONTINUITY_OF_STANDING").FULL_BOOK.true_book_net_cents}c.\n- Strict print-cross build verification V49b -> V51: ${attributionByName.get("V49B_FROZEN_BASELINE").STRICT_PRINT_CROSS.completed_pairs} -> ${strictScore.completed_pairs}.\n- Converted-leg falling-knife cost, entry minus lowest lawful true trade: ${JSON.stringify(v51Package.acceptance.falling_knife_cost_cents)}. Per-volume-tier and per-category deltas are frozen separately.\n- V51 remains measured, not automatically ratified; operative baseline remains V49b pending operator ruling.\n`
    : isV49b
