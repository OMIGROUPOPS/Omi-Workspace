...
function strictFinite(value) { return typeof value === "number" && Number.isFinite(value) ? value : null; }
...
function normalizeVariant(run, evidenceMap) {
  ...
  for (const rawEvent of run.replay.events) {
    ...
    for (const evLeg of evidence.legs) {
      const raw = rawEvent.legs?.[evLeg.leg_id] || {}; const actionEntry = strictInteger(raw.proposed_entry_cents), candidateEntry = strictInteger(raw.honest_credited_entry_cents), fillClass = raw.honest_fill_class || "UNPROVEN", credited = honestFillCredited(fillClass, candidateEntry), entry = credited ? candidateEntry : null, acted = actionEntry !== null;
...
function developmentComparison(repo) {
  ...
  for (const old of v11.filter((x) => x.acted)) {
    ...
    const gap = strictFinite(old.entry_minus_qualifying_ask_floor_cents); const supported = old.credited && Number.isInteger(old.qualifying_ask_floor_cents);
...
module.exports = { tickerIdentity, capacityFloor, pairNegative, distribution, metrics, honestFillCredited, strictInteger, strictFinite, readFrozenTape, fetchTapeResumable, prepareInputs };
