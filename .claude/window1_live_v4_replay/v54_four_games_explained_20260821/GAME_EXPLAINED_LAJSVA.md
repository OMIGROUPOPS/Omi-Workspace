...
NEIGHBOR-GRAIN [NG-LAJSVA]: receipt-bearing comparisons below are either RANGE_SPECTRUM_PATH polling paths (R-CORPUS + R-RANGE, approximately 100 ticks per leg) or HISTORICAL_EVENT_AGGREGATE rows (R-CORPUS + R-HIST, no intramatch path). RESOURCE-GAP [GAP-NG-LAJSVA]: no raw-tape order-book depth receipt exists at the matched-neighbor stage; range-path best-five summaries are not raw depth.
