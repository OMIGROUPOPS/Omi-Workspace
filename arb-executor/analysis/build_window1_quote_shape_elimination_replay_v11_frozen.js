...
const { narrowSiblingShapes } = require("./window1_second_leg_x_pricer_v17.js");
...
const secondLegXPricerV17 = args.includes("--second-leg-x-pricer-v17");
const secondLegXLibraryPath = path.resolve(value("--second-leg-x-library-v17", path.join(repo, ".claude/window1_live_v4_replay/second_leg_x_pricer_fit_v17_20260803/SECOND_LEG_X_CONDITIONAL_LIBRARY.json")));
...
function replayGame(eventId, sources, library, refs, persistenceLibrary = null, secondLegXLibrary = null) {
  ...
  for (const ts of clock) {
    ...
    if (secondLegXPricerV17 && secondLegXLibrary) {
      for (const leg of legs) {
        const firstLeg = legs.find((candidate) => candidate !== leg && candidate.order && candidate.order.action_ts < ts);
        if (!firstLeg || leg.order || !leg.last) continue;
        const priorShapes = [...leg.survivor_shapes];
        let decision = narrowSiblingShapes(secondLegXLibrary, {
          eventId,
          category: leg.category,
          startingPriceSplit: secondLegXLibrary.event_starting_price_split[eventId],
          firstFillX: firstLeg.order.price_cents,
          shapeIds: priorShapes,
        });
        if (decision.applied && decision.all_shapes_eliminated && priorShapes.length < leg.all_shape_ids.length) {
          const reopened = narrowSiblingShapes(secondLegXLibrary, {
            eventId,
            category: leg.category,
            startingPriceSplit: secondLegXLibrary.event_starting_price_split[eventId],
            firstFillX: firstLeg.order.price_cents,
            shapeIds: leg.all_shape_ids,
          });
          if (reopened.retained_shape_ids.length) decision = { ...reopened, contradiction_reopen: { reason: "CURRENT_SURVIVOR_SET_DISJOINT_FROM_X_CONDITIONAL_DISTRIBUTION; FULL_FITTED_SET_REOPENED_BEFORE_RENARROW", contradicted_shape_ids: priorShapes } };
        }
        if (decision.applied && decision.retained_shape_ids.length) leg.survivor_shapes = [...decision.retained_shape_ids];
        const signature = `${decision.reason}|${decision.retained_shape_ids.join(",")}|${decision.eliminated_shapes.map((item) => item.shape_id).join(",")}`;
        const priorDecision = leg.second_leg_x_pricer_decisions[leg.second_leg_x_pricer_decisions.length - 1];
        if (!priorDecision || priorDecision.signature !== signature) leg.second_leg_x_pricer_decisions.push({
          signature,
          timestamp_epoch: ts,
          t_minus_scheduled_seconds: leg.scheduled - ts,
          t_minus_actual_bell_seconds: Number.isFinite(leg.bell) ? leg.bell - ts : null,
          trigger_receipt: leg.last.receipt,
          first_leg_id: firstLeg.leg,
          first_fill_x_cents: firstLeg.order.price_cents,
          first_fill_timestamp_epoch: firstLeg.order.action_ts,
          strictly_after_first_fill: ts > firstLeg.order.action_ts,
          current_ask_cents: leg.last.ask,
          prior_shape_ids: priorShapes,
          ...decision,
        });
      }
  ...
  return {
    ...
    legs: Object.fromEntries(Object.entries(event.legs).map(([legId, leg]) => {
      ...
      return [legId, {
        ...
        second_leg_x_pricer_decisions: leg.second_leg_x_pricer_decisions || [],
...
function main() {
  ...
  const secondLegXLibrary = secondLegXPricerV17 ? JSON.parse(fs.readFileSync(secondLegXLibraryPath)) : null;
  ...
  const fullEvents = [...TARGETS].sort().map((eventId) => {
    try { return replayGame(eventId, sourcesByEvent[eventId], library, refs, persistenceLibrary, secondLegXLibrary); }
  ...
  const receipt = {
    ...
    second_leg_x_pricer_v17: secondLegXPricerV17,
    second_leg_x_library_sha256: secondLegXPricerV17 ? sha256(fs.readFileSync(secondLegXLibraryPath)) : null,
