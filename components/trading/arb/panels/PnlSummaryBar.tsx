...
export function PnlSummaryBar({ balances, positions, totalPnl, pnlSummary, isStale }: Props) {
  ...
  const items = [
    { label: "Starting", value: startingBalance, color: "text-white" },
  ...
  return (
    <div className="rounded-lg border border-gray-800 bg-[#111] p-3">
      ...
      <div className="grid grid-cols-6 gap-3">
        {items.map((item) => (
          <div key={item.label}>
            <p className="text-[10px] text-white uppercase">{item.label}</p>
      ...
      {recon && (
        <div className="mt-3 border-t border-gray-800 pt-2">
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-[10px] font-semibold text-white uppercase tracking-wide">API Truth</span>
            {recon.stale && (
