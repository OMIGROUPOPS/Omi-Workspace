...
function replayGame(eventId, sources, library, refs, persistenceLibrary = null) {
  ...
  let tuples = allTuples, pairSurvivorIds = pairHypothesisGroup?.hypotheses?.map((row) => row.pair_hypothesis_id) || [], lastPairNarrowing = null, indices = Object.fromEntries(legs.map((l) => [l.leg, 0])); const dynamicPairClosures = [], pairNarrowingTransitions = [], clock = [...new Set(legs.flatMap((l) => l.rows.map((r) => r.ts)))].sort((a, b) => a - b);
  for (const ts of clock) {
    ...
    for (const leg of legs) {
      ...
      if (pairInterimV18) {
        ...
        pairOnlyResolution = { single_leg_shape_ids: singleShapes, single_leg_verdicts: singleVerdicts, single_leg_resolved_floor: singleAllFloor, joint_shape_ids: shapes, joint_resolved_floor: allFloor, resolved_by_pair_hypothesis_not_single_leg_hypothesis: allFloor && !singleAllFloor, pair_hypothesis_ids: lastPairNarrowing?.signable_pair_survivor_ids || [], pair_status: lastPairNarrowing?.status || "PAIR_SOURCE_PENDING" };
      }
...
function main() {
  ...
  const receipt = {
    schema_version: pairInterimV18 ? "WINDOW1_SYNCHRONIZED_PAIR_INTERIM_ELIMINATION_REPLAY_V18" : interimEliminationV13 ? "WINDOW1_INTERIM_ELIMINATION_REPLAY_V13" : dynamicRenarrowV6 ? "WINDOW1_QUOTE_SHAPE_DYNAMIC_RENARROW_REPLAY_V6" : "WINDOW1_QUOTE_SHAPE_REPLAY",
    cold: true,
