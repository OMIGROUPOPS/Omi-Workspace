#!/usr/bin/env python3
"""Deterministic one-shot Round-2 Window-1 grid execution package.
    ...
    *,
    ...
    if mode == "execute":
        ...
        if branch != "codex/window1-definition":
            raise GridExecutionError(
                "validation-only requires the frozen Codex branch"
            )
    ...
    assert_exact_candidate_order(list(candidate_receipts))
    if (
...
def execute_grid(repo: Path, package_path: Path) -> int:
    ...
    try:
        ...
        (result_dir / "WINDOW1_ROUND2_DEVELOPMENT_REPORT.md").write_text(
