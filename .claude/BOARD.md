...
1. ~~Fused gun + orphan fix~~ ✓ **BUILT + DEPLOYING 07-08 night** (`ac0c0ffe`, one gated deploy; close-out INVALID until tonight's live proof table lands — see IN-FLIGHT)
...
13. ~~ORPHAN-CLASS FIX~~ — **SHIPPED 07-08 night (`ac0c0ffe` C-ORPHAN-FINGERPRINT, with the fused gun)**: boot re-adopts by jsonl order_placed lineage; C47 asserts zero fingerprinted orders in manual class. Regression watch: `fingerprint_in_manual` / `fingerprint_untracked` / `orphan_readopted_fingerprint` at every restart.
