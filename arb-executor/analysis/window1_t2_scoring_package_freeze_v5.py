#!/usr/bin/env python3
"""Two-clean-build freezer for the score-free T2 package V5."""
...
from window1_t2_scoring_package_builder_v5 import build


VERSION = "window1-t2-scoring-package-freeze-v5"


class FreezeError(RuntimeError):
    """The two V5 builds were not byte-identical."""
...
def freeze(*, repo: Path, output: Path) -> dict[str, object]:
    if output.exists():
        raise FreezeError("frozen V5 output already exists")
    with tempfile.TemporaryDirectory(
        prefix="w1-t2-scorepkg-v5-a-"
    ) as a_text, tempfile.TemporaryDirectory(
        prefix="w1-t2-scorepkg-v5-b-"
    ) as b_text:
        ...
        if hashes_a != hashes_b:
            ...
            raise FreezeError(
                "clean V5 builds differ: " + ",".join(mismatch[:20])
            )
    ...
    if frozen_regenerable != build_regenerable:
        raise FreezeError("frozen V5 differs from clean builds")
