...
- **Phase 2 (corpus subset, stratified by MATCHES not tickers):** 100 **matches** × 2 tickers per match = **200 tickers**. Sampling unit is the match (paired event_ticker), not the independent ticker, because the paired-leg signals in Section 2.9 require both sides of every sampled match. Stratification: 5 categories × 4 premarket-length quartiles = **20 strata, 5 matches per stratum**. Deterministic random seed (`PHASE2_SAMPLE_SEED = 42` in producer). Validates corpus-scale runtime and memory behavior. Computes per-ticker compute cost distribution to project Phase 3 runtime accurately. Runtime budget: <40 min (doubled vs single-ticker stratification because both partners get processed; per-pair amortization noted in producer commit). Sampling-by-tickers (the prior framing) would split RUN/MCD into different samples ~50% of the time and break paired-leg coverage on those sampled matches — sampling-by-matches preserves pair integrity.
