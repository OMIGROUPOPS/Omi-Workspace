...
T31c. **Layer B v1 coherence read.** Validation gate per spec doc Section "Validation gate". Four checks: (1) capture distributions bounded by physical limits (limit policy capture_p90 <= limit threshold + tolerance); (2) fire rates trend monotonically with policy aggressiveness; (3) time-stop horizons produce monotonically larger captures in positive-mean-bounce cells; (4) premarket vs in-match capture distributions differ in the direction T21 Check 2 predicted. PASS verdict required before promoting Layer C (G11) to T-item. INCONCLUSIVE results that surface methodology issues get a lessons commit and may gate iteration on T31b before promoting G11.
