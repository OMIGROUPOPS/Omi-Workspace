...
const isV52MacroRecognition = isV52m || isV52n;
...
function simulate(base, tapes, prints, mode, clauses = {}) {
  ...
  for (const row of timeline) {
    ...
    const macroRecognition = isV52MacroRecognition ? policy.classifyShapeState(leg.v52m_shape_state, { timestamp_epoch: row.ts, receipt: row.receipt, category: base.category, confidence_gate_enabled: clauses.recognition_confidence_gate === true }) : null;
...
async function main() {
  ...
  if (isV52ReadAuthority && !isV52eExam) {
    ...
    if (isV52MacroRecognition) for (let index = 0; index < candidateFlow.trace.length; index += 1) {
      const row = candidateFlow.trace[index];
      if ((isV52n && row.recognition_confidence_gate) || (!isV52n && row.per_shape_floor_depth?.applicable === true)) {
    ...
    const causalPerGameOutcomeTable = isV52CausalOnset ? reportedCandidateFourStateRows.map((row) => {
      ...
      return {
        ...
        ...(isV52MacroRecognition ? { leg_macro_recognition: Object.fromEntries(Object.entries(event.legs).sort(([a], [b]) => a.localeCompare(b)).map(([legId, leg]) => {
          ...
          return [legId, { leg_identity: leg.leg_identity, credited: leg.credited, entry_cents: leg.entry_cents, fill_timestamp_epoch: leg.fill_timestamp_epoch, family_at_entry_or_terminal: last?.macro_recognition?.binding_family ?? last?.macro_recognition?.family ?? null, proposed_family: last?.macro_recognition?.proposed_family ?? last?.macro_recognition?.family ?? null, confidence: last?.macro_recognition?.confidence ?? null, confidence_gate: last?.recognition_confidence_gate ?? null, signable: last?.macro_recognition?.signable ?? false, receipt: last?.receipt ?? null, per_shape_floor_depth: last?.per_shape_floor_depth ?? null }];
        })) } : {}),
      };
    ...
    for (const event of candidateRun.marketEvents) for (const leg of Object.values(event.legs)) {
      ...
      if (beforeBehaviorSha !== afterBehaviorSha) {
        ...
        const firstBoundCandidates = candidateFlow.trace.filter((row) => row.event_id === event.event_id && (isV52n ? Boolean(row.recognition_confidence_gate) : isV52m ? row.per_shape_floor_depth?.applicable === true : isV52l ? (() => {
          const before = baselineTrace.get(traceKey(row));
...
- Outcomes are observations only. No full-804, sealed, deployment, authorization, live, order, position, or holdout action occurred.
` : null;
    const clauseNumber = isV52n ? "3_RECOGNITION_CONFIDENCE_GATE" : isV52m ? "3_MACRO_RECOGNITION" : isV52l ? "1_CAUSAL_ONSET" : isV52k ? "3_LIBRARY_EVIDENCE" : isV52j ? "3_N4_ROLE" : isV52i ? "3_N4_DEPTH" : isV52h ? "4_MARKET_PROOF" : isV52g ? "6" : isV52f ? "5" : isV52e ? "N9" : isV52d ? "4" : isV52c ? "2" : "3";
    const parentCommit = isV52n ? V52M_COMMIT : isV52m ? V52L_COMMIT : isV52l ? "fc17d0d3ec3db4795d2e25a986bb9bfa1806714b" : isV52k ? V52J_COMMIT : isV52j ? V52I_COMMIT : isV52i ? V52H_COMMIT : isV52h ? V52G_COMMIT : isV52g ? V52F_COMMIT : isV52f ? V52F_PARENT_COMMIT : isV52e ? V52D_COMMIT : isV52d ? V52D_PARENT_COMMIT : isV52c ? V52B_COMMIT : V52_COMMIT;
    ...
    const baselinePrefix = isV52n ? "V52M" : isV52m ? "V52L" : (isV52l || isV52DepthValidation) ? "V52H" : isV52h ? "V52G" : isV52g ? "V52F" : isV52f ? "V52E" : isV52e ? "V52D" : isV52d ? "V52C" : isV52c ? "V52B" : "V52";
    const candidatePrefix = isV52n ? "V52N" : isV52m ? "V52M" : isV52l ? "V52L" : isV52k ? "V52K" : isV52j ? "V52J" : isV52i ? "V52I" : isV52h ? "V52H" : isV52g ? "V52G" : isV52f ? "V52F" : isV52e ? "V52E" : isV52d ? "V52D" : isV52c ? "V52C" : "V52B";
    const core = {
      "REPORT.md": isV52n ? v52nReport : isV52m ? v52mReport : isV52l ? v52lReport : isV52k ? v52kReport : isV52j ? v52jReport : isV52i ? v52iReport : isV52h ? v52hReport : isV52g ? v52gReport : isV52f ? v52fReport : isV52e ? v52eReport : isV52d ? v52dReport : isV52c ? v52cReport : report,
