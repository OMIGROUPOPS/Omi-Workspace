...
async function runExam() {
  ...
  fs.mkdirSync(output, { recursive: true });
  writeJson(output, "EXECUTION_START_RECEIPT.json", { schema_version: "window1-fresh-holdout-exam-start-v3", authorization: "OPERATOR_PROMPT_SERIALIZER_REPAIR_FRESH_EXAM_20260807", authorization_consumed: true, authorized_process_attempt: 2, prior_mechanical_attempts_before_score_row: 1, score_rows_before_this_attempt: 0, prior_consumed_failure_commit: "a746d17582284736f9b3a9e6c8db2bf61e9204e1", prior_retryable_mechanical_failure_receipt: ".claude/window1_fresh_holdout_exam_serializer_repair_20260807/MECHANICAL_FAILURE_ATTEMPT1_RECEIPT.json", brains: ["V36", "V35"], R3_excluded: true, event_N: 171, event_list_sha256: hashFile(eventListFile), boundary_sha256: hashFile(boundaryFile), prints_sha256: hashFile(printsFile), started_at_utc: new Date().toISOString() });
  ...
  writeJson(output, "EXECUTION_COMPLETE_RECEIPT.json", { status: "COMPLETE", authorized_process_attempts: 2, permitted_mechanical_retries_before_score_rows: 1, score_emitting_runs: 1, brain_invocations_in_final_run: { V36: 1, V35: 1, R3: 0 }, score_rows: bellRows.length, authorization_consumed: true, completed_at_utc: new Date().toISOString() });
  ...
  reportLines.push("", "One expressly permitted mechanical retry occurred after the first attempt emitted zero score rows. No tuning, live engine, account, order, position, or trading access occurred.", "");
  ...
  process.stdout.write(canonical({ status: "COMPLETE", authorized_process_attempts: 2, permitted_mechanical_retries_before_score_rows: 1, score_emitting_runs: 1, exact_headline: bellRows.filter((row) => row.headline) }));
}
