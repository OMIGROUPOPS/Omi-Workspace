...
T33. **Per-minute Open Interest reconstruction producer (per-side).** Per LESSONS F31 + B24. Foundation: g9_trades.parquet (T28 sha256-pinned at ea84e74, 33.7M rows). Open Interest per minute per side is a load-bearing trading signal that we do not currently track but is reconstructable: walk g9_trades chronologically per market, compute signed cumulative `count_fp` by `taker_side` direction, bin to per-minute timestamps. Output: data/durable/oi_v1/oi_per_minute.parquet, columns [ticker, ts_minute, yes_oi, no_oi, yes_oi_delta_1min, no_oi_delta_1min]. Per-market aggregation; ~20,018 markets × premarket+in_match minute span. C28 streaming discipline mandatory (33.7M trade rows, must NOT materialize full dataframe; iterate row-groups). Estimated runtime ~2-3 hours. T33 chain (planned, mirrors T29/T31b shape): T33a (spec doc oi_v1_spec.md with column definitions + computation method + per-market correctness check sample), T33b (producer build_oi_v1.py + output parquet + log), T33c (coherence read check_oi_v1_coherence.py + report — 4-check shape: monotone non-negative OI, per-market OI sum equals total open positions at last observed minute, per-side delta sums equal trade counts on that side, sample-cell anchor evidence reproduces Dzumhur=2,070 + Mannarino=8,594). C27 treatment: MANIFEST sha256 entry + ANALYSIS_LIBRARY subsection with foundation-pointer to T28. Validity gate: T33c PASS gates downstream Layer C v2/v3 fill-probability modeling (T32d v2). Cross-reference: F31 (the lesson), B24 (4× OI asymmetry anchor evidence), B22 (cell anchoring framework — OI growth tells us which cells are reachable when), E16 (in-match liquidity), C28 (streaming discipline mandate).
