...
1. **T31b-beta — Layer B v1 producer single-cell test mode** (immediate next work). Implement evaluate_policy + walk_trajectory + --test-cell mode. No parquet write, no visuals. Verification target: --test-cell in_match__1__medium__high__ATP_CHALL runs end-to-end with bounded capture distributions, monotonic fire rates with policy aggressiveness, no runtime errors.
2. **T31b-gamma — Layer B v1 full sweep** (gated on T31b-beta). All 356 in-scope substantial cells. Estimated 30-60 min runtime. Writes exit_policy_per_cell.parquet + per-(channel, category) visuals + producer log. Output gets full LESSONS C27 treatment: MANIFEST sha256 + ANALYSIS_LIBRARY foundation pointer entry.
3. **T31c — Layer B v1 coherence read** (gated on T31b-gamma). Four validation checks per spec validation gate. PASS gates Layer C (G11).
4. **G11 — Layer C realized economics** (gated on T31c PASS). Layer A + Layer B + fees + slippage + fill probability + capital constraints. Final per-(cell, policy) realized P&L distribution.
