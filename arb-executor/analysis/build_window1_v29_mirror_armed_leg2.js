...
function targetRows(events, trace) {
  ...
  for (const event of events) {
    ...
    const decision = mirrorArmDecision({ incumbentHandled: !structurallyEligible, siblingDiscountAuthority: "NOT_BOUND", closeBar: null, lawfulFloorBar: null, firstFillCents: first.entry_cents });
    rows.push({ target_class: "V28_CARRIED_PAIR", event_id: event.event_id, category: event.category, starting_price_region: event.starting_price_split, target_leg_identity: positive.leg_identity, leg1_identity: first.leg_identity, leg1_fill_cents: first.entry_cents, ex_post_leg1_below_audited_close: first.entry_cents < first.audited_close_cents, target_was_second_fill: positive.leg_identity === ordered[1].leg_identity, structurally_eligible_before_authority_checks: structurallyEligible, incumbent_handled_at_leg1_fill: !structurallyEligible, later_handled_by_V28: true, decision, conversion: "UNCHANGED_V28_CARRIED" });
  }
