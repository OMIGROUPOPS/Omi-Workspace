...
assert(source.includes("await writeGzipRowsFile(traceFile, results[name].result.decisions)"));
assert(source.includes("await writeGzipRowsFile(traceCheck, results[name].result.decisions)"));
assert(source.includes("streaming_full_trace: true"));
...
console.log(JSON.stringify({ pass: true, assertions: 18 }));
