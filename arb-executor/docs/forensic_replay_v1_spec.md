...
**Convention invariant (verified Session 9 from on-disk probe of 5,878 trade-candle pairs across 5 tickers):** Kalshi's `taker_side` field names the side the taker BOUGHT. A trade with `taker_side == "yes"` means the taker bought YES contracts (paying the ask); a trade with `taker_side == "no"` means the taker bought NO contracts (which is equivalent to selling YES, taking the bid). Therefore: a hypothetical maker BID for yes is filled by a `taker_side == "no"` trade (the taker buys no, hitting our bid); a hypothetical maker SELL for yes is filled by a `taker_side == "yes"` trade (the taker buys yes, paying our ask). Producer must use this convention on BOTH the entry-fill check (step 3) and the exit-fill check (step 6); inverting either leg matches phantom trades against other makers' orders rather than against our hypothetical resting orders. The empirical cross-tab anchor: across 5,878 trade-candle pairs (3 ATP_CHALL + 2 WTA_MAIN tickers), `taker_side == "yes"` clusters at AT_ASK (56% AT_ASK vs 24% AT_BID) and `taker_side == "no"` clusters at AT_BID (49% AT_BID vs 26% AT_ASK), unambiguous in column dominance.
...
3. **Walk g9_trades forward second-by-second from T0.** Stream `g9_trades` rows for this ticker with `created_time >= T0` ordered ascending. For each trade event:
   ...
   - **Fill check:** if `taker_side == "no"` and `yes_price_dollars <= our_bid_price`, the trade hits our bid — we are filled. Record `fill_time = trade.created_time`, `fill_price = our_bid_price`. Stop walking forward for fill purposes. (Per Session 9 empirical convention probe — `taker_side` names the side the taker BOUGHT; a maker BID for yes is filled by a taker who BUYS no = SELLS yes, taking the bid.)
...
6. **Walk g9_trades forward from fill_time looking for the kiss.** Continue streaming g9_trades with `created_time >= fill_time` ordered ascending. For each trade:
   - **Scenario A exit check:** if `taker_side == "yes"` (taker buys yes, paying the ask; our resting sell at target would be hit) and `yes_price_dollars >= scenario_A_target`, scenario A exits. Record `exit_time_A`, `exit_price_A`, `capture_A_dollars = exit_price_A - fill_price`.
   - **Scenario B exit check:** same logic against `scenario_B_target` (using `yes_price_dollars`) — also requires `taker_side == "yes"` per the corrected convention. Both scenarios may fire at different trades.
