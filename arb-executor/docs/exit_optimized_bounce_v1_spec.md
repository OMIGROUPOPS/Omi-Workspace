...
- **G2 frame purity**: Frame P entry marks are 100% `regime=="premarket"` with `time_to_match_start_min` in [+18,+22]; Frame I entry marks are 100% `regime=="in_match"` with `time_to_match_start_min > 0` as the post-boundary violation test (the match-start boundary minute, `ttms == 0`, is regime==in_match INCLUSIVELY per the foundation -- probe /tmp/g2_probe.log; v0.1 over-specified this as `>= 0`, re-introducing the already-fixed Layer-A G2 defect, corrected here per the G23/4f55339 honest-provenance discipline). Zero cross-frame leakage (B14/G17).
...
- **G7 band support**: frame-aware AND phase-aware (probe /tmp/g7_probe.log, full-cohort measured). Frame I (dense) keeps the hard `n_positions >= 200` reliability bar. Frame P is structurally sparse BY CONSTRUCTION (one T-20m+/-2 conservative fill per ticker); the calibrated floor `30` is probe-grounded for the FULL COHORT (7,115 positions, 39/40 bands >=30; lone extreme band idx39 n=27 carries `low_support`, not hard-failed). The Frame-P floor is PHASE-SCALED by the sampling fraction (== 30 at Phase-2, the deliverable run, probe-grounded; proportionate at the downsampled Phase-1 sanity run) -- applying the full-cohort constant phase-blind to a Phase-1 subsample is the identical defect class as the original G1 phase-blindness, corrected here per the G23/4f55339 honest-provenance discipline (v0.2 was frame-aware but phase-blind; the floor magnitude 30 is unchanged, only its phase-scaling added). Gate behavior (made explicit v0.4): G7's violation count EXCLUDES low_support=True bands -- a sub-floor band is surfaced via the low_support flag in the robustness output, never a hard gate failure; only a NON-flagged Frame-P band below the phase-scaled floor counts as a real violation (catching a genuine Frame-P collapse). The v0.3 gate logic counted the flagged band as a violation, contradicting this stated design; corrected v0.4 per the G23/4f55339 honest-provenance discipline (the spec intent was already correct; the code is now aligned to it).
...
- 2026-05-19 ET (v0.2): G2/G7 gate-spec defects probe-corrected (Phase-1 halt adjudication). G2: Frame-I purity violation is `ttms > 0` not `>= 0` -- the v0.1 re-introduced the already-fixed Layer-A G2 over-specification (boundary minute ttms==0 is regime==in_match inclusive per foundation; probe /tmp/g2_probe.log). G7: made frame-aware -- Frame I keeps >=200, Frame P calibrated floor >=30 per the full-cohort measured distribution (probe /tmp/g7_probe.log: 7,115 positions, min 27, zero bands <10, 39/40 >=30; lone extreme band idx39 n=27 flagged low_support not hard-failed). The v0.1 flat-200 was frame-blind and would have failed Phase-2 too regardless of scale (Frame P structurally sparse by the conservative-fill design). Science unchanged + confirmed sound (Phase-1 direction-sanity PASS: Frame-I exit-optimized <= Layer-A unconditional 0/40 bands, no wiring bug; A38/no-stop/ranking-integrity all PASS). Probe-grounded, not assumed.
- 2026-05-19 ET (v0.3): G7 Frame-P floor made phase-aware (Phase-1 v0.2 halt adjudication). v0.2 made G7 frame-aware but applied the full-cohort-calibrated Frame-P floor (30) as an absolute constant to the downsampled 1000-ticker Phase-1 run -> 27/40 Frame-P bands structurally false-failed purely by subsampling (Phase-1 Frame-P ~24/band vs full-cohort 39/40 bands >=30). Identical defect class as the original G1 phase-blindness; fixed by the same pattern -- scale the Frame-P floor by the phase sampling fraction (==30 at Phase-2 deliverable, probe-grounded unchanged; ~4 at Phase-1). Floor magnitude (30, /tmp/g7_probe.log) NOT re-derived -- correct for the deliverable, only phase-scaling added. G2 confirmed fixed (frameI_post_boundary_ttms=0). Science provably untouched (direction-sanity byte-identical to v0.1: max(Frame-I_eo - LayerA_unc) = -0.03187, 0/40 bands; A38/no-stop/ranking PASS). Probe-grounded + pattern-established (the G1 lesson), not assumed.
- 2026-05-19 ET (v0.4): G7 gate logic aligned to its own stated spec design -- low_support=True Frame-P bands are EXEMPT from the violation count (flagged-into-robustness, not hard-failed), as Sec 4.1 already prescribed and /tmp/g7_probe.log verdicted. Phase-2 v0.3 halted with n_violations=1 = exactly the lone extreme-dislocation Frame-P band (idx39, n=27) the g7 probe predicted; the producer correctly set its low_support=True but the gate still hard-counted it. Floor magnitude (30), frame-awareness, phase-awareness all correct/probe-grounded and UNCHANGED -- only the violation-count exemption added. Science provably untouched: direction-sanity invariant holds (Frame-I exit-optimized <= Layer-A unconditional, 0/40 bands, cap strictly negative; full-cohort -0.03221 vs Phase-1-subsample -0.03187 is the expected population delta, gate-spec edits structurally cannot alter exit_bounce_c_mean). Spec-design-aligned + probe-grounded, not assumed.
