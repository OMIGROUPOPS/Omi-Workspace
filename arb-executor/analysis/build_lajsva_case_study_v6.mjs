...
const REPAIR_ROOT = ".claude/window1_live_v4_replay/v54_repair_iteration7_fitness_weighted_bases_20260822";
const OUT_ROOT = ".claude/window1_live_v4_replay/lajsva_case_study_v8_20260822";
const V7_ROOT = ".claude/window1_live_v4_replay/lajsva_case_study_v7_20260822";
...
function main() {
  ...
  source = source
    ...
    .replaceAll("CONDITIONAL_DIP_RECEIPT.json", "FITNESS_WEIGHTED_BASES_RECEIPT.json")
    .replaceAll("V1_V2_V3_SIDE_BY_SIDE.md", "V1_V2_V3_V4_V5_V6_V7_V8_SIDE_BY_SIDE.md")
    .replaceAll("repair iteration 2", "repair iteration 7")
    .replaceAll("Foundation + conditional dip + early riser", "fitness-weighted evidence bases")
    .replaceAll("this leg's own dip/no-dip state and the q50 remaining dip of same-state, native-bell-bounded MINUTE neighbors", "a continuous receipt-level blend of time-bearing neighbors, graded neighbors, own-tape presence, and reflex tracking")
    .replaceAll("LAJSVA v3", "LAJSVA v8")
    .replaceAll("v3 repaired machine", "v8 repaired machine")
    .replaceAll("FOUNDATION_CONDITIONAL_DIP_EARLY_RISER", "FITNESS_WEIGHTED_BASES")
  ...
  const v7ReceiptFile = path.join(repo, V7_ROOT, "CASE_STUDY_RECEIPT.json");
  const v7 = JSON.parse(fs.readFileSync(v7ReceiptFile, "utf8"));
  const result = caseReceipt.v1_v2_v3?.v3 ?? null;
  const prior = v7.v1_v2_v3_v4_v5_v6_v7?.v7 ?? null;
...
| v7 | evidence ladder: timed neighbors → neighbors+own-window → own tape → reflex | ${outcome(prior)} | self-stopped |
| v8 | fitness-weighted bases: timed neighbors + graded neighbors + own-tape presence + reflex | ${outcome(result)} | ${gate.self_stop ? `self-stop: ${gate.stop_reason}` : "gate pass"} |

V7 receipt: ${V7_ROOT}/CASE_STUDY_RECEIPT.json@sha256:${hash(fs.readFileSync(v7ReceiptFile))}
...
`;
  ...
  caseReceipt.label = "LAJSVA_CASE_STUDY_V8_FITNESS_WEIGHTED_BASES";
